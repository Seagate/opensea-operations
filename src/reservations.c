...
// ******************************************************************************************
//
// \file reservations.c   SCSI reservations based functions. NOTE: THis is SCSI-3 persistent reservations only at this
// time - TJE

// NOTE: Old reservations from SCSI2 (and up to SPC2) are not currently supported. These are much more limited than what
// is available in the Persistent Reservations feature NOTE: Some additional enhancement for NVMe is probably possible.
// The current NVMe implementation is more-or-less what is part of NVMe to SCSI translation. - TJE
...
M_PARAM_RO(1) OPENSEA_OPERATIONS_API bool is_Persistent_Reservations_Supported(const tDevice* M_NONNULL device)
{
    ...
    {
        // try a persistent reserve in with no data transferred. This SHOULD not return an error if this feature is
        // supported - TJE
...
typedef struct s_reservationTypesSupportedV1
{
...
typedef enum eAllowedCommandDetailV1Enum
{
...
typedef struct s_allowedCommandsV1
{
...
typedef struct s_persistentReservationCapabilitiesV1
{
    size_t            size;
    uint32_t          version;
                                                                ...
                                                                uint16_t* M_NONNULL      keyCount)
{
    ...
    else if (get_Device_DriveType(device) == NVME_DRIVE)
    {
        DECLARE_ZERO_INIT_ARRAY(
            ...
            24); // may be able to get away with only 8 bytes, but this read up until the list begins - TJE
...
typedef struct s_registrationKeysDataV1
{
    size_t   size;
    uint32_t version;
    ...
    uint64_t registrationKey[1]; // This is variable sized depending on how many are requested to be read and how many
                                                           ...
                                                           ptrRegistrationKeysData M_NONNULL keys)
{
    ...
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
                                                         ...
                                                         registrationKeys)))
        {
            ...
            keys->numberOfKeys = 0;
            for (uint32_t keyIter = UINT32_C(0), offset = UINT32_C(8);
                 keyIter < reportedKeys && keyIter < numberOfKeys && offset < dataLength;
                 ++keys->numberOfKeys, ++keyIter, offset += UINT32_C(8))
            {
    ...
    {
        uint32_t dataLength =
            (C_CAST(uint32_t, numberOfKeys) * UINT32_C(24)) +
        ...
        if (SUCCESS == (ret = nvme_Reservation_Report(device, false, registrationKeys, dataLength)))
        {
            ...
            // loop through and save each key
            for (uint32_t keyIter = UINT32_C(0), offset = UINT32_C(24);
                 keyIter < reportedKeys && keyIter < numberOfKeys && offset < dataLength;
                 ++keys->numberOfKeys, ++keyIter, offset += UINT32_C(24))
            {
                keys->registrationKey[keyIter] = M_BytesTo8ByteValue(
...
typedef struct s_reservationInfoV1
{
...
typedef struct s_reservationsDataV1
{
    size_t   size;
    uint32_t version;
    uint32_t generation;
                                                      ...
                                                      ptrReservationsData M_NONNULL reservations)
{
    ...
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
        ...
        uint8_t* reservationKeys    = C_CAST(uint8_t*, safe_calloc_aligned(reservationsLength, sizeof(uint8_t),
                                                         ...
                                                         reservationsLength, reservationKeys)))
        {
            ...
            for (uint32_t reservationIter = UINT32_C(0), offset = UINT32_C(8);
                 reservationIter < reservationKeyCount && reservationIter < numberReservations &&
                 offset < reservationsLength;
                 ++reservations->numberOfReservations, ++reservationIter, offset += UINT32_C(16))
            {
    ...
    {
        // due to how the API was written and NVMe works, we need to call this instead to read all the keys. The get
        // reservations key count will only return 0 or 1 since there is at MOST 1 active reservation. But we need to go
        // through and find out how many keys are currently registered to loop through them later.
        uint16_t totalReservationKeys = UINT16_C(0);
        if (SUCCESS == get_Registration_Key_Count(device, &totalReservationKeys))
        {
            ...
            reservations->numberOfReservations = UINT16_C(0);
            if (SUCCESS == (ret = nvme_Reservation_Report(device, false, reservationKeys, reservationsLength)))
            {
                ...
                if (reservationKeys[4] > 0 && numberReservations > 0)
                {
                    ...
                    if (reservationKeys[4] >
                        0) // if this doesn't have an active registration right now, no need to loop through.
                    {
                        // loop through and find the reservatio nthat is currently active.
                        for (uint32_t reservationIter = UINT32_C(0), offset = UINT32_C(24);
                             reservationIter < reservationCount && offset < reservationsLength;
                             offset += UINT32_C(24), ++reservationIter)
                        {
                            if (reservationKeys[2] & BIT0)
                            {
                                ...
                                reservations->reservation[0].scope =
                                    RESERVATION_SCOPE_LOGICAL_UNIT; // scope is always zero on NVMe (at least for now)
                                switch (reservationKeys[4])
                                {
                                case 0:
                                    reservations->reservation[0].type =
                                                               ...
                                                               uint16_t* M_NONNULL      keyCount)
{
    ...
    *keyCount = UINT16_C(0);
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
        ...
        uint8_t* fullStatusData = C_CAST(uint8_t*, safe_calloc_aligned(fullStatusDataLength, sizeof(uint8_t),
                                                                       get_Device_IO_Minimum_Alignment(device)));
        ...
        if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3 &&
            SUCCESS == (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_FULL_STATUS,
                                                         ...
                                                         fullStatusData)))
        {
            // since the transport ID can vary in size, we cannot calculate this on length alone, so we need to re-read
            // with the full length of the data just reported and count them.
            ...
            fullStatusData = C_CAST(uint8_t*, safe_calloc_aligned(fullStatusDataLength, sizeof(uint8_t),
                                                                  get_Device_IO_Minimum_Alignment(device)));
            if (fullStatusData == M_NULLPTR)
            {
                return MEMORY_FAILURE;
                                                             ...
                                                             fullStatusData)))
            {
                ...
                uint32_t offsetAdditionalLength = UINT32_C(0);
                for (uint32_t offset = UINT32_C(8); offset < fullStatusDataLength;
                     offset += 24 + offsetAdditionalLength)
                {
...
typedef struct s_fullReservationKeyInfoV1
{
    uint64_t          key;
    bool              allTargetPorts;
    ...
    uint8_t transportID[24]; // NOTE: This is 24 bytes as that is the common size. iSCSI is variable in size, so it will
                             // be truncated in this case -TJE
} fullReservationKeyInfoV1;

#define FULL_RESERVATION_INFO_VERSION_V1 1

typedef struct s_fullReservationInfoV1
{
    size_t   size;
    uint32_t version;
    uint32_t generation;
                                                     ...
                                                     ptrFullReservationInfo M_NONNULL fullReservation)
{
    ...
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
                                                         ...
                                                         fullStatusData)))
        {
                                                             ...
                                                             fullStatusData)))
            {
                ...
                // each will be at least 24 bytes before the transport ID
                uint32_t offsetAdditionalLength = UINT32_C(0);
                for (uint32_t offset = UINT32_C(8), keyIter = UINT32_C(0);
                     offset < fullStatusDataLength && keyIter < numberOfKeys;
                     offset += UINT32_C(24) + offsetAdditionalLength, ++keyIter)
                {
                    offsetAdditionalLength =
                        M_BytesTo4ByteValue(fullStatusData[offset + 20], fullStatusData[offset + 21],
                                            fullStatusData[offset + 22], fullStatusData[offset + 23]);
        ...
        {
            // Older drive, or just doesn't support the full status capability, so we can read registrations and
            // reservations and then combine the two outputs into the expected full status results. read registrations
            // and reservations to match things up to the "equivalent" of the full status.
            uint16_t registrationCount = UINT16_C(0);
            uint16_t reservationCount  = UINT16_C(0);
            if (SUCCESS == get_Registration_Key_Count(device, &registrationCount) &&
                SUCCESS == get_Reservation_Count(device, &reservationCount))
            {
                // start with reading registrations first, then if the reservation count is > 0 read and map that
                // information too.
                ...
                {
                    // not sure which of these failed, but these macros should be safe to use to make sure we don't
                    // leave any memory out there
                ...
                {
                    // got both things we need to map to the full data as best we can...some parts may be omitted, but
                    // that's OK...it's the best we can do right now. registrations will hold all the keys. reservations
                    // will have a list of the keys with an active reservation. So match this to the registration key to
                    // set the r_holder bit and set scope and type...everything else will be left at zero first fill in
                    // the keys from registrations
                    fullReservation->generation = registrations->generation;
                    for (uint32_t keyIter = UINT32_C(0);
                         ...
                         ++keyIter)
                    {
                        ...
                        // check if this key matches any reported in the reservations data and add more fields from
                        // that...
                        for (uint32_t resKeyIter = UINT32_C(0);
    ...
    else if (get_Device_DriveType(device) == NVME_DRIVE)
    {
        ...
        uint8_t* nvmeFullData    = M_REINTERPRET_CAST(
            uint8_t*, safe_calloc_aligned(nvmeFullDataLen, sizeof(uint8_t), get_Device_IO_Minimum_Alignment(device)));
        ...
        if (SUCCESS == (ret = nvme_Reservation_Report(device, false, nvmeFullData, nvmeFullDataLen)))
        {
            ...
            for (uint32_t keyIter = UINT32_C(0), offset = UINT32_C(24); keyIter < keyCount && keyIter < numberOfKeys;
                 ++keyIter, offset += UINT32_C(24))
            {
                fullReservation->reservationKey[keyIter].relativeTargetPortIdentifier =
                ...
                if (fullReservation->reservationKey[keyIter].reservationHolder)
                {
                    // set the reservation scope and type...only 1 reservation allowed at a time per namespace, so check
                    // offset 4 for this information.
...
M_PARAM_RO(1) OPENSEA_OPERATIONS_API void show_Full_Status(ptrFullReservationInfo M_NONNULL fullReservation)
{
        ...
        fullReservation->size >= sizeof(fullReservationInfoV1))
    {
        ...
        for (uint32_t keyIter = UINT32_C(0); keyIter < UINT16_MAX && keyIter < fullReservation->numberOfKeys; ++keyIter)
        {
            ...
            {
                for (uint32_t transportIDoffset = UINT32_C(0);
...
typedef struct s_persistentReserveOutBasic
{
    ...
    uint8_t  transportID[1]; // overallocate to add transport IDs. Length is not needed in this data and is set from the
                             // parameter above
                              ...
                              ptrPersistentReserveOutBasic M_NONNULL basicInfo)
{
        ...
        basicInfo != M_NULLPTR) // 24 is minimum length for this buffer
    {
        ...
        if (dataLength > PR_OUT_BASIC_MIN_LENGTH && basicInfo->transportIDLength > 0 &&
            (basicInfo->transportIDLength +
             PR_OUT_BASIC_MIN_LENGTH /*length of basic data buffer before transport IDs*/ +
                                                  ...
                                                  bool                     ignoreExisting)
{
    ...
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
        ...
        // NOTE: Not currently supporting the specifiy initiator ports field and transport IDs, but the data formatter
        // does support creating that buffer.
    ...
    else if (get_Device_DriveType(device) == NVME_DRIVE)
    {
        ...
        // NOTE: SCSI translation mentions sending set features first if persist through power loss is not already
        // activated...not sure if this is needed or not in this case since translation is not being used...don't really
...
M_PARAM_RO(1)
OPENSEA_OPERATIONS_API eReturnValues unregister_Key(const tDevice* M_NONNULL device, uint64_t currentRegistrationKey)
{
    eReturnValues ret = NOT_SUPPORTED;
    if (get_Device_DriveType(device) == SCSI_DRIVE)
    {
                                                         ...
                                                         eReservationType         resType)
{
    ...
    else if (get_Device_DriveType(device) == NVME_DRIVE)
    {
        ...
        uint8_t nvmeReservationType = UINT8_C(0);
...
// TODO: register and move: Requires transport ID which there is not a good way to deal with right now with how these
// tools work-TJE
