...
//
// Copyright (c) 2021-2024 Seagate Technology LLC and/or its Affiliates, All
// Rights Reserved
//
...
// ******************************************************************************************
//
// \file reservations.c   SCSI reservations based functions. NOTE: THis is
// SCSI-3 persistent reservations only at this time - TJE

// NOTE: Old reservations from SCSI2 (and up to SPC2) are not currently
// supported. These are much more limited than what is available in the
// Persistent Reservations feature NOTE: Some additional enhancement for NVMe is
// probably possible. The current NVMe implementation is more-or-less what is
// part of NVMe to SCSI translation. - TJE
...
bool is_Persistent_Reservations_Supported(tDevice* device)
{
    ...
    {
        // try a persistent reserve in with no data transferred. This SHOULD not
        // return an error if this feature is supported - TJE
    ...
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        // NS identify says which types of reservations are supported...should have
        // both non-zero if supported - TJE
...
{
    bool readShared; // marked as reserved in spec...but pretty sure it was
                     // reserved for this
    bool writeExclusive;
    bool readExclusive; // marked as reserved in spec...but pretty sure it was
                        // reserved for this
    bool exclusiveAccess;
    bool sharedAccess; // marked as reserved in spec...but pretty sure it was
                       // reserved for this
    ...
    // remaining are reserved, but making each a bool indicating SCSI mode number
    // in case it expands in the future
    bool reserved9h;
    bool reservedAh;
    bool reservedBh;
    bool reservedCh;
...
{
    uint8_t allowedCommandsRawValue; // 0 - 7 as reported by the drive in case the
                                     // remaining info is not useful enough
...
typedef struct _persistentReservationCapabilitiesV1
{
    ...
    bool     compatibleReservationHandling; // scsi2 reservation reelease/reserve
                                            // exceptions
    ...
    bool              reservationTypesSupportedValid; // If set to true, the device reported the
                                                      // type mask indicating which reservation
                                                      // types are supported (below)
...
{
    // note: some older drives don't support report capabilities...need to figure
    // out what to do about those - TJE
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS ==
            (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_REPORT_CAPABILITIES, 8, capabilities)))
        {
            ...
            if (prCapabilitiesLength >= 8)
            {
                ...
                case 0:
                    ...
                    prCapabilities->allowedCommandsInfo.reportSupportedTaskManagementFunctions =
                ...
                case 5:
                    ...
                    prCapabilities->allowedCommandsInfo.readBuffer10             = RES_CMD_PERSIST_ALLOWED_WE;
                    prCapabilities->allowedCommandsInfo.receiveDiagnosticResults = RES_CMD_PERSIST_ALLOWED_WE;
                    prCapabilities->allowedCommandsInfo.reportSupportedOperationCodes =
    ...
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        prCapabilities->replaceLostReservationCapable =
            true; // not in NVMe translation, but seems to be part of NVMe spec
        prCapabilities->compatibleReservationHandling = false;
        prCapabilities->specifyInitiatorPortCapable   = false;
        prCapabilities->allTargetPortsCapable         = true;
        prCapabilities->persistThroughPowerLossCapable =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT0);
        // need to do get features to figure out if persist through power loss
        // activated is true or false
        ...
        prCapabilities->reservationsCapabilities.writeExclusive =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT1);
        prCapabilities->reservationsCapabilities.readExclusive = false;
        prCapabilities->reservationsCapabilities.exclusiveAccess =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT2);
        prCapabilities->reservationsCapabilities.sharedAccess = false;
        prCapabilities->reservationsCapabilities.writeExclusiveRegistrantsOnly =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT3);
        prCapabilities->reservationsCapabilities.exclusiveAccessRegistrantsOnly =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT4);
        prCapabilities->reservationsCapabilities.writeExclusiveAllRegistrants =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT5);
        prCapabilities->reservationsCapabilities.exclusiveAccessAllRegistrants =
            M_ToBool(device->drive_info.IdentifyData.nvme.ns.rescap & BIT6);
        ...
        {
            // in dw0 completion
            prCapabilities->persistThroughPowerLossActivated =
                M_ToBool(device->drive_info.lastNVMeResult.lastNVMeCommandSpecific & BIT0);
        }
    }
    return ret;
}
    ...
    if (prCapabilities)
    {
             ...
             prCapabilities->size >= sizeof(persistentReservationCapabilitiesV1)))
        {
            ...
            if (prCapabilities->allowedCommandsInfo.allowedCommandsRawValue <
                6) // restricted like this since this is reserved at the time of
                   // writing this code - TJE
            {
            ...
            {
                printf("\tAllowed Commands: Not Reportable. Reserved value reported: "
                       "%" PRIu8 "\n",
        ...
        {
            printf("Error: Incorrect reservations capabilities structure version or "
                   "bad structure size.\n");
        }
    }
    return;
...
eReturnValues get_Registration_Key_Count(tDevice* device, uint16_t* keyCount)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS ==
            (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_KEYS, 8, readKeyCount)))
        {
            *keyCount = C_CAST(uint16_t,
                               M_BytesTo4ByteValue(readKeyCount[4], readKeyCount[5], readKeyCount[6], readKeyCount[7]) /
                                   UINT32_C(8)); // each registered key is 8 bytes in length
        }
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        DECLARE_ZERO_INIT_ARRAY(uint8_t, readKeyCount,
                                24); // may be able to get away with only 8 bytes,
                                     // but this read up until the list begins - TJE
...
typedef struct _registrationKeysDataV1
{
    size_t   size;
    uint32_t version;
    uint32_t generation;         // counter that updates each time new registration is
                                 // added or removed
    uint16_t numberOfKeys;       // number of keys reported below
    uint64_t registrationKey[1]; // This is variable sized depending on how many
                                 // are requested to be read and how many are
                                 // filled in when read.
...
eReturnValues get_Registration_Keys(tDevice* device, uint16_t numberOfKeys, ptrRegistrationKeysData keys)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        uint8_t* registrationKeys =
            C_CAST(uint8_t*, safe_calloc_aligned(dataLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!registrationKeys)
        {
            return MEMORY_FAILURE;
        }
        if (SUCCESS == (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_KEYS, dataLength,
                                                         registrationKeys)))
        {
            uint16_t reportedKeys = C_CAST(uint16_t, M_BytesTo4ByteValue(registrationKeys[4], registrationKeys[5],
                                                                         registrationKeys[6], registrationKeys[7]) /
                                                         UINT32_C(8)); // each registered key is 8 bytes in length
            keys->generation =
                M_BytesTo4ByteValue(registrationKeys[0], registrationKeys[1], registrationKeys[2], registrationKeys[3]);
            // loop through and save each key
            keys->numberOfKeys = 0;
            for (uint32_t keyIter = 0, offset = 8;
                 keyIter < reportedKeys && keyIter < numberOfKeys && offset < dataLength;
                 ++keys->numberOfKeys, ++keyIter, offset += 8)
            {
                keys->registrationKey[keyIter] = M_BytesTo8ByteValue(
                    registrationKeys[offset + 0], registrationKeys[offset + 1], registrationKeys[offset + 2],
                    registrationKeys[offset + 3], registrationKeys[offset + 4], registrationKeys[offset + 5],
                    registrationKeys[offset + 6], registrationKeys[offset + 7]);
            }
        }
    ...
    {
        uint32_t dataLength = (C_CAST(uint32_t, numberOfKeys) * UINT32_C(24)) +
                              UINT32_C(24); // 24 byte header, then 24 bytes per key....if extended,
                                            // then it is even larger.
        uint8_t* registrationKeys =
            C_CAST(uint8_t*, safe_calloc_aligned(dataLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!registrationKeys)
        {
        ...
        if (SUCCESS == (ret = nvme_Reservation_Report(device, false, registrationKeys, dataLength)))
        {
            ...
            keys->generation =
                M_BytesTo4ByteValue(registrationKeys[3], registrationKeys[2], registrationKeys[1], registrationKeys[0]);
            // loop through and save each key
            for (uint32_t keyIter = 0, offset = 24;
                 keyIter < reportedKeys && keyIter < numberOfKeys && offset < dataLength;
                 ++keys->numberOfKeys, ++keyIter, offset += 24)
            {
                keys->registrationKey[keyIter] = M_BytesTo8ByteValue(
                    registrationKeys[offset + 23], registrationKeys[offset + 22], registrationKeys[offset + 21],
                    registrationKeys[offset + 20], registrationKeys[offset + 19], registrationKeys[offset + 18],
                    registrationKeys[offset + 17], registrationKeys[offset + 16]);
            }
        }
        safe_free_aligned(&registrationKeys);
    }
...
// If supporting "extents", multiple can be reported, but this capability is
// obsolete, so this will likely return 1 or 0
eReturnValues get_Reservation_Count(tDevice* device, uint16_t* reservationKeyCount)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS ==
            (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_RESERVATION, 8, reservationKeys)))
        {
            *reservationKeyCount = C_CAST(uint16_t, M_BytesTo4ByteValue(reservationKeys[4], reservationKeys[5],
                                                                        reservationKeys[6], reservationKeys[7]) /
                                                        UINT32_C(16));
        }
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
...
typedef struct _reservationsDataV1
{
    ...
    uint32_t version;
    uint32_t generation;
    uint16_t numberOfReservations;    // will most likely be 0 or 1 since element and
                                      // extent types are obsolete.
    reservationInfoV1 reservation[1]; // variable length depending on how it was allocated.
                                      // Should always be AT LEAST one of these
...
eReturnValues get_Reservations(tDevice* device, uint16_t numberReservations, ptrReservationsData reservations)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS == (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_RESERVATION,
                                                         reservationsLength, reservationKeys)))
        {
            uint16_t reservationKeyCount =
                C_CAST(uint16_t, M_BytesTo4ByteValue(reservationKeys[4], reservationKeys[5], reservationKeys[6],
                                                     reservationKeys[7]) /
                                     UINT32_C(16));
            reservations->generation =
                M_BytesTo4ByteValue(reservationKeys[0], reservationKeys[1], reservationKeys[2], reservationKeys[3]);
            for (uint32_t reservationIter = 0, offset = 8;
                 reservationIter < reservationKeyCount && reservationIter < numberReservations &&
                 offset < reservationsLength;
                 ++reservations->numberOfReservations, ++reservationIter, offset += 16)
            {
                reservations->reservation[reservationIter].reservationKey = M_BytesTo8ByteValue(
                    reservationKeys[offset + 0], reservationKeys[offset + 1], reservationKeys[offset + 2],
                    reservationKeys[offset + 3], reservationKeys[offset + 4], reservationKeys[offset + 5],
                    reservationKeys[offset + 6], reservationKeys[offset + 7]);
                reservations->reservation[reservationIter].scopeSpecificAddress = M_BytesTo4ByteValue(
                    reservationKeys[offset + 8], reservationKeys[offset + 9], reservationKeys[offset + 10],
                    reservationKeys[offset + 11]); // obsolete on newer drives
                reservations->reservation[reservationIter].extentLength =
                    M_BytesTo2ByteValue(reservationKeys[offset + 14],
                                        reservationKeys[offset + 15]); // obsolete on newer drives
    ...
    {
        // due to how the API was written and NVMe works, we need to call this
        // instead to read all the keys. The get reservations key count will only
        // return 0 or 1 since there is at MOST 1 active reservation. But we need to
        // go through and find out how many keys are currently registered to loop
        // through them later.
        ...
        if (SUCCESS == get_Registration_Key_Count(device, &totalReservationKeys))
        {
            ...
            {
                reservations->generation =
                    M_BytesTo4ByteValue(reservationKeys[3], reservationKeys[2], reservationKeys[1], reservationKeys[0]);
                if (reservationKeys[4] > 0 && numberReservations > 0)
                {
                    ...
                    if (reservationKeys[4] > 0) // if this doesn't have an active registration right now, no
                                                // need to loop through.
                    {
                        // loop through and find the reservatio nthat is currently active.
                        for (uint32_t reservationIter = 0, offset = 24;
                             reservationIter < reservationCount && offset < reservationsLength;
                             offset += 24, ++reservationIter)
                        {
                            if (reservationKeys[2] & BIT0)
                            {
                                reservations->reservation[0].reservationKey =
                                    M_BytesTo8ByteValue(reservationKeys[offset + 23], reservationKeys[offset + 22],
                                                        reservationKeys[offset + 21], reservationKeys[offset + 20],
                                                        reservationKeys[offset + 19], reservationKeys[offset + 18],
                                                        reservationKeys[offset + 17], reservationKeys[offset + 16]);
                                reservations->numberOfReservations = 1;
                                reservations->reservation[0].scope =
                                    RESERVATION_SCOPE_LOGICAL_UNIT; // scope is always zero on
                                                                    // NVMe (at least for now)
                                switch (reservationKeys[4])
                                {
                                case 0:
                                    reservations->reservation[0].type = RES_TYPE_NO_RESERVATION; // or unknown? THis is
                                                                                                 // reserved in NVMe
                                    break;
                                case 1:
...
eReturnValues get_Full_Status_Key_Count(tDevice* device, uint16_t* keyCount)
{
    ...
    *keyCount = 0;
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        uint32_t fullStatusDataLength = 32; // some drive FW have a bug where if this is read as 8 bytes, it
        ...
        // check SCSI version and fall back if "new enough" but command doesn't
        // complete successfully???
        if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3 &&
            SUCCESS == (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_FULL_STATUS,
                                                         C_CAST(uint16_t, M_Min(fullStatusDataLength, UINT16_MAX)),
                                                         fullStatusData)))
        {
            // since the transport ID can vary in size, we cannot calculate this on
            // length alone, so we need to re-read with the full length of the data
            // just reported and count them.
            fullStatusDataLength =
                8 + M_BytesTo4ByteValue(fullStatusData[4], fullStatusData[5], fullStatusData[6], fullStatusData[7]);
            ...
            // reread the data
            if (SUCCESS == (ret = scsi_Persistent_Reserve_In(device, SCSI_PERSISTENT_RESERVE_IN_READ_FULL_STATUS,
                                                             C_CAST(uint16_t, M_Min(fullStatusDataLength, UINT16_MAX)),
                                                             fullStatusData)))
            {
                ...
                {
                    offsetAdditionalLength =
                        M_BytesTo4ByteValue(fullStatusData[offset + 20], fullStatusData[offset + 21],
                                            fullStatusData[offset + 22], fullStatusData[offset + 23]);
                    ++(*keyCount);
                }
            }
        }
...
typedef struct _fullReservationKeyInfoV1
{
    uint64_t key;
    bool     allTargetPorts;
    bool     reservationHolder; // NOTE: This must be set to true for scope and type
                                // to be valid
    uint16_t          relativeTargetPortIdentifier;
    eReservationScope scope;
    eReservationType  type;
    uint32_t          transportIDLength;
    uint8_t           transportID[24]; // NOTE: This is 24 bytes as that is the common size.
                                       // iSCSI is variable in size, so it will be truncated
                                       // in this case -TJE
...
typedef struct _fullReservationInfoV1
{
    ...
    fullReservationKeyInfoV1 reservationKey[1]; // Variable size depending on how many will be reported
...
eReturnValues get_Full_Status(tDevice* device, uint16_t numberOfKeys, ptrFullReservationInfo fullReservation)
{
    ...
    // If older SPC, use the get_Registrations and get_Reservations functions to
    // get all the data we need to collect. - TJE
    eReturnValues ret = NOT_SUPPORTED;
    if (!fullReservation)
    {
        return BAD_PARAMETER;
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
                                                         ...
                                                         fullStatusData)))
        {
                                                             ...
                                                             fullStatusData)))
            {
                fullReservation->generation =
                    M_BytesTo4ByteValue(fullStatusData[0], fullStatusData[1], fullStatusData[2], fullStatusData[3]);

                // loop through each descriptor to count them.
                // each will be at least 24 bytes before the transport ID
                uint32_t offsetAdditionalLength = 0;
                for (uint32_t offset = 8, keyIter = 0; offset < fullStatusDataLength && keyIter < numberOfKeys;
                     offset += 24 + offsetAdditionalLength, ++keyIter)
                {
                    offsetAdditionalLength =
                        M_BytesTo4ByteValue(fullStatusData[offset + 20], fullStatusData[offset + 21],
                                            fullStatusData[offset + 22], fullStatusData[offset + 23]);
                    ...
                    fullReservation->reservationKey[keyIter].key = M_BytesTo8ByteValue(
                        fullStatusData[offset + 0], fullStatusData[offset + 1], fullStatusData[offset + 2],
                        fullStatusData[offset + 3], fullStatusData[offset + 4], fullStatusData[offset + 5],
                        fullStatusData[offset + 6], fullStatusData[offset + 7]);
                    fullReservation->reservationKey[keyIter].allTargetPorts =
                        M_ToBool(fullStatusData[offset + 12] & BIT1);
                    fullReservation->reservationKey[keyIter].reservationHolder =
                    ...
                    if (offsetAdditionalLength > 0)
                    {
                        ...
                        memcpy(fullReservation->reservationKey[keyIter].transportID, &fullStatusData[offset + 24],
                               M_Min(24, offsetAdditionalLength));
                    }
                    else
                    {
        ...
        {
            // Older drive, or just doesn't support the full status capability, so we
            // can read registrations and reservations and then combine the two
            // outputs into the expected full status results. read registrations and
            // reservations to match things up to the "equivalent" of the full status.
            uint16_t registrationCount = 0;
            uint16_t reservationCount  = 0;
            if (SUCCESS == get_Registration_Key_Count(device, &registrationCount) &&
                SUCCESS == get_Reservation_Count(device, &reservationCount))
            {
                // start with reading registrations first, then if the reservation count
                // is > 0 read and map that information too.
                ptrRegistrationKeysData registrations = C_CAST(
                    ptrRegistrationKeysData,
                    safe_calloc(sizeof(registrationKeysData) + registrationCount * sizeof(uint64_t), sizeof(uint8_t)));
                ptrReservationsData reservations =
                    C_CAST(ptrReservationsData,
                           safe_calloc(sizeof(reservationsData) + reservationCount * sizeof(reservationInfo),
                                       sizeof(uint8_t)));
                if (!registrations || !reservations)
                {
                    // not sure which of these failed, but these macros should be safe to
                    // use to make sure we don't leave any memory out there
                ...
                if (SUCCESS == (ret = get_Registration_Keys(device, registrationCount, registrations)) &&
                    SUCCESS == (ret = get_Reservations(device, reservationCount, reservations)))
                {
                    // got both things we need to map to the full data as best we
                    // can...some parts may be omitted, but that's OK...it's the best we
                    // can do right now. registrations will hold all the keys.
                    // reservations will have a list of the keys with an active
                    // reservation. So match this to the registration key to set the
                    // r_holder bit and set scope and type...everything else will be left
                    // at zero first fill in the keys from registrations
                    fullReservation->generation = registrations->generation;
                    for (uint32_t keyIter = 0;
                         keyIter < registrationCount && keyIter < registrations->numberOfKeys && keyIter < numberOfKeys;
                         ++keyIter)
                    {
                        ...
                        // initialize the following fields before we check the reservations
                        // data
                        ...
                        // check if this key matches any reported in the reservations data
                        // and add more fields from that...
                        for (uint32_t resKeyIter = 0;
                             resKeyIter < reservationCount && resKeyIter < reservations->numberOfReservations;
                             ++resKeyIter)
                        {
                            if (registrations->registrationKey[keyIter] ==
                                reservations->reservation[resKeyIter].reservationKey)
                            {
                                // found a matching key, so update remaining fields.
                                fullReservation->reservationKey[keyIter].reservationHolder = true;
                                fullReservation->reservationKey[keyIter].scope =
                                    reservations->reservation[resKeyIter].scope;
                                fullReservation->reservationKey[keyIter].type =
                                    reservations->reservation[resKeyIter].type;
                                break;
                            }
                        }
                        ++(fullReservation->numberOfKeys);
    ...
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        uint8_t* nvmeFullData =
            C_CAST(uint8_t*, safe_calloc_aligned(nvmeFullDataLen, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!nvmeFullData)
        {
            return MEMORY_FAILURE;
        }
        if (SUCCESS == (ret = nvme_Reservation_Report(device, false, nvmeFullData, nvmeFullDataLen)))
        {
            ...
            fullReservation->generation =
                M_BytesTo4ByteValue(nvmeFullData[3], nvmeFullData[2], nvmeFullData[1], nvmeFullData[0]);
            for (uint32_t keyIter = 0, offset = 24; keyIter < keyCount && keyIter < numberOfKeys;
                 ++keyIter, offset += 24)
            {
                fullReservation->reservationKey[keyIter].relativeTargetPortIdentifier =
                    M_BytesTo2ByteValue(nvmeFullData[offset + 1], nvmeFullData[offset + 2]);
                fullReservation->reservationKey[keyIter].scope =
                    RESERVATION_SCOPE_LOGICAL_UNIT; // all nvme scops are like this
                ...
                if (fullReservation->reservationKey[keyIter].reservationHolder)
                {
                    // set the reservation scope and type...only 1 reservation allowed at
                    // a time per namespace, so check offset 4 for this information.
                    switch (nvmeFullData[4])
                    {
                    case 0:
                        fullReservation->reservationKey[keyIter].type =
                            RES_TYPE_NO_RESERVATION; // or unknown? THis is reserved in NVMe
                        break;
                    case 1:
                        fullReservation->reservationKey[keyIter].type = RES_TYPE_WRITE_EXCLUSIVE;
                        break;
                ...
                fullReservation->reservationKey[keyIter].key =
                    M_BytesTo8ByteValue(nvmeFullData[offset + 23], nvmeFullData[offset + 22], nvmeFullData[offset + 21],
                                        nvmeFullData[offset + 20], nvmeFullData[offset + 19], nvmeFullData[offset + 18],
                                        nvmeFullData[offset + 17], nvmeFullData[offset + 16]);
            }
        }
...
{
    if (fullReservation && fullReservation->version >= FULL_RESERVATION_INFO_VERSION_V1 &&
        fullReservation->size >= sizeof(fullReservationInfoV1))
    {
        printf("Full Reservation Status:\n");
        printf("\tGeneration: %" PRIX32 "h\n", fullReservation->generation);

        printf("      Key        | ATP | Res Holder | Scope |         Type         "
               "|  RTPID  | Transport ID \n");
        for (uint32_t keyIter = 0; keyIter < UINT16_MAX && keyIter < fullReservation->numberOfKeys; ++keyIter)
        {
            ...
            {
                for (uint32_t transportIDoffset = 0;
                     transportIDoffset < 24 &&
                     transportIDoffset <= fullReservation->reservationKey[keyIter].transportIDLength;
                     ++transportIDoffset)
                {
...
typedef struct _persistentReserveOutBasic
{
    ...
    uint8_t  transportID[1];    // overallocate to add transport IDs. Length is not needed
                                // in this data and is set from the parameter above
...
static void format_Basic_Info(uint8_t* ptrData, uint32_t dataLength, ptrPersistentReserveOutBasic basicInfo)
{
    if (ptrData && dataLength >= PR_OUT_BASIC_MIN_LENGTH && basicInfo) // 24 is minimum length for this buffer
    {
        ...
        if (dataLength > PR_OUT_BASIC_MIN_LENGTH && basicInfo->transportIDLength > 0 &&
            (basicInfo->transportIDLength + PR_OUT_BASIC_MIN_LENGTH /*length of basic data buffer before transport
                                                                       IDs*/
             + 4 /*for the length that is set before the transport ids start */) <= dataLength)
        {
                           ...
                           bool     ignoreExisting)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // NOTE: Not currently supporting the specifiy initiator ports field and
        // transport IDs, but the data formatter does support creating that buffer.

        format_Basic_Info(registerData, PR_OUT_BASIC_MIN_LENGTH, &prData);
        ret = scsi_Persistent_Reserve_Out(device,
                                          ignoreExisting ? SCSI_PERSISTENT_RESERVE_OUT_REGISTER_AND_IGNORE_EXISTING_KEY
                                                         : SCSI_PERSISTENT_RESERVE_OUT_REGISTER,
                                          0, 0, PR_OUT_BASIC_MIN_LENGTH, registerData);
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        // NOTE: SCSI translation mentions sending set features first if persist
        // through power loss is not already activated...not sure if this is needed
        // or not in this case since translation is not being used...don't really
        // see it in the spec- TJE
        ret = nvme_Reservation_Register(device, persistThroughPowerLoss ? 3 : 2, ignoreExisting, 0, registerData,
                                        16); // TODO: Move values to enums/definitions
    }
    return ret;
}
...
eReturnValues unregister_Key(tDevice* device, uint64_t currentRegistrationKey)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        format_Basic_Info(registerData, PR_OUT_BASIC_MIN_LENGTH, &prData);
        ret = scsi_Persistent_Reserve_Out(device, SCSI_PERSISTENT_RESERVE_OUT_REGISTER, 0, 0, PR_OUT_BASIC_MIN_LENGTH,
                                          registerData);
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        ret             = nvme_Reservation_Register(device, 0, 0, 1, registerData,
                                                    16); // TODO: Move values to enums/definitions
    }
    return ret;
}
...
eReturnValues acquire_Reservation(tDevice* device, uint64_t key, eReservationType resType)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        ret = scsi_Persistent_Reserve_Out(device, SCSI_PERSISTENT_RESERVE_OUT_RESERVE, 0, scsiReservationType,
                                          PR_OUT_BASIC_MIN_LENGTH, acquireRes);
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
...
eReturnValues release_Reservation(tDevice* device, uint64_t key, eReservationType resType)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        ret = scsi_Persistent_Reserve_Out(device, SCSI_PERSISTENT_RESERVE_OUT_RELEASE, 0, scsiReservationType,
                                          PR_OUT_BASIC_MIN_LENGTH, releaseRes);
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
                                  ...
                                  eReservationType resType)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        ret = scsi_Persistent_Reserve_Out(
            device, abort ? SCSI_PERSISTENT_RESERVE_OUT_PREEMPT_AND_ABORT : SCSI_PERSISTENT_RESERVE_OUT_PREEMPT, 0,
            scsiReservationType, PR_OUT_BASIC_MIN_LENGTH, preemptRes);
    }
    else if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        ret = nvme_Reservation_Acquire(device, nvmeReservationType, false,
                                       abort ? 2 /*preempt & abort*/ : 1 /*preempt*/, preemptRes, 16);
    }
    return ret;
}

// TODO: replace lost reservation
// TODO: register and move: Requires transport ID which there is not a good way
// to deal with right now with how these tools work-TJE
