...
// \file ata_Security.c
// \brief This file defines the function calls for performing some ATA Security
// operations
...
bool sat_ATA_Security_Protocol_Supported(tDevice* device)
{
    ...
    // For non-ATA/IDE interfaces, we need to check if the translator (SATL)
    // supports the ATA security protocol.
    if (device->drive_info.interface_type != IDE_INTERFACE)
    {
                                                ...
                                                securityBuf))
        {
            ...
            for (; (bufIter - 8) < length && bufIter < LEGACY_DRIVE_SEC_SIZE; ++bufIter)
            {
                ...
                {
                    // the supported list shows this protocol, but try reading the page
                    // too...if that fails then we know it's only a partial
                    // implementation.
                    ...
                    {
                        if (ataSecurityInfo[1] == 0x0E) // Checking that the length matches to make sure we got a
                                                        // good response
                        {
                            supported = true;
                        }
...
{
    if (useSAT) // if SAT ATA security supported, use it so the SATL manages the
                // erase.
    {
    ...
    else if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word128) &&
            device->drive_info.IdentifyData.ata.Word128 & BIT0)
        {
            securityStatus->securitySupported = true;
            ...
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word089))
            {
                if (device->drive_info.IdentifyData.ata.Word089 & BIT15)
                {
                    ...
                    // bits 14:0
                    securityStatus->securityEraseUnitTimeMinutes =
                        (device->drive_info.IdentifyData.ata.Word089 & 0x7FFF) * ATA_SECURITY_TIME_MULTIPLIER;
                    if (securityStatus->securityEraseUnitTimeMinutes ==
                        (ATA_SECURITY_GREATER_THAN_MAX_EXTENDED_TIME_VALUE * ATA_SECURITY_TIME_MULTIPLIER))
                    {
                        securityStatus->securityEraseUnitTimeMinutes = UINT16_MAX;
                ...
                else
                {
                    // bits 7:0
                    securityStatus->securityEraseUnitTimeMinutes =
                        M_Byte0(device->drive_info.IdentifyData.ata.Word089) * ATA_SECURITY_TIME_MULTIPLIER;
                    if (securityStatus->securityEraseUnitTimeMinutes ==
            ...
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word090))
            {
                if (device->drive_info.IdentifyData.ata.Word090 & BIT15)
                {
                    ...
                    // bits 14:0
                    securityStatus->enhancedSecurityEraseUnitTimeMinutes =
                        (device->drive_info.IdentifyData.ata.Word090 & 0x7FFF) * ATA_SECURITY_TIME_MULTIPLIER;
                    if (securityStatus->enhancedSecurityEraseUnitTimeMinutes ==
                        (ATA_SECURITY_GREATER_THAN_MAX_EXTENDED_TIME_VALUE * ATA_SECURITY_TIME_MULTIPLIER))
                    {
                        securityStatus->enhancedSecurityEraseUnitTimeMinutes = UINT16_MAX;
                ...
                else
                {
                    // bits 7:0
                    securityStatus->enhancedSecurityEraseUnitTimeMinutes =
                        M_Byte0(device->drive_info.IdentifyData.ata.Word090) * ATA_SECURITY_TIME_MULTIPLIER;
                    if (securityStatus->enhancedSecurityEraseUnitTimeMinutes ==
                        (ATA_SECURITY_GREATER_THAN_MAX_TIME_VALUE * ATA_SECURITY_TIME_MULTIPLIER))
                    {
                        securityStatus->enhancedSecurityEraseUnitTimeMinutes = UINT16_MAX;
    ...
    // read ID data log page for security bits to get
    // restrictedSanitizeOverridesSecurity bit
    if (device->drive_info.drive_type == ATA_DRIVE && device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        ...
        {
            if (M_BytesTo2ByteValue(securityPage[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2) + 1],
                                    securityPage[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2)]) *
                    512 >
                0)
            {
                memset(&securityPage, 0, 512);
                // IDData log suppored. Read first page to see if security subpage (06h)
                // is supported
...
static void print_ATA_Security_Password(ptrATASecurityPassword ataPassword)
{
    if (ataPassword)
    {
        ...
        {
            // now we need to check if it's a printable string or not and print it
            // between quotes if it is, otherwise as a sequence of hex bytes
            ...
            for (uint8_t iter = 0; iter < ataPassword->passwordLength && iter < ATA_SECURITY_MAX_PW_LENGTH; ++iter)
            {
                if (safe_iscntrl(ataPassword->password[iter]) ||
                    ataPassword->password[iter] & BIT7) // this function is the opposite of isPrint and should
                                                        // work for what we want. We also want to check if bit 7
                                                        // is set
                {
                    isASCIIString = false;
                    break;
                                         ...
                                         bool                   useSAT)
{
    ...
    {
        // copy the password in, but the max length is 32 bytes according to the
        // spec!
        memcpy(&ptrData[2], ataPassword->password, M_Min(ataPassword->passwordLength, ATA_SECURITY_MAX_PW_LENGTH));
        if (setPassword) // if setting the password in the set password command, we
                         // need to set a few other things up
        {
        ...
        if (ataPassword->passwordType == ATA_PASSWORD_MASTER)
        {
            ...
            ptrData[0] |= BIT0;
            if (setPassword) // if setting the password in the set password command,
                             // we need to set a few other things up
            {
                // set the master password identifier.
                // Since this is ATA, this is little endian format.
                // Word 17
                // NOTE: SAT uses SCSI big endian format whereas ATA uses little endian,
                // so need to swap this.
                //       May need SATL specific workarounds if this is not copied and
                //       sent to the drive correctly
                if (useSAT)
                {
...
void set_ATA_Security_Erase_Type_In_Buffer(uint8_t* ptrData, eATASecurityEraseType eraseType, bool useSAT)
{
    if (ptrData)
    {
        if (eraseType == ATA_SECURITY_ERASE_ENHANCED_ERASE)
        {
            if (useSAT)
            {
                // NOTE: SAT spec has this bit in a different place so a memcpy will NOT
                // work on this buffer unlike all other SAT
                //       security protocol buffers will allow.
                //       Because of this it is possible a SATL is not implemented
                //       correctly and not setting the enhanced erase bit correctly.
                //       This code will set this bit per the SAT spec, however device
                //       unique workarounds may be necessary in the future. This
                //       filtering should be based off of only known translators with
                //       errors when they are discovered.
                ptrData[0] |= BIT0;
            }
            else
            {
                                                ...
                                                bool                forceSAT)
{
                ...
                else
                {
                    if (securityStatus.securityLocked)
                    {
                        if (securityStatus.securityCountExpired)
                        {
                            if (VERBOSITY_QUIET < device->deviceVerbosity)
                            {
                                printf("Password attempts exceeded. You must power cycle the "
                                       "drive to clear the attempt counter and retry the "
                                       "operation.\n");
                            }
                            return FAILURE;
                        }
                    ...
                    // now check security locked again because the above if statement
                    // should change it if the unlock was successful
                    if (!securityStatus.securityLocked)
                    {
    ...
    else // this is ATA specific and there's nothing to do on other drives since
...
eReturnValues run_Freeze_ATA_Security(tDevice* device, bool forceSATvalid, bool forceSAT)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE || satATASecuritySupported)
    {
        ...
        if (securityStatus.securitySupported)
        {
            if (satATASecuritySupported) // if SAT ATA security supported, use it so
                                         // the SATL manages the commands.
            {
                ret = scsi_SecurityProtocol_Out(device, SECURITY_PROTOCOL_ATA_DEVICE_SERVER_PASSWORD,
                                            ...
                                            bool                forceSAT)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE || satATASecuritySupported)
    {
        ...
        if (securityStatus.securitySupported)
        {
            ...
            {
                // ATA security is already enabled. The password must be disabled before
                // a new one is set (for user password) Master password should be able
                // to be set.
                if (VERBOSITY_QUIET < device->deviceVerbosity)
                {
                    printf("Security is Locked. Cannot set a password without unlocking "
                           "or erasing the device (with the master password).\n");
                }
                ret = FAILURE;
            }
            else
                                     ...
                                     bool                  forceSAT)
{
    ...
    {
        // this will catch nvme drives that support this protocol (for some reason
        // it was implemented by some vendors)
    ...
    if (forceSATvalid)
    {
        // this allows forcing the security protocol commands instead of SAT
        // passthrough
        satATASecuritySupported = forceSAT;
    }
    ...
    if (securityStatus.securityCountExpired)
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            printf("Password attempts exceeded. You must power cycle the drive to "
                   "clear the attempt counter and retry the operation.\n");
        }
        return FAILURE;
    }
    // ATA spec shows you can erase without unlocking the drive.
    // We may want to put this back in as a "just in case" or a "try it to see if
    // the password was right" though.
    //      if (securityStatus.securityLocked && ataPassword.passwordType !=
    //      ATA_PASSWORD_MASTER)//master shouldn't need to unlock since it can be
    //      used to repurpose the drive
    //      {
    //          if (VERBOSITY_QUIET < device->deviceVerbosity)
    //          {
    //              printf("Attempting to unlock security with password = ");
    //              print_ATA_Security_Password(&ataPassword);
    //          }
    //          if (SUCCESS == unlock_ATA_Security(device, ataPassword,
    //          satATASecuritySupported))
    //          {
    //              securityStatus.securityLocked = false;
    //          }
    //          else
    ...
    //          }
    //      }
    if (!securityStatus.securityEnabled) // ATA spec is not clear on whether an erase can be
                                         // done at anytime with the master PW or not, so
                                         // we'll try to set one for the erase.
    {
        // set the password
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
    ...
    if (VERBOSITY_QUIET < device->deviceVerbosity)
    {
        ...
        else
        {
            ...
            printf("from now.\n");
            memset(timeFormat, 0,
                   TIME_STRING_LENGTH); // clear this again before reusing it
        ...
        printf("\tas this will leave it in an uninitialized state with the "
               "password set.\n");
    ...
    // before we read the bitfield again...try requesting sense data to see if
    // that says there was a reset on the bus. (6h/29h/00h)
    bool hostResetDuringErase = false;
    if (!satATASecuritySupported) // Only do the code below if we aren't using the
                                  // SAT security protocol to perform the erase.
    {
        // If the host issued a reset, we may not get back correct status, but the
        // status after the reset. After talking to a firmware engineer, a status of
        // 50h and error of 01h would be what we could see. I added this check as a
        // test to set the "host reset the drive" message at the end of the
        // operation. - TJE
        if (device->drive_info.lastCommandRTFRs.status == 0x50 && device->drive_info.lastCommandRTFRs.error == 0x01)
        {
            // ataEraseResult = FAILURE; //This is commented out because I'm not sure
            // this is the right place to do this... The code that checks the security
            // bits should still catch a failure. - TJE
...
#endif
    {
        ...
        {
            printf("ATA Security Validate Erase Completion, validate completion "
                   "buffer:\n");
        ...
        if (device->deviceVerbosity >= VERBOSITY_BUFFERS)
        {
            printf("ATA Security Validate Erase Completion, request sense command "
                   "completion:\n");
            print_Data_Buffer(validateCompletion, SPC3_SENSE_LEN, false);
        }
        if (senseKey == SENSE_KEY_UNIT_ATTENTION && asc == 0x29 && ascq == 0)
    ...
    // Read the bitfield again to check if the drive is in a good or bad security
    // state...a success should only leave it at supported. If locked or enabled,
    // we need to fail and disable the password.
    if (satATASecuritySupported)
    {
        // force an identify information update to make sure the security protocol
        // information is not being cached by the controller. VPD 89h will force the
        // controller to issue a new identify command and it SHOULD also update
        ...
        {
            // commenting this out because a controller could change this data from
            // what the drive reported and we don't want to store that.
            // memcpy((uint8_t*)&device->drive_info.IdentifyData.ata.Word000,
            // &ataVPDPage[60], LEGACY_DRIVE_SEC_SIZE);
        }
    }
    // issue an identify device command before we read the ATA security bits to
    // make sure the data isn't stale in our structure.
    ...
    if (VERBOSITY_QUIET < device->deviceVerbosity)
    {
        ...
        {
            // response was in less than a second so display the time in those much
            // smaller units. This is uncommon unless there was an error reported from
            // the drive or it was a crypto erase
    ...
    {
        // check the initial state to see if security was already enabled. If it
        // was, do not try to clear the password.
        if (!securityStatus.securityEnabled)
        {
            ...
            if (finalSecurityStatus.securityEnabled && !finalSecurityStatus.securityLocked)
            {
                if (SUCCESS == disable_ATA_Security_Password(device, ataPassword, satATASecuritySupported))
                {
                    if (VERBOSITY_QUIET < device->deviceVerbosity)
                    {
                        printf("\tThe ATA Security password used during erase has been "
                               "cleared.\n\n");
                    }
                }
                else
                {
                    if (VERBOSITY_QUIET < device->deviceVerbosity)
                    {
                        printf("\tWARNING!!! Unable to remove the ATA security password "
                               "used during erase!!\n");
            ...
            {
                printf("\tWARNING!!! The drive is in a security state where clearing "
                       "the password is not possible!\n");
                printf("\tPlease power cycle the drive and try clearing the password "
                       "upon powerup.\n");
                printf("\tErase password that was used was: ");
                print_ATA_Security_Password(&ataPassword);
                printf("\n");
        ...
        if (hostResetDuringErase)
        {
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
                printf("\tThe host reset the drive during the erase.\n\tEnsure no "
                       "other applications are trying to access\n\tthe drive while it "
                       "is erasing.\n\n");
            }
        }
    }
