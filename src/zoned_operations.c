                                   ...
                                   ptrZoneDescriptor     zoneDescriptors)
{
    ...
    reportZones = C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE * sectorCount, sizeof(uint8_t),
    ...
    for (uint32_t pullIter = 0; pullIter < dataBytesToRequest; pullIter += (sectorCount * LEGACY_DRIVE_SEC_SIZE - 64))
    {
        ...
        for (uint32_t byteIter = 64; zoneIter < numberOfZoneDescriptors && byteIter <= localListLength &&
             ...
             ++zoneIter, byteIter += 64)
        {
            ...
            if (device->drive_info.drive_type == ATA_DRIVE)
            {
                // byte swap the zone length, start lba, and write pointer lba. All
                // other fields match SCSI
        ...
        // now that we are here, go to the last zone and get the LBA and length to
        // figure out what to request next.
...
#define ZONE_ATTR_OTHER_FLAGS_LENGTH 4
        ...
        // zone start and WP LBA could be at max FFFFFFFFFFFFh which is 15 digits in
        // decimal typical zone length is 524288 (256MiB) which is 6 digits, and +1
        // in case extend in future
                            ...
                            ptrZoneDescriptor     zoneDescriptors)
{
    ...
    printf("\t  P - PREDICTED UNRECOVERED ERRORS bit, Predicted Unrecovered "
           "Errors Present\n");
    printf("---------------------------------------------------------------------"
           "-----------\n");
