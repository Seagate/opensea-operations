...
#define ATA_WORKLOAD_TRACE_PAGE_SIZE                                                                                   \
    (UINT32_C(2048) * UINT32_C(1024))                                                // 2048
                                                                                     // KB
...
char farmSubPageSignatureId[SUBPAGE_TYPE_MAX][FARM_DATASET_SIGNATURE_LENGTH + 1] = {
    "FARM", "FACTORY", "FARMTIME", "FARMSTCK", "WORKLDTC", "FARMSAVE", "FARMLONG"};
...
static eReturnValues pullATAFarmLogs(tDevice*                device,
                                     ...
                                     uint8_t*                farmCurrentLog,
                                     uint8_t*                farmFactoryLog,
                                     uint8_t*                farmSavedLog,
                                     uint8_t*                farmTimeSeriesLog,
                                     uint8_t*                farmLongSavedLog,
                                     uint8_t*                farmStickyLog,
                                     uint8_t*                farmWorkLoadTraceLog)
{
    ...
    if (sataFarmCopyType == SATA_FARM_COPY_TYPE_DISC)
    {
        if (is_FARM_Log_Supported(device))
        {
            ...
            if (SUCCESS == get_ATA_Log(device, SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS, M_NULLPTR,
                                       M_NULLPTR, true, false, true, farmCurrentLog, ATA_FARM_LOG_PAGE_SIZE, M_NULLPTR,
                                       transferSizeBytes, SEAGATE_FARM_CURRENT))
            {
                ...
                {
                    if (fwrite(farmCurrentLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                        ferror(tempFile))
                    {
            ...
            startTimeInMilliSecs = get_Milliseconds_Since_Unix_Epoch();
            if (SUCCESS == get_ATA_Log(device, SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS, M_NULLPTR,
                                       M_NULLPTR, true, false, true, farmFactoryLog, ATA_FARM_LOG_PAGE_SIZE, M_NULLPTR,
                                       transferSizeBytes, SEAGATE_FARM_REPORT_FACTORY_DATA))
            {
                ...
                if (tempFile != M_NULLPTR)
                {
                    if (fwrite(farmFactoryLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                        ferror(tempFile))
                    {
            ...
            startTimeInMilliSecs = get_Milliseconds_Since_Unix_Epoch();
            if (SUCCESS == get_ATA_Log(device, SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS, M_NULLPTR,
                                       M_NULLPTR, true, false, true, farmSavedLog, ATA_FARM_LOG_PAGE_SIZE, M_NULLPTR,
                                       transferSizeBytes, SEAGATE_FARM_REPORT_SAVED))
            {
                ...
                if (tempFile != M_NULLPTR)
                {
                    if (fwrite(farmSavedLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                        ferror(tempFile))
                    {
        ...
        if (is_FARM_Time_Series_Log_Supported(device))
        {
            ...
            uint8_t* farmTimeSeriesFramesLog =
                C_CAST(uint8_t*, safe_calloc_aligned(ATA_TIMESERIES_FRAME_LOG_SIZE, sizeof(uint8_t),
                                       ...
                                       transferSizeBytes, SEAGATE_FARM_TIME_SERIES_DISC))
            {
                ...
                addDataSetEntry(SUBPAGE_TYPE_FARM_TIMESERIES, farmTimeSeriesHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 16 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmTimeSeriesLog, sizeof(uint8_t), 16 * ATA_FARM_LOG_PAGE_SIZE, tempFileTimeseries) !=
                            C_CAST(size_t, 16 * ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                memcpy(farmLongSavedLog, farmTimeSeriesFramesLog + (16 * ATA_FARM_LOG_PAGE_SIZE),
                       2 * ATA_FARM_LOG_PAGE_SIZE);
                addDataSetEntry(SUBPAGE_TYPE_FARM_LONG_SAVE, farmLongSavedHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 2 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmLongSavedLog, sizeof(uint8_t), 2 * ATA_FARM_LOG_PAGE_SIZE, tempFileLongSave) !=
                            C_CAST(size_t, 2 * ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                memcpy(farmStickyLog, farmTimeSeriesFramesLog + (18 * ATA_FARM_LOG_PAGE_SIZE),
                       6 * ATA_FARM_LOG_PAGE_SIZE);
                addDataSetEntry(SUBPAGE_TYPE_FARM_STICKY, farmStickyHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 6 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmStickyLog, sizeof(uint8_t), 6 * ATA_FARM_LOG_PAGE_SIZE, tempFileSticky) !=
                            C_CAST(size_t, 6 * ATA_FARM_LOG_PAGE_SIZE) ||
                                       ...
                                       transferSizeBytes, SEAGATE_FARM_TIME_SERIES_WLTR))
            {
                ...
                addDataSetEntry(SUBPAGE_TYPE_FARM_WORKLOAD_TRACE, farmWorkLoadTraceHeader, &numberOfDataSets,
                                &headerLength, &farmContentField, ATA_WORKLOAD_TRACE_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                if (tempFile != M_NULLPTR)
                {
                    if (fwrite(farmWorkLoadTraceLog, sizeof(uint8_t), ATA_WORKLOAD_TRACE_PAGE_SIZE, tempFile) !=
                            C_CAST(size_t, ATA_WORKLOAD_TRACE_PAGE_SIZE) ||
                        ferror(tempFile))
                    {
    ...
    else if (sataFarmCopyType == SATA_FARM_COPY_TYPE_FLASH)
    {
        if (is_FARM_Time_Series_Log_Supported(device))
        {
                                       ...
                                       transferSizeBytes, SEAGATE_FARM_TIME_SERIES_FLASH))
            {
                ...
                {
                    if (fwrite(farmCurrentLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempCurrentFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                {
                    if (fwrite(farmSavedLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempSavedFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                memcpy(farmTimeSeriesLog, farmTimeSeriesFramesLog + (2 * ATA_FARM_LOG_PAGE_SIZE),
                       16 * ATA_FARM_LOG_PAGE_SIZE);
                addDataSetEntry(SUBPAGE_TYPE_FARM_TIMESERIES, farmTimeSeriesHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 16 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmTimeSeriesLog, sizeof(uint8_t), 16 * ATA_FARM_LOG_PAGE_SIZE, tempTimeSeriesFile) !=
                            C_CAST(size_t, 16 * ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                memcpy(farmLongSavedLog, farmTimeSeriesFramesLog + (18 * ATA_FARM_LOG_PAGE_SIZE),
                       2 * ATA_FARM_LOG_PAGE_SIZE);
                addDataSetEntry(SUBPAGE_TYPE_FARM_LONG_SAVE, farmLongSavedHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 2 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmLongSavedLog, sizeof(uint8_t), 2 * ATA_FARM_LOG_PAGE_SIZE, tempLongSavedFile) !=
                            C_CAST(size_t, 2 * ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                memcpy(farmStickyLog, farmTimeSeriesFramesLog + (20 * ATA_FARM_LOG_PAGE_SIZE),
                       6 * ATA_FARM_LOG_PAGE_SIZE);
                addDataSetEntry(SUBPAGE_TYPE_FARM_STICKY, farmStickyHeader, &numberOfDataSets, &headerLength,
                                &farmContentField, 6 * ATA_FARM_LOG_PAGE_SIZE, startTimeInMilliSecs,
                                endTimeInMilliSecs);
                ...
                {
                    if (fwrite(farmStickyLog, sizeof(uint8_t), 6 * ATA_FARM_LOG_PAGE_SIZE, tempStickyFile) !=
                            C_CAST(size_t, 6 * ATA_FARM_LOG_PAGE_SIZE) ||
                ...
                {
                    if (fwrite(farmFactoryLog, sizeof(uint8_t), ATA_FARM_LOG_PAGE_SIZE, tempFactoryFile) !=
                            C_CAST(size_t, ATA_FARM_LOG_PAGE_SIZE) ||
                                      ...
                                      tSASLogpageSize         logpageSize)
{
    ...
    if (is_FARM_Log_Supported(device))
    {
        ...
        returnValue = get_SCSI_Log(device, SEAGATE_LP_FARM, SEAGATE_FARM_SP_CURRENT, M_NULLPTR, M_NULLPTR, true,
                                   farmCurrentLog, logpageSize.currentLog, M_NULLPTR);
        ...
        else
        {
            ...
            if (tempFile != M_NULLPTR)
            {
                if (fwrite(farmCurrentLog, sizeof(uint8_t), logpageSize.currentLog, tempFile) !=
                        C_CAST(size_t, logpageSize.currentLog) ||
                    ferror(tempFile))
                {
                    printf("error in writing farmcurrent.bin file\n");
                    return ERROR_WRITING_FILE;
    ...
    if (is_Factory_FARM_Log_Supported(device))
    {
        ...
        returnValue = get_SCSI_Log(device, SEAGATE_LP_FARM, SEAGATE_FARM_SP_FACTORY, M_NULLPTR, M_NULLPTR, true,
                                   farmFactoryLog, logpageSize.factoryLog, M_NULLPTR);
        if (returnValue != SUCCESS)
        {
            // print error
            if (VERBOSITY_QUIET < device->deviceVerbosity)
        ...
        else
        {
            ...
            if (tempFile != M_NULLPTR)
            {
                if (fwrite(farmFactoryLog, sizeof(uint8_t), logpageSize.factoryLog, tempFile) !=
                        C_CAST(size_t, logpageSize.factoryLog) ||
                    ferror(tempFile))
                {
                    printf("error in writing farmfactory.bin file\n");
                    return ERROR_WRITING_FILE;
    ...
    if (is_FARM_Time_Series_Log_Supported(device))
    {
        ...
        {
            returnValue = get_SCSI_Log(device, SEAGATE_LP_FARM, i, M_NULLPTR, M_NULLPTR, true, tempTimeSeries,
                                       logpageSize.timeSeriesLog, M_NULLPTR);
            if (returnValue != SUCCESS)
            {
                // print error
                if (VERBOSITY_QUIET < device->deviceVerbosity)
            ...
            {
                // set the flag to true, which indicates atleast one timeseries frame
                // was pulled
        ...
        if (isTimeSeriesPulled) // if atleast one time series frame was pulled then
                                // add into the dataset entry
        {
            endTimeInMilliSecs = get_Milliseconds_Since_Unix_Epoch();
            addDataSetEntry(SUBPAGE_TYPE_FARM_TIMESERIES, farmTimeSeriesHeader, &numberOfDataSets, &headerLength,
            ...
            if (tempFile != M_NULLPTR)
            {
                ...
                if (fwrite(farmTimeSeriesLog, sizeof(uint8_t), timeSeriesLogLength, tempFile) !=
                        C_CAST(size_t, timeSeriesLogLength) ||
                    ferror(tempFile))
                {
                    printf("error in writing farmtimeseries.bin file\n");
                    return ERROR_WRITING_FILE;
    ...
    if (is_FARM_Long_Saved_Log_Supported(device))
    {
        ...
        {
            // set the flag to true, which indicates atleast one saved frame was
            // pulled
            isLongSavedPulled = true;
            tempLongSavedFarm = tempLongSavedFarm + logpageSize.longSavedLog;
            longSavedLogLength += logpageSize.longSavedLog;
        ...
        if (isLongSavedPulled) // if atleast one saved frame was pulled then add
                               // into the dataset entry
        {
            endTimeInMilliSecs = get_Milliseconds_Since_Unix_Epoch();
            addDataSetEntry(SUBPAGE_TYPE_FARM_LONG_SAVE, farmLongSavedHeader, &numberOfDataSets, &headerLength,
            ...
            if (tempFile != M_NULLPTR)
            {
                if (fwrite(farmLongSavedLog, sizeof(uint8_t), longSavedLogLength, tempFile) !=
                        C_CAST(size_t, longSavedLogLength) ||
                    ferror(tempFile))
                {
                    printf("error in writing farmlongsaved.bin file\n");
                    return ERROR_WRITING_FILE;
    ...
    if (is_FARM_Sticky_Log_Supported(device))
    {
        ...
        {
            returnValue = get_SCSI_Log(device, SEAGATE_LP_FARM, i, M_NULLPTR, M_NULLPTR, true, tempSticky,
                                       logpageSize.stickyLog, M_NULLPTR);
            if (returnValue != SUCCESS)
            {
                // print error
                if (VERBOSITY_QUIET < device->deviceVerbosity)
            ...
            {
                // set the flag to true, which indicates atleast one sticky frame was
                // pulled
                isStickyPulled = true;
        ...
        if (isStickyPulled)
        {
            ...
            if (tempFile != M_NULLPTR)
            {
                if (fwrite(farmStickyLog, sizeof(uint8_t), stickyLogLength, tempFile) !=
                        C_CAST(size_t, stickyLogLength) ||
                    ferror(tempFile))
                {
                    printf("error in writing farmsticky.bin file\n");
                    return ERROR_WRITING_FILE;
                                     ...
                                     int               sataFarmCopyType)
{
    ...
    do
    {
        ...
        if (device->drive_info.drive_type == ATA_DRIVE)
        {
            ...
            farmCurrentLog       = C_CAST(uint8_t*, safe_calloc_aligned(ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 96KB
            farmFactoryLog       = C_CAST(uint8_t*, safe_calloc_aligned(ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 96KB
            farmSavedLog         = C_CAST(uint8_t*, safe_calloc_aligned(ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 96KB
            farmTimeSeriesLog    = C_CAST(uint8_t*, safe_calloc_aligned(16 * ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 16 * 96KB
            farmLongSavedLog     = C_CAST(uint8_t*, safe_calloc_aligned(2 * ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 2 * 96KB
            farmStickyLog        = C_CAST(uint8_t*, safe_calloc_aligned(6 * ATA_FARM_LOG_PAGE_SIZE, sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment)); // 6 * 96KB
            farmWorkLoadTraceLog = C_CAST(uint8_t*, safe_calloc_aligned(ATA_WORKLOAD_TRACE_PAGE_SIZE, sizeof(uint8_t),
        ...
        else if (device->drive_info.drive_type == SCSI_DRIVE)
        {
            ...
            if (is_FARM_Log_Supported(device))
            {
                ...
                farmCurrentLog         = C_CAST(uint8_t*, safe_calloc_aligned(logpageSize.currentLog, sizeof(uint8_t),
                                                                              device->os_info.minimumAlignment));
                if (!farmCurrentLog)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
            ...
            if (is_Factory_FARM_Log_Supported(device))
            {
                ...
                farmFactoryLog         = C_CAST(uint8_t*, safe_calloc_aligned(logpageSize.factoryLog, sizeof(uint8_t),
                                                                              device->os_info.minimumAlignment));
                if (!farmFactoryLog)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
            ...
            if (is_FARM_Time_Series_Log_Supported(device))
            {
                ...
                farmTimeSeriesLog =
                    C_CAST(uint8_t*, safe_calloc_aligned((logpageSize.timeSeriesLog * 16), sizeof(uint8_t),
                                                         device->os_info.minimumAlignment));
                if (!farmTimeSeriesLog)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
            ...
            if (is_FARM_Long_Saved_Log_Supported(device))
            {
                ...
                farmLongSavedLog = C_CAST(uint8_t*, safe_calloc_aligned((logpageSize.longSavedLog * 2), sizeof(uint8_t),
                                                                        device->os_info.minimumAlignment));
                if (!farmLongSavedLog)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
            ...
            if (is_FARM_Sticky_Log_Supported(device))
            {
                ...
                farmStickyLog = C_CAST(uint8_t*, safe_calloc_aligned((logpageSize.stickyLog * 6), sizeof(uint8_t),
                                                                     device->os_info.minimumAlignment));
                if (!farmStickyLog)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
        ...
        // write header
        if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, header, FARMC_LOG_HEADER_LENGTH, sizeof(uint8_t),
                                                  FARMC_LOG_HEADER_LENGTH, M_NULLPTR))
        {
            if (device->deviceVerbosity > VERBOSITY_QUIET)
            {
        ...
        // write dataset entry header
        {
            ...
            if (farmContentField & BIT6)
            {
                                                          ...
                                                          FARMC_LOG_DATA_SET_HEADER_LENGTH, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
                        perror("Error in writing farm long term saved header data to a "
                               "file!\n");
                    }
                    if (SEC_FILE_SUCCESS != secure_Close_File(farmCombinedLog))
                    {
            ...
            if (farmContentField & BIT4)
            {
                                                          ...
                                                          FARMC_LOG_DATA_SET_HEADER_LENGTH, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
                        perror("Error in writing farm workload trace header data to a "
                               "file!\n");
                    }
                    if (SEC_FILE_SUCCESS != secure_Close_File(farmCombinedLog))
                    {
        ...
        // write log buffer
        {
            if (farmContentField & BIT0)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmCurrentLog, *datasetlength,
            ...
            if (farmContentField & BIT1)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmFactoryLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
            ...
            if (farmContentField & BIT5)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmSavedLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
            ...
            if (farmContentField & BIT2)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmTimeSeriesLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
                        perror("Error in writing farm time series frame log data to a "
                               "file!\n");
                    }
                    if (SEC_FILE_SUCCESS != secure_Close_File(farmCombinedLog))
                    {
            ...
            if (farmContentField & BIT6)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmLongSavedLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
                        perror("Error in writing farm long term saved frame log data to a "
                               "file!\n");
                    }
                    if (SEC_FILE_SUCCESS != secure_Close_File(farmCombinedLog))
                    {
            ...
            if (farmContentField & BIT3)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmStickyLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
            ...
            if (farmContentField & BIT4)
            {
                ...
                if (SEC_FILE_SUCCESS != secure_Write_File(farmCombinedLog, farmWorkLoadTraceLog, *datasetlength,
                                                          sizeof(uint8_t), *datasetlength, M_NULLPTR))
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
