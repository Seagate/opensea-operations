...
//
// \file dst.c
// \brief This file defines the function calls for dst and dst related
// operations
...
eReturnValues nvme_Get_DST_Progress(tDevice* device, uint32_t* percentComplete, uint8_t* status)
{
    ...
    if (SUCCESS == nvme_Get_Log_Page(device, &getDSTLog))
    {
        ...
        if (nvmeSelfTestLogBuf[0] == 0)
        {
            ...
            *status = M_Nibble0(nvmeSelfTestLogBuf[newestResultOffset + 0]); // This should be fine for the rest
                                                                             // of the running DST code.
            // According to spec, if status bit is 0x0F, that means entry is not
            // valid(doesn't cantain valid test results. and in that case, we have to
            // ignore this bit, and consider this as completed/success. I have seen
            // this issue on the drive, where DST was never run. - Nidhi
            if (*status == 0x0F)
            {
                *status = 0;
            }
        ...
        {
            // NVMe made this simple...if this is 25, then it's 25% complete. no silly
            // business
            *percentComplete = nvmeSelfTestLogBuf[1];
            // Setting the status to Fh to work with existing SCSI/ATA code in the
            // run_DST_Function - TJE
            *status = 0x0F;
        }
    }
    else
...
void translate_DST_Status_To_String(uint8_t status, char* translatedString, bool justRanDST, bool isNVMeDrive)
{
    ...
    if (isNVMeDrive)
    {
        ...
        case 0x03:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "Operation was aborted due to a removal of a namespace from the "
                     "namespace inventory.");
            break;
        case 0x04:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
        ...
        case 0x06:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "Operation completed with a segment that failed and the segment "
                     "that failed is not known.");
            break;
        case 0x07:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "Operation completed with one or more failed segments and the first "
                     "segment that failed is indicated in the Segment Number field.");
            break;
        case 0x08:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH, "Operation was aborted for unknown reason.");
            break;
        case 0x0F: // NOTE: The spec says that this is NOT used. We are dummying
                   // this up to work with existing SAS/SATA code which is why this
                   // is here - TJE
    ...
    else
    {
        ...
        case 0x03:
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "A fatal error or unknown test error occurred while the device "
                     "was executing its self-test routine and the device was unable "
                     "to complete the self-test routine.");
            break;
        case 0x04:
            if (justRanDST)
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The self-test completed having a test element that failed "
                         "and the test element that failed is not known.");
            }
            else
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The previous self-test completed having a test element that "
        ...
        case 0x05:
            ...
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The previous self-test completed having the electrical "
        ...
        case 0x06:
            ...
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The self-test completed having the servo (and/or seek) test "
                         "element of the test failed.");
            }
            else
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The previous self-test completed having the servo (and/or "
                         "seek) test element of the test failed.");
            }
            break;
        case 0x07:
            if (justRanDST)
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The self-test completed having the read element of the test "
                         "failed.");
            }
            else
            {
                snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                         "The previous self-test completed having the read element of "
                         "the test failed.");
            }
            break;
        case 0x08:
...
bool is_Self_Test_Supported(tDevice* device)
{
    ...
    case ATA_DRIVE:
        if (is_SMART_Enabled(device))
        {
            ...
            {
                // NOTE: Also need to check the SMART read data as it also contains a
                // bit to indicate if DST is supported or not!
                //       That field also indicates whether Short, extended, or
                //       conveyance tests are supported! Since the SAMART read data has
                //       been made obsolete on newer standards, we may need a version
                //       check or something to keep proper behavior as new devices show
                //       up without support for this information.
                ...
                if (SUCCESS == ata_SMART_Read_Data(device, smartData, 512))
                {
                    ...
                    // assume this is more accurate since this seems to be the case with
                    // some older products
                    if ((smartData[367] & BIT0) && (smartData[367] & BIT4)) // bit0 = the subcommand, bit4 = self-test
                                                                            // routine implemented (short and extended)
                    {
                        supported = true;
                    }
                }
    ...
    if (is_SMART_Enabled(device))
    {
        ...
        if ((is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word084) &&
            ...
            (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word087) &&
             device->drive_info.IdentifyData.ata.Word087 & BIT1))
        {
            // NOTE: Also need to check the SMART read data as it also contains a bit
            // to indicate if DST is supported or not!
            //       That field also indicates whether Short, extended, or conveyance
            //       tests are supported! Since the SAMART read data has been made
            //       obsolete on newer standards, we may need a version check or
            //       something to keep proper behavior as new devices show up without
...
// NOTE: During my testing, even a drive that doesn't "abort" due to a command
// interruption never continues when trying to poll.
//       But the "in progress" wasn't added until ATA/ATAPI-7, so the polling
//       may need to be restricted to drives with that standard compliance. The
//       drive I have been testing is older and does not support that and does
//       not seem to ever restart on its own. The standards just say it restarts
//       after a "vendor specific event". Because of this, the polling code is
//       removed entirely unless the following #define is set to reenable it.
//       -TJE
...
eReturnValues run_SMART_Offline(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (is_ATA_SMART_Offline_Supported(device, &abortRestart, &offlineTimeInSeconds))
        {
            ...
            if (abortRestart)
            {
                printf("\tInterrupting commands will cause data collection to abort "
                       "and will require manually restarting.\n");
            }
            else
            {
                printf("\tInterrupting commands will cause data collection to suspend "
                       "and will restart after a vendor specific event.\n");
            }
            ...
            {
                uint8_t status = 0;
                // count down for the abount of time the drive reports for how long this
                // should take
                uint16_t countDownSecondsRemaining = offlineTimeInSeconds;
#if defined ENABLE_SMART_OFFLINE_ROUTINE_POLLING
                bool     inProgress  = true;
                uint16_t pollingTime = offlineTimeInSeconds / 10; // attempting to check enough times that it's in 10%
                                                                  // increments, even though the device will not tell us
                                                                  // a real percent complete here.
                ...
                )
                {
                    ...
                    {
                        // time to to a progress check to determine if we should keep
                        // waiting or exit early.
                        if (SUCCESS == get_SMART_Offline_Status(device, &status))
                        {
                            ...
                            case 4:
                            case 0x84:
                                // command "suspended" can be returned just by making the query
                                // to get progress. So treat this case as "in progress" as well
                                // and do not stop. SEE NOTE above. This does not seem to work
                                // like expected as this never seems to restart -TJE
                                inProgress = false;
                                break;
                            case 3:
                                ...
                                break;
                            default:
                                // reserved or vendor unique status....leave as "in progress"
                                // until we get one of the completion codes. Leaving this as "in
                                // progress" until the time runs out because some early drives
                                // may have used a vendor unique code to indicate "in progress"
                                // versus some other error since the case "3" above was not
                                // defined until later.-TJE
                                break;
                            }
                        }
                        // else just let it keep running for the time the drive reported
                        // earlier.
                        pollCounter = 0;
                    }
#endif // ENABLE_SMART_OFFLINE_ROUTINE_POLLING
    ...
    if (is_Self_Test_Supported(device))
    {
        ...
        if (ret == SUCCESS || ret == IN_PROGRESS)
        {
            ...
            case DST_TYPE_SHORT: // short
                ...
                if (captiveForeground)
                {
                    commandTimeout = 120; // two minutes as per ATA and SCSI
                                          // specifications
                }
                break;
            case DST_TYPE_LONG: // extended
                // Since long DST is slower in terms of when progress is achieved, we
                // need to change some things such as how often we poll, how often we
                // expect progress increments, and the maximum number of times to allow
                // for increasing the polling interval. For now, I'm just multiplying
                // everything by 3. Not sure if there is a better way to do this or not.
                delayTime *= 3;
                timeDiff *= 3;
                maxTimeIncreases *= 3;
                timeIncreaseWarningCount *= 3;
                {
                    ...
                    else
                    {
                        ...
                        totalDSTTimeSeconds = 14400; // a fallback for drives not reporting a time, so using 4
                                                     // hours for now. This likely will not ever happen
            ...
            {
                delay_Seconds(1); // delay for a second before starting to poll for
                                  // progress to give it time to start
                ...
                while (status == 0x0F && (ret == SUCCESS || ret == IN_PROGRESS))
                {
                    ...
                    // make the time between progress polls bigger if it isn't changing
                    // quickly to allow the drive time to finish any recovery it's doing.
                    if (difftime(time(M_NULLPTR), dstProgressTimer) > timeDiff &&
                        lastProgressIndication == percentComplete)
                    {
                        // We are likely pinging the drive too quickly during the read test
                        // and error recovery isn't finishing...extend the delay time
                        ...
                        dstProgressTimer = time(M_NULLPTR); // reset this beginning timer since we changed
                                                            // the polling time
                        if (!ignoreMaxTime && timeExtensionCount > maxTimeIncreases &&
                            difftime(dstProgressTimer, startTime) > maxDSTWaitTimeSeconds)
                        {
                            // only abort if we are past the total DST time and have already
                            // increased the delays multiple times. we've extended the polling
                            // time too much. Something else is wrong in the drive. Just abort
                            // it and exit.
                ...
                {
                    // printf 35 characters + width of warning message to clear the line
                    // before printing this final status update
                    printf("\r                                    %.*s",
                ...
                if (VERBOSITY_QUIET < device->deviceVerbosity)
                {
                    if (abortForTooLong)
                    {
                        printf("\nDST was aborted for taking too long. This may happen if "
                               "other disc activity\n");
                        printf("is too high! Please check to make sure no other disk IO is "
                               "occurring so that DST\n");
            ...
            {
                // need to check the result! Probably best to do this from DST log in
                // case ATA RTFRs are not reported back correctly. - TJE
#if !defined(DISABLE_NVME_PASSTHORUGH)
                if (device->drive_info.drive_type == NVME_DRIVE)
                {
                    // simulate a "captive" test on NVMe by polling and waiting to get
                    // status.
                ...
                {
#endif
                    // if the LBA registers have C2-4F or 2C-F4, then we have pass vs fail
                    // results.
                    if (device->drive_info.drive_type == ATA_DRIVE &&
                    ...
                    else // SCSI drive, or ATA rtfrs not available
                    {
                        ...
                        if (SUCCESS == get_DST_Log_Entries(device, &logEntries))
                        {
                            ...
                            {
                                // NOTE: this shouldn't ever happen, but I've seen weird things
                                // before...-TJE
                                ret = IN_PROGRESS;
                            }
...
eReturnValues get_Long_DST_Time(tDevice* device, uint8_t* hours, uint8_t* minutes)
{
    ...
    case ATA_DRIVE:
        if (is_Self_Test_Supported(device))
        {
            ...
            uint8_t* smartData   = C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
    ...
    case SCSI_DRIVE:
    {
        ...
        uint8_t* controlMP = C_CAST(uint8_t*, safe_calloc_aligned(MP_CONTROL_LEN + MODE_PARAMETER_HEADER_10_LEN,
        ...
        // read the control MP to get the long DST time, but it is reported in
        // SECONDS here
        ...
        else
        {
            ...
            {
                ret                            = NOT_SUPPORTED;
                getTimeFromExtendedInquiryData = true; // some crappy USB bridges may not support the mode page, but
                                                       // will support the VPD page, so attempt to read the VPD page
                                                       // anyways
            }
        }
        ...
        {
            uint8_t* extendedInqyData = C_CAST(uint8_t*, safe_calloc_aligned(VPD_EXTENDED_INQUIRY_LEN, sizeof(uint8_t),
                                                                             device->os_info.minimumAlignment));
...
bool get_Error_LBA_From_DST_Log(tDevice* device, uint64_t* lba)
{
    ...
    *lba            = UINT64_MAX; // set to something crazy in case caller ignores return
                                  // type
                                ...
                                bool*                   repaired)
{
    ...
    {
        passthroughWrite = true; // in this case, since sector size emulation is active, we need to
                                 // issue a passthrough command for the repair instead of a
                                 // standard interface command. - TJE
    }
    if (!externalErrorList)
    {
        ...
        if (errorLimit < 1)
        {
            errorListAllocation = sizeof(errorLBA); // allocate a list length of 1..that way we have
                                                    // something if the rest of the function does try
    ...
    while (totalErrors <= errorLimit)
    {
        ...
        if (SUCCESS == run_DST(device, DST_TYPE_SHORT, false, false, true))
        {
            ...
            uint32_t percentComplete        = 0;
            uint32_t lastProgressIndication = 0;
            uint8_t  delayTime              = 5; // Start with 5. If after 30 seconds, progress has not changed,
                                                 // increase the delay to 15seconds. If it still fails to update
                                                 // after the next minute, break out and abort the DST.
            time_t   dstProgressTimer   = time(M_NULLPTR);
            time_t   startTime          = time(M_NULLPTR);
            uint8_t  timeExtensionCount = 0;
            ...
            else
            {
                if (get_Error_LBA_From_DST_Log(device, &errorList[*errorIndex].errorAddress))
                {
                    ...
                    // Now we need to read around the LBA we repaired to make sure there
                    // aren't others around
                    uint64_t readAroundStart = 0;
                    uint64_t readAroundRange = 10000; // 10000 LBAs total (as long as we don't go over the end of
                                                      // the drive)
                    ...
                    // not using generic_tests.h since we don't have a way to force ATA vs
                    // SCSI passthrough command for this, and we have times where we must
                    // do a passthrough write (USB emulation nonsense) first try verifying
                    // the whole thing at once so we can skip the loop below if it is good
    ...
    if (!externalErrorList)
    {
        if (device->deviceVerbosity > VERBOSITY_QUIET)
        {
            if (totalErrors > 0)
            {
                ...
                if (unableToRepair)
                {
                    printf("Other errors were found during DST, but were unable to be "
                           "repaired.\n");
                }
            }
            else if (unableToRepair)
            {
                printf("An error was detected during DST but it is unable to be "
                       "repaired.\n");
            }
            else
            {
...
static eReturnValues get_ATA_DST_Log_Entries(tDevice* device, ptrDstLogEntries entries)
{
    ...
    uint32_t      logSize         = 0; // used for compatibility purposes with drives that may
                                       // have GPL, but not support the ext log...
    // device->drive_info.ata_Options.generalPurposeLoggingSupported = false;//for
    // debugging SMART log version
        ...
        SUCCESS == get_ATA_Log_Size(device, ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG, &logSize, true, false) && logSize > 0)
    {
        ...
        selfTestResults =
            C_CAST(uint8_t*, safe_calloc_aligned(extLogSize, sizeof(uint8_t), device->os_info.minimumAlignment));
        ...
        // if (SUCCESS == ata_Read_Log_Ext(device,
        // ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG, 0, selfTestResults,
        ...
        // device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
        {
            ...
            if (selfTestIndex > 0) // we know DST has been run at least once...
            {
                ...
                uint16_t counter     = UINT16_C(0); // when this get's larger than our max, we need to break out of
                                                    // the loop. This is always incremented. - TJE
                uint16_t maxEntries =
                    C_CAST(uint16_t,
                           (lastPage + UINT16_C(1)) * UINT16_C(19)); // this should get us some multiple of 19
                                                                     // based on the number of pages we read.
#if ENABLE_DST_LOG_DEBUG
                printf("maxEntries = %u\n", maxEntries);
#endif
                while (counter <= maxEntries)
                {
                    ...
                    if (!is_Empty(&selfTestResults[offset],
                                  descriptorLength)) // invalid entires will be all zeros-TJE
                    {
                        ...
                        // if LBA field is all F's, this is meant to signify an invalid
                        // value like T10 specs say to do - TJE
                        ...
                        memcpy(&entries->dstEntry[entries->numberOfEntries].ataVendorSpecificData[0],
                    ...
                    if (offset > descriptorLength)
                    {
                        ...
                        // offset needs to subtract since we work backwards from the end
                        // (until rollover)
...
#endif
                        if (pageNumber > 0 && offsetCheck > C_CAST(uint32_t,
                                                                   (LEGACY_DRIVE_SEC_SIZE *
                                                                    pageNumber))) // this cast is extremely stupid.
                                                                                  // Apparently MSFT's UINT16_C doesn't
                                                                                  // actually force it to unsigned
                                                                                  // properly, so this generates an
             ...
             SUCCESS == get_ATA_Log_Size(device, ATA_LOG_SMART_SELF_TEST_LOG, &logSize, false, true) && logSize > 0)
    {
        ...
        if (SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_SMART_SELF_TEST_LOG, selfTestResults, LEGACY_DRIVE_SEC_SIZE))
        {
            ...
            if (selfTestIndex > 0)
            {
                ...
                uint16_t counter = 0; // when this get's larger than our max, we need to break out of
                                      // the loop. This is always incremented. - TJE
                while (counter < MAX_DST_ENTRIES /*&& counter < 21*/) // max of 21 dst entries in
                                                                      // this log
                {
                    if (!is_Empty(&selfTestResults[offset], descriptorLength))
                    {
                        ...
                        // if LBA field is all F's, this is meant to signify an invalid
                        // value like T10 specs say to do - TJE filtering 28bit all F's and
                        // 32bit all F's since it is not clear exactly how many drives will
                        // report this invalid value -TJE
                    ...
                    if (offset > descriptorLength)
                    {
                        ...
                        {
                            // need to adjust offset to 482 (since we are rolling over back to
                            // beginning)
                            offset = 482;
                        }
                    }
...
static eReturnValues get_SCSI_DST_Log_Entries(tDevice* device, ptrDstLogEntries entries)
{
                                      ...
                                      LP_SELF_TEST_RESULTS_LEN))
    {
        ...
        for (uint32_t offset = 4; offset < C_CAST(uint32_t, pageLength + UINT16_C(4)) &&
                                  offset < LP_SELF_TEST_RESULTS_LEN && entries->numberOfEntries < MAX_DST_ENTRIES;
             offset += 20)
        {
            if (memcmp(&dstLog[offset + 4], zeroCompare,
                       16)) // if this doesn't match, we have an entry...-TJE
            {
                entries->dstEntry[entries->numberOfEntries].descriptorValid = true;
                entries->dstEntry[entries->numberOfEntries].selfTestExecutionStatus =
...
static eReturnValues get_NVMe_DST_Log_Entries(tDevice* device, ptrDstLogEntries entries)
{
    ...
    if (is_Self_Test_Supported(device))
    {
        ...
        if (SUCCESS == nvme_Get_Log_Page(device, &dstLogParms))
        {
            ...
            entries->numberOfEntries = 0;
            for (uint32_t offset = 4; offset < 564 && entries->numberOfEntries < 20;
                 offset += 28) // maximum of 20 NVMe DST log entires
            {
                    ...
                    M_Nibble0(nvmeDSTLog[offset + 0]) != 0x0F) // 0F in NVMe is an unused entry.
                {
                    ...
                    {
                        // this is an invalid LBA value, so it can be filtered out with
                        // existing SCSI/ATA code
                        entries->dstEntry[entries->numberOfEntries].lbaOfFailure = UINT64_MAX;
                    }
...
#define NVM_STATUS_CODE_STR_LEN 10
        ...
        printf("NOTE: DST Log entries are printed out in order from newest to "
               "oldest based ATA/SCSI/NVMe\n");
