            ...
            device->drive_info.IdentifyData.ata.Word076 & BIT10)
        {
            ...
            if (SUCCESS == ret || WARN_INVALID_CHECKSUM == ret)
            {
                ...
                if (WARN_INVALID_CHECKSUM == ret)
                {
                    ...
                    ret                             = SUCCESS; // changing this since we are indicating an invalid
                                                               // checksum another way.-TJE
                }
                else
                {
...
void print_SATA_Phy_Event_Counters(ptrSATAPhyEventCounters counters)
{
    ...
    if (counters->valid)
    {
        ...
        // Figure out how to keep names of each event short before printing them
        // out. -TJE
...
#define PHY_COUNTER_DESCRIPTION_LEN 56
        ...
        {
            printf("\nWARNING: Invalid checksum was received. Data may not be "
                   "accurate!\n");
        }
    }
    return;
