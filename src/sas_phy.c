...
eReturnValues stop_SAS_Test_Pattern(tDevice* device, uint8_t phyIdentifier, eSASPhyPhysicalLinkRate linkRate)
{
    eReturnValues ret = SUCCESS;
    DECLARE_ZERO_INIT_ARRAY(uint8_t, sasDiagPage, 32);
    ret = build_SAS_SSP_Diagnostic_Page(sasDiagPage, phyIdentifier, SAS_PHY_FUNC_STOP, 0, false, 0, linkRate, 0,
                                        0); // I'm assuming the stop command doesn't need to specify anything else
                                            // that matches the running test. - TJE
    if (ret == SUCCESS)
    {
        ret = scsi_Send_Diagnostic(device, 0, 1, 0, 0, 0, 32, sasDiagPage, 32, 15);
    }
