    ...
    if (mbrDataBuf && mbrDataSize >= UINT32_C(512) && mbr)
    {
             ...
             partitionTableOffset += UINT32_C(16))
        {
            ...
            mbr->partition[partitionOffset].partitionType =
                mbrDataBuf[partitionTableOffset + 4]; // if zero, this is empty! This seems to be the most common way to
                                                      // figure this out anyways-TJE
...
static eReturnValues fill_MBR_Data(uint8_t* mbrDataBuf, uint32_t mbrDataSize, ptrMBRData mbr)
{
    ...
    {
        fill_OG_MBR_Partitions(
            mbrDataBuf, mbrDataSize,
            mbr); // fill in the original 4 records first since these are in the same place for any format
        // While all MBR's have 4 partitions, some variants may have more. Check for their signtures first to get the
        // subtype first
            ...
            mbrDataBuf[6] == 'D' && mbrDataBuf[7] == 'R')
        {
            ...
            if (mbrDataBuf[428] == 0x78 && mbrDataBuf[429] == 0x56) // AAP signature (listed as optional)
            {
                ...
                mbr->numberOfPartitions += 1;
                 ...
                 mbrDataBuf[430] <= 0xFE) // AAP signature (listed as optional) and physical drive number
        {
            // AAP format only
        ...
        else if (mbrDataBuf[380] == 0x5A && mbrDataBuf[381] == 0xA5)
        {
                 ...
                 partitionTableOffset -= UINT32_C(16))
            {
                ...
                mbr->partition[partitionOffset].startingAddress.cylinder = mbrDataBuf[partitionTableOffset + 3];
                mbr->partition[partitionOffset].partitionType =
                    mbrDataBuf[partitionTableOffset + 4]; // if zero, this is empty! This seems to be the most common
                                                          // way to figure this out anyways-TJE
                mbr->partition[partitionOffset].endingAddress.head     = mbrDataBuf[partitionTableOffset + 5];
                mbr->partition[partitionOffset].endingAddress.sector   = mbrDataBuf[partitionTableOffset + 6];
                mbr->partition[partitionOffset].endingAddress.cylinder = mbrDataBuf[partitionTableOffset + 7];
                // assuming next two are little endian.
...
// Rather than ordering this ourselves, put these in a list that is manageable and trackable ourselves, then sort it
// before using it.-TJE https://en.m.wikipedia.org/wiki/GUID_Partition_Table#Partition_type_GUIDs
                                   ...
                                   uint64_t   lba)
{
        ...
        gptDataSize >= (UINT32_C(2) * device->drive_info.deviceBlockSize))
    {
        ...
        if (strcmp(gptSignature, "EFI PART") == 0)
        {
            ...
            if (lba == UINT64_C(0) && SUCCESS == fill_MBR_Data(gptDataBuf, UINT32_C(512), &gpt->protectiveMBR))
            {
                gpt->mbrValid = true;
            }
            ...
            // to calculate the header value, we need the header size (at least 92) and to zero out the crc32 in the
            // data before recalculating it
            ...
            if (crc32HeaderValue == gpt_CRC_32(&gptDataBuf[gptHeaderOffset], gptHeaderSize))
            {
                ...
                {
                    uint64_t partitionArrayLBA = UINT64_C(
                        2); // point to beginning of the partition data. This is assuming reading at primary GPT!
                    ...
                    gpt_CRC_32(gptPartitionArray, (sizeOfPartitionEntry * gpt->numberOfPartitionEntries)))
                {
                    ...
                    for (uint64_t partIter = UINT64_C(0), dataOffset = UINT64_C(0);
                         ...
                         ++partIter, dataOffset += sizeOfPartitionEntry)
                    {
                        ...
                        {
                            // only increment this count when the GUID is non-zero. A zero for the GUID is an unused
                            // GUID
                            ++gpt->partitionDataAvailable;
                        }
                    }
                    // Now that the entire array has been read and all CRCs have been validated, we need to compare to
                    // the backup location. NOTE: If the code populated from the backup LBA already, this verifies the
                    // primary LBA...which is expected to be empty otherwise the backup would not have been used.
                }
...
ptrPartitionInfo get_Partition_Info(tDevice* device)
{
    ...
    // If everything is zeros, it will read the last 32KiB of the drive to see if a backup of the boot sector is
    // available.
    ...
    if (dataBuffer && partitionData)
    {
        ...
        do
        {
            if (SUCCESS == read_LBA(device, lba, false, dataBuffer,
                                    dataSize)) // using fua to make sure we are reading from the media, not cache
            {
                ...
                // NOTE: all documentation I have does not show any backups of the MBR. If a backup is available, this
                // needs to change to support it.-TJE
                if (lba == 0 && dataBuffer[510] == MBR_SIGNATURE_LO && dataBuffer[511] == MBR_SIGNATURE_HI)
                {
                    ...
                    else
                    {
                        ...
                        partitionData->mbrTable = M_REINTERPRET_CAST(ptrMBRData, safe_calloc(1, sizeof(mbrData)));
...
static void print_GPT_Info(ptrGPTData gptTable)
{
    if (gptTable)
    {
             ...
             ++partIter)
        {
            ...
            // TODO: Convert the partition name string into something that can be printed. This is UTF-16, so there are
