...
//
// \file format.c
// \brief This file defines the functions for performing some format unit
// operations
...
bool is_Format_Unit_Supported(tDevice* device, bool* fastFormatSupported)
{
    ...
    if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3 &&
        SUCCESS ==
            scsi_Report_Supported_Operation_Codes(device, false, 1, SCSI_FORMAT_UNIT_CMD, 0, 10, formatSupportData))
    {
        // uint16_t cdbSize = M_BytesTo2ByteValue(formatSupportData[2],
        // formatSupportData[3]);
        ...
        // if we made it here, then it's at least supported...not check the bit
        // field for fast format support
        if (fastFormatSupported) // make sure the pointer is valid
        {
            if (!(formatSupportData[7] == 0xFF &&
                  formatSupportData[8] == 0xFF)) // if both these bytes are FFh, then the drive conforms
                                                 // to SCSI2 where this was the "interleave" field
            {
    ...
    {
        // hack for old scsi drives that don't support the report supported op codes
        // command. - TJE
        return true;
    }
    else
...
eReturnValues show_Format_Unit_Progress(tDevice* device)
{
    ...
    if (ret == IN_PROGRESS)
    {
        ...
        // add 0.005 to round up since this is what is happening in the %f print
        // above (more or less) and we really don't need a call to round() to
        // accomplish this. This is also simple enough and close enough to warn the
        // user that the drive is not yet done with the format
        if (percentComplete + 0.005 >= 100.0)
        {
            printf("\tWARNING: Even though progress reports 100%%, the sense data "
                   "indicates\n");
            printf("\t         that a format is still in progress! Please wait an "
                   "additional\n");
            printf("\t         30 seconds and check again to see when the sense data "
                   "no longer\n");
    ...
    {
        printf("\tFormat Unit command is not currently in progress. It is either "
               "complete or has not been run.\n");
    }
    else
    {
...
eReturnValues run_Format_Unit(tDevice* device, runFormatUnitParameters formatParameters, bool pollForProgress)
{
    ...
    // dataSize += 1;//adding 1 to make sure we don't go over the end of out
    ...
    if (!(formatParameters.defaultFormat && formatParameters.disableImmediate)) // Only set the fmtpInfo bit if we
                                                                                // are sending data to the drive via
                                                                                // parameter list...otherwise this
                                                                                // is an illegal combination
    {
        ...
        default: // don't set protection. There is a condition before this that
    ...
    if (longList)
    {
        if ((formatParameters.protectionType == 2 || formatParameters.protectionType == 3) &&
            formatParameters.protectionIntervalExponent != 0)
        {
            // P_I_Information should be left as zero always! the exponent is in the
            // lower nibble of this same byte
    ...
    if (!formatParameters.currentBlockSize || formatParameters.newMaxLBA)
    {
        ...
        // set the block size to the new block size and the max lba to all
        // F's...they can change the sector size later with another command if they
        // want to
        if (modeSelect10)
        {
            ...
            // zero out the mode data length since we will not actually send it the
            // mode page, just header and block descriptor
            modeParameterData[0] = 0;
            modeParameterData[1] = 0;
            // zero out device specific parameter since those bits are mostly reserved
            // in this case and don't really matter for a reformat.
        ...
        if (blockDescriptorLength == 8)
        {
            ...
            {
                formatParameters.newMaxLBA += 1; // Need to add 1 for SCSI so that this will match the -i report.
                                                 // If this is not done, then  we end up with 1 less than the
                                                 // value provided.
        ...
        if (modeSelect10)
        {
            ret = scsi_Mode_Select_10(device, (blockDescriptorLength + blockDescriptorOffset), false, true, false,
                                      modeParameterData,
        ...
        {
            ret = scsi_Mode_Select_6(device, C_CAST(uint8_t, (blockDescriptorLength + blockDescriptorOffset)), false,
    ...
    if (ret == SUCCESS)
    {
        ...
        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
        {
            ...
            printf("Depending on the format request, this could take minutes to "
                   "hours or days.\n");
            printf("Do not remove power or attempt other access as interrupting it "
                   "may make\n");
        ...
        if (pollForProgress && ret == SUCCESS && !formatParameters.disableImmediate)
        {
            ...
            delay_Seconds(2); // 2 second delay to make sure it starts (and on SSD
                              // this may be enough for it to finish immediately)
        ...
        {
            // check if there was an invalid parameter field specifying the security
            // initialize bit...if so, print a message and return not supported - TJE
        }
    }
    safe_free_aligned(&dataBuf);
    return ret;
...
eReturnValues get_Format_Status(tDevice* device, ptrFormatStatus formatStatus)
{
    ...
    uint8_t* formatStatusPage =
        C_CAST(uint8_t*, safe_calloc_aligned(307, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!formatStatusPage)
    {
        return MEMORY_FAILURE;
    }
    ...
    {
        // NOTE: Parameters will be all F's when the data is not available or new or
        // the last format failed
                ...
                case 0: // format data out
                    formatStatus->lastFormatParametersValid = true;
                    {
                        ...
                        else
                        {
                            ...
                            switch (formatStatusPage[offset + 3]) // based on the length of the data,
                                                                  // it may be a short or long list...
                            {
                            case 4: // short list
...
void show_Format_Status_Log(ptrFormatStatus formatStatus)
{
    if (formatStatus)
    {
        ...
        if (!formatStatus->formatParametersAllFs)
        {
            ...
            {
                printf("The last format unit was performed with the following "
                       "parameters:\n");
        ...
        {
            printf("Format unit currently in progress or the last format command "
                   "failed!\n");
        }
    }
    return;
                                                 ...
                                                 LEGACY_DRIVE_SEC_SIZE, 0))
        {
                ...
                M_Word0(qword0) >= 0x0001)
            {
                ...
                if (supportedCapabilitiesQWord & BIT63 &&
                    supportedCapabilitiesQWord & BIT49) // check bit63 since it should always be 1, then bit 49
                                                        // for set sector configuration
                {
                    return true;
                }
                else
...
uint32_t get_Number_Of_Supported_Sector_Sizes(tDevice* device)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        return MAX_NUMBER_SUPPORTED_SECTOR_SIZES; // This should be ok on ATA...we
                                                  // would have to pull the log and
                                                  // count to know for sure, but
                                                  // this is the max available in
                                                  // the log
    }
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        // pull the VPD page and determine how many are supported based on
        // descriptor length and the VPD page length
        ...
        else
        {
            ...
            // This device either doesn't support any other sector sizes, or supports
            // legacy sector sizes...
                                                 ...
                                                 LEGACY_DRIVE_SEC_SIZE, 0))
        {
                 ...
                 iter += 16, ++sectorSizeCounter)
            {
                    ...
                    formats->sectorSizes[sectorSizeCounter].ataSetSectorFields.descriptorCheck != 0)
                {
                    ...
                    // special case for USB attached ATA drives to set the indicator for
                    // the current configuration. Since this is done as a SAT passthrough
                    // command on USB, we need to check the child drive information rather
                    // than the primary information.
                    ...
                    else if (formats->sectorSizes[sectorSizeCounter].logicalBlockLength ==
...
static eReturnValues scsi_Get_Supported_Formats(tDevice* device, ptrSupportedFormats formats)
{
        ...
        if (inquiryData[5] & BIT0)
        {
                ...
                scsi_Inquiry(device, inquiryData, INQ_RETURN_DATA_LENGTH, EXTENDED_INQUIRY_DATA, true, false))
            {
                ...
                case 0:                                   // direct access block device
                    ...
                    break;
                case 1: // sequential access block device (we don't care...it's a tape
                        // drive...)
                default:
                    break;
                }
    ...
    if (formats->protectionInformationSupported.protectionReportedPerSectorSize || supportedSectorSizesDataLength)
    {
        uint8_t* supportedBlockLengthsData =
            C_CAST(uint8_t*, safe_calloc_aligned(supportedSectorSizesDataLength, sizeof(uint8_t),
                                    ...
                                    supportedBlockLengthsData, supportedSectorSizesDataLength, M_NULLPTR))
        {
            ...
            uint32_t numberOfSizes       = formats->numberOfSectorSizes;
            formats->numberOfSectorSizes = 0;
            for (uint32_t iter = 4, sectorSizeCounter = 0;
                 (iter + 8) < supportedSectorSizesDataLength && sectorSizeCounter < numberOfSizes;
                 iter += 8, ++sectorSizeCounter, ++formats->numberOfSectorSizes)
            {
                formats->sectorSizes[sectorSizeCounter].valid = true;
                formats->sectorSizes[sectorSizeCounter].logicalBlockLength =
    ...
    if (is_Format_Unit_Supported(device, &formats->scsiFastFormatSupported))
    {
        if (dummyUpCommonSizes)
        {
            ...
            ret                                 = SUCCESS;
            // dummy up the support based on what is known from traditional formatting
            // support - don't include any PI stuff here for now. Need more
            // refactoring
...
void show_Supported_Formats(ptrSupportedFormats formats)
{
    ...
    printf("  ? - unable to determine support for protection type at specified "
           "block size\n");
...
#define META_STRING_SIZE 10
    ...
    {
        printf("NOTE: Device is not capable of showing all sizes it supports. Only "
               "common\n");
        printf("      sizes are listed. Please consult the product manual for all "
               "supported\n");
        printf("      combinations.\n");
    }
    if (formats->protectionInformationSupported.deviceSupportsProtection)
    {
        printf("NOTE: This device supports protection information (PI) (a.k.a. End "
               "to End protection).\n");
        ...
        if (formats->protectionInformationSupported.protectionType2Supported)
        {
            printf("\tType 2 - Logical Block Guard and Logical Block Reference Tag "
        ...
        {
            printf("      Not all forms of PI are supported on all sector sizes "
    ...
    if (formats->scsiFastFormatSupported)
    {
        printf("NOTE: This device supports Fast Format. Fast format is not "
               "instantaneous and is used for\n");
        printf("      switching between 5xx and 4xxx sector sizes. A fast format "
               "may take a few minutes or longer\n");
        printf("      but may take longer depending on the size of the drive. Fast "
               "format support does not necessarily\n");
        printf("      mean switching sector sizes AND changing PI at the same time "
               "is supported. In most cases, a\n");
        printf("      switch of PI type will require a full device format.\n");
        printf("      Fast format mode 1 is typically used to switch from 512 to "
               "4096 block sizes with the current\n");
        printf("          PI scheme.\n");
    }
...
// this function takes a sector size and maps it to the descriptor check code to
...
eReturnValues set_Sector_Configuration_With_Force(tDevice* device, uint32_t sectorSize, bool force)
{
    ...
    if (is_Set_Sector_Configuration_Supported(device) || force)
    {
        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
        {
            ...
            printf("This command may appear to hang the utiliy. Do NOT interrupt "
                   "this\n");
            printf("command for at least 1 hour if it appears hung. The drive is "
                   "busy\n");
            printf("performing the sector size change and is not able to indicate "
                   "its\n");
            printf("progress during this time.\n");
            printf("This often only takes a few minutes to complete and return "
                   "completion\n");
            printf("of this command to the utility but in some cases it may take "
                   "this full\n");
            printf("hour before the drive is ready to use.\n");
            printf("After this command has completed successfully the drive is ready "
                   "to\n");
            printf("read and write data.\n");
            printf("Be aware that the drive may begin performing vendor unique "
                   "background\n");
            printf("activity which may prevent idle and standby timers from taking "
                   "affect\n");
            printf("until the background activity has completed.\n");
            printf("If this command takes an hour or the command reports a failure "
                   "due to\n");
            printf("interruption by the system with a reset, recovery will be "
                   "attempted\n");
            printf("automatically. You may attempt to run this command again if "
                   "recovery\n");
            printf("does not appear successfull.\n");
        }
        os_Lock_Device(device);
        os_Unmount_File_Systems_On_Device(device);
        // a weird case was found when changing the sector size on a drive with an
        // existing partition on it. Since the MBR was a "dummy" for GPT, it is
        // setup to look like the entire disk has a partition to stop an old OS from
        // overwriting partitions setup with GPT. So Windows blocks the ability to
        // change the partition. The solution is simple: erase the MBR before the
        // format. This option already requires a confirmation of data deletion to
        // run, so this should be safe enough. -TJE
        bool     mbrEraseWarning = false;
        uint8_t* eraseMBR = C_CAST(uint8_t*, safe_calloc_aligned(device->drive_info.deviceBlockSize, sizeof(uint8_t),
                                                                 device->os_info.minimumAlignment));
        if (eraseMBR)
        {
            // write the allocated zeros over the MBR (first sector), and the last
            // sector (maxLBA) to ensure it is erased and not causing a problem NOTE:
            // last sector is sometimes used as a backup of the MBR, which is why it
            // will also be erased
        ...
        if (mbrEraseWarning)
        {
            if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
            {
                printf("WARNING: Unable to erase MBR. If unable to write a partition "
                       "after this operation, erase the first sector of the device\n");
                printf("         and the last sector (max LBA) then try creating new "
                       "partitions again.\n");
            }
        }
        if (device->drive_info.drive_type == ATA_DRIVE)
        {
            ...
            if (!setSizeSupported || force)
            {
                ...
                {
                    printf("ERROR: The device was reset during sector size change. "
                           "Device may not be usable!\n");
                }
                if (is_Seagate_Family(device) == SEAGATE && !is_SSD(device)) // HDDs only
                {
                    ...
                    if (SUCCESS != seagate_Quick_Format(device))
                    {
                        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                        {
                            printf("WARNING: Seagate quick format did not complete "
                                   "successfully!\n");
                        }
                    }
                    // try refreshing the device one more time incase the status was just
                    // not right.
                    fill_Drive_Info_Data(device);
                    if (!is_Set_Sector_Configuration_Supported(device))
                    {
                        ...
                        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                        {
                            printf("ERROR: Quick format did not recover the device. The "
                                   "device may not be usable!\n");
                        }
                    }
                    else
                    {
                        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                        {
                            ...
                            printf("If sector size change is attempted again, format only "
                                   "single disks at a time,\n");
                            printf("disable all background software, disable any management "
                                   "hardware or software, and then\n");
        ...
        else // Assume SCSI
        {
            ...
            formatUnitParameters.defaultFormat = true; // Don't need any option bits! In fact, this could cause an
                                                       // error if not set!
            ...
            formatUnitParameters.disableImmediate = false; // this will require polling for progress until complete with
                                                           // this disabled, but reduces the likelyhood of a reset going
                                                           // to the drive.
            // make this smarter to know which type of fast format to use!
            // FAST_FORMAT_WRITE_NOT_REQUIRED is a power of 2 change (512 to 4096),
            // FAST_FORMAT_WRITE_REQUIRED is any other size change
...
eReturnValues show_NVM_Format_Progress(tDevice* device)
{
    ...
    {
        printf("\tA format is not detected as running. Either it is complete or "
               "the device does not report its progress\n");
    }
    else
    {
...
eReturnValues get_NVMe_Format_Support(tDevice* device, ptrNvmeFormatSupport formatSupport)
{
    ...
    if (device->drive_info.drive_type == NVME_DRIVE && formatSupport)
    {
        ...
        // bit 0 = format applies to all namespaces. If zero, applies only to the
        // specified namespace bit 1 = secure erase applies to all namespaces. if
        // zero, applies only to the specified namespace bit 2 = cryptographic erase
        // is supported
...
eReturnValues run_NVMe_Format(tDevice* device, runNVMFormatParameters nvmParams, bool pollForProgress)
{
    ...
    else
    {
        ...
        // get the LBAF number. THis field varies depending on other things reported
        // by the drive in NVMe 2.0
        if (device->drive_info.IdentifyData.nvme.ns.nlbaf > 16)
        {
            // need to append 2 more bits to interpret this correctly since number of
            // formats > 16
    ...
    if (pollForProgress && ret == SUCCESS)
    {
        ...
        uint8_t  progress         = 0;
        delay_Seconds(2); // 2 second delay to make sure it starts (and on SSD this
                          // may be enough for it to finish immediately)
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            uint8_t seconds = 0;
            uint8_t minutes = 0;
