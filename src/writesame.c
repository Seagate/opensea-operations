...
//
// \file writesame.c
// \brief This file defines the functions related to the writesame command on a
// drive
                             ...
                             uint64_t*              maxNumberOfLogicalBlocksPerCommand)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        {
            supported = true;
            // as far as I know, ATA drives don't have a limit so just return the
            // range from the requested start and the maxLBA of the device - TJE
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // Can use report supported opcodes & MAYBE older inquiry cmdDT to try to
        // figure out if the device definitely supports the command or not.
        //   NOTE: SPC added cmdDT. SPC3 obsoletes this for report supported
        //   operation codes
        // Can use these to get a good idea if the command is supported or not for
        // the request. 16B max range is UINT32 (unless you can set zero as the
        // range). 10B max range is UINT16 (unless zero can be used as the range)

        // If SCSI 2, just say supported and we'll check sense data for an error
        // later. Otherwise try getting the report about if it is supported from
        // cmdDT/report supported op.
        //   If these complete and show definitive support or definitive no support,
        //   we can easily decide how to proceed. If these don't work (not
        //   supported), then try checking the block limits page to see what it
        //   says.
        // If block limits shows zeros, assume support and report support/lack of
        // support based on trying the command.
        if (device->drive_info.scsiVersion >= SCSI_VERSION_SCSI2) // SCSI 2 or higher
        {
            bool driveReportsSupport = false; // This tracks if the report supported opcodes or cmdDT reports
                                              // with absolute certainty that the drive supports the command.
            if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3)
            {
                // for scsi ask for supported op code and look for write same 16....we
                // don't care about the 10 byte or 32byte commands right now
                uint32_t supportLengthCheck = 20; // enough space to read all support data bytes for 16B version
                uint8_t* writeSameSupported = C_CAST(uint8_t*, safe_calloc_aligned(supportLengthCheck, sizeof(uint8_t),
                ...
                if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE,
                                                                     WRITE_SAME_16_CMD, 0, supportLengthCheck,
                                                                     writeSameSupported))
                {
                    gotData = true;
                    if (*maxNumberOfLogicalBlocksPerCommand)
                ...
                if (!driveReportsSupport && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                           device, false, REPORT_OPERATION_CODE, WRITE_SAME_10_CMD, 0,
                                                           supportLengthCheck, writeSameSupported))
                {
                    gotData = true;
                    if (*maxNumberOfLogicalBlocksPerCommand)
                    {
                     ...
                     device->drive_info.scsiVersion < SCSI_VERSION_SPC_3)
            {
                // for scsi ask for supported op code and look for write same 16....we
                // don't care about the 10 byte or 32byte commands right now
                uint32_t supportLengthCheck = 22; // enough space to read all support data bytes for 16B version
                                                  // with CmdDT
                uint8_t* writeSameSupported = C_CAST(uint8_t*, safe_calloc_aligned(supportLengthCheck, sizeof(uint8_t),
                                                                                   device->os_info.minimumAlignment));
                if (!writeSameSupported)
                ...
                bool gotData = false;
                if (SUCCESS ==
                    scsi_Inquiry(device, writeSameSupported, supportLengthCheck, WRITE_SAME_16_CMD, false, true))
                {
                    gotData = true;
                    if (*maxNumberOfLogicalBlocksPerCommand)
                    {
                ...
                if (!driveReportsSupport && SUCCESS == scsi_Inquiry(device, writeSameSupported, supportLengthCheck,
                                                                    WRITE_SAME_10_CMD, false, true))
                {
                    gotData = true;
                    if (*maxNumberOfLogicalBlocksPerCommand)
                    {
            ...
            // SPC4 will have full block limits. Check for SPC2 and up since it's
            // ambiguous about when exactly the fields we want to check may have been
            // supported by a given drive
            if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_2 && maxNumberOfLogicalBlocksPerCommand)
            {
                // also check the block limits vpd page to see what the maximum number
                // of logical blocks is so that we don't get in a trouble spot...(we may
                // need chunk the write same command...ugh).
                    ...
                    if (pageLength >= 0x3C) // earlier specs, this page was shorter
                    {
                        ...
                        *maxNumberOfLogicalBlocksPerCommand =
                        ...
                        if (*maxNumberOfLogicalBlocksPerCommand >= requesedNumberOfLogicalBlocks)
                        {
                            ...
                            {
                                // this case is that the drive supports a range LARGER than is
                                // allowed in a single command and a range must be
                                // specified...which is weird, but stupid.
                                supported = false;
                            }
                            else
                            {
                                supported = true;
                                 ...
                                 !wsnz) // checking for write-same non-zero bit. If this
                                        // is set, then there SHOULD be a limit listed. If
                                        // not, then I guess this is not supported on this
                                        // device-TJE
                        {
                            // Device does not report a limit. This can be a
                            // backwards-compatible thing, or it could mean the device
                            // supports any length. Because of this, call it supported since
                            // we don't have any reason to otherwise think write same is not
                            // supported.
                            supported = true;
                        }
                        else
                        {
                            // This case should only be hit when the requested range is larger
                            // than the device supports, or no max range was reported AND the
                            // WSNZ bit is set (meaning the command is not really supported)
                            supported = false;
                        }
                    }
                }
...
// we need to know where we started at and the range in order to properly
// calculate progress
                                     ...
                                     uint64_t range)
{
    ...
    *writeSameInProgress = false;
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            if (sctState == SCT_STATE_SCT_COMMAND_PROCESSING_IN_BACKGROUND ||
                sctStatus == SCT_EXT_STATUS_SCT_COMMAND_PROCESSING_IN_BACKGROUND) // sct
                                                                                  // commmand
                                                                                  // processing
                                                                                  // in
                                                                                  // background
    ...
    {
        //request sense...hopefully we get something here....we don't, FYI. The
    drive doesn't report the write same progress at all. I'll leave this code in
    place anyways in case someone ever is interested in it, or progress reporting
    is added. - TJE
        //Not sure what we'll get for sense data, so we'll have to do some testing
    to make sure we get at least one of these
        //asc = 0x04, ascq = 0x07 - Logical Unit Not Ready, Operation In Progress
        //asc = 0x00, ascq = 0x16 - Operation In Progress
        //if the progress is reported, then we just have to return it... i
    think... uint8_t *senseData = C_CAST(uint8_t*,
    safe_calloc_aligned(SPC3_SENSE_LEN, sizeof(uint8_t),
    device->os_info.minimumAlignment)); if (!senseData)
        ...
        uint8_t senseKey = 0;
        uint8_t fru = 0;
        ret = scsi_Request_Sense_Cmd(device, false, senseData,
    SPC3_SENSE_LEN);//get fixed format sense data to make this easier to parse the
    progress from. get_Sense_Key_ASC_ASCQ_FRU(&senseData[0], SPC3_SENSE_LEN,
    &senseKey, &asc, &ascq, &fru); if (VERBOSITY_BUFFERS <=
    device->deviceVerbosity)
        ...
        {
            *progress = M_BytesTo2ByteValue(senseData[16], senseData[17]);//sense
    key specific information if (asc == 0x04 && ascq == 0x07) //this is making
    sure that something is in progress
            {
                *writeSameInProgress = true;
            }
...
eReturnValues show_Write_Same_Current_LBA(tDevice* device)
{
    ...
    case SUCCESS:
        ...
        printf("\tA Write same is not currently in progress or has completed "
               "successfully\n");
        break;
    case IN_PROGRESS:
        // currently running. Current LBA = %llu, calculate progress with this
        // formula:
                        ...
                        uint32_t patternLength)
{
         ...
         (startingLba + numberOfLogicalBlocks) == (device->drive_info.deviceMaxLba + UINT64_C(1))))
    {
        ...
        if (device->drive_info.drive_type != ATA_DRIVE)
        {
            ...
            {
                // only allocate this memory for SCSI drives because they need a sector
                // telling what to use as a pattern, whereas ATA has a feature that does
                // not require this, and why bother sending an extra command/data
                // transfer when it isn't neded for our application
                zeroPatternBufLen = device->drive_info.deviceBlockSize;
                zeroPatternBuf    = C_CAST(uint8_t*, safe_calloc_aligned(zeroPatternBufLen, sizeof(uint8_t),
                                                                         device->os_info.minimumAlignment));
                if (!zeroPatternBuf)
                {
                    perror("Error allocating logical sector sized buffer for zero "
                           "pattern\n");
                }
            }
            ...
            {
                // in this case, erasing the whole drive is requested. To do this on
                // SAS/SCSI, set the range to zero. NOTE: This *might* not work, but
                // it's not super straight forward to check this. - TJE
        ...
        {
            ret = write_Same(device, startingLba, numberOfLogicalBlocks,
                             zeroPatternBuf); // null for the pattern means we'll
        ...
        if (ret == SUCCESS && pollForProgress && device->drive_info.drive_type == ATA_DRIVE)
        {
            ...
            delay_Seconds(1); // delay one second before we start polling to let the
                              // drive get started
