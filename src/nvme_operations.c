...
void nvme_Print_Feature_Identifiers_Help(void)
{
    ...
    printf("15h       \t M \t   NO       \tLBA Status Information Report "
           "Interval\n");
    ...
    printf("84h       \t O \t   NO       \tNamespace Write Protection Config "
           "(NVM)\n");
    printf("C0h-FFh   \t   \t            \tVendor Specific\n");
    printf("====================================================\n");
    printf("NOTE: Some \"Mandatory\" features may not be supported by some "
           "drives due to\n");
    printf("      conforming to older specifications from before these features "
           "were\n");
    printf("      standardized.\n");
    printf("NOTE: This list is not exhaustive and not an indication of what is "
           "supported\n");
    printf("      by a given device. It is simply a list of known feature IDs as "
           "of NVMe 1.4\n");
}

// This is far from perfect. Not all features will be supported, so would be
// better to have something check if
//       a feature is supported in the identify data then request information
//       about it as needed. it would also be helpful to have the name of the
//       feature output as well.-TJE
eReturnValues nvme_Print_All_Feature_Identifiers(tDevice*                 device,
...
static eReturnValues nvme_Print_Temperature_Feature_Details(tDevice* device, eNvmeFeaturesSelectValue selectType)
{
    ...
    for (TMPSEL = 1; TMPSEL <= 8; TMPSEL++)
    {
        ...
        featureCmd.featSetGetValue =
            C_CAST(uint32_t, (C_CAST(uint32_t, BIT20) | (C_CAST(uint32_t, TMPSEL) << UINT32_C(16))));
        ret = nvme_Get_Features(device, &featureCmd);
        if (ret == SUCCESS)
        {
...
//       It needs to also check if a given page is supported as well, which
//       is....complicated Older devices won't have the supported pages LID, but
//       newer will Additionally we may need to check for specific features or
//       other bits to determine what is or is not supported.
eReturnValues nvme_Get_Log_Size(tDevice* device, uint8_t logPageId, uint64_t* logSize)
{
    ...
    if (logSize)
    {
        ...
        getLogHeader.nsid    = NVME_ALL_NAMESPACES; // change this as needed when
                                                    // calculating sizes below
        getLogHeader.rae = true;                    // do not clear any asynchronous events
        *logSize         = UINT64_C(0);             // make sure this is initialized to zero as some logs below may
                                                    // require calculating the size by reading info from the drive
        switch (logPageId)
        {
        ...
        case NVME_LOG_CHANGED_NAMESPACE_LIST: // up to 1024 entries of
                                              // namespaces...just returning the
                                              // maximum size
        ...
        case NVME_LOG_TELEMETRY_CTRL_ID:
        {
            ...
            if (SUCCESS == nvme_Get_Log_Page(device, &getLogHeader))
            {
                *logSize = UINT64_C(512) + (UINT64_C(512) * M_BytesTo2ByteValue(logPageHeader[13], logPageHeader[12]));
                // TODO: Data area 4 support. Need to check host behavior support
                // feature as well as identify data
        ...
        case NVME_LOG_ASYMMETRIC_NAMESPACE_ACCESS_ID:
            ...
            // device->drive_info.IdentifyData.nvme.ctrl.anagrpmax) * (UINT64_C(32) +
            //(C_CAST(uint64_t, device->drive_info.IdentifyData.nvme.ctrl.nn) *
            // UINT64_C(4))));
            break;
        case NVME_LOG_PERSISTENT_EVENT_LOG_ID:
            // 512B header, each event is24B + vendor specific info (max 65535B) +
            // event data PELS is maximum persistent events in 64KiB units
        ...
        case NVME_LOG_MEDIA_UNIT_STATUS_ID:
            ...
            // Selected Configuration will play a part in the size calculation as
            // well. If zero, MUCS in each descriptor will be zero
            ...
            if (SUCCESS == nvme_Get_Log_Page(device, &getLogHeader))
            {
                ...
                {
                    // This assumes channel 0 will start at offset 16...this is not
                    // required by the spec, just that it's a multiple of 16
                    *logSize =
                        UINT64_C(16) /*log header*/ + (nmu * (UINT64_C(16) /*descriptor header bytes*/ +
                                                              (cchans * UINT64_C(2) /*bytes per channel identifier*/)));
                }
            }
            break;
        case NVME_LOG_SUPPORTED_CAPACITY_CONFIGURATION_LIST_ID:
            // TODO: read 16B header to get SCCN. NOTE: configuration descriptors can
            // be different lengths...this is due to number of endurance groups
            // accessible by the controller. Each is variable by number of endurance
            // groups, number NVM sets, number of channels and number of media units.
        ...
        case NVME_LOG_DISCOVERY_ID:
            // NOTE: Using first 16B for now. Change if record format ever changes to
            // read more info to properly determine the size
            getLogHeader.lid = NVME_LOG_DISCOVERY_ID;
            if (SUCCESS == nvme_Get_Log_Page(device, &getLogHeader))
            {
                uint64_t numberOfRecords =
                    M_BytesTo8ByteValue(logPageHeader[15], logPageHeader[14], logPageHeader[13], logPageHeader[12],
                                        logPageHeader[11], logPageHeader[10], logPageHeader[9], logPageHeader[8]);
...
eReturnValues nvme_Print_DevSelfTest_Log_Page(tDevice* device)
{
    ...
    const char*     test_res[10] = {"Operation completed without error",
                                    ...
                                    "A fatal error or unknown test error occurred while the controller was "
                                        "executing the"
                                        " device self-test operation andthe operation did not complete",
                                    "Operation completed with a segment that failed and the segment that "
                                        "failed is not known",
    ...
    if (ret == SUCCESS)
    {
        ...
        for (i = 0; i < NVME_SELF_TEST_REPORTS; i++)
        {
            ...
            printf("  Vendor Specific                      : %x %x\n", selfTestLogInfo.result[i].vendorSpecific[0],
                   selfTestLogInfo.result[i].vendorSpecific[1]);
        }
    }
...
eReturnValues nvme_Print_ERROR_Log_Page(tDevice* device, uint64_t numOfErrToPrint)
{
    ...
    pErrLogBuf = C_CAST(nvmeErrLogEntry*, safe_calloc_aligned(C_CAST(size_t, numOfErrToPrint), sizeof(nvmeErrLogEntry),
                                                              device->os_info.minimumAlignment));
    if (pErrLogBuf != M_NULLPTR)
    {
        ret = nvme_Get_ERROR_Log_Page(device, C_CAST(uint8_t*, pErrLogBuf),
