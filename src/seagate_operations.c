...
eReturnValues seagate_ata_SCT_SATA_phy_speed(tDevice* device, uint8_t speedGen)
{
    ...
    // speedGen = 1 means generation 1 (1.5Gb/s), 2 =  2nd Generation (3.0Gb/s), 3
    // = 3rd Generation (6.0Gb/s)
    if (speedGen > 3)
    {
    ...
    // option flags
    sctSATAPhySpeed[8] = 0x01; // save feature to disk...I'm assuming this is always what's wanted
                               // since the other flags don't make sense since this feature
                               // requires a power cycle
    sctSATAPhySpeed[9] = 0x00;
    // reserved words are from byte 10:27
    // the data transferspeed goes in bytes 28 and 29, although byte 28 will be
    // zero
...
// valid phySpeedGen values are 1 - 5. This will need to be modified if SAS
// get's higher link rates than 22.5Gb/s
eReturnValues scsi_Set_Phy_Speed(tDevice* device, uint8_t phySpeedGen, bool allPhys, uint8_t phyNumber)
{
    ...
    uint16_t phyControlLength =
        UINT16_C(104) + MODE_PARAMETER_HEADER_10_LEN; // size of 104 comes from 8 byte page header
                                                      // + (2 * 48bytes) for 2 phy descriptors +
                                                      // then add 8 bytes for mode parameter
                                                      // header. This is assuming drives only
                                                      // have 2...which is true right now, but the
                                                      // code will detect when it needs to
                                                      // reallocate and read more from the drive.
    uint8_t* sasPhyControl =
        C_CAST(uint8_t*, safe_calloc_aligned(phyControlLength, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!sasPhyControl)
    {
        return MEMORY_FAILURE;
    }
    ...
    {
        // make sure we got the header as we expect it, then validate we got all the
        // data we needed. uint16_t modeDataLength =
        // M_BytesTo2ByteValue(sasPhyControl[0], sasPhyControl[1]);
            ...
            (sasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & BIT6) > 0)
        {
            ...
            // check that we were able to read the full page! If we didn't get the
            // entire thing, we need to reread it and adjust the phyControlLength
            // variable!
                ...
                6) // make sure it's the SAS protocol page
            {
                ...
                uint32_t phyDescriptorOffset = MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength +
                                               8; // this will be set to the beginning of the phy descriptors so
                     ...
                     ++phyIter, phyDescriptorOffset += 48)
                {
                    ...
                    if (allPhys || phyNumber == phyIdentifier)
                    {
                        ...
                        if (phySpeedGen == 0)
                        {
                            // they want it back to default, so read the hardware maximum
                            // physical link rate and set it to the programmed maximum
                            uint8_t matchedRate =
                        ...
                        {
                            // they are requesting a specific speed, so set the value in the
                            // page.
                            switch (phySpeedGen)
                            {
                            case 1: // 1.5 Gb/s
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word206) &&
                device->drive_info.IdentifyData.ata.Word206 & BIT7 && !is_SSD(device))
            {
        ...
        else
        {
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
                printf("Setting the PHY speed of a device is only available on Seagate "
                       "Drives.\n");
            }
            ret = NOT_SUPPORTED;
        }
...
bool is_SCT_Low_Current_Spinup_Supported(tDevice* device)
{
    ...
    {
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word206) &&
            device->drive_info.IdentifyData.ata.Word206 & BIT4)
        {
...
int is_Low_Current_Spin_Up_Enabled(tDevice* device, bool sctCommandSupported)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE && is_Seagate_Family(device) == SEAGATE)
    {
        ...
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word206) &&
            (device->drive_info.IdentifyData.ata.Word206 & BIT4) && sctCommandSupported)
        {
            uint16_t optionFlags = 0x0000;
            uint16_t state       = 0x0000;
            if (SUCCESS == send_ATA_SCT_Feature_Control(device, SCT_FEATURE_FUNCTION_RETURN_CURRENT_STATE,
        ...
        else if (!sctCommandSupported) // check the identify data for a bit (2.5"
                                       // drives only I think) - TJE
        {
...
eReturnValues seagate_SCT_Low_Current_Spinup(tDevice* device, eSeagateLCSpinLevel spinupLevel)
{
    ...
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word206) &&
        device->drive_info.IdentifyData.ata.Word206 & BIT4)
    {
        uint16_t saveToDrive = 0x0001; // always set this because this feature
                                       // requires saving for it to even work.
...
eReturnValues set_Low_Current_Spin_Up(tDevice* device, bool useSCTCommand, eSeagateLCSpinLevel state)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE && is_Seagate_Family(device) == SEAGATE)
    {
        ...
        else
        {
            ...
            if (state == SEAGATE_LOW_CURRENT_SPINUP_STATE_DEFAULT) // 0 means disable, 2 is
                                                                   // here for compatibility
                                                                   // with SCT command inputs
            {
...
eReturnValues set_SSC_Feature_SATA(tDevice* device, eSSCFeatureState mode)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        else
        {
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
                printf("Setting the SSC feature of a device is only available on "
                       "Seagate Drives.\n");
            }
            ret = NOT_SUPPORTED;
        }
...
eReturnValues get_SSC_Feature_SATA(tDevice* device, eSSCFeatureState* mode)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        else
        {
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
                printf("Getting the SSC feature of a device is only available on "
                       "Seagate Drives.\n");
            }
            ret = NOT_SUPPORTED;
        }
                                               ...
                                               bool     nonvolatile)
{
    ...
    if (family == SEAGATE || family == SEAGATE_VENDOR_A)
    {
        if (!is_SSD(device))
        {
            ...
            if (revertToDefaults)
            {
                // We need to read the default mode page to get the status of the JIT
                // bits, save them, then pass them along...
            ...
            if (readPage)
            {
                ...
                seagateUnitAttentionParameters[headerLength + 4] &=
                    0x70; // clear all bits to zero, except 4, 5, & 6 since those are
                          // reserved (and if they are ever used, we don't want to touch
                          // them now) - TJE
                if (!disableVjit)
                {
                    seagateUnitAttentionParameters[headerLength + 4] |= BIT7;
                }
                switch (jitMode) // Spec says that faster modes allow the drive to
                                 // continue accessing slower modes, so might as well
                                 // set each bit below the requested mode.
                {
                default:
                case 0:
...
eReturnValues seagate_Get_Power_Balance(tDevice* device, bool* supported, bool* enabled)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_Seagate_Family(device) == SEAGATE)
        {
            ...
            if (supported)
            {
                ...
                if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word149) &&
                    (device->drive_info.IdentifyData.ata.Word149 & BIT8 ||
                     device->drive_info.IdentifyData.ata.Word149 & BIT10))
                {
                    *supported = true;
                }
                else
            ...
            if (enabled)
            {
                ...
                if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word149) &&
                    (device->drive_info.IdentifyData.ata.Word149 & BIT9 ||
                     device->drive_info.IdentifyData.ata.Word149 & BIT11))
                {
                    *enabled = true;
                }
                else
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        //       This feature conflicts with use other use of this page, at least on
        //       old drives. New drives use the active power mode identifier and the
        //       "highest" means disabled and "lowest" means enabled for this
        //       feature - TJE
        if (is_Seagate_Family(device) == SEAGATE)
        {
            ...
            {
                // If this page is supported, we're calling power balance on SAS not
                // supported. Note: This may need changing in the future, but right now
                // this is still accurate - TJE
                if (supported)
                {
                    *supported = false;
                }
            ...
            {
                ret = SUCCESS;
                // This is as close as I can figure the best way to check for power
                // balance support - TJE
                /// Active mode cannot be changable, then the power consumption VPD page
                /// must also not be supported.
                // The above comment was true for OLD drives. New ones now use the
                // active power mode field to enable/disable this feature
                if (pcModePage[MODE_PARAMETER_HEADER_10_LEN + 7] == 0xFF &&
                    (M_GETBITRANGE(pcModePage[MODE_PARAMETER_HEADER_10_LEN + 6], 2, 0) == 0))
                {
                    // If in here, this is an old drive since it doesn't allow setting the
                    // active power mode.
                    if (supported)
                    {
                        *supported = true;
                ...
                {
                    // if in here, this is a new drive which only allows this change via
                    // the active mode field. On these drives, we can check to make sure
                    // the changable fields apply to the active mode field, but NOT the
                    // power condition identifier.
                    if (supported)
                    {
                        *supported = true;
                    }
...
eReturnValues seagate_Set_Power_Balance(tDevice* device, ePowerBalanceMode powerMode)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // First, need to read changable values page to see if this is a drive
        // needing the old method, or the new one. - TJE
        ...
        {
            // Detect the old method by seeing if active mode is not changable, but
            // power condition identifier is.
            if (pcModePage[MODE_PARAMETER_HEADER_10_LEN + 7] == 0xFF &&
                (M_GETBITRANGE(pcModePage[MODE_PARAMETER_HEADER_10_LEN + 6], 2, 0) == 0))
            {
                oldMethod = true;
            }
            // Assume for now that otherwise this is changable by modifying the active
            // field.
        }
                                          ...
                                          false, MPC_CURRENT_VALUES, pcModePage))
        {
            ...
            if (oldMethod)
            {
                // Active field is NOT used, only the power condition identifier, which
                // must be 0 or 1.
            ...
            {
                // Active field IS used and must be set to highest (disabled) or lowest
                // (enabled)
                if (powerMode == POWER_BAL_ENABLE)
                {
...
eReturnValues get_IDD_Support(tDevice* device, ptrIDDSupportedFeatures iddSupport)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        {
            uint8_t* iddDiagPage =
                C_CAST(uint8_t*, safe_calloc_aligned(12, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (iddDiagPage)
            {
...
eReturnValues get_IDD_Status(tDevice* device, uint8_t* status)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (iddDiagPage)
        {
            // do not use the return value from this since IDD can return a few
            // different sense codes with unit attention, that we may otherwise call
            // an error
            ...
            if (ret != SUCCESS)
            {
                ...
                get_Sense_Key_ASC_ASCQ_FRU(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, &senseKey, &asc,
                                           &ascq, &fru);
                // We are checking if the reset part of the test is still in progress by
                // checking for this sense data and will try again after a second until
                // we know this part of the test is complete.
                if (senseKey == SENSE_KEY_UNIT_ATTENTION && asc == 0x29 && ascq == 0x01 && fru == 0x0A)
                {
                    *status = 0x0F; // this is setting that the IDD is still in progress.
                                    // This is because we got specific sense data related
                                    // to this test, so that is why we are setting this
                                    // here instead of trying to look at the data buffer.
                    return SUCCESS;
                }
            }
...
void translate_IDD_Status_To_String(uint8_t status, char* translatedString, bool justRanDST)
{
    ...
    case 0x00:
        ...
        else
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The previous IDD routine completed without error or no IDD has "
                     "ever been run.");
        }
        break;
    case 0x01:
    ...
    case 0x02:
        snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                 "The IDD routine was interrupted by the host with a hardware or "
                 "software reset.");
        break;
    case 0x03:
        snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                 "A fatal error or unknown test error occurred while the device "
                 "was executing its IDD routine and the device was unable to "
                 "complete the IDD routine.");
        break;
    case 0x04:
        if (justRanDST)
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The IDD completed having a test element that failed and the "
                     "test element that failed is not known.");
        }
        else
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The previous IDD completed having a test element that failed "
    ...
    case 0x05:
        ...
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The IDD completed having the electrical element of the test "
                     "failed.");
        }
        else
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The previous IDD completed having the electrical element of "
                     "the test failed.");
        }
        break;
    case 0x06:
        ...
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The IDD completed having the servo (and/or seek) test element "
                     "of the test failed.");
        }
        else
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The previous IDD completed having the servo (and/or seek) test "
    ...
    case 0x07:
        ...
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The previous IDD completed having the read element of the test "
                     "failed.");
        }
        break;
    case 0x08:
        if (justRanDST)
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
                     "The IDD completed having a test element that failed and the "
                     "device is suspected of having handling damage.");
        }
        else
        {
            snprintf(translatedString, MAX_DST_STATUS_STRING_LENGTH,
...
static eReturnValues start_IDD_Operation(tDevice* device, eIDDTests iddOperation, bool captiveForeground)
{
    ...
    if (ret != SUCCESS)
    {
        if (device->drive_info.drive_type == SCSI_DRIVE)
        {
            // check the sense data. The problem may be that captive/foreground mode
            // isn't supported for the long test
            uint8_t senseKey = 0;
            uint8_t asc      = 0;
            uint8_t ascq     = 0;
            uint8_t fru      = 0;
            ...
            {
                // Do we need to check for asc = 26h, ascq = 0h? For now this should be
                // ok
                return NOT_SUPPORTED;
            }
            else
    ...
    {
        // we need to make sure we waited at least 2 minutes since command was sent
        // to the drive before pinging it with another command. It needs time to
        // spin back up and be ready to accept commands again. This is being done in
        // both captive/foreground and offline/background modes due to differences
        // between some drive firmwares.
    ...
    if (is_Seagate_Family(device) != NON_SEAGATE)
    {
        ...
        if (SUCCESS == get_IDD_Support(device, &iddSupport))
        {
                ...
                (IDDtest == SEAGATE_IDD_LONG && iddSupport.iddLong))
            {
                ...
                {
                    // SCSI says that the short test must be run in foreground...so let's
                    // do that for both ATA and SCSI... Long test may be ran in background
                    // or foreground
                ...
                // if we are here, then an operation isn't already in progress so time
                // to start it
                ...
                if (SUCCESS == result && captiveForeground)
                {
                    ...
                    else
                    {
                        ...
                        case 9: // ready to start...guessing this means success?
                            result = SUCCESS;
                            break;
                        case 0x0F: // still in progress. Go into the polling loop below to
                                   // finish waiting
                            result          = SUCCESS;
                        ...
                        default:
                            ...
                            else
                            {
                                ...
                                // just to make sure it finished. This is a special case for the
                                // long test since it can take more than 2 minutes to complete.
                                pollForProgress = true;
                            }
                            break;
                        }
                ...
                {
                    status = 0xF;                // assume that the operation is in progress until it
                                                 // isn't anymore
                    eReturnValues ret = SUCCESS; // for use in the loop below...assume
...
eReturnValues get_Power_Telemetry_Data(tDevice* device, ptrSeagatePwrTelemetry pwrTelData)
{
    ...
    // first, determine how much data there is, allocate memory, then read it all
    // into that buffer
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        {
            powerTelemetryLog = C_CAST(uint8_t*, safe_calloc_aligned(powerTelemetryLogSize, sizeof(uint8_t),
                                                                     device->os_info.minimumAlignment));
    ...
    if (ret == SUCCESS && powerTelemetryLogSize > 0)
    {
        ...
        // This makes it easy, so just need to convert to the host's endianness if
        // necessary
        ...
        // drive timestamps will be reported as uint64 in this structure so that
        // they can be converted to whatever is easy by other users
        ...
        for (uint16_t measurementNumber = 0;
             ...
             ++measurementNumber, measurementOffset += 6)
        {
            ...
            {
                // invalid or empty entry should only happen once at the end! loop
                // conditions should also protect from this!
                break;
            }
        }
    }
...
void show_Power_Telemetry_Data(ptrSeagatePwrTelemetry pwrTelData)
{
    if (pwrTelData)
    {
        ...
        {
            printf("\tMeasurement Time (seconds): 600\t (No previous request. "
                   "Free-running mode)\n");
        }
        else
        {
        ...
        // Note, while the spacing may not make much sense, it definitely works with
        // the widths below.
        printf("    #\t     Time       \t  5V Pwr (W)\t  12V Pwr (W)\t  Total (W)\n");
        uint16_t measurementCounter = 0;
        for (uint16_t measurementNumber = 0; measurementNumber < pwrTelData->numberOfMeasurements &&
             ...
             ++measurementNumber)
        {
            ...
            // NOTE: Original format was 10.6, 6.3, 6.3, 6.3. Trying to widen to match
            // the header
        ...
        if (measurementCounter > 0)
        {
            ...
            {
                printf(" 5 Volt Power (W):\tAverage: %6.3f \tMinimum: %6.3f \tMaximum: "
                       "%6.3f\n",
                       sum5v / measurementCounter, min5v, max5v);
            }
            if (pwrTelData->measurementFormat == 0 || pwrTelData->measurementFormat == 12)
            {
                printf("12 Volt Power (W):\tAverage: %6.3f \tMinimum: %6.3f \tMaximum: "
                       "%6.3f\n",
        ...
        {
            printf("NOTE: All power measurements are for the full device, not "
...
bool is_Seagate_Quick_Format_Supported(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE) // This is only available on SATA drives.
    {
        ...
        if (family == SEAGATE)
        {
            ...
            if (!supported)
            {
                // This above support bit was discontinued, so need to check for support
                // of other features..This is not a guaranteed "it will work" but it is
                // what we have to work with.
                bool stdDepopSupported = is_Depopulation_Feature_Supported(device, M_NULLPTR);
                // bool stdRepopSupported = is_Repopulate_Feature_Supported(device,
                // M_NULLPTR);
...
const char* print_ext_smart_id(uint8_t attrId)
{
    ...
    case VS_ATTR_ID_MAX_LIFE_TEMPERATURE:
        return "Max lifetime temperature"; /*VS_ATTR_ID_MAX_LIFE_TEMPERATURE for
                                              extended*/
...
{
    static uint64_t lsbGbErased = 0, msbGbErased = 0, lsbLifWrtToFlash = 0, msbLifWrtToFlash = 0, lsbLifWrtFrmHost = 0,
                    msbLifWrtFrmHost = 0, lsbLifRdToHost = 0, msbLifRdToHost = 0, lsbTrimCnt = 0, msbTrimCnt = 0;
...
eReturnValues clr_Pcie_Correctable_Errs(tDevice* device)
{
    if (is_Seagate_Family(device) == SEAGATE_VENDOR_SSD_PJ)
    {
        // const char *desc = "Clear Seagate PCIe Correctable counters for the given
        // device "; const char *save = "specifies that the controller shall save
        // the attribute";
...
static eReturnValues get_Seagate_ATA_DeviceStatistics(tDevice* device, ptrSeagateDeviceStatistics seagateDeviceStats)
{
    ...
    {
        uint8_t* deviceStatsLog =
            C_CAST(uint8_t*, safe_calloc_aligned(deviceStatsSize, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!deviceStatsLog)
        {
            return MEMORY_FAILURE;
        }
...
static void print_Seagate_ATA_DeviceStatistics(ptrSeagateDeviceStatistics seagateDeviceStats)
{
    ...
    for (uint8_t logEntry = 0; logEntry < maxLogEntries; ++logEntry)
    {
        ...
        case 11:
            print_TimeStamp_Statistics("ATA Security Erase Unit Enhanced Timestamp",
        ...
        case 21:
            print_TimeStamp_Statistics("ATA Security Erase Unit Enhanced Failure Timestamp",
...
eReturnValues get_Seagate_SCSI_Firmware_Numbers(tDevice* device, ptrSeagateSCSIFWNumbers fwNumbers)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE && SEAGATE == is_Seagate_Family(device))
    {
        ...
        if (SUCCESS == scsi_Inquiry(device, firmwareNumbersPage, 60, 0xC0, true, false))
        {
            ...
            memcpy(fwNumbers->servoFirmwareProductFamilyAndProductFamilyMemberIDs, &firmwareNumbersPage[56],
                   SERVO_PRODUCT_FAMILY_LEN);
        }
    }
    return ret;
}
