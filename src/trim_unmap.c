...
static bool is_ATA_Data_Set_Management_XL_Supported(tDevice* device)
{
    ...
    // NOTE: we can add reading the ID Data log through SMART read log, but there is no point. Everything should have it
    // in GPL that would support this command.
    return supported;
}
    ...
    case NVME_DRIVE:
        if (device->drive_info.IdentifyData.nvme.ctrl.oncs & BIT2)
        {
            ...
            if (maxTrimOrUnmapBlockDescriptors && maxLBACount)
            {
                ...
                {
                    // in Windows passthrough we rely on translation through SCSI unmap, so we need to meet the
                    // limitations we're given in it...-TJE For other drivers/interfaces we will assume that it is
                    // possible to issue this for now-TJE NOTE: If we find other OS's with limitations we may need to
...
eReturnValues nvme_Deallocate_Range(tDevice* device, uint64_t startLBA, uint64_t range)
{
    ...
    uint32_t      maxLBACount                    = UINT32_C(0);
    if (is_Trim_Or_Unmap_Supported(device, &maxTrimOrUnmapBlockDescriptors, &maxLBACount))
    {
        // We SHOULD only need one command since each range is a uint32 and more than large enough to get a majority of
        // the drive. Maybe even get a whole drive done in 2 ranges... BUT we may be limited by the OS.
        ...
        DECLARE_ZERO_INIT_ARRAY(uint8_t, deallocate,
                                4096); // This will hold the maximum number of ranges/descriptors we can.
...
eReturnValues ata_Trim_Range(tDevice* device, uint64_t startLBA, uint64_t range)
{
    ...
    if (is_Trim_Or_Unmap_Supported(device, &maxTrimOrUnmapBlockDescriptors, &maxLBACount))
    {
        ...
        uint64_t trimRange =
            C_CAST(uint16_t, M_Min(numberOfLBAsToTrim,
                                   maxRange)); // range must be FFFFh or less, so take the minimum of these two
        uint64_t trimDescriptors =
            ((numberOfLBAsToTrim + trimRange) - UINT64_C(1)) /
            trimRange; // need to make sure this division rounds up as necessary so the whole range gets TRIMed
        uint64_t trimBufferLen = C_CAST(uint64_t, (((trimDescriptors + maxEntriesPerPage) - 1) / maxEntriesPerPage) *
        ...
        uint32_t trimOffset = UINT32_C(0);
    ...
    if (is_Trim_Or_Unmap_Supported(device, &maxTrimOrUnmapBlockDescriptors, &maxLBACount))
    {
        ...
        uint32_t unmapDescriptors = C_CAST(
            ...
            ((((startLBA + range) - startLBA) + unmapRange) - 1) /
                unmapRange); // need to round this division up o we won't unmap the whole range that was requested
        ...
        uint32_t unmapCommandDataLen   = UINT32_C(0);
        uint32_t descriptorsPerCommand = UINT32_C(0);
        ...
        for (unmapCommands = 0; unmapCommands < numberOfUnmapCommandsRequired; unmapCommands++)
        {
            ...
            // unmap block descriptor data length (number of entries * 16...this should ALWAYS fit inside the buffer
            // length we calculated earlier)
