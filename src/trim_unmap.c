...
static bool is_ATA_Data_Set_Management_XL_Supported(tDevice* device)
{
    ...
    if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        ...
        if (SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_IDENTIFY_DEVICE_DATA, ATA_ID_DATA_LOG_SUPPORTED_PAGES,
                                                 logBuffer, LEGACY_DRIVE_SEC_SIZE, 0))
        {
            ...
            if (pageNumber == C_CAST(uint8_t, ATA_ID_DATA_LOG_SUPPORTED_PAGES) && revision >= 0x0001)
            {
                ...
                for (uint16_t iter = ATA_ID_DATA_SUP_PG_LIST_OFFSET;
                     iter < C_CAST(uint16_t, listLen + ATA_ID_DATA_SUP_PG_LIST_OFFSET) && iter < LEGACY_DRIVE_SEC_SIZE;
                     ++iter)
                {
    ...
    // NOTE: we can add reading the ID Data log through SMART read log, but there
    // is no point. Everything should have it in GPL that would support this
    // command.
    return supported;
}
...
bool is_Trim_Or_Unmap_Supported(tDevice* device, uint32_t* maxTrimOrUnmapBlockDescriptors, uint32_t* maxLBACount)
{
        ...
        if (M_NULLPTR != maxTrimOrUnmapBlockDescriptors)
        {
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word105))
            {
                *maxTrimOrUnmapBlockDescriptors =
                    device->drive_info.IdentifyData.ata.Word105 * 64; // multiple by 64 since you can fit a maximum of
                                                                      // 64 descriptors in each 512 byte block
            }
            else
            {
                *maxTrimOrUnmapBlockDescriptors = 64; // assume 1 512B block is supported since we didn't get a valid
                                                      // value otherwise.
            }
        }
        break;
        ...
        if (device->drive_info.IdentifyData.nvme.ctrl.oncs & BIT2)
        {
            ...
            if (maxTrimOrUnmapBlockDescriptors && maxLBACount)
            {
                ...
                if (!device->os_info.openFabricsNVMePassthroughSupported &&
                    !device->os_info.intelNVMePassthroughSupported)
                {
                    // in Windows passthrough we rely on translation through SCSI unmap,
                    // so we need to meet the limitations we're given in it...-TJE For
                    // other drivers/interfaces we will assume that it is possible to
                    // issue this for now-TJE NOTE: If we find other OS's with limitations
                    // we may need to change the #if or use some other kind of check
                    // instead.
                    ...
                    {
                        *maxTrimOrUnmapBlockDescriptors =
                            M_BytesTo4ByteValue(blockLimits[24], blockLimits[25], blockLimits[26], blockLimits[27]);
                        *maxLBACount =
                            M_BytesTo4ByteValue(blockLimits[20], blockLimits[21], blockLimits[22], blockLimits[23]);
                    }
    ...
    case SCSI_DRIVE:
    {
        ...
        uint8_t* lbpPage = C_CAST(uint8_t*, safe_calloc_aligned(VPD_LOGICAL_BLOCK_PROVISIONING_LEN, sizeof(uint8_t),
                                                                device->os_info.minimumAlignment));
        ...
        if (SUCCESS ==
            scsi_Inquiry(device, lbpPage, VPD_LOGICAL_BLOCK_PROVISIONING_LEN, LOGICAL_BLOCK_PROVISIONING, true, false))
        {
            if ((lbpPage[5] & BIT7) > 0)
            {
                supported = true;
...
eReturnValues nvme_Deallocate_Range(tDevice* device, uint64_t startLBA, uint64_t range)
{
    ...
    {
        // We SHOULD only need one command since each range is a uint32 and more
        // than large enough to get a majority of the drive. Maybe even get a whole
        // drive done in 2 ranges... BUT we may be limited by the OS.
        uint32_t contextAttributes = 0; // this is here in case we want to enable
                                        // setting these bits some time later. - TJE
        DECLARE_ZERO_INIT_ARRAY(uint8_t, deallocate,
                                4096); // This will hold the maximum number of
                                       // ranges/descriptors we can.
        ...
        // send the command(s) to the drive....currently only 1 command to do this.
        // May need to revisit later - TJE
        ret =
            nvme_Dataset_Management(device, C_CAST(uint8_t, descriptorCount - 1), true, false, false, deallocate, 4096);
...
eReturnValues ata_Trim_Range(tDevice* device, uint64_t startLBA, uint64_t range)
{
    ...
    if (is_Trim_Or_Unmap_Supported(device, &maxTrimOrUnmapBlockDescriptors, &maxLBACount))
    {
        ...
        uint64_t trimRange          = C_CAST(uint16_t, M_Min(numberOfLBAsToTrim,
                                                             maxRange)); // range must be FFFFh or less, so
                                                                         // take the minimum of these two
        uint64_t trimDescriptors =
            ((numberOfLBAsToTrim + trimRange) - UINT64_C(1)) / trimRange; // need to make sure this division rounds up
                                                                          // as necessary so the whole range gets TRIMed
        uint64_t trimBufferLen = C_CAST(uint64_t,
                                        (((trimDescriptors + maxEntriesPerPage) - 1) / maxEntriesPerPage) *
                                            LEGACY_DRIVE_SEC_SIZE); // maximum of 64 TRIM entries per sector
        uint8_t* trimBuffer    = C_CAST(uint8_t*, safe_calloc_aligned(C_CAST(size_t, trimBufferLen), sizeof(uint8_t),
                                                                      device->os_info.minimumAlignment));
        uint64_t trimLBA       = 0;
        ...
        uint16_t numberOfTRIMCommandsRequired =
        ...
        for (trimLBA = startLBA; trimLBA < finalLBA && (bufferIter + (entryLengthBytes - 1)) < trimBufferLen;
             trimLBA += trimRange, bufferIter += entryLengthBytes)
        {
            // make sure we don't go beyond the ending LBA for our TRIM, so adjust the
            // range to fit
        ...
        for (trimCommands = 0; trimCommands < numberOfTRIMCommandsRequired; trimCommands++)
        {
            ...
            {
                trimCommandLen = C_CAST(
                    uint32_t, trimBufferLen - (C_CAST(uint64_t, trimCommandLen) * C_CAST(uint64_t, trimCommands)));
            }
...
eReturnValues scsi_Unmap_Range(tDevice* device, uint64_t startLBA, uint64_t range)
{
    ...
    if (is_Trim_Or_Unmap_Supported(device, &maxTrimOrUnmapBlockDescriptors, &maxLBACount))
    {
        ...
        uint32_t unmapRange = C_CAST(uint32_t, M_Min(((startLBA + range) - startLBA),
                                                     maxLBACount)); // this may truncate but that is expected
                                                                    // since the maximum range you can specify
                                                                    // for a UNMAP command is 0xFFFFFFFF
        uint32_t unmapDescriptors = C_CAST(uint32_t, ((((startLBA + range) - startLBA) + unmapRange) - 1) /
                                                         unmapRange); // need to round this division up o we won't
                                                                      // unmap the whole range that was requested
        uint32_t unmapBufferLen =
            unmapDescriptors * UINT32_C(16); // this is JUST the descriptors. The header will need to
                                             // be tacked onto the beginning of this for each command.
        uint8_t* unmapBuffer =
            C_CAST(uint8_t*, safe_calloc_aligned(unmapBufferLen, sizeof(uint8_t), device->os_info.minimumAlignment));
        uint64_t unmapLBA      = 0;
        uint64_t bufferIter    = 0;
        uint32_t unmapCommands = 0;
        uint32_t numberOfUnmapCommandsRequired =
            ((unmapDescriptors + maxTrimOrUnmapBlockDescriptors) - UINT32_C(1)) / maxTrimOrUnmapBlockDescriptors;
        ...
        unmapCommandDataLen =
            M_Min(descriptorsPerCommand * 16, unmapBufferLen) + 8; // add 8 for the length of the header
        ...
        for (unmapLBA = startLBA, bufferIter = 0; unmapLBA < (startLBA + range) && bufferIter < unmapBufferLen;
             unmapLBA += unmapRange, bufferIter += UINT64_C(16))
        {
            // make sure we don't go beyond the ending LBA for our TRIM, so adjust the
            // range to fit
        ...
        // allocate a buffer to hold the descriptors AND header. Earlier buffer was
        // just to build the descriptors into it.
        ...
        for (unmapCommands = 0; unmapCommands < numberOfUnmapCommandsRequired; unmapCommands++)
        {
...
#endif
            // adjust the size to allocate a buffer based on the remaining number of
            // descriptors in the buffer
            ...
            // unmap block descriptor data length (number of entries * 16...this
            // should ALWAYS fit inside the buffer length we calculated earlier)
            ...
            // now copy the number of descriptors for this command into the allocated
            // buffer
