                             ...
                             bool                        hideLBACounter)
{
    ...
    {
        dataBuf = C_CAST(uint8_t*, safe_calloc_aligned(C_CAST(size_t, sectorCount * device->drive_info.deviceBlockSize),
    ...
    *failingLBA = UINT64_MAX; // this means LBA access failed
    ...
    {
        // check that current LBA + sector count doesn't go beyond the maxLBA for
        // the loop
            ...
            if (rwvCommand != RWV_COMMAND_VERIFY)
            {
                ...
                temp = C_CAST(uint8_t*,
                              safe_reallocf_aligned(C_CAST(void**, &dataBuf), 0,
                                                    C_CAST(size_t, sectorCount * device->drive_info.deviceBlockSize),
                                               ...
                                               C_CAST(uint32_t, sectorCount * device->drive_info.deviceBlockSize)))
        {
            ...
            // read command failure...so we need to read until we find the exact
            // failing lba
            for (; lbaIter <= maxSingleLoopLBA; lbaIter += logicalPerPhysical)
            {
                ...
                if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
                {
                    ...
                    case RWV_COMMAND_WRITE:
                        printf("\rWriting LBA: %-20" PRIu64 "",
                               lbaIter); // 20 wide is the max width for a unsigned 64bit
                                         // number
                        break;
                    case RWV_COMMAND_READ:
                        printf("\rReading LBA: %-20" PRIu64 "",
                                 ...
                                 bool                        hideLBACounter)
{
    ...
    uint64_t* randomLBAList  = C_CAST(uint64_t*, safe_calloc(randomLBACount, sizeof(uint64_t)));
    uint64_t  iterator       = 0;
    uint64_t  onePercentOfDrive =
        C_CAST(uint64_t, C_CAST(double, device->drive_info.deviceMaxLba) * 0.01); // calculate how many LBAs are 1% of
                                                                                  // the drive so that we read that many
    ...
    if (rwvCommand != RWV_COMMAND_VERIFY)
    {
        dataBuf = C_CAST(uint8_t*, safe_malloc(device->drive_info.deviceBlockSize * sizeof(uint8_t)));
        if (!dataBuf)
        {
    ...
    for (iterator = 0; iterator < randomLBACount; iterator++)
    {
        ...
        if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
        {
            ...
            case RWV_COMMAND_READ:
                printf("\rReading LBA: %-20" PRIu64 "",
                       randomLBAList[iterator]); // 20 wide is the max width for a
...
// TODO: Move this to generic_tests.h...also, should this be a separate
// parameter so some of these functions so the caller can get this data or
                                      ...
                                      bool                        hideLBACounter)
{
    ...
    uint8_t*      dataBuf                = M_NULLPTR;
    uint32_t      sectorCount            = get_Sector_Count_For_Read_Write(device);
    uint8_t       IDODTimeSeconds        = 45; // can be made into a function input if we
                                               // wanted
    ...
    {
        dataBufSize =
            C_CAST(size_t, device->drive_info.deviceBlockSize) * C_CAST(size_t, sectorCount) * sizeof(uint8_t);
        dataBuf = C_CAST(uint8_t*, safe_malloc_aligned(dataBufSize, device->os_info.minimumAlignment));
        if (!dataBuf)
        {
    ...
    while (difftime(time(M_NULLPTR), startTime) < IDODTimeSeconds && ODEndingLBA < device->drive_info.deviceMaxLba)
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
        {
            ...
            case RWV_COMMAND_READ:
                printf("\rReading LBA: %-20" PRIu64 "",
                       ODEndingLBA); // 20 wide is the max width for a unsigned 64bit
                                     // number
                break;
            case RWV_COMMAND_VERIFY:
                printf("\rVerifying LBA: %-20" PRIu64 "",
        ...
        // if (SUCCESS != read_LBA(device, ODEndingLBA, false, dataBuf,
        // C_CAST(uint32_t, sectorCount * device->drive_info.deviceBlockSize)))
    ...
    // issue this read to get the heads in the right place before starting the ID
    // test.
                                   ...
                                   bool                        hideLBACounter)
{
    ...
    uint32_t      sectorCount       = get_Sector_Count_For_Read_Write(device);
    // only one of these flags should be set. If they are both set, this makes no
    // sense
    ...
    if (errorLimit < 1)
    {
        ...
        errorList = C_CAST(errorLBA*, safe_calloc(1 * sizeof(errorLBA), sizeof(errorLBA)));
    }
    else
    {
        errorList = C_CAST(errorLBA*, safe_calloc(errorLimit * sizeof(errorLBA), sizeof(errorLBA)));
    }
    if (!errorList)
    {
    ...
    while (!errorLimitReached)
    {
                                      ...
                                      &errorList[errorIndex].errorAddress, updateFunction, updateData, hideLBACounter))
        {
            ...
            // set a new start for next time through the loop to 1 lba past the last
            // error LBA
            ...
            if (repairOnTheFly)
            {
                repair_LBA(device, &errorList[errorIndex], false, autoWriteReassign,
                           autoReadReassign); // This function will set the repair
                                              // status for us. - TJE
            }
            if (errorLimit != 0)
                errorIndex++;
    ...
    if (repairAtEnd)
    {
        ...
        for (errorIter = 0; errorIter < errorIndex; errorIter++)
        {
            ...
            {
                // check if the LBA we want to repair is within the same physical sector
                // as the last LBA
                if ((lastLBARepaired + logicalPerPhysicalSectors) > errorList[errorIter].errorAddress)
                {
                    // in this case, we have already repaired this LBA since the repair is
                    // issued to the physical sector, so move on to the next thing in the
                    // list
                              ...
                              bool                        hideLBACounter)
{
    ...
    errorList = C_CAST(errorLBA*, safe_calloc(errorLimit, sizeof(errorLBA)));
    if (!errorList)
    {
        perror("calloc failure\n");
        return MEMORY_FAILURE;
    ...
    if (rwvCommand == RWV_COMMAND_READ || rwvCommand == RWV_COMMAND_WRITE)
    {
        ...
        dataBuf = C_CAST(uint8_t*, safe_calloc_aligned(dataBufSize, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!dataBuf)
        {
    ...
    // make sure the starting LBA is alligned? If we do this, we need to make sure
    // we don't mess with the data of the LBAs we don't mean to start at...mostly
    // don't want to erase an LBA we shouldn't be starting at. startingLBA =
    // align_LBA(device, startingLBA); this is escentially a loop over the
           ...
           startingLBA < device->drive_info.deviceMaxLba)
    {
        ...
        if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
        {
            ...
            case RWV_COMMAND_WRITE:
                printf("\rWriting LBA: %-20" PRIu64 "",
                       startingLBA); // 20 wide is the max width for a unsigned 64bit
                                     // number
                break;
            case RWV_COMMAND_READ:
                printf("\rReading LBA: %-20" PRIu64 "",
                                               ...
                                               C_CAST(uint32_t, sectorCount * device->drive_info.deviceBlockSize)))
        {
            uint64_t maxSingleLoopLBA =
                startingLBA + sectorCount; // limits the loop to trying to only a certain number of
                                           // sectors without getting stuck at single LBA reads.
            // read command failure...so we need to read until we find the exact
            // failing lba
            for (; startingLBA <= maxSingleLoopLBA; startingLBA += 1)
            {
                ...
                if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
                {
                    ...
                    case RWV_COMMAND_WRITE:
                        printf("\rWriting LBA: %-20" PRIu64 "",
                               startingLBA); // 20 wide is the max width for a unsigned
                                             // 64bit number
                        break;
                    case RWV_COMMAND_READ:
                        printf("\rReading LBA: %-20" PRIu64 "",
            ...
            if (errorIndex < errorLimit)
            {
                startingLBA = errorList[errorIndex].errorAddress + 1;
                errorIndex++;
                continue; // continuing here since startingLBA will get incremented
                          // beyond the error so we pick up where we left off.
            }
            else
            {
                errorLimitReached = true;
                             ...
                             bool                        hideLBACounter)
{
    ...
    while (C_CAST(uint64_t, (lastTime = difftime(time(M_NULLPTR), startTime))) < timeLimitSeconds)
    {
        ...
        if (C_CAST(int64_t, innerLBA - sectorCount) < 0)
        {
            ...
            currentSectorCount = C_CAST(uint32_t,
                                        innerLBA); // this should set us up to read the remaining sectors to 0
        }
        if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
        {
            switch (rwvcommand)
        ...
        // set the sector count back to what it was before the next iteration in the
        // loop
                          ...
                          bool                        hideLBACounter)
{
    ...
    if (rwvcommand != RWV_COMMAND_VERIFY)
    {
        dataBuf = C_CAST(uint8_t*, safe_malloc(device->drive_info.deviceBlockSize * sectorCount * sizeof(uint8_t)));
        if (!dataBuf)
        {
            return MEMORY_FAILURE;
        }
    }
    seed_64(C_CAST(uint64_t,
                   time(M_NULLPTR))); // start the seed for the random number generator
    time(&startTime);                 // get the starting time before starting the loop
    double lastTime = 0.0;
    while (C_CAST(uint64_t, (lastTime = difftime(time(M_NULLPTR), startTime))) < timeLimitSeconds)
    {
...
// This function is very similar to the "user_Sequential_Test" call, but the
// error list is allocated outside of this function instead of having it self
// containted. Rather than change the user_Sequential_Test and make it
// potentially break others or complicated its already long list of parameters,
// I wrote this function instead.
                                  ...
                                  bool            hideLBACounter)
{
    ...
    // eReturnValues user_Sequential_Test(tDevice *device, eRWVCommandType
    // rwvCommand, uint64_t startingLBA, uint64_t range, uint16_t errorLimit, bool
    // stopOnError, bool repairOnTheFly, bool repairAtEnd, custom_Update
    // updateFunction, void *updateData)
    errorLBA* errorList   = C_CAST(errorLBA*, safe_calloc(errorLimit * sizeof(errorLBA), sizeof(errorLBA)));
...
// this function is similar to the range function, but looks for a time limit to
// run for instead.
                                           ...
                                           bool            hideLBACounter)
{
           ...
           startingLBA < device->drive_info.deviceMaxLba)
    {
                                               ...
                                               C_CAST(uint32_t, sectorCount * device->drive_info.deviceBlockSize)))
        {
            ...
            (*errorOffset)++;
            continue; // continuing here since startingLBA will get incremented beyond
                      // the error so we pick up where we left off.
        }
        startingLBA += sectorCount;
    }
    if (VERBOSITY_QUIET < device->deviceVerbosity && !hideLBACounter)
    ...
    if (numberOfLbasAccessed)
    {
        *numberOfLbasAccessed =
            startingLBA + sectorCount - *numberOfLbasAccessed; // subtract itself since it gets set to where we
                                                               // start at when we begin.
    }
    safe_free_aligned(&dataBuf);
    return ret;
}
                                 ...
                                 bool            hideLBACounter)
{
    ...
    if (inner && (ret == SUCCESS || (errorOffset < errorLimit && !stopOnError)))
    {
        ...
        {
            // NOTE: this guestimate can be improved by reading the negotiated
            // interface speed and using that as a maximum performance to get the
            // gestimate closer to the best place to start the ID scan...this would
            // work at least on ATA and SCSI well...USB would be more difficult to
            // guess properly. need to make a guess based on the amount of time we're
            // running where we'll start...let's assume the worse case of an accessing
            // at 550MB/s (max 6Gb/s transfer an SSD on SATA can get...shouldn't
            // happen)
...
eReturnValues full_Zero_Verify_Test(tDevice* device, bool hideLBACounter)
{
    ...
    for (uint64_t lbaIterator = 0; lbaIterator < device->drive_info.deviceMaxLba; lbaIterator += sectorCount)
    {
        ...
        uint8_t* dataBuffer = C_CAST(uint8_t*, safe_calloc_aligned(C_CAST(size_t, bufferSize), sizeof(uint8_t),
                                                                   device->os_info.minimumAlignment));
        if (!dataBuffer)
        {
        ...
        if (SUCCESS == read_Write_Seek_Command(device, RWV_COMMAND_READ, lbaIterator, dataBuffer, bufferSize))
        {
            ...
            uint8_t* validationBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(C_CAST(size_t, bufferSize), sizeof(uint8_t),
                                                     device->os_info.minimumAlignment));
            if (!validationBuffer)
            {
...
#define DRIVE_SECTIONS                                                                                                 \
    10000 // divide drive into these many sections and then pick 2 random LBA from
          // each section
...
eReturnValues quick_Zero_Verify_Test(tDevice* device, bool hideLBACounter)
{
    ...
    for (uint64_t sectionCounter = 0, counter = 0; sectionCounter < DRIVE_SECTIONS; sectionCounter++, counter += 2)
    {
        // first random LBA from section
        {
            ...
            uint8_t* dataBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(C_CAST(size_t, device->drive_info.deviceBlockSize),
                                                     sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!dataBuffer)
            {
                perror("\nfailed to allocate memory for reading data\n");
