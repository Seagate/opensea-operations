...
)
{
    ...
    if (logPath && logPathLen > 0)
    {
        ...
        // if there is no path separator at the end of this string, then we need to
        // append one before using this path below
    ...
    {
        // set to relative current working directory ./ or .\\ in Windows. This will
        // be expanded by secure file to a full path
        snprintf(path, OPENSEA_PATH_MAX, ".%c", SYSTEM_PATH_SEPARATOR);
    }
    if (logExt && logExtLen > 0)
    {
        // check if there is already an extension with a dot, otherwise add a dot
        // then the extension
        if (logExt[0] == '.')
        {
    ...
    {
        // no extension provided, but allocate an empty string so as not to cause
        // undefined behavrior in the format string interpretation below
...
// Idea: Try to recursively call this with identify commands to retry up to 5
// times to get a valid SN or ID before returning unknown
...
)
{
    ...
    // for us. NOTE: Always opened with X for exclusive so as not to overwrite an
    // existing file)
    *file = secure_Open_File(logGeneratedName, "wxb", M_NULLPTR, M_NULLPTR, M_NULLPTR);
    // Step 4: return error code depending on what happened from attempting to do
    // all of these steps
    if (!file)
    {
...
eReturnValues get_ATA_Log_Size(tDevice* device, uint8_t logAddress, uint32_t* logFileSize, bool gpl, bool smart)
{
    ...
    *logFileSize = 0; // make sure we set this to zero in case we don't find it.
    if (gpl && device->drive_info.ata_Options.generalPurposeLoggingSupported) // greater than one means check for
                                                                              // it in the GPL directory
    {
        ...
        {
            *logFileSize = M_BytesTo2ByteValue(logBuffer[(logAddress * 2) + 1], logBuffer[(logAddress * 2)]) *
    ...
    if (smart && !foundInGPL)
    {
        if (gpl)
        {
            // if we already tried the GPL buffer, make sure we clean it back up
            // before we check again just to be safe.
        ...
        {
            // The directory log is marked as optional and is supposed to return
            // aborted IF the drive does not support multi-sector logs. Only do this
            // for summary error, comprehensive error, self-test log, and selective
            // self-test log
            switch (logAddress)
            {
            ...
            case ATA_LOG_COMPREHENSIVE_SMART_ERROR_LOG:
                ...
                // If the drive does not report at least this version, do not return a
                // size for it as it will not be supported
...
eReturnValues get_SCSI_Log_Size(tDevice* device, uint8_t logPage, uint8_t logSubPage, uint32_t* logFileSize)
{
    ...
    *logFileSize = 0;
                                                         ...
                                                         LP_SUPPORTED_LOG_PAGES_AND_SUBPAGES, 0xFF, 0, logBuffer, 255))
    {
        ...
        {
            // the device did not return the data that is was asked to, so it does not
            // appear subpages are supported at all since this is only looking on this
            // page for subpages, call this page not supported since the returned list
            // was not as expected
            ret          = NOT_SUPPORTED;
            *logFileSize = 0;
        }
    ...
    // we know the page is supported, but to get the size, we need to try reading
    // it.
    if (ret == SUCCESS)
    {
        memset(logBuffer, 0, 255);
        // only requesting the header since this should get us the total length.
        // If this fails, we return success, but a size of zero. This shouldn't
        // happen, but there are firmware bugs...
        ...
        else // trust that the page is supported, but this method of getting the
             // size is broken on this firmware/device
        {
            *logFileSize = UINT16_MAX; // maximum transfer for a log page, so return this so that
                                       // the page can at least be read....
        }
    }
    safe_free_aligned(&logBuffer);
    return ret;
...
eReturnValues get_SCSI_VPD_Page_Size(tDevice* device, uint8_t vpdPage, uint32_t* vpdPageSize)
{
    ...
    uint8_t*      vpdBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(vpdBufferLength, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!vpdBuffer)
    {
                                      ...
                                      uint32_t*            modePageSize)
{
    ...
    // If device is older than SCSI2, DBD is not available and will be limited to
    // 6 byte command checking for this for old drives that may support mode
    // pages, but not the dbd bit properly Earlier than SCSI 2, RBC devices, and
    // CCS compliant devices are assumed to only support mode sense 6 commands.
    ...
    uint8_t* modeBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(modeLength, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!modeBuffer)
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
    ...
    *modePageSize = 0;
    if (!sixByte)
    {
        ...
        else
        {
            ...
            {
                // could be a mode page is not supported, or MPC is not a correct value,
                // among other errors Try checking sense key speciic
                if (senseFields.senseKeySpecificInformation.type == SENSE_KEY_SPECIFIC_FIELD_POINTER)
                {
                    // if we are getting a sense key specific field pointer, this is a SAS
                    // drive and there is no need to retry anthing. set this hack to valid
                    // as there is no need to retry the 6 byte command
                    ret = NOT_SUPPORTED;
                }
                ...
                {
                    // this hack has not already been tested for, so retry with mode sense
                    // 6
                    sixByte    = true;
                    retriedMP6 = true;
                }
    ...
    if (sixByte) // not an else because in the above if, we can retry the command
                 // as 6 byte if it doesn't work.
    {
        if (SUCCESS == scsi_Mode_Sense_6(device, modePage, C_CAST(uint8_t, modeLength), subpage, false, mpc,
                                         modeBuffer)) // don't disable block descriptors here since this is
                                                      // mostly to support old drives.
        {
            // validate the correct page was returned!
    ...
    // if we are here and this hack has not already been validated, then validate
    // it to skip future retries.
                                 ...
                                 bool*                used6ByteCmd)
{
    ...
    if (!sixByte)
    {
        // do not disable block descriptor for consistency between mode sense 6
        // and 10.
        bool longlba = false;
        if (device->drive_info.deviceMaxLba > UINT32_MAX)
        {
...
bool is_SCSI_Read_Buffer_16_Supported(tDevice* device)
{
    ...
    if (!device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations &&
        SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE, READ_BUFFER_16_CMD, 0,
                                                         20, reportSupportedOperationCode))
    {
                                          ...
                                          bool      useReadBuffer16)
{
    ...
    uint8_t* errorHistoryDirectory =
        C_CAST(uint8_t*, safe_calloc_aligned(SCSI_ERROR_HISTORY_DIRECTORY_LEN, sizeof(uint8_t),
                                             device->os_info.minimumAlignment));
                                     ...
                                     char*             fileNameUsed)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        uint32_t increment = 65536; // pulling in 64K chunks...this should be ok,
        ...
        historyBuffer =
            C_CAST(uint8_t*, safe_calloc_aligned(increment, sizeof(uint8_t), device->os_info.minimumAlignment));
        ...
        for (uint64_t offset = 0; offset < historyLen && offset <= maxOffset; offset += increment)
        {
            ...
            {
                // adjusting the pull size so we don't accidentally get an error from a
                // drive that doesn't want to return more than the maximum it told is in
                // this buffer ID.
            ...
            if (dataRetrieved)
            {
                ...
                if (logName && fileExtension) // Because you can also get a log file &
                                              // get it in buffer.
                {
                    if (!logFileOpened)
                    {
        ...
        {
            // size of error history is greater than the maximum possible offset for
            // the read buffer command, then need to return an error for truncated
            // data.
            ret = TRUNCATED_FILE;
        }
...
eReturnValues get_Pending_Defect_List(tDevice* device, const char* const filePath)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        // new is ACS4. Can be read with standard read log command if the drive
        // supports the log.
    ...
    {
        // this is new in SBC4. We can read this with a logsense command. (if the
        // drive supports it)
...
eReturnValues get_EPC_log(tDevice* device, const char* const filePath)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        // old code was reading address 0x12, however the ACS3 spec says 0x12 is the
        // NCQ Queue Management log and 0x08 is the Power Conditions log
        ret = get_ATA_Log(device, ATA_LOG_POWER_CONDITIONS, "EPC", "EPC", true, false, false, M_NULLPTR, 0, filePath,
                          LEGACY_DRIVE_SEC_SIZE * 2,
                          0); // sending in an override to read both pages in one command - TJE
    }
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
...
eReturnValues pull_SCSI_G_List(tDevice* device, const char* const filePath)
{
    ...
    uint32_t      defectDataSize         = 8; // set to size of defect data without any address descriptors so we
                                              // know how much we will be pulling
    uint8_t* defectData =
        C_CAST(uint8_t*, safe_calloc_aligned(defectDataSize, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!defectData)
    {
        return MEMORY_FAILURE;
    }
                          ...
                          uint16_t          featureRegister)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        if (GPL)
        {
            // read each log 1 page at a time since some can get to be so large some
            // controllers won't let you pull it.
            ...
            {
                // USB and IEEE 1394 should only ever be read 1 page at a time since
                // these interfaces use cheap bridge chips that typically don't allow
                // larger transfers.
            ...
            for (currentPage = 0; currentPage < numberOfLogPages; currentPage += pagesToReadAtATime)
            {
                ...
                {
                    // special case to allow skipping reading unavailable pages. Need to
                    // have already read page 0 Both of these logs use the same structure
                    // in the first 512B to indicate a list of supported pages. If reading
                    // either of these logs, we can skip a drive request when the page
                    // will come back as zeroes anyways This can be especially helpful for
                    // USB devices reading single sectors at a time on logs like device
                    // statistics.
                    bool skipAhead    = true;
                    bool doneChecking = false;
                    for (uint8_t pageIter = 0;
                         !doneChecking && pageIter < logBuffer[ATA_DEV_STATS_SUP_PG_LIST_LEN_OFFSET]; ++pageIter)
                    {
                        // offset = deviceStatsLog[ATA_DEV_STATS_SUP_PG_LIST_OFFSET +
                        // pageIter] * LEGACY_DRIVE_SEC_SIZE; check if the current page is
                        // in the list PLUS additional transfer length if reading more than
                    ...
                    if (skipAhead)
                    {
                        if (fileOpened)
                        {
                            // write out to a file
                            if (SEC_FILE_SUCCESS !=
                                secure_Write_File(fp_log, &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                                  C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE,
                                                  sizeof(uint8_t),
                                                  C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE, M_NULLPTR))
                            {
                                if (VERBOSITY_QUIET < device->deviceVerbosity)
                                {
                        ...
                        if (toBuffer)
                        {
                            if (bufSize >= logSize)
                            {
                                memset(&myBuf[currentPage * LEGACY_DRIVE_SEC_SIZE], 0,
                                       C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE);
                            }
                            else
                            {
                ...
                ret = send_ATA_Read_Log_Ext_Cmd(device, logAddress, currentPage,
                                                &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                ...
                if (ret == SUCCESS)
                {
                    ...
                    if (fileOpened)
                    {
                        ...
                        if (SEC_FILE_SUCCESS !=
                            secure_Write_File(fp_log, &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                              C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
                                              C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE, M_NULLPTR))
                        {
                            if (VERBOSITY_QUIET < device->deviceVerbosity)
                            {
                    ...
                    if (toBuffer)
                    {
                        if (0 != safe_memcpy(&myBuf[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                             bufSize - currentPage * LEGACY_DRIVE_SEC_SIZE,
                                             &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                             C_CAST(size_t, pagesToReadNow) * LEGACY_DRIVE_SEC_SIZE))
                        {
                            return BAD_PARAMETER;
        ...
        // if the log wasn't found in the GPL directory, then try reading from the
        // SMART directory
        if (!logFromGPL && SMART)
        {
...
//!   \param[in]  subpage - set this to something other than zero if looking for
//!   a specific subpage to a log \param[in]  logName - a string that is the
//!   name of the log (NO SPACES please!) M_NULLPTR if no file output needed
//!   \param[in]  fileExtension - a string for the file extension. You do not
//!   need to include a dot character. \param[in]  toBuffer - boolean flag
//!   specifying if you want to return data in buffer \param[in]  myBuf - buffer
//!   to return data in if toBuffer is true \param[in]  bufSize - size of the
//!   buffer to get data filled into it (use get_SCSI_Log_Size) \param[in]
//!   filePath - string with path to output the file to. Can be M_NULLPTR for
//!   current directory.
//!
//  Exit:
                           ...
                           const char* const filePath)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        {
            logBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(pageLen, sizeof(uint8_t), device->os_info.minimumAlignment));
        }
                               ...
                               C_CAST(uint16_t, pageLen)) == SUCCESS)
        {
            ...
            ret                         = SUCCESS;
            if (logName && fileExtension) // Because you can also get a log file & get
                                          // it in buffer.
            {
                                            ...
                                            uint32_t          transferSizeBytes)
{
    ...
    // check the GPL directory to make sure that the internal status log is
    ...
    if (SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_DIRECTORY, 0, dataBuffer, LEGACY_DRIVE_SEC_SIZE, 0))
    {
        ...
        if (M_BytesTo2ByteValue(dataBuffer[(islLogToPull * 2) + 1], dataBuffer[(islLogToPull * 2)]) > 0)
        {
            ...
            if (SUCCESS ==
                send_ATA_Read_Log_Ext_Cmd(device, islLogToPull, 0, dataBuffer, LEGACY_DRIVE_SEC_SIZE, 0x0001))
            {
                // now we need to check the sizes reported for the log and what the user
                // is requesting to pull (and save what we just read to a file)
                ...
                // check what the user requested us try and pull and set a size based
                // off of what the drive reports supporting (ex, if they asked for
                // large, but only small is available, return the small information set)
                switch (islDataSet)
                {
                case 3: // large
                     ...
                     pageNumber += C_CAST(uint16_t, (pullChunkSize / LEGACY_DRIVE_SEC_SIZE)))
                {
                    ...
                    // adjust pullcheck size so we don't try and request anything that's
                        ...
                        send_ATA_Read_Log_Ext_Cmd(device, islLogToPull, pageNumber, dataBuffer, pullChunkSize, 0))
                    {
                        ...
                        {
                            if (0 != safe_memcpy(&ptrData[pageNumber * LEGACY_DRIVE_SEC_SIZE],
                                                 dataSize - (pageNumber * LEGACY_DRIVE_SEC_SIZE), dataBuffer,
                                                 pullChunkSize))
                            {
                                safe_free_aligned(&dataBuffer);
                                return BAD_PARAMETER;
                                             ...
                                             uint32_t          transferSizeBytes)
{
    ...
    if (transferSizeBytes % LEGACY_DRIVE_SEC_SIZE)
    {
        // NOTE: We may be able to pull this in any size, but for now and for
        // compatibility only allow 512B sizes.
        return BAD_PARAMETER;
    }
    uint8_t* dataBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));
    ...
    if (SUCCESS == scsi_Read_Buffer(device, 0x3C, 0x01, 0, LEGACY_DRIVE_SEC_SIZE, dataBuffer))
    {
        ...
        // we just pulled the error history directory. We need to go through each of
        // the error history parameters until we find the current/saved parameter
        // for internal status log, otherwise the drive doesn't support internal
        // status log.
        for (errorHistoryIter = 32; errorHistoryIter < M_Min(errorHistoryLength, LEGACY_DRIVE_SEC_SIZE);
             errorHistoryIter += 8) // each error history parameter is 8 bytes long
        {
            if (currentOrSaved && dataBuffer[errorHistoryIter + 1] == 0x01)
            {
                ...
                else
                {
                    if (VERBOSITY_QUIET < device->deviceVerbosity)
                    {
                        printf("Found telemetry log in error history but length is 0! "
                               "Cannot pull the log!\n");
                    }
                    ret = NOT_SUPPORTED;
                }
        ...
        if (islSupported)
        {
            ...
            // now we can pull the first page of internal status log and find whether
            // the short/long pages are supported by the device
            if (SUCCESS == scsi_Read_Buffer(device, 0x1C, islLogToPull, pageNumber, LEGACY_DRIVE_SEC_SIZE, dataBuffer))
            {
                ...
                for (pageNumber = 1; pageNumber < islPullingSize; pageNumber += (pullChunkSize / LEGACY_DRIVE_SEC_SIZE))
                {
                    ...
                    if (SUCCESS == scsi_Read_Buffer(device, 0x1C, islLogToPull, pageNumber * LEGACY_DRIVE_SEC_SIZE,
                                                    pullChunkSize, dataBuffer))
                    {
                        // save to file, or copy to the ptr we were given
                        if (saveToFile)
                        {
                                             ...
                                             uint32_t          transferSizeBytes)
{
    ...
    if (device->drive_info.IdentifyData.nvme.ctrl.lpa & BIT3) // If this bit is set, then BOTH host and controller
                                                              // initiated are supported
    {
        uint8_t islLogToPull = 0;
        if (currentOrSaved)
        {
        ...
        }
        {
            ...
            if (SUCCESS == nvme_Get_Log_Page(device, &telemOpts))
            {
                     ...
                     pageNumber += C_CAST(uint16_t, (pullChunkSize / LEGACY_DRIVE_SEC_SIZE)))
                {
                    ...
                    // read each remaining chunk with the trigger bit set to 1 as thats
                    // what nvme-cli is doing - Deb
                    telemOpts.lsp     = 1;
...
eReturnValues print_Supported_SCSI_Logs(tDevice* device, uint64_t flags)
{
    ...
    {
        // either device doesn't support logs, or it just doesn't support subpages,
        // so let's try reading the list of supported pages (no subpages) before
        // saying we need to dummy up the list
    ...
    {
        retStatus            = SUCCESS;
        uint16_t logPageIter = LOG_PAGE_HEADER_LENGTH; // log page descriptors start on offset 4 and
...
// NOTE: smartAccess & gplAccess are used to show how a log can be read based on
// drive reporting.
//       driveReportBug exists for noting that a drive is incorrectly reporting
//       access for certain logs.
static void format_print_ata_logs_info(uint16_t log,
                                       uint32_t logSize,
                                       bool     smartAccess,
                                       bool     gplAccess,
                                       bool     driveReportBug)
...
#define ATA_LOG_ACCESS_STRING_LENGTH 10
    ...
    printf("   %3" PRIu16 " (%02" PRIX16 "h)   :     %-5" PRIu32 "      :    %-10" PRIu32 " :   %-10s\n", log, log,
           (logSize / LEGACY_DRIVE_SEC_SIZE), logSize, access);
}

// To be portable between old & new, SMART and GPL, we need to read both GPL and
// SMART directory. Combine the results, then show them on screen.
eReturnValues print_Supported_ATA_Logs(tDevice* device, uint64_t flags)
{
    eReturnValues retStatus           = NOT_SUPPORTED;
    bool          legacyDriveNoLogDir = false;
    uint8_t*      gplLogBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));
    uint8_t* smartLogBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));
    M_USE_UNUSED(flags);
    if (smartLogBuffer)
    {
        if (is_SMART_Enabled(device) && is_SMART_Error_Logging_Supported(device))
        {
            ...
            {
                legacyDriveNoLogDir = true; // for old drives that do not support multi-sector logs we
                                            // will rely on Identify/SMART data bits to generate the
                                            // output
    ...
    if (gplLogBuffer)
    {
        ...
        {
            if (SUCCESS !=
                send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_DIRECTORY, 0, gplLogBuffer, LEGACY_DRIVE_SEC_SIZE, 0))
            {
    ...
    if (gplLogBuffer || smartLogBuffer || legacyDriveNoLogDir)
    {
        ...
        for (log = 0; log < 0x80; log++)
        {
            ...
            {
                gplLogSize =
                    M_BytesTo2ByteValue(gplLogBuffer[(log * 2) + 1], gplLogBuffer[(log * 2)]) * LEGACY_DRIVE_SEC_SIZE;
            }
            if (smartLogBuffer)
            {
                smartLogSize = M_BytesTo2ByteValue(smartLogBuffer[(log * 2) + 1], smartLogBuffer[(log * 2)]) *
                               LEGACY_DRIVE_SEC_SIZE;
            }
            else if (legacyDriveNoLogDir)
            {
                // special case for old drives that support only single sector logs. In
                // this case they do not support the smart directory So for each log
                // being looked at, dummy up the size
                switch (log)
                {
                case ATA_LOG_SUMMARY_SMART_ERROR_LOG:
            ...
            // Here we need to check if the drive is reporting certain logs that are
            // only accessible with GPL or only with SMART to set the "bug" field
            switch (log)
            {
                // smart only
            case ATA_LOG_SUMMARY_SMART_ERROR_LOG:
            ...
            case ATA_LOG_CAPACITY_MODELNUMBER_MAPPING:
                ...
                break;
            default: // unknown log, or not a possible problem log. At this point in
                     // time, any thing not already in this list, likely no longer
                     // matters as it's almost all GPL only access or combination.
                bug = false;
                break;
            }
            if (bug)
        ...
        safe_free_aligned(&smartLogBuffer);
        safe_free_aligned(&gplLogBuffer);
        retStatus = SUCCESS; // set success if we were able to get at least one of
                             // the log directories to use
        if (legacyDriveNoLogDir)
        {
            printf("\nNOTE: SMART log detection came from identify & smart data "
                   "bits. This device\n");
            printf("      does not support the SMART log directory, and therefore no "
                   "multi-sector\n");
            printf("      logs either. It may not be possible to get all possible "
                   "logs on this device.\n\n");
        }
        if (atLeastOneBug)
        {
            printf("\nWARNING - At least one log was reported in a non-standard way "
                   "(GPL log in SMART\n");
            printf("          or SMART in GPL). Because of this, the access type may "
                   "be incorrect on\n");
            printf("          other non-standard logs. This may also lead to strange "
                   "behavior when\n");
            printf("          trying to get access to some of the logs since they "
                   "are not being\n");
            printf("          reported appropriately. The incorrectly reported logs "
                   "will have a \"!\"\n");
...
eReturnValues print_Supported_NVMe_Logs(tDevice* device, uint64_t flags)
{
    ...
    else if (is_Seagate_Family(device) == SEAGATE_VENDOR_SSD_PJ)
    {
        ...
        if (SUCCESS == nvme_Get_Log_Page(device, &suptLogOpts))
        {
            ...
            // Check if a bogus number is returned as the C5 log may be used
            // differently on some products There needs to be a better filter as this
            // C5 page was for 2 known products and newer ones have very different
            // designs
    ...
    else
    {
        ...
        // So if it is not supported, dummy up a response based on other reported
        // identify data bits and which logs are madatory in the NVMe specs.
        readSupporteLogPagesLog = true;
        dummyFromIdentify       = true;
    }
    ...
    if (readSupporteLogPagesLog)
    {
        uint8_t* supportedLogsPage =
            C_CAST(uint8_t*, safe_calloc_aligned(1024, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (supportedLogsPage)
        {
            ...
            if (SUCCESS == nvme_Get_Log_Page(device, &suptLogOpts))
            {
                ...
                {
                    // Using this macro so we don't have endianness issues with straight
                    // assignments or pointers - TJE
                    uint32_t currentLog =
            ...
            {
                // something went wrong, so fall back to dummying it up from identify
                // bits
                dummyFromIdentify = true;
            }
    ...
    if (dummyFromIdentify)
    {
        ...
        // 06 = device self test (look for bit in identify data for support of DST
        // feature)
        if (device->drive_info.IdentifyData.nvme.ctrl.oacs & BIT4)
        {
            printf("   6 (06h)\n");
        }
        // 07 & 08 = lpa bit 3telemetry host initiated and telepemtry controller
        // initiated log pages
        ...
        // 0A = predictable latency per NVM set - support in controller attributes
        // ctratt bit5
        ...
        // 0C = asymestric namespace access - anacap bit0 in controller identify???
        // or bit3 CMIC??? or oaes bit11???
...
// This function needs a proper rewrite to allow pulling with offsets, other log
// sizes, pulling to a buffer, and more like the SCSI and ATA functions.
...
#define NVME_LOG_NAME_SIZE 16
    ...
    /*switch (logNum) {
        case NVME_LOG_SMART_ID:
            ...
            {
            case SUCCESS:
                //nothing to print here since if it was successful, the log will
    be printed to the screen break; default: retStatus = 3; break;
            }
            break;
    ...
    be printed to the screen break; default: retStatus = 3; break;
        ...
        case NVME_LOG_FW_SLOT_ID:
            ...
            case SUCCESS:
                //nothing to print here since if it was successful, the log will
    be printed to the screen break; default:
...
eReturnValues print_Supported_SCSI_Error_History_Buffer_IDs(tDevice* device, uint64_t flags)
{
    ...
    uint8_t*      errorHistoryDirectory =
        C_CAST(uint8_t*, safe_calloc_aligned(errorHistorySize, sizeof(uint8_t), device->os_info.minimumAlignment));
    M_USE_UNUSED(flags);
    ...
    if (errorHistoryDirectory)
    {
            ...
            SUCCESS == scsi_Read_Buffer(device, 0x1C, 0, 0, errorHistorySize, errorHistoryDirectory))
        {
            ...
            if ((C_CAST(uint32_t, directoryLength) + (UINT32_C(32)) > errorHistorySize))
            {
                ...
                // realloc and re-read
                uint8_t* temp = C_CAST(uint8_t*, safe_realloc_aligned(errorHistoryDirectory, 2048, errorHistorySize,
                                                                      device->os_info.minimumAlignment));
                if (temp)
                {
                                          ...
                                          char*             logFileName)
{
    ...
    // First, setting up bools for GPL and SMART logging features based on drive
    // capabilities
    ...
    // Now, using switch case to handle KNOWN logs from ATA spec. Only flipping
    // certain logs as most every modern drive uses GPL and most logs are GPL
    // access now (but it wasn't always that way, and this works around some bugs
    // in drive firmware!!!)
    switch (logNum)
    {
    case ATA_LOG_SUMMARY_SMART_ERROR_LOG:
    ...
    case ATA_LOG_SMART_SELF_TEST_LOG:
    case ATA_LOG_SELECTIVE_SELF_TEST_LOG:
        // All of these logs are specified as access with SMART read log only, so
        // disabling GPL. All others should be accessible with GPL when supported.
        gpl = false;
        break;
    default:
        break;
                                ...
                                eLogPullMode      mode)
{
    ...
    uint8_t*        logBuffer          = C_CAST(
        uint8_t*, safe_calloc_aligned((32 * LEGACY_DRIVE_SEC_SIZE), sizeof(uint8_t), device->os_info.minimumAlignment));
            ...
            for (currentPage = 0; currentPage < numberOfLogPages; currentPage += pagesToReadAtATime)
            {
                ...
                if (SUCCESS ==
                    send_ATA_Read_Log_Ext_Cmd(device, logAddress,
                                              ...
                                              &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                              pagesToReadNow * LEGACY_DRIVE_SEC_SIZE, (uint16_t)issueFactory))
                {
                    if (!fileOpened)
                    {
                        if (SUCCESS == create_And_Open_Secure_Log_File_Dev_EZ(
                    ...
                    if (fileOpened)
                    {
                        ...
                        if (SEC_FILE_SUCCESS != secure_Write_File(fp_log,
                                                                  &logBuffer[currentPage * LEGACY_DRIVE_SEC_SIZE],
                                                                  (32 * LEGACY_DRIVE_SEC_SIZE), sizeof(uint8_t),
                                                                  (pagesToReadNow * LEGACY_DRIVE_SEC_SIZE), M_NULLPTR))
                        {
                            if (VERBOSITY_QUIET < device->deviceVerbosity)
                            {
                                perror("Error writing vpd data to a file!\n");
                            ...
                            eLogPullMode      mode)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        case SEAGATE_ATA_LOG_FARM_TIME_SERIES:
            ...
            default:
                ...
                // 1 (feature register 0) - Default: Report all FARM frames from disc
                // (~250ms) (SATA only) 2 (feature register 1) - Report all FARM data
                // (~250ms)(SATA only) 3 (feature register 2) - Return WLTR data (SATA
                // only) 4 (feature register 3) - Return Neural N/W data (SATA only)
                if (issueFactory == 2)
                {
        ...
        default:
            ...
            case PULL_LOG_RAW_MODE:
                ...
                // 0 - Default: Generate and report new FARM data but do not save to
                // disc (~7ms) (SATA only) 1 - Generate and report new FARM data and
                // save to disc(~45ms)(SATA only) 2 - Report previous FARM data from
                // disc(~20ms)(SATA only) 3 - Report FARM factory data from
                // disc(~20ms)(SATA only)
                ret = get_ATA_Log_Size(device, logAddress, &logSize, true, false);
                if (ret == SUCCESS && logSize > 0)
                {
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        case PULL_LOG_RAW_MODE:
            // FARM pull Factory subpages
            // 0 - Default: Generate and report new FARM data but do not save to disc
            // (~7ms) (SATA only) 4 - factory subpage (SAS only)
            if (issueFactory == 4)
            {
