...
//
// \file smart.c
// \brief This file defines the functions related to SMART features on a drive
...
eReturnValues get_SMART_Attributes(tDevice* device, smartLogData* smartAttrs)
{
        ...
        if (ret == SUCCESS)
        {
                 ...
                 smartIter += ATA_SMART_ATTRIBUTE_SIZE)
            {
                ...
                if (currentAttribute.attributeNumber > 0 && currentAttribute.attributeNumber < 255)
                {
                    ...
                    memcpy(&smartAttrs->attributes.ataSMARTAttr.attributes[currentAttribute.attributeNumber].data,
                           &currentAttribute, sizeof(ataSMARTAttribute));
                    // check if it's warrantied (This should work on Seagate drives at
                    // least)
            ...
            if (SUCCESS == ata_SMART_Read_Thresholds(device, ATAdataBuffer, LEGACY_DRIVE_SEC_SIZE))
            {
                     ...
                     smartIter += ATA_SMART_ATTRIBUTE_SIZE)
                {
                    ...
                    if (currentThreshold.attributeNumber > 0 && currentThreshold.attributeNumber < 255)
                    {
                        ...
                        memcpy(&smartAttrs->attributes.ataSMARTAttr.attributes[currentThreshold.attributeNumber]
                                    .thresholdData,
                               &currentThreshold, sizeof(ataSMARTThreshold));
                    }
                }
            }
        }
    ...
    else
    {
        ...
        {
            printf("Getting SMART attributes is not supported on this drive type at "
                   "this time\n");
        }
    }
    return ret;
...
void get_Attribute_Name(tDevice* device, uint8_t attributeNumber, char** attributeName)
{
    ...
    /*
    I broke the attribute name finder apart because sometimes there's overlap and
    sometimes there isn't. Also, this will let me name the attributes according to
    the respective specs for each drive.
    */
    ...
    case MAXTOR:
        ...
        case 8: // seek time performance
            ...
            break;
        case 9: // power on hours
            // internal spec says this is minutes, but not sure which drives report in
            // minutes. Old drives I have tested seem to do hours. may need to use
            // revision number
            snprintf(*attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "Power-On Hours");
            break;
    ...
    case SEAGATE_VENDOR_C:
        ...
        case 179: // Used Reserved Block Count for SSD
            ...
            break;
        case 180: // reported IOEDC Error In Interval (Seagate/Samsung), End to End
        ...
        case 182: // Erase Fail Count
            snprintf(*attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "Erase Fail Count");
            break;
        case 183: // PHY Counter Events (Seagate), SATA Downshift Count
                  // (Seagate-vendor-b-c)
            snprintf(*attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "SATA Downshift Count");
            break;
        case 184: // IOEDC Count (Seagate), End to End Error Detection Count
                  // (Seagate-vendor-b-c)
        ...
        case 188: // Command Timeout
            snprintf(*attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "Command Timeout");
            break;
        case 190: // Airflow Temperature (Seagate), SATA Error Counters
                  // (Seagate-vendor-b-c)
...
#define ATA_SMART_RAW_ATTRIBUTES_FLAGS_STRING_LEN (5)
    ...
    // clear out the attribute name before looping again so we don't show
    // dulicates
...
{
    // making the attribute name seperate so that if we add is_Seagate() logic in
    // we can turn on and off printing the name
    ...
    printf("       # Attribute Name:                     Status: Current: Worst: "
           "Thresh: Raw (hex):\n");
    ...
    printf("\n* Indicates warranty attribute type, also called Pre-fail "
           "attribute type\n");
    ...
    printf("~ - attribute has previously warned about its condition (thresholds "
           "required)\n");
    printf("\"Current\" is also referred to as the \"Nominal\" value in "
           "specifications.\n");
...
{
    ATA_SMART_ATTRIBUTE_RAW_HEX,                      // default, we don't know how to interpret so
                                                      // show the raw hex bytes
    ATA_SMART_ATTRIBUTE_TEMPERATURE_WST_LOW,          // Seagate format where raw 1:0 is
                                                      // current (same as nominal), 5:4 is
                                                      // lowest, worst ever is highest temp
    ATA_SMART_ATTRIBUTE_DECIMAL,                      // interpret specified raw bytes as a decimal
                                                      // value
    ATA_SMART_ATTRIBUTE_AIRFLOW_TEMP,                 // Seagate format where raw 1:0 is current,
                                                      // 2 is lowest, 3 is highest during this
                                                      // power cycle
    ATA_SMART_ATTRIBUTE_TEMPERATURE_RAW_CURRENT_ONLY, // Maxtor where raw 1:0
                                                      // handles current
                                                      // temperature, but no other
                                                      // values are reported
    ATA_SMART_ATTRIBUTE_TEMPERATURE_NOM_WST,          // Nominal is current temperature,
                                                      // worst is hottest temp. Lowest not
                                                      // reported.
    ATA_SMART_ATTRIBUTE_DECIMAL_UNIT_MB,              // Counter is in decimal and represents
                                                      // Mega Bytes
    ATA_SMART_ATTRIBUTE_PERCENTAGE,                   // attribute reports a percentage value
    ATA_SMART_ATTRIBUTE_TEMPERATURE_RAW_HIGH_CUR,     // reports current in raw 1:0
                                                      // and highest in 3:2. No lowest
    ATA_SMART_ATTRIBUTE_DECIMAL_UNIT_GIB,             // Reports a decimal counter using the
                                                      // units GiB NOT GB
...
#define ATTR_HYBRID_RAW_STRING_LENGTH                                                                                  \
    24 // Setting 24 to prevent truncation warnings from the temperature setup,
       // but real max is 16
...
#define ATTR_HYBRID_OTHER_FLAGS_LENGTH     4
        ...
        if (currentAttribute->thresholdDataValid)
        {
            ...
            {
                snprintf(thresholdValue, ATTR_HYBRID_THRESHOLD_VALUE_LENGTH, "%" PRIu8,
                         currentAttribute->thresholdData.thresholdValue);
            }
            if (currentAttribute->thresholdData.thresholdValue != ATA_SMART_THRESHOLD_ALWAYS_PASSING &&
        ...
        case ATA_SMART_ATTRIBUTE_DECIMAL:
            // use rawCounterMSB and rawCounterLSB to setup the decimal number for
            // display First things first, check that MSB is larger or smaller than
            // LSB offset to interpret correctly
        ...
        case ATA_SMART_ATTRIBUTE_TEMPERATURE_WST_LOW:
            ...
            // NOTE: This should always fit within 16 chars as temperatures should
            // never exceed 3 characters wide for any of them. Anything wider would be
            // a drive bug or garbage.
            //       Min temps will never be -100C or more and max will never be 120C
            //       or more, let alone 999C or more. This should be ok as the output
            //       below will be truncated. At worst, the final parenthesis will be
            //       cut off. - TJE
            snprintf(rawDataString, ATTR_HYBRID_RAW_STRING_LENGTH, "%" PRId16 " (m/M %" PRId16 "/%" PRId16 ")",
                     currentTemp, lowestTemp, highestTemp);
            break;
        case ATA_SMART_ATTRIBUTE_TEMPERATURE_RAW_HIGH_CUR:
            ...
            snprintf(rawDataString, ATTR_HYBRID_RAW_STRING_LENGTH, "%" PRId16 " (M %" PRId16 ")", currentTemp,
                     highestTemp);
            break;
        ...
        default: // if not known, use hex
            snprintf(rawDataString, ATTR_HYBRID_RAW_STRING_LENGTH,
...
static void print_Hybrid_ATA_Attributes(tDevice* device, smartLogData* smartData)
{
    ...
    printf("\t  ? - See analyzed output for more information on an attribute's "
           "raw data\n");
    ...
    printf("\t  m = minimum\n");
    printf("\t  M = maximum\n");
    printf("\t  All temperatures are reported in Celcius unless otherwise "
           "specified.\n");
    printf("\tColumns:\n");
    ...
    printf("\t  Raw - raw data associated with the attribute. Vendor specific "
           "definition.\n");
    ...
    for (uint8_t iter = 0; iter < 255; ++iter)
    {
        ...
        {
            get_Attribute_Name(device, iter, &attributeName);
            // The value printed in RAW for a given attribute in this mode depends on
            // the drive type and specific attribute.
            ...
            case SEAGATE:
                ...
                case 9: // power on hours
                    print_ATA_SMART_Attribute_Hybrid(&smartData->attributes.ataSMARTAttr.attributes[iter],
                                                     attributeName, ATA_SMART_ATTRIBUTE_DECIMAL, 3, 0, true);
                    break;
                case 10: // spin retry count
                    // raw unused
                    print_ATA_SMART_Attribute_Hybrid(&smartData->attributes.ataSMARTAttr.attributes[iter],
            ...
            case MAXTOR:
                ...
                case 194:
                    ...
                    break;
                default:
                    // From what I can tell in maxtor specs, everything is just a single
                    // counter - TJE
                    print_ATA_SMART_Attribute_Hybrid(&smartData->attributes.ataSMARTAttr.attributes[iter],
                                                     attributeName, ATA_SMART_ATTRIBUTE_DECIMAL, 3, 0, false);
                    break;
            ...
            case SEAGATE_VENDOR_E: // with Seagate for now. Might move sometime
                ...
                break;
            default: // unknown, not seagate, or we don't have enough information to
                     // provide a better interpretation at this time - TJE
                switch (iter)
                {
                case 1:
                case 4:
                ...
                case 194:
                    ...
                    // getting min/max seems to come from different locations if it is
                    // supported at all.
                    print_ATA_SMART_Attribute_Hybrid(&smartData->attributes.ataSMARTAttr.attributes[iter],
    ...
    {
        printf("WARNING: Interpretation of RAW data has not been verified on this "
               "device/firmware.\n");
        printf("         Product manuals and/or specifications are required for "
    ...
    for (uint8_t iter = 0; iter < UINT8_MAX; ++iter)
    {
        if (smartData->attributes.ataSMARTAttr.attributes[iter].valid)
        {
            ...
            if (smartData->attributes.ataSMARTAttr.attributes[iter].valid)
            {
                ...
                {
                    printf("\t\tPre-fail/warranty. Indicates a cause of known impending "
                           "failure.\n");
                }
                if (smartData->attributes.ataSMARTAttr.attributes[iter].data.status &
                ...
                {
                    printf("\t\tPerformance. Degredation of this attribute will affect "
                           "performance.\n");
                }
                ...
                case SEAGATE:
                    ...
                    case 9: // power on hours
                    {
                        ...
                        powerOnMinutes += (millisecondsSinceIncrement /
                                           UINT32_C(60000)); // convert the milliseconds to minutes, then add
                                                             // that to the amount of time we already know
                    ...
                    case 187: // Reported Un-correctable
                        printf(
                            "\tTotal # of Reported Uncorrectable Errors To The Host: "
                            "%" PRIu16 "",
                            M_BytesTo2ByteValue(smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[1],
                                                smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[0]));
                ...
                case SEAGATE_VENDOR_G:
                    ...
                    case 1:
                        printf(
                            "\tCorrectable, Soft LDPC correctable errors since last "
                            "power cycle: %" PRIu32 "\n",
                            M_BytesTo4ByteValue(smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[3],
                                                smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[2],
                                                smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[1],
                    ...
                    case 195:
                        ...
                        printf(
                            ...
                            M_BytesTo2ByteValue(smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[3],
                                                smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[2]));
                        printf(
                            "\tNumber of Times RAISE is Used to Restore Date Being "
                            "Programmed After a Program Failure: %" PRIu16 "\n",
                            M_BytesTo2ByteValue(smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[5],
                                                smartData->attributes.ataSMARTAttr.attributes[iter].data.rawData[4]));
                        break;
                    case 198:
...
eReturnValues print_SMART_Attributes(tDevice* device, eSMARTAttrOutMode outputMode)
{
    ...
    if (ret != SUCCESS)
    {
        ...
        {
            printf("Printing SMART attributes is not supported on this drive type at "
                   "this time\n");
        }
        else
        {
...
eReturnValues show_NVMe_Health(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == NVME_DRIVE)
    {
        ...
        if (ret != SUCCESS)
        {
            if (ret == NOT_SUPPORTED)
            {
                printf("Printing SMART/Health data is not supported on this drive type "
                       "at this time\n");
            }
            else
            {
        ...
        else
        {
            ...
            {
                printf("\tTemperature >= over temperature threshold or <= under "
                       "temperature threshold.\n");
            }
            if (smartData.attributes.nvmeSMARTAttr.criticalWarning & BIT2)
            {
                printf("\tNVM Subsystem reliability has been degraded due to media "
...
eReturnValues ata_SMART_Check(tDevice* device, ptrSmartTripInfo tripInfo)
{
    ...
    if (is_SMART_Enabled(device))
    {
        ...
        // NOTE: ATA-3 and up all report the return status as mandatory when smart
        // is supported and enabled
        //       HOWEVER: SFF-8035i lists this as an optional command.
        //       Always attempt a SMART return status command, then perform
        //       workarounds to get the status if it fails.
        ...
        else
        {
            ...
            // this MIGHT be available earlier than that in ACS-3 compliant drives,
            // but it is not super likely. this will be attempted, but may need to do
            // a attributes to thresholds comparison to know for sure.
            ret = get_ATA_SMART_Status_From_SCT_Log(device);
        }
        // Even though we may have already determined pass/fail, attempt to read the
        // attributes and thresholds for more comparison and detail It is possible
        // for some drives to give "warnings" for attributes that are not
        // warrantied, which would be useful to report when possible.
        if (SUCCESS == get_SMART_Attributes(device, &attributes))
        {
            // go through and compare attirbutes to thresholds (as long as the
            // thresholds were able to be read!!!)
            for (uint16_t counter = 0; counter < ATA_SMART_LOG_MAX_ATTRIBUTES; ++counter)
            {
                if (attributes.attributes.ataSMARTAttr.attributes[counter].valid)
                {
                    if (attributes.attributes.ataSMARTAttr.attributes[counter].thresholdDataValid)
                    {
                        ...
                        {
                            ret = SUCCESS; // need to set this to "pass" since we will otherwise
                                           // keep a unknown status or not supported status
                        }
                                 ...
                                 ATA_SMART_THRESHOLD_ALWAYS_FAILING)
                        {
                            ...
                            ret = FAILURE; // this should override the "unknown" return value
                                           // if it was set
                            if (tripInfo)
                            {
                                ...
                                char* attributeName =
                                    C_CAST(char*, safe_calloc(MAX_ATTRIBUTE_NAME_LENGTH, sizeof(char)));
                                get_Attribute_Name(device, tripInfo->ataAttribute.attributeNumber, &attributeName);
                                if (safe_strlen(attributeName))
                                {
                                    // use the name in the error reason
                                    snprintf(tripInfo->reasonString, UINT8_MAX, "%s [%" PRIu8 "] set to test trip!",
                                ...
                                else
                                {
                                    ...
                                    snprintf(tripInfo->reasonString, UINT8_MAX,
                        ...
                        /*before evaluating attributes, make sure all the values are in the
                           valid range per SFF-8035 (01h-FDh)*/
...
#define ATA_SMART_WHEN_FAILED_MAX_STR_LEN (12)
                            if (attributes.attributes.ataSMARTAttr.attributes[counter].isWarrantied)
                            {
                                ...
                                ret = FAILURE; // this should override the "unknown" return
                                               // value if it was set
                                if (tripInfo)
                                {
                            ...
                            {
                                // This attribute is not a warrantied failure, but it is
                                // generating a warning that could be helpful to report -TJE
                                // Using IN_PROGRESS for warning like SCSI code uses.
                                // Do not break if this is found because it is possible for
                                // warnings and failure to exist on different attributes. So
                                // store this until more detail is uncovered.
                                ret = IN_PROGRESS;
                                if (tripInfo)
                                {
...
#define SCSI_FAILURE_REASON_STRING_LENGTH 40
            ...
            if (failureReasonMissing || impendingFailureMissing)
            {
                ...
                {
                    snprintf(reasonString, reasonStringMaxLength, "%s - unknown ascq %" PRIu8 "", impendingFailure,
                             ascq);
                }
            }
            else
    ...
    case 0x0B:
        ...
        {
        case 0x00:
            // This only means "WARNING" which isn't very useful....so I'm not
            // translating it right now. - TJE
            break;
        case 0x01:
...
eReturnValues scsi_SMART_Check(tDevice* device, ptrSmartTripInfo tripInfo)
{
    ...
    bool temporarilyEnableMRIEMode6 = false; // This will hold if we are changing the mode from a value of 1-5
                                             // to 6. DO NOT CHANGE IT IF IT IS ZERO! We should return
                                             // NOT_SUPPORTED in this case. - TJE
    uint32_t delayTimeMilliseconds = 0;      // This will be used to make a delay only if the interval is a value
        ...
        infoExceptionsLog.isValid)
    {
        ...
        {
            // This is supposed to be the most consistent way of determining this...it
            // should work always so long as the page is supported.
            ...
            {
                ret = IN_PROGRESS; // using this to signify that a warning is being
        ...
        {
            // got the log and mode page...need to check mode page settings to see if
            // an error get's logged and the MRIE value so we can attempt a request
            // sense.
            if (infoExceptionsControl.isValid)
            {
                ...
                {
                case 1: // asynchronous event reporting (not supported on Seagate
                        // drives)
                case 2: // Generate Unit attention (sense key 6, asc = 5D. Fail command,
                        // no data transfer)
                case 3: // Conditionally generate recovered error (sense key 1, asc 5D
                        // on command that normall generates good status. Honors PER
                        // bit)
                case 4: // Unconditionally generate recovered error (sense key 1, asc 5D
                        // on command that normall generates good status. Independent of
                        // PER bit)
                case 5: // Generate No Sense (sense key 0, asc 5D)
                    temporarilyEnableMRIEMode6 = true;
                    sendRequestSense           = true;
                    break;
                case 6: // issue request sense. We may need to change the interval or
                        // reporting count first....
                    sendRequestSense = true;
                    // we need to check the interval and the report count
                    // fields...depending on what these are, we may need to either wait or
                    // make a mode page change
                ...
                default: // unknown or not supported value
                    // not enabled, return NOT_SUPPORTED. Make them use the --setMRIE
                    // option to change to something else first
                    ret = NOT_SUPPORTED;
                    break;
                }
            }
            ...
            {
                // uhh...just try request sense or return NOT_SUPPORTED???...I don't
                // think this case should ever get hit - TJE
                sendRequestSense = true;
            }
        }
    }
    ...
    {
        // This device doesn't support the log page or mode page...so just try a
        // request sense and see what the sense data gives.
        sendRequestSense = true;
    }
    if (temporarilyEnableMRIEMode6)
    {
        delayTimeMilliseconds = 100; // 100 milliseconds to match our temporary change
        // change MRIE mode to 6, PS = 0 and SP = false to change this temporarily
        // so we can issue a request sense.
        ...
        tempControl.ewasc         = true; // turn on warnings for the check since we are making a temporary
                                          // change...TODO: determine above if we should turn this on all
                                          // the time or not (if it's not already on)
    ...
    if (delayTimeMilliseconds > 0 && delayTimeMilliseconds <= 1000) // do not wait longer than a second...should be
                                                                    // caught above, but just in case....-TJE
    {
    ...
    {
        uint8_t* senseData =
            C_CAST(uint8_t*, safe_calloc_aligned(SPC3_SENSE_LEN, sizeof(uint8_t), device->os_info.minimumAlignment));
    ...
    if (temporarilyEnableMRIEMode6)
    {
            ...
            get_SCSI_Informational_Exceptions_Info(device, MPC_SAVED_VALUES, &savedControlSettings, M_NULLPTR))
        {
            ...
            {
                // try again with the save bit set to false...shouldn't happen but we
                // need to try to get this back to the user's other settings.
    ...
    if (SUCCESS == nvme_Get_Log_Page(device, &smartPageOpts))
    {
        ...
        if (tripInfo && ret == FAILURE)
        {
            ...
            if (smartLogPage[0] & BIT1)
            {
                ...
                snprintf(tripInfo->reasonString, UINT8_MAX,
                         "Temperature is above an over temperature threshold or below "
                         "an under temperature threshold");
                tripInfo->reasonStringLength = C_CAST(uint8_t, safe_strlen(tripInfo->reasonString));
            }
            if (smartLogPage[0] & BIT2)
            {
                ...
                snprintf(tripInfo->reasonString, UINT8_MAX,
                         "NVM subsystem reliability has been degraded due to "
                         "significant media related errors or an internal error that "
...
bool is_SMART_Enabled(tDevice* device)
{
    ...
    case SCSI_DRIVE:
    {
        // read the informational exceptions mode page and check MRIE value for
        // something other than 0
        uint8_t* infoExceptionsControl =
            C_CAST(uint8_t*, safe_calloc_aligned(12 + MODE_PARAMETER_HEADER_10_LEN, sizeof(uint8_t),
...
bool is_SMART_Check_Supported(tDevice* device)
{
    ...
    case NVME_DRIVE:
        ...
        break;
    case SCSI_DRIVE:
        // For SMART Check on SCSI, first look for the informational exceptions log
        // page to be supported...then look for the mode page. At least one of these
        // has to be available to do this.
        {
            uint32_t logSize = 0;
            ...
            {
                // check if the mode page is supported...at least then we can attempt
                // the other methods we have in this code to check for a trip.
...
eReturnValues get_Pending_List_Count(tDevice* device, uint32_t* pendingCount)
{
    ...
    {
        // get by reading the pending defects log page (SBC4) parameter 0, which is
        // a count
...
eReturnValues get_Grown_List_Count(tDevice* device, uint32_t* grownCount)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // get by reading the grown list since it contains a number of entries at
        // the beggining
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
            ...
            device->drive_info.IdentifyData.ata.Word206 & BIT0 && device->drive_info.IdentifyData.ata.Word206 & BIT4)
        {
            ...
            case SCT_FEATURE_CONTROL_SET_HDA_TEMPERATURE_INTERVAL:
                ...
                if (defaultValue)
                {
                    // for this we need to read the "sample period" from the SCT data
                    // tables command...not supported for now
...
eReturnValues enable_Disable_SMART_Feature(tDevice* device, bool enable)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        {
            ret = NOT_SUPPORTED; // leave as this since the drive doesn't support this
                                 // mode page
        }
    }
    return ret;
                                                     ...
                                                     ptrInformationalExceptionsLog     logData)
{
    ...
    // if logData is non-null, read the log page...do this first in case a mode
    // select is being performed after this function call!
    if (logData)
    {
        uint8_t* infoLogPage = C_CAST(uint8_t*, safe_calloc_aligned(LP_INFORMATION_EXCEPTIONS_LEN, sizeof(uint8_t),
                                                                    device->os_info.minimumAlignment));
        if (infoLogPage)
        {
    ...
    uint8_t* infoControlPage =
        C_CAST(uint8_t*, safe_calloc_aligned(MODE_PARAMETER_HEADER_10_LEN + MP_INFORMATION_EXCEPTIONS_LEN,
...
eReturnValues print_SMART_Info(tDevice* device, ptrSmartFeatureInfo smartInfo)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        case 4:
        case 0x84:
            printf("Off-line data collection activity was suspended by an "
                   "interrupting command from host");
            break;
        case 5:
        case 0x85:
            printf("Off-line data collection activity was aborted by an interrupting "
                   "command from host");
            break;
        case 6:
        case 0x86:
            printf("Off-line data collection activity was aborted by the device with "
                   "a fatal error");
            break;
        default:
            // vendor specific
        ...
        {
        case 0:
            printf("Self-test routine completed without error or no self-test status "
                   "is available");
            break;
        case 1:
            ...
            break;
        case 2:
            printf("The self-test routine was interrupted by the host with a "
                   "hardware or software reset");
            break;
        case 3:
            printf("A fatal error or unknown test error occurred while the device "
                   "was executing its self-test routine and the device was unable to "
                   "complete the self-test routine");
            break;
        case 4:
            printf("The previous self-test completed having a test element that "
                   "failed and the test element that failed is not known");
            break;
        case 5:
            printf("The previous self-test completed having the electrical element "
                   "of the test failed");
            break;
        case 6:
            printf("The previous self-test completed having the servo and/or seek "
                   "test element of the test failed");
            break;
        case 7:
            printf("The previous self-test completed having the read element of the "
                   "test failed");
            break;
        case 8:
            printf("The previous self-test completed having a test element that "
        ...
        if (is_SMART_Enabled(device) && is_SMART_Error_Logging_Supported(device)) // must be enabled to read this page
        {
            ...
            if (smartErrorLogSize > 0)
            {
                ...
                if (SUCCESS == getLog || WARN_INVALID_CHECKSUM == getLog)
                {
                    ...
                    {
                        uint32_t offset = UINT32_C(2) + ((C_CAST(uint32_t, errorLogIndex) - UINT32_C(1)) *
                                                         SUMMARY_SMART_ERROR_LOG_ENTRY_SIZE); // first entry is at
                                                                                              // offset 2, each entry is
                                                                                              // 90 bytes long
                               ...
                               entryCount < smartErrorLog->deviceErrorCount)
                        {
                            ...
                            if (is_Empty(&errorLog[offset], SUMMARY_SMART_ERROR_LOG_ENTRY_SIZE))
                            {
                                ...
                                else // we must be at the 1st entry, so we need to reset to
                                     // the end
                                {
                            ...
                            // each entry has 5 command data structures to fill in followed by
                            // error data
                            ...
                            for (uint8_t commandEntry = 0; commandEntry < 5;
                            ...
                            smartErrorLog->smartError[smartErrorLog->numberOfEntries].error.status =
                                errorLog[offset + 67];
                            memcpy(smartErrorLog->smartError[smartErrorLog->numberOfEntries]
                                       .error.extendedErrorInformation,
                                   &errorLog[offset + 68], 19);
                            ...
                            else // we must be at the 1st entry, so we need to reset to the
                                 // end
                            {
                                offset = UINT32_C(362); // final entry in the log
                            }
                                                    ...
                                                    bool                          forceSMARTLog)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (is_SMART_Enabled(device) && is_SMART_Error_Logging_Supported(device)) // must be enabled to read this page
        {
            ...
            // now check for GPL summort so we know if we are reading the ext log or
            // not
                ...
                compErrLogSize > 0)
            {
                ...
                // We will read each sector of the log as we need it to help with some
                // USB compatibility (and so we don't read more than we need)
                ...
                if (compErrLogSize > 0)
                {
                    ...
                    if (getLog == SUCCESS || getLog == WARN_INVALID_CHECKSUM)
                    {
                        ...
                        if (errorLogIndex > 0)
                        {
                            ...
                            pageNumber = errorLogIndex / EXT_COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE; // 4 entries
                                                                                                        // per page
                            while (
                                ...
                                smartErrorLog->numberOfEntries < smartErrorLog->deviceErrorCount && smartErrorLog->numberOfEntries < (UINT8_C(4) * maxPage) /*make sure we don't go beyond the number of pages the drive actually has*/)
                            {
                                while (pageIter <= maxPage)
                                {
                                    ...
                                    if (getLog == SUCCESS || getLog == WARN_INVALID_CHECKSUM)
                                    {
                                        ...
                                        while (pageEntryNumber <
                               EXT_COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE &&
                           pageEntryCounter <
                               EXT_COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE &&
                           smartErrorLog
                                   ->numberOfEntries < (UINT8_C(4) * maxPage) /*make sure we don't go beyond the number of pages the drive actually has*/) // 4 entries per page in the ext log
                                        {
                                            uint32_t offset = (C_CAST(uint32_t, pageEntryNumber) *
                                                               EXT_COMP_SMART_ERROR_LOG_ENTRY_SIZE) +
                                                              EXT_COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE;
                                            --pageEntryNumber; // decrement now before we forget. This
                                                               // is so that we roll backwards since
                                                               // this log appends. If this rolls over
                                                               // to UINT8_MAX, we'll break this loop
                                            ...
                                            // each entry has 5 command data structures to fill in
                                            // followed by error data
                                            smartErrorLog->extSmartError[smartErrorLog->numberOfEntries]
                                                .extDataStructures = true;
                                            // NOTE: don't memcpy since we aren't packing the structs
                                            uint32_t commandEntryOffset = offset;
                                            for (uint8_t commandEntry = 0; commandEntry < 5; ++commandEntry,
                                            ...
                                            memcpy(smartErrorLog->extSmartError[smartErrorLog->numberOfEntries]
                                                       .extError.extendedErrorInformation,
                                                   &errorLog[offset + 102], 19);
                                            smartErrorLog->extSmartError[smartErrorLog->numberOfEntries]
                                        ...
                                        // reset the pageEntry number to 4 for next page (start at
                                    ...
                                    if (pageNumber > 0)
                                    {
                                        --pageNumber; // go to the previous page of the log to read
                                                      // entries in order
                                    }
                                    else
                                    {
            ...
            else // GPL log was not available or did not read correctly.
            {
                ...
                // read the first sector to get index and device error count. Will read
                // the full thing if those are non-zero
                get_ATA_Log_Size(device, ATA_LOG_COMPREHENSIVE_SMART_ERROR_LOG, &compErrLogSize, false, true);
                if (compErrLogSize > 0)
                {
                    ret = SUCCESS;
                    uint8_t* errorLog =
                        C_CAST(uint8_t*, safe_calloc_aligned(512, sizeof(uint8_t), device->os_info.minimumAlignment));
                    if (!errorLog)
                    {
                        return MEMORY_FAILURE;
                    }
                    ...
                    if (getLog == SUCCESS || getLog == WARN_INVALID_CHECKSUM)
                    {
                        ...
                        if (errorLogIndex > 0)
                        {
                            ...
                            uint8_t* temp =
                                C_CAST(uint8_t*, safe_realloc_aligned(errorLog, 512, compErrLogSize * sizeof(uint8_t),
                                                                      device->os_info.minimumAlignment));
                            if (!temp)
                            {
                            ...
                            if (getLog == SUCCESS || getLog == WARN_INVALID_CHECKSUM)
                            {
                                ...
                                // First, figure out the first page to read. Next: need to
                                // handle switching between pages as we fill in the structure
                                // with data.
                                uint16_t pageNumber =
                                    errorLogIndex / COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE; // 5 entries per
                                                                                               // page
                                ...
                                //               3 * 90 + 2 gets us to the entry offset on the
                                //               page we need = 272, which is 4th entry on the
                                //               page (5th page) this gets us entry 4 on page 5
                                //               which is entry number 28
                                // Now, we need to loop through the data and jump between pages.
                                // go until we fill up our structure with a max number of
                                // entries
                                while (
                                    ...
                                    smartErrorLog->numberOfEntries < smartErrorLog->deviceErrorCount && smartErrorLog->numberOfEntries < (UINT8_C(5) * maxPages) /*make sure we don't go beyond the number of pages the drive actually has*/)
                                {
                                    while (pageIter <= maxPages)
                                    {
                                        ...
                                        while (pageEntryNumber <
                               COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE &&
                           pageEntryCounter <
                               COMP_SMART_ERROR_LOG_MAX_ENTRIES_PER_PAGE &&
                           smartErrorLog
                                   ->numberOfEntries < (UINT8_C(5) * maxPages) /*make sure we don't go beyond the number of pages the drive actually has*/)
                                        {
                                            // calculate the offset of the first entry we need to read
                                            // from this page
                                            offset =
                                            ...
                                            --pageEntryNumber; // decrement now before we forget. This
                                                               ...
                                                               // to UINT8_MAX, we'll break this loop
                                                               // and read another page.
                                            // read the entry into memory if it is valid, otherwise
                                            // continue the loop check if the entry is empty
                                        ...
                                        if (pageNumber > 0)
                                        {
                                            --pageNumber; // go to the previous page of the log to
...
// only to be used for the commands defined in the switch! Other commands are
...
{
    bool isLBAMode = (device & LBA_MODE_BIT); // almost everything should be LBA mode. Only a few CHS
                                              // things should be here, but we need to handle them
    ...
    if (async)
    {
        ...
        else // old dma queued commands
        {
            ...
            if (isLBAMode) // probably not necessary since these commands only ever
                           // reference LBA mode...
            {
                uint32_t readSecLBA = C_CAST(uint32_t, M_Nibble0(device)) << 24;
                readSecLBA |= M_DoubleWord0(lba) & UINT32_C(0x00FFFFFF); // grabbing first 24 bits only since
    ...
    else
    {
        if (ext)
        {
            ...
            if (isLBAMode)
            {
                if (stream)
                {
                    ...
                    {
                        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                                 "%s - LBA: %" PRIu64 " Count: %" PRIu32 " StreamID: %" PRIu8 " CCTL: %" PRIu8
                                 " Urgent: %d WC: %d Flush %d HSE: %d",
                ...
                {
                    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - LBA: %" PRIu64 " Count: %" PRIu32 "",
                             commandName, lba, sectorsToTransfer);
                }
            }
            else // unlikely...most or all transfers should be LBA mode for this
                 // command...ATA6 does not require LBA mode bit set like later
                 // specifications do
            {
        ...
        else
        {
            ...
            if (isLBAMode)
            {
                ...
                readSecLBA |= M_DoubleWord0(lba) & UINT32_C(0x00FFFFFF); // grabbing first 24 bits only since
                                                                         // the others should be zero
                snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - LBA: %" PRIu32 " Count: %" PRIu32 "",
                         commandName, readSecLBA, sectorsToTransfer);
            }
            else
            {
                uint16_t cylinder = M_BytesTo2ByteValue(M_Byte2(lba), M_Byte1(lba));
                                     ...
                                     char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    if (commandOpCode == ATA_SEND_FPDMA || commandOpCode == ATA_RECEIVE_FPDMA) // these commands can encapsulate
                                                                               // read/write log ext commands
    {
    ...
    case ATA_LOG_DIRECTORY:
        ...
        break;
    case ATA_LOG_SUMMARY_SMART_ERROR_LOG: // smart log...should be an error using
                                          // this command!
        snprintf(logAddressName, GPL_LOG_NAME_LENGTH, "Summary SMART Error");
        invalidLog = true;
        break;
    case ATA_LOG_COMPREHENSIVE_SMART_ERROR_LOG: // smart log...should be an error
                                                // using this command!
    ...
    case ATA_LOG_DEVICE_STATISTICS:
        ...
        break;
    case ATA_LOG_SMART_SELF_TEST_LOG: // smart log...should be an error using this
                                      // command!
    ...
    case ATA_LOG_POWER_CONDITIONS:
        ...
        break;
    case ATA_LOG_SELECTIVE_SELF_TEST_LOG: // smart log...should be an error using
                                          // this command!
    ...
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Log: %s (Invalid Address) Page Number: %" PRIu16 " PageCount: %" PRIu32 " Features: %" PRIX16
                 "h",
    ...
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Log: %s Page Number: %" PRIu16 " PageCount: %" PRIu32 " Features: %" PRIX16 "h", commandName,
                 logAddressName, pageNumber, logPageCount, features);
    }
}
                                      ...
                                      char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
             "%s - Mode: %s Block Count: %" PRIu16 " Buffer Offset: %" PRIu16 "", commandName, subCommandName,
             blockCount, bufferOffset);
}
                                     ...
                                     char        commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    default:
        ...
        {
            snprintf(securityProtocolName, TRUSTED_CMD_SECURITY_PROTOCOL_NAME_LENGTH, "Vendor Specific (%02" PRIX8 "h)",
                     securityProtocol);
            break;
        }
        else
        {
            snprintf(securityProtocolName, TRUSTED_CMD_SECURITY_PROTOCOL_NAME_LENGTH, "Unknown (%02" PRIX8 "h)",
                     securityProtocol);
            break;
        }
    }
    ...
    {
        transferLength = 0;
        if (device & BIT0) // spec is a little misleading, but the bits 24:27 are in
                           // the device/head register on 28 bit commands
        {
            // receive
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s (Receive) - Protocol: %s Protocol Specific: %" PRIu16 "", commandName, securityProtocolName,
        ...
        else
        {
            // send
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s (Send) - Protocol: %s Protocol Specific: %" PRIu16 "", commandName, securityProtocolName,
                     securityProtocolSpecific);
        }
    }
    else
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Protocol: %s Protocol Specific: %" PRIu16 " Transfer Length: %" PRIu16 "", commandName,
                 securityProtocolName, securityProtocolSpecific, transferLength);
    }
}
                                             ...
                                             const char*            smartSigValid)
{
    ...
    default:
        ...
        else if (offlineImmdTest >= 0x40 && offlineImmdTest <= 0x7E)
        {
            ...
            snprintf(offlineTestName, SMART_OFFLINE_TEST_NAME_LENGTH, "Vendor Specific %" PRIX8 "h (offline)",
                     offlineImmdTest);
        }
        else if (offlineImmdTest >= 0x90 /* && offlineImmdTest <= 0xFF*/)
        {
            // vendor unique (captive)
            snprintf(offlineTestName, SMART_OFFLINE_TEST_NAME_LENGTH, "Vendor Specific %" PRIX8 "h (captive)",
                     offlineImmdTest);
        }
        else
        {
    ...
    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Offline Immediate: %s, SMART Signature %s", commandName,
             offlineTestName, smartSigValid);
}
                               ...
                               const char*           smartSigValid)
{
    ...
    case ATA_LOG_COMPREHENSIVE_SMART_ERROR_LOG:
        ...
        break;
    case ATA_LOG_EXTENDED_COMPREHENSIVE_SMART_ERROR_LOG: // GPL log...should be an
                                                         // error using this
                                                         // command!
    ...
    case ATA_LOG_SMART_SELF_TEST_LOG:
        ...
        break;
    case ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG: // GPL log...should be an error
                                               // using this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Ext SMART Self-Test");
        invalidLog = true;
        break;
    case ATA_LOG_POWER_CONDITIONS: // GPL log...should be an error using this
                                   // command!
    ...
    case ATA_LOG_SELECTIVE_SELF_TEST_LOG:
        ...
        break;
    case ATA_LOG_DEVICE_STATISTICS_NOTIFICATION: // GPL log...should be an error
                                                 // using this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Device Statistics Notification");
        invalidLog = true;
        break;
    case ATA_LOG_PENDING_DEFECTS_LOG: // GPL log...should be an error using this
                                      // command!
    ...
    case ATA_LOG_LPS_MISALIGNMENT_LOG:
        ...
        break;
    case ATA_LOG_SENSE_DATA_FOR_SUCCESSFUL_NCQ_COMMANDS: // GPL log...should be an
                                                         // error using this
                                                         // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Sense Data for Successful NCQ");
        invalidLog = true;
        break;
    case ATA_LOG_NCQ_COMMAND_ERROR_LOG: // GPL log...should be an error using this
                                        // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "NCQ Command Errors");
        invalidLog = true;
        break;
    case ATA_LOG_SATA_PHY_EVENT_COUNTERS_LOG: // GPL log...should be an error
                                              // using this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "SATA Phy Event Counters");
        invalidLog = true;
        break;
    case ATA_LOG_SATA_NCQ_QUEUE_MANAGEMENT_LOG: // GPL log...should be an error
                                                // using this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "NCQ Queue Management");
        invalidLog = true;
        break;
    case ATA_LOG_SATA_NCQ_SEND_AND_RECEIVE_LOG: // GPL log...should be an error
                                                // using this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "NCQ Send and Receive");
        invalidLog = true;
        break;
    case ATA_LOG_HYBRID_INFORMATION: // GPL log...should be an error using this
                                     // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Hybrid Information");
        invalidLog = true;
        break;
    case ATA_LOG_REBUILD_ASSIST: // GPL log...should be an error using this
                                 // command!
    ...
    case ATA_LOG_LBA_STATUS: // GPL log...should be an error using this command!
        ...
        invalidLog = true;
        break;
    case ATA_LOG_STREAMING_PERFORMANCE: // GPL log...should be an error using this
                                        // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Streaming Performance");
        invalidLog = true;
        break;
    case ATA_LOG_WRITE_STREAM_ERROR_LOG: // GPL log...should be an error using
                                         // this command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Write Stream Errors");
        invalidLog = true;
        break;
    case ATA_LOG_READ_STREAM_ERROR_LOG: // GPL log...should be an error using this
                                        // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Read Stream Errors");
        invalidLog = true;
        break;
    case ATA_LOG_DELAYED_LBA_LOG: // GPL log...should be an error using this
                                  // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Delayed LBA");
        invalidLog = true;
        break;
    case ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG: // GPL log...should be
                                                          // an error using this
                                                          // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Current Device Internal Status");
        invalidLog = true;
        break;
    case ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG: // GPL log...should be an
                                                        // error using this
                                                        // command!
        snprintf(logAddressName, SMART_LOG_ADDRESS_NAME_LENGTH, "Saved Device Internal Status");
        invalidLog = true;
        break;
    case ATA_LOG_SECTOR_CONFIGURATION_LOG: // GPL log...should be an error using
                                           // this command!
    ...
    if (invalidLog)
    {
        ...
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s Read Log - Log: %s (Invalid Address) PageCount: %" PRIu8 ", SMART Signature %s", commandName,
                     logAddressName, logPageCount, smartSigValid);
        }
        else
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s Write Log - Log: %s (Invalid Address) PageCount: %" PRIu8 ", SMART Signature %s", commandName,
                     logAddressName, logPageCount, smartSigValid);
        }
    }
    else
    {
        ...
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s Read Log- Log: %s PageCount: %" PRIu8 ", SMART Signature %s", commandName, logAddressName,
                     logPageCount, smartSigValid);
        }
        else
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s Write Log- Log: %s PageCount: %" PRIu8 ", SMART Signature %s", commandName, logAddressName,
                     logPageCount, smartSigValid);
        }
    }
}
                                   ...
                                   char        commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    case ATA_SMART_READ_DATA:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Read SMART Data, SMART Signature %s", commandName,
                 smartSigValid);
        break;
    case ATA_SMART_RDATTR_THRESH:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Read SMART Threshold Data, SMART Signature %s",
                 commandName, smartSigValid);
        break;
    case ATA_SMART_SW_AUTOSAVE:
        if (M_Byte0(count) == UINT8_C(0xF1)) // enable
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable Attribute Autosave, SMART Signature %s",
                     commandName, smartSigValid);
        }
        else if (M_Byte0(count) == UINT8_C(0)) // disable
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Disable Attribute Autosave, SMART Signature %s",
                     commandName, smartSigValid);
        }
    ...
    case ATA_SMART_SAVE_ATTRVALUE:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Save Attributes, SMART Signature %s", commandName,
                 smartSigValid);
        break;
    ...
    case ATA_SMART_WRITE_LOG:
        get_SMART_Log_Info(commandName, commandOpCode, features, count, lba, device, commandInfo,
                           C_CAST(const char*, smartSigValid));
        break;
    // case ATA_SMART_WRATTR_THRESH:some things say vendor specific, others say
    // obsolete
    case ATA_SMART_ENABLE:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable Operations, SMART Signature %s", commandName,
                 smartSigValid);
        break;
    case ATA_SMART_DISABLE:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Disable Operations, SMART Signature %s", commandName,
                 smartSigValid);
        break;
    case ATA_SMART_RTSMART:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Return Status, SMART Signature %s", commandName,
                 smartSigValid);
        break;
    case ATA_SMART_AUTO_OFFLINE:
        if (M_Byte0(count) == 0xF8) // enable
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable Auto Offline, SMART Signature %s",
                     commandName, smartSigValid);
        }
        else if (M_Byte0(count) == 0) // disable
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Disable Auto Offline, SMART Signature %s",
                     commandName, smartSigValid);
        }
        else // invalid field for this command
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Unknown Auto Offline request %" PRIX8 "h, SMART Signature %s", commandName, M_Byte0(count),
                     smartSigValid);
        }
        break;
    default:
            ...
            (subcommand >= UINT8_C(0xDC) && subcommand <= UINT8_C(0xDF)))
        {
            // reserved
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Unknown Command %" PRIX8 "h, LBA Low: %" PRIX8 "h, Device: %" PRIX8 "h SMART Signature %s",
        ...
        else
        {
            // vendor unique
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Vendor Unique %" PRIX8 "h, LBA Low: %" PRIX8 "h, Device: %" PRIX8 "h SMART Signature %s",
                     commandName, subcommand, M_Byte0(lba), device, smartSigValid);
        }
        break;
    }
                                      ...
                                      char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    uint32_t signature  = M_DoubleWord0(lba); // NOTE: for overwrite, this is the
                                              // pattern. 47:32 contain a signature
    ...
    case ATA_SANITIZE_STATUS:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Status, Clear Failure: %d", commandName,
    ...
    case ATA_SANITIZE_CRYPTO_SCRAMBLE:
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Crypto Scramble, ZNR: %d, Failure Mode: %d, Signature %s", commandName, zoneNoReset, failure,
    ...
    case ATA_SANITIZE_BLOCK_ERASE:
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Block Erase, ZNR: %d, Failure Mode: %d, Signature %s",
                 commandName, zoneNoReset, failure, sanitizeSignatureValid);
        break;
    case ATA_SANITIZE_OVERWRITE_ERASE:
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Overwrite Erase, ZNR: %d, Invert: %d, Definitive Pattern: "
                 "%d, Failure Mode: %d, Passes: %" PRIu8 ", Pattern: %08" PRIX32 "h, Signature %s",
    ...
    case ATA_SANITIZE_FREEZE_LOCK:
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Freeze Lock, Signature %s", commandName,
                 sanitizeSignatureValid);
        break;
    case ATA_SANITIZE_ANTI_FREEZE_LOCK:
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Anti-Freeze Lock, Signature %s", commandName,
                 sanitizeSignatureValid);
        break;
                                 ...
                                 char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    default: // reserved
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Unknown (%02" PRIX8 "h), LBA = %07" PRIX32 "h, Count = %02" PRIX8 "h", commandName, subcommand,
                 C_CAST(uint32_t, lba), C_CAST(uint8_t, count));
        break;
    }
}
                                             ...
                                             char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    if (commandOpCode == ATA_SET_MAX_EXT)
    {
        ...
        if (volatileValue)
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Maximum LBA: %" PRIu64 " (Volatile)", commandName,
                     lba);
        }
        else
        {
    ...
    else
    {
        ...
        case HPA_SET_MAX_ADDRESS:
            if (volatileValue)
            {
                snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Maximum LBA: %" PRIu32 " (Volatile)",
                         commandName, C_CAST(uint32_t, lba));
            }
            else
            {
                snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Maximum LBA: %" PRIu32 "", commandName,
                         C_CAST(uint32_t, lba));
            }
            break;
                                             ...
                                             char                   commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    default:
        if (standbyTimerPeriod >= 0x01 && standbyTimerPeriod <= 0xF0)
        {
            ...
            {
                snprintf(standbyTimerPeriodString, STANDBY_TIMER_PERIOD_LENGTH, "%" PRIu8 " Minutes %" PRIu8 " Seconds",
                         minutes, seconds);
            }
            else
            {
        ...
        else if (standbyTimerPeriod >= 0xF1 && standbyTimerPeriod <= 0xFB)
        {
            ...
            uint8_t minutes = 0, hours = 0;         // no seconds since it would always be
                                                    // zero
            ...
            {
                snprintf(standbyTimerPeriodString, STANDBY_TIMER_PERIOD_LENGTH, "%" PRIu8 " Hours %" PRIu8 " Minutes",
                         hours, minutes);
            }
            else
            {
        ...
        {
            snprintf(standbyTimerPeriodString, STANDBY_TIMER_PERIOD_LENGTH, "Unknown Timer Value (%02" PRIX8 "h)",
                     standbyTimerPeriod);
        }
        break;
    }
    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Standby Timer Period: %s", commandName,
                                      ...
                                      char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    case NV_SET_NV_CACHE_POWER_MODE:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Set NV Cache Power Mode. Minimum High-Power Time: %" PRIu8 " hours %" PRIu8 " minutes %" PRIu8
                 " seconds",
    ...
    case NV_ADD_LBAS_TO_NV_CACHE_PINNED_SET:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Add LBAs to NV Cache Pinned Set, Populate Immediately: %d, "
                 "Count = %" PRIu32 "",
    ...
    case NV_REMOVE_LBAS_FROM_NV_CACHE_PINNED_SET:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Remove LBAs to NV Cache Pinned Set, Unpin All: %d, Count = "
                 "%" PRIu32 "",
    ...
    case NV_QUERY_NV_CACHE_PINNED_SET:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Query NV Cache Pinned Set, Starting 512B block: %" PRIu64 ", Count = %" PRIu32 "", commandName,
                 lba, blockCount);
    }
    break;
                                  ...
                                  char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    case AMAC_SET_ACCESSIBLE_MAX_ADDRESS:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set Accessible Max Address - LBA: %" PRIu64 "",
                 commandName, lba);
        break;
    ...
    default: // reserved - unknown
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Unknown (%04" PRIX16 "h), LBA = %012" PRIX64 "h, Count = %04" PRIX16 "h", commandName, features,
                 lba, count);
        break;
    }
}
                                       ...
                                       char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    if (commandOpCode == ATA_FPDMA_NON_DATA)
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - TRIM: (Unknown), LBA: %" PRIu64 " Count: %" PRIu32 "",
                 commandName, lba, numberOfSectorsToWriteZeros);
    }
    else
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - TRIM: %d, LBA: %" PRIu64 " Count: %" PRIu32 "",
                 commandName, trimBit, lba, numberOfSectorsToWriteZeros);
    }
}
...
#define HARDWARE_FEATURE_NAME_LENGTH UINT8_C(31)
        ...
        default:
            ...
            {
                snprintf(hardwareFeatureName, HARDWARE_FEATURE_NAME_LENGTH, "Vendor Specific (%04" PRIX16 "h)",
                         functionID);
            }
            else
            {
                snprintf(hardwareFeatureName, HARDWARE_FEATURE_NAME_LENGTH, "Unknown Function (%04" PRIX16 "h)",
                         functionID);
            }
            break;
        }
    ...
    if (enable)
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable SATA Feature - %s", commandName,
                 sataFeatureString);
    }
    else
    {
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Disable SATA Feature - %s", commandName,
                 sataFeatureString);
    }
}
...
#define TRANSFER_MODE_LENGTH UINT8_C(31)
    ...
    case SF_ENABLE_ALL_AUTOMATIC_DEFECT_REASSIGNMENT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable All Automatic Defect Reassignment",
                 commandName);
        break;
...
#define APM_LEVEL_STRING_LENGTH UINT8_C(51)
        ...
        if (apmLevel == 1)
        {
            snprintf(apmLevelString, APM_LEVEL_STRING_LENGTH, "Minimum Power Consumption w/ Standby (%02" PRIX8 "h)",
                     apmLevel);
        }
        else if (apmLevel >= UINT8_C(0x02) && apmLevel <= UINT8_C(0x7F))
        {
            snprintf(apmLevelString, APM_LEVEL_STRING_LENGTH,
                     "Intermediate Power Management w/ Standby (%02" PRIX8 "h)", apmLevel);
        }
        else if (apmLevel == UINT8_C(0x80))
        {
            snprintf(apmLevelString, APM_LEVEL_STRING_LENGTH, "Minimum Power Consumption w/o Standby (%02" PRIX8 "h)",
                     apmLevel);
        }
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable Advanced Power Management - %s", commandName,
                 apmLevelString);
    }
    break;
    ...
    case SF_ADDRESS_OFFSET_RESERVED_BOOT_AREA_METHOD_TECH_REPORT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Address Offser Reserved Boot Area Method %02" PRIX8 "h", commandName, setFeaturesSubcommand);
        break;
...
#define WRV_MODE_STRING_LENGTH 41
        ...
        case 0x02:
            ...
            break;
        case 0x03:
            snprintf(wrvModeString, WRV_MODE_STRING_LENGTH, "Mode 3 (1st %" PRIu32 " Sectors))",
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Enable Write-Read-Verify: %s", commandName,
                 wrvModeString);
    }
    break;
    ...
    case SF_TLC_SET_CCTL:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - TCL Set CCTL - %" PRIu32 " milliseconds", commandName,
                 C_CAST(uint32_t, subcommandCount) * UINT32_C(10));
        break;
    case SF_TCL_SET_ERROR_HANDLING:
        if (subcommandCount == UINT8_C(1))
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - TCL Error Handling - Read/Write Continuous",
                     commandName);
        }
        else
        {
    ...
    case SF_ENABLE_FREE_FALL_CONTROL_FEATURE:
        ...
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Enable Free-Fall Control - Sensitivity: %02" PRIu8 "h", commandName, subcommandCount);
        }
        break;
    case SF_ENABLE_AUTOMATIC_ACOUSTIC_MANAGEMENT_FEATURE:
        ...
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Enable Automatic Acoustic Management - Minimum Acoustic "
                     "Emanation",
                     commandName);
        }
        ...
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Enable Automatic Acoustic Management - Intermediate "
                     "Acoustic Mangement Levels (%02" PRIX8 "h)",
    ...
    case SF_MAXIMUM_HOST_INTERFACE_SECTOR_TIMES:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Set Maximum Host Interface Sector Times - PIO: %" PRIu16 " DMA: %" PRIu8 "", commandName,
                 typicalPIOTime, typicalDMATime);
    }
    break;
    ...
    case SF_SET_RATE_BASIS:
        ...
        case 0x00:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Set Rate Basis - Time Of Manufacture Until Time Indicated "
                     "by Date and Time Timestamp",
                     commandName);
            break;
        case 0x04:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
        ...
        case 0x08:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Set Rate Basis - Time Indicated By Power On Hours Device "
                     "Statistic",
                     commandName);
            break;
        case 0x0F:
            ...
            break;
        default:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set Rate Basis - Unknown(%02" PRIX8 "h)",
                     commandName, subcommandCount);
            break;
        }
        break;
...
#define POWER_CONDITION_STRING_LENGTH 31
        ...
        case PWR_CND_ALL:
            ...
            break;
        default:
            snprintf(powerConditionString, POWER_CONDITION_STRING_LENGTH, "Unknown Pwr Cond (%02" PRIX8 "h)",
                     powerConditionCode);
            break;
        }
        switch (subcommand)
        ...
        case EPC_GO_TO_POWER_CONDITION:
        {
            ...
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Go To Power Condition - %s Delay: %d Hold: %d",
        ...
        case EPC_SET_POWER_CONDITION_TIMER:
        {
            ...
            {
                snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                         "%s - Set Power Condition Timer - %s Timer: %" PRIu32 " minutes, Enable: %d, Save: %d",
        ...
        case EPC_SET_POWER_CONDITION_STATE:
        {
            ...
            bool save   = epcLBA & BIT4;
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Set Power Condition State - %s Enable: %d, Save: %d", commandName, powerConditionString,
                     enable, save);
        }
        break;
...
#define POWER_SOURCE_STRING_LENGTH 21
            ...
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set EPC Power Source - %s", commandName,
                     powerSourceString);
        }
        break;
        default:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Unknown EPC Subcommand (%02" PRIX8 "h) - %s LBA: %07" PRIu32 "h", commandName, subcommand,
                     powerConditionString, epcLBA);
            break;
        }
    }
    break;
    case SF_SET_CACHE_SEGMENTS:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set Cache Segments - %" PRIu8 " Segments", commandName,
                 subcommandCount);
        break;
    ...
    case SF_LONG_PHYSICAL_SECTOR_ALIGNMENT_ERROR_REPORTING:
        ...
        case SF_LPS_REPORT_ALIGNMENT_ERROR_DATA_CONDITION_UNKNOWN:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Long Physical Sector Alignment Error Reporting - Report "
                     "Error, Data Condition Unknown",
                     commandName);
            break;
        default:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Long Physical Sector Alignment Error Reporting - Unknown "
                     "Mode (%02" PRIX8 "h)",
                     commandName, subcommandCount);
            break;
        }
    ...
    case SF_ENABLE_DISABLE_DSN_FEATURE:
        ...
        {
        case SF_DSN_ENABLE:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Device Statistics Notification - Enable",
                     commandName);
            break;
        case SF_DSN_DISABLE:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Device Statistics Notification - Disable",
                     commandName);
            break;
        default:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
    ...
    case SF_CFA_NOP_ACCEPTED_FOR_BACKWARDS_COMPATIBILITY:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - CFA (%02" PRIX8 "h) - NOP, Accepted for Compatibility",
    ...
    case SF_DISABLE_ALL_AUTOMATIC_DEFECT_REASSIGNMENT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Disable All Automatic Defect Reassignment",
                 commandName);
        break;
    ...
    case SF_SET_DEVICE_MAXIMUM_AVERAGE_CURRENT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set Device Maximum Average Current: %" PRIu16 " mA",
    ...
    case SF_SET_MAXIMUM_PREFETCH:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set Maximum Prefetch: %" PRIu8 " sectors", commandName,
                 subcommandCount);
        break;
    case SF_LEGACY_SET_4_BYTES_ECC_FOR_READ_WRITE_LONG:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Set 4 Bytes ECC Data For Read/Write Long",
                 commandName);
        break;
    ...
    default:
            ...
            setFeaturesSubcommand == UINT8_C(0xE0))
        {
            // vendor specific
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Vendor Specific (%" PRIX8 "h), LBA: %07" PRIX32 " Count: %02" PRIX8 "h", commandName,
                     setFeaturesSubcommand, C_CAST(uint32_t, lba), subcommandCount);
        }
        else if ((setFeaturesSubcommand >= UINT8_C(0xF0) /* && setFeaturesSubcommand <= UINT8_C(0xFF) */))
        {
            // reserved for CFA
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Reserved for CFA (%" PRIX8 "h), LBA: %07" PRIX32 " Count: %02" PRIX8 "h", commandName,
                     setFeaturesSubcommand, C_CAST(uint32_t, lba), subcommandCount);
        }
        else
        {
            // unknown/reserved
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                     "%s - Unknown Feature (%" PRIX8 "h), LBA: %07" PRIX32 " Count: %02" PRIX8 "h", commandName,
                     setFeaturesSubcommand, C_CAST(uint32_t, lba), subcommandCount);
        }
        break;
    }
                                               ...
                                               char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    if (commandOpCode == ATA_RECEIVE_FPDMA)
    {
        ...
        featuresActionSpecific = 0; // this is in the AUX register and we cannot
                                    // read this in this log page.
...
#define ZONE_REPORT_OPTIONS_STRING_LENGTH 61
            ...
            case ZONE_REPORT_LIST_ZONES_WITH_NON_SEQ_SET_TO_ONE:
                snprintf(reportOptionString, ZONE_REPORT_OPTIONS_STRING_LENGTH,
    ...
    default:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Unknown ZAC In Command, LBA: %012" PRIX64 " Features: %04" PRIX16 "h Count: %04" PRIX16 "h",
                                                ...
                                                char                  commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    if (commandOpCode == ATA_FPDMA_NON_DATA || commandOpCode == ATA_SEND_FPDMA)
    {
        //      if (commandOpCode == ATA_FPDMA_NON_DATA)
        //      {
        //          countActionSpecific = M_BytesTo2ByteValue(M_Byte1(count),
        //          M_Byte1(features));
        //      }
        featuresActionSpecific = 0; // this is in the AUX register and we cannot
                                    // read this in this log page.
    ...
    case ZM_ACTION_CLOSE_ZONE:
    {
        ...
        if (featureActionSpecificAvailable)
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Close Zone, Zone ID: %" PRIu64 "  Close All: %d",
                     commandName, lba, closeAll);
        }
        else
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
    ...
    case ZM_ACTION_FINISH_ZONE:
    {
        ...
        if (featureActionSpecificAvailable)
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Finish Zone, Zone ID: %" PRIu64 "  Finish All: %d",
    ...
    case ZM_ACTION_OPEN_ZONE:
    {
        ...
        if (featureActionSpecificAvailable)
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Open Zone, Zone ID: %" PRIu64 "  Open All: %d",
                     commandName, lba, openAll);
        }
        else
        {
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
    ...
    default:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Unknown ZAC Out Command, LBA: %012" PRIX64 " Features: %04" PRIX16 "h Count: %04" PRIX16 "h",
                 commandName, lba, features, count);
        break;
    }
}
...
#define ABORT_TYPE_STRING_LENGTH 31
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Abort NCQ Queue: %s. Tag: %" PRIu8 " PRIO: %" PRIu8 "",
    ...
    case NCQ_NON_DATA_DEADLINE_HANDLING:
    {
        ...
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "%s - Deadline Handling. Tag: %" PRIu8 " WDNC: %d RDNC: %d",
...
#define NCQ_SET_FEATURES_STRING_LENGTH UINT8_C(48)
        ...
        snprintf(ncqSetFeaturesString, NCQ_SET_FEATURES_STRING_LENGTH, "%s - Set Features. Tag: %" PRIu8 "",
                 commandName, tag);
...
#define NCQ_ZAC_MANAGEMENT_OUT_STRING_LENGTH UINT8_C(48)
        ...
        snprintf(ncqZacMgmtOutString, NCQ_ZAC_MANAGEMENT_OUT_STRING_LENGTH, "%s - ZAC Management Out. Tag: %" PRIu8 "",
                 commandName, tag);
    ...
    default:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Unknown Subcommand (%" PRIX8 "h). Tag: %" PRIu8 " Feature: %04" PRIX16 "h Count: %0" PRIX16
                 "h LBA: %012" PRIX64 "h",
...
#define RECEIVE_FPDMA_READ_LOG_STRING_LENGTH UINT8_C(53)
        ...
        snprintf(recieveFPDMAReadLogString, RECEIVE_FPDMA_READ_LOG_STRING_LENGTH,
...
#define NCQ_ZAC_MANAGEMENT_IN_STRING_LENGTH UINT8_C(54)
        ...
        snprintf(ncqZacMgmtInString, NCQ_ZAC_MANAGEMENT_IN_STRING_LENGTH,
                                        ...
                                        char        commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    case SEND_FPDMA_HYBRID_EVICT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "%s - Hybrid Evicy. Tag: %" PRIu8 " PRIO: %" PRIu8 " Evict All: (Unknown) Blocks To Transfer: %" PRIu32
                 "",
...
#define SEND_FPDMA_READ_LOG_STRING_LENGTH UINT8_C(53)
        ...
        snprintf(sendFPDMAReadLogString, SEND_FPDMA_READ_LOG_STRING_LENGTH,
...
#define SEND_FPDMA_ZAC_MANAGEMENT_OUT_LENGTH UINT8_C(53)
        ...
        snprintf(ncqZacMgmtOutString, SEND_FPDMA_ZAC_MANAGEMENT_OUT_LENGTH,
                             ...
                             char     commandInfo[ATA_COMMAND_INFO_MAX_LENGTH])
{
    ...
    case ATA_NOP_CMD:
        ...
        case 1:
            ...
            break;
        default:
            snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "NOP (subcommand %02" PRIx8 "h",
    ...
    case ATA_REQUEST_SENSE_DATA:
        ...
        break;
    case ATA_RECALIBRATE_CMD: // this can have various values for the lower nibble
                              // which conflict with new command standards
    case 0x11:
    // case ATA_GET_PHYSICAL_ELEMENT_STATUS://or recalibrate? check the count
    // register...it should be non-zero
    case 0x12:
    case 0x13:
    case 0x14:
    case 0x15:
    ...
    case ATA_READ_MAX_ADDRESS_EXT:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "Read Max Address Ext"); // no other worthwhile inputs to this command
                                          // to report...every other register is N/A
        break;
    case ATA_READ_READ_MULTIPLE_EXT:
...
#define UNCORRECTABLE_OPTION_STRING_LENGTH 31
        ...
        case WRITE_UNCORRECTABLE_VENDOR_SPECIFIC_A5H: // vendor specific
            snprintf(uncorrectableOptionString, UNCORRECTABLE_OPTION_STRING_LENGTH, "Vendor Specific (%02" PRIX8 "h)",
                     uncorrectableOption);
            break;
        default: // reserved/unknown
            snprintf(uncorrectableOptionString, UNCORRECTABLE_OPTION_STRING_LENGTH, "Unknown Mode (%02" PRIX8 "h)",
                     uncorrectableOption);
            break;
        }
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                 "Write Uncorrectable Ext - %s  LBA: %" PRIu64 "  Count: %" PRIu32 "", uncorrectableOptionString, lba,
                 numberOfSectors);
    }
    break;
    ...
    case ATA_RECEIVE_FPDMA:
        ...
        break;
    case ATA_SEEK_CMD: // NOTE: seek can be 7xh....but that also conflicts with
    ...
    case 0x7F:
    {
        ...
        if (commandOpCode == ATA_ACCESSABLE_MAX_ADDR)
        {
                ...
                (features == 0 && lba > 0 /*this check is for legacy CHS seek AND Any other LBA to seek to*/))
            {
                ...
                if (isLBAMode)
                {
                    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "Seek (%02" PRIX8 "h) - LBA: %" PRIu32 "",
        ...
        {
            // this one will be difficult to tell the difference between...LBA should
            // be less that 24bits, LBAmode should not be on, and head field should be
            // zero
            ...
            else // timestamp is # milliseconds since Jan 1 1970...so we should
                 // reliably get here since we shouldn't ever have an LBA value less
                 // than 24bits...
            {
        ...
        {
            // features and count will be used for remove and truncate, but not the
            // legacy seek command :)
            if (features || count || lba > MAX_28_BIT_LBA ||
                !(device & LBA_MODE_BIT)) // LBA may be zero. device register should
                                          // NOT have the lba bit set for this command
            {
                ...
                {
                    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "Remove And Truncate - Element ID: %" PRIX32 "h",
                             elementIdentifier);
                }
            }
            else
    ...
    case ATA_SET_MULTIPLE:
        snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH, "Set Multiple - DRQ Data Block Count: %" PRIu8 "",
                 M_Byte0(count));
        break;
    case ATA_READ_DMA_QUEUED_CMD:
            ...
            M_Byte0(features) == LEGACY_WRITE_SAME_INITIALIZE_ALL_SECTORS)
        {
            if (M_Byte0(features) == LEGACY_WRITE_SAME_INITIALIZE_SPECIFIED_SECTORS)
            {
                if (device & LBA_MODE_BIT)
                {
                    ...
                    writeSameLBA |= M_DoubleWord0(lba) & UINT32_C(0x00FFFFFF); // grabbing first 24 bits only since the
                                                                               // others should be zero
                    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                             "Write Same - LBA: %" PRIu32 " Count: %" PRIu8 "", writeSameLBA, M_Byte0(count));
                }
                else
                {
                    uint16_t cylinder = M_BytesTo2ByteValue(M_Byte2(lba), M_Byte1(lba));
                    ...
                    uint8_t  sector   = M_Byte0(lba);
                    snprintf(commandInfo, ATA_COMMAND_INFO_MAX_LENGTH,
                             "Write Same - Cylinder: %" PRIu16 " Head: %" PRIu8 " Sector: %" PRIu8 " Count: %" PRIu8 "",
    ...
    default:
        ...
        {
            // NOTE: The above if is far from perfect...there are some commands that
            // were once VU in old standards that have been defined in newer
            // ones...this is as close as I care to get this. NOTE2: A couple of the
            // op codes above may be for CFA, or reserved for CFA. Don't care right
            // now since we are unlikely to see a CFA device with this code.
...
static void convert_Milliseconds_To_Time_String(uint64_t milliseconds, char timeString[TIMESTRING_MAX_LEN + 1])
{
    ...
    snprintf(timeString, TIMESTRING_MAX_LEN, "%" PRIu8 "D:%" PRIu8 "H:%" PRIu8 "M:%" PRIu8 "S:%" PRIu64 "MS", days,
...
{
    if (M_Nibble1(commandOpCodeThatCausedError) ==
        0x1) // All possible recalibrate commands start with nibble 0 set to 1
    {
        return true;
    }
    else
                           ...
                           char                   errorInfo[ATA_ERROR_INFO_MAX_LENGTH + 1])
{
    ...
    bool isRecal     = is_Recalibrate_Command(commandOpCodeThatCausedError); // NOTE: This will only catch case 0x10.
                                                                             // The function
                                                                             // is_Possible_Recalibrate_Command can also
                                                                             // be used, but some of those op-codes HAVE
                                                                             // been repurposed so it is less accurate!
    ...
    if (status & ATA_STATUS_BIT_ERROR)
    {
        ...
        if (error & ATA_ERROR_BIT_INTERFACE_CRC) // abort bit will also be set to 1
                                                 // if this is set to 1
        {
        ...
        if (error & ATA_ERROR_BIT_ID_NOT_FOUND) // - media access and possibly
                                                // commands to set max lba
        {
            if (safe_strlen(errorMessage) > 0)
            {
                common_String_Concat(errorMessage, ATA_ERROR_MESSAGE_MAX_LENGTH, ", ");
        ...
        if (safe_strlen(errorMessage) == 0)
        {
            ...
            else
            {
                ...
                // unknown error, possibly recalibrate command + track zero not
                // found....
    ...
    if (isReadWrite)
    {
        if (commandDeviceReg & LBA_MODE_BIT)
        {
            ...
            {
                snprintf(errorInfo, ATA_ERROR_INFO_MAX_LENGTH,
                         "Status: %s\tError: %s\tLBA: %" PRIu64 "\tDevice: %02" PRIX8 "", statusMessage, errorMessage,
                         lba, device);
            }
            else
            {
                ...
                snprintf(errorInfo, ATA_ERROR_INFO_MAX_LENGTH,
                         "Status: %s\tError: %s\tLBA: %" PRIu32 "\tDevice: %02" PRIX8 "", statusMessage, errorMessage,
                         smallLba, device);
            }
        }
        else // CHS read/write command
        {
            // not handling ext...shouldn't be an issue since CHS and 48bit don't
            // really go together.
            snprintf(errorInfo, ATA_ERROR_INFO_MAX_LENGTH,
    ...
    {
        snprintf(errorInfo, ATA_ERROR_INFO_MAX_LENGTH,
                 "Status: %s\tError: %s\tLBA: %012" PRIX64 "h\tDevice: %02" PRIX8 "", statusMessage, errorMessage, lba,
                 device);
    }
}
...
void print_ATA_Comprehensive_SMART_Error_Log(ptrComprehensiveSMARTErrorLog errorLogData, bool genericOutput)
{
    if (errorLogData)
    {
        ...
        else
        {
            ...
            {
                printf("\tWARNING: Invalid checksum was detected when reading SMART "
                       "Error log data!\n");
            }
            ...
            if (genericOutput)
            {
                ...
                // Command & Error need to know when it's a entry from the ext log or
                // not so they can print it correctly! Command:
                //  CD=Command  FT=Feature  LL=LbaLo  LM=LBAMid  LH=LBAHi DH=Device/Head
                //  SC=SectorCount  DC=DeviceControl CD FT SC LL LM LH DH DC
                // Ext Command:
                //  CD=Command  FT=Feature  FTe=FeatureExt  LL=LbaLo  LM=LBAMid LH=LBAHi
                //  LLe=LbaLoExt  LMe=LBAMidExt  LHe=LBAHiExt  DH=Device/Head
                //  SC=SectorCount SCe=SectorCountExt  DC=DeviceControl CD FT FTe SC SCe
                //  LL LM LH LLe LMe LHe DH DC
                // Error:
                //  ST=Status  ER=Error  LL=LbaLo  LM=LBAMid  LH=LBAHi  DH=Device/Head
                //  SC=SectorCount  DC=DeviceControl ST ER SC LL LM LH DH DC
                // Ext Error:
                //  ST=Status  ER=Error  LL=LbaLo  LM=LBAMid  LH=LBAHi  LLe=LbaLoExt
                //  LMe=LBAMidExt  LHe=LBAHiExt  DH=Device/Head  SC=SectorCount
                //  SCe=SectorCountExt  DC=DeviceControl ST ER SC SCe LL LM LH LLe LMe
                //  LHe DH DC
                if (errorLogData->extLog)
                {
                    printf("\t-----Command Key-----\n");
                    ...
                    printf("\tDH - Device/Head \tDC - Device Control (transport "
                           "specific)\n");
                    printf("\t------Error Key------\n");
                    ...
                    printf("\tLL - LBA Low     \tLM - LBA Mid     \tLH - LBA Hi\n");
                    printf("\tLLe - LBA Low Ext\tLMe - LBA Mid Ext\tLHe - LBA Hi Ext\n");
                    printf("\tDH - Device/Head \tDC - Device Control\tVU Bytes - "
                           "Extended Error Info (Vendor Unique)\n");
                    printf("\t---------------------\n");
                }
                else
                {
                    ...
                    printf("\tSC - Sector Count\tLL - LBA Low\n");
                    printf("\tLM - LBA Mid     \tLH - LBA Hi\n");
                    printf("\tDH - Device/Head \tVU Bytes - Extended Error Info (Vendor "
                           "Unique)\n");
                    printf("\t---------------------\n");
                }
            }
            ...
            for (uint8_t iter = 0; iter < errorLogData->numberOfEntries && iter < totalErrorCountLimit; ++iter)
            {
                ...
                // Putting these vars here because we may need to look at them while
                // parsing the error reason.
                uint16_t features      = 0;
                uint16_t count         = 0;
                     ...
                     ++commandIter)
                {
                    ...
                    if (genericOutput)
                    {
                        if (errorLogData->extLog)
                        {
                            // printf("CD FT FTe SC SCe LL LM LH LLe LMe LHe DH
                            // DC\tTimeStamp\n");
...
// Ext commands reported in the summary log will be truncated to 28bits! Data
