...
OPENSEA_OPERATIONS_API errno_t
get_Samsung_HDD_Attribute_Name(uint8_t attributeNumber, char attributeName[M_NONNULL_ARRAY MAX_ATTRIBUTE_NAME_LENGTH])
{
    errno_t error = 0;
    switch (attributeNumber)
    {
                                      ...
                                      char           attributeName[M_NONNULL_ARRAY MAX_ATTRIBUTE_NAME_LENGTH])
{
    ...
    case 180: // reported IOEDC Error In Interval (Seagate/Samsung), End to End Error Detection Rate
        ...
        {
            error =
                safe_strcpy_no_overlap(attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "Unused Reserved Block Count (Total)");
        }
        break;
    case 181: // Program Fail Count
        error = safe_strcpy_no_overlap(attributeName, MAX_ATTRIBUTE_NAME_LENGTH, "Program Fail Count");
...
{
    if (attributeName != M_NULLPTR && *attributeName != M_NULLPTR)
    {
...
M_PARAM_WO(3)
OPENSEA_OPERATIONS_API errno_t
get_Device_SMART_Attribute_Name(const tDevice* M_NONNULL device,
                                uint8_t                  attributeNumber,
                                char                     attributeName[M_NONNULL_ARRAY MAX_ATTRIBUTE_NAME_LENGTH])
{
    ...
    {
    case SEAGATE:
        error = get_Seagate_Attribute_Name(attributeNumber, attributeName);
        break;
    ...
    case SEAGATE_VENDOR_E: // with Seagate for now. Might move sometime
        error = get_Seagate_Vendor_D_E_Attribute_Name(isSeagateDrive, attributeNumber, attributeName);
        break;
    case SAMSUNG:
        error = get_Samsung_HDD_Attribute_Name(attributeNumber, attributeName);
        break;
    case MAXTOR:
        error = get_Maxtor_Attribute_Name(attributeNumber, attributeName);
        break;
    case SEAGATE_VENDOR_B:
    case SEAGATE_VENDOR_C:
        error = get_Seagate_Vendor_B_C_Attribute_Name(isSeagateDrive, attributeNumber, attributeName);
        break;
    case SEAGATE_VENDOR_F:
        error = get_Seagate_Vendor_F_Attribute_Name(attributeNumber, attributeName);
        break;
    case SEAGATE_VENDOR_G:
        error = get_Seagate_Vendor_G_Attribute_Name(attributeNumber, attributeName);
        break;
    case SEAGATE_CONNER:
        error = get_Conner_Attribute_Name(attributeNumber, attributeName);
        break;
    case SEAGATE_VENDOR_K:
        error = get_Seagate_Vendor_K_Attribute_Name(attributeNumber, attributeName);
        break;
    case SEAGATE_QUANTUM:
        error = get_Quantum_Attribute_Name(attributeNumber, attributeName);
        break;
    default:
    ...
    if (get_Device_DriveType(device) == ATA_DRIVE)
    {
        ...
        for (uint8_t iter = UINT8_C(0); iter < UINT8_MAX; ++iter)
        {
            if (smartData->attributes.ataSMARTAttr.attributes[iter].valid)
            {
                ...
                if (0 !=
                    get_Device_SMART_Attribute_Name(device, iter, smartAnylyzedData->attributes[iter].attributeName))
                    M_UNLIKELY
                    {
                                ...
                                if (tripInfo)
                                {
                                    ...
                                    tripInfo->ataAttribute.thresholdValue =
                                        attributes.attributes.ataSMARTAttr.attributes[counter]
                                            .thresholdData.thresholdValue;
                                    DECLARE_ZERO_INIT_ARRAY(char, attributeName, MAX_ATTRIBUTE_NAME_LENGTH);
                                    if (0 == get_Device_SMART_Attribute_Name(
                                                 device, tripInfo->ataAttribute.attributeNumber, attributeName) &&
                                        safe_strlen(attributeName) > 0)
                                    {
                                        // use the name in the error reason
                                        snprintfres = snprintf_err_handle(
