...
//
// \file defect.h
// \brief This file defines the functions for creating and reading defect
// information
                                   ...
                                   scsiDefectList**        defects)
{
    ...
    if (defects)
    {
        ...
        if (gotDefectData)
        {
            if (defectListLength > 0)
            {
                ...
                if (ret == SUCCESS)
                {
                    if (tenByte)
                    {
                        ...
                        {
                            dataLength = UINT16_MAX; // we cannot pull more than this with
                                                     // this command. - TJE
                        }
                        uint8_t* temp = C_CAST(uint8_t*, safe_realloc_aligned(defectData, 0, dataLength,
                                                                              device->os_info.minimumAlignment));
                        if (temp)
                        {
                            defectData = temp;
                            memset(defectData, 0, dataLength);
                            if (SUCCESS == (ret = scsi_Read_Defect_Data_10(device, primaryList, grownList,
                                                                           ...
                                                                           C_CAST(uint16_t, dataLength), defectData)))
                            {
                                ...
                                *defects = C_CAST(ptrSCSIDefectList, safe_malloc(sizeof(scsiDefectList) + defectAlloc));
                                if (*defects)
                                {
                                    ...
                                    ptrDefects->deviceHasMultipleLogicalUnits =
                                        M_ToBool(device->drive_info.numberOfLUs);
                                    uint8_t increment = 0;
                                    switch (returnedDefectListFormat)
                                    {
                                    case AD_SHORT_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                         ...
                                         ++elementNumber, offset += increment)
                                    {
                                        ...
                                        case AD_SHORT_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                            ...
                                            break;
                                        case AD_LONG_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                            ptrDefects->block[elementNumber].longBlockAddress = M_BytesTo8ByteValue(
                                                defectData[offset + 0], defectData[offset + 1], defectData[offset + 2],
                                                defectData[offset + 3], defectData[offset + 4], defectData[offset + 5],
                                                defectData[offset + 6], defectData[offset + 7]);
                                            break;
                                        case AD_EXTENDED_BYTES_FROM_INDEX_FORMAT_ADDRESS_DESCRIPTOR:
                                            ptrDefects->bfi[elementNumber].cylinderNumber =
                                                M_BytesTo4ByteValue(0, defectData[offset + 0], defectData[offset + 1],
                                                                    defectData[offset + 2]);
                    ...
                    else
                    {
                        ...
                        // possibly multiple commands (if address descriptor index is
                        // supported in the command...added in SBC3)
                        ...
                        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE,
                                                                             READ_DEFECT_DATA_12_CMD, 0, 16,
                                                                             reportOPCodeSupport))
                        {
                            // skipping the support data since we shouldn't be this far if the
                            // command isn't supported!
                            uint32_t addressIndexSupport =
                                M_BytesTo4ByteValue(reportOPCodeSupport[6], reportOPCodeSupport[7],
                                                    reportOPCodeSupport[8], reportOPCodeSupport[9]);
                            if (addressIndexSupport > 0)
                            {
                                multipleCommandsSupported = true;
                                // before we say this works, we need to do one more test due to
                                // some drive firmware bugs try setting the offset to the end of
                                // the element list and see what the reported defect length is.
                                // If it is zero, then this is supported, otherwise it's a
                                // firmware bug reporting incorrectly
                                if (SUCCESS == scsi_Read_Defect_Data_12(device, primaryList, grownList,
                                                                        C_CAST(uint8_t, defectListFormat),
                                                                        numberOfElements + 1, dataLength, defectData))
                                {
                                    // If this reported length is less than the saved list length
                                    // we already have, then this is responding to the index
                                    // properly, and is therefore supported.
                            ...
                            if (temp)
                            {
                                ...
                                *defects = C_CAST(ptrSCSIDefectList, safe_malloc(sizeof(scsiDefectList) + defectAlloc));
                                if (*defects)
                                {
                                    ...
                                    {
                                        offset = 8; // reset the offset to 8 each time through the
                                                    // while loop since we will start reading the
                                                    // list over and over after each command
                                        memset(defectData, 0, dataLength);
                                        if (SUCCESS ==
                                            (ret = scsi_Read_Defect_Data_12(
                                                 device, primaryList, grownList, C_CAST(uint8_t, defectListFormat),
                                                 elementNumber * increment, dataLength, defectData)))
                                        {
                                            defectListLength = M_BytesTo4ByteValue(defectData[4], defectData[5],
                                            ...
                                            if (increment > 0)
                                            {
                                                for (; elementNumber < numberOfElements && offset < defectListLength &&
                                                       offset < (dataLength + 8);
                                                     ++elementNumber, offset += increment)
                                                {
                                                    switch (returnedDefectListFormat)
                                                    ...
                                                    case AD_SHORT_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                                        ptrDefects->block[elementNumber].shortBlockAddress =
                                                            M_BytesTo4ByteValue(
                                                                defectData[offset + 0], defectData[offset + 1],
                                                                defectData[offset + 2], defectData[offset + 3]);
                                                        break;
                                                    case AD_LONG_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                                        ptrDefects->block[elementNumber].longBlockAddress =
                                                            M_BytesTo8ByteValue(
                                                                defectData[offset + 0], defectData[offset + 1],
                                                                defectData[offset + 2], defectData[offset + 3],
                                                    ...
                                                    case AD_EXTENDED_BYTES_FROM_INDEX_FORMAT_ADDRESS_DESCRIPTOR:
                                                        ptrDefects->bfi[elementNumber].cylinderNumber =
                                                            M_BytesTo4ByteValue(0, defectData[offset + 0],
                                                                                defectData[offset + 1],
                                                                                defectData[offset + 2]);
                                                        ptrDefects->bfi[elementNumber].headNumber =
                                                            defectData[offset + 3];
                                                        ptrDefects->bfi[elementNumber].multiAddressDescriptorStart =
                                                            defectData[offset + 4] & BIT7;
                                                        ptrDefects->bfi[elementNumber].bytesFromIndex =
                                                    ...
                                                    case AD_EXTENDED_PHYSICAL_SECTOR_FORMAT_ADDRESS_DESCRIPTOR:
                                                        ptrDefects->physical[elementNumber].cylinderNumber =
                                                            M_BytesTo4ByteValue(0, defectData[offset + 0],
                                                                                defectData[offset + 1],
                                                                                defectData[offset + 2]);
                                                        ptrDefects->physical[elementNumber].headNumber =
                                                            defectData[offset + 3];
                                                        ptrDefects->physical[elementNumber]
                                                            .multiAddressDescriptorStart =
                                                            defectData[offset + 4] & BIT7;
                                                        ptrDefects->physical[elementNumber].sectorNumber =
                                                            M_BytesTo4ByteValue(
                                                                M_GETBITRANGE(defectData[offset + 4], 3, 0),
                        ...
                        else
                        {
                            ...
                            {
                                dataLength = UINT32_MAX; // we cannot pull more than this with
                                                         // this command. - TJE
                            }
                            else
                            {
                            ...
                            if (temp)
                            {
                                defectData = temp;
                                memset(defectData, 0, dataLength);
                                if (SUCCESS == (ret = scsi_Read_Defect_Data_12(device, primaryList, grownList,
                                                                               C_CAST(uint8_t, defectListFormat), 0,
                                                                               dataLength, defectData)))
                                {
                                    ...
                                    // now allocate our list to return to the caller!
                                    *defects =
                                        C_CAST(ptrSCSIDefectList, safe_malloc(sizeof(scsiDefectList) + defectAlloc));
                                    if (*defects)
                                    {
                                        ptrSCSIDefectList ptrDefects = *defects;
                                        memset(ptrDefects, 0, sizeof(scsiDefectList) + defectAlloc);
                                             ...
                                             ++elementNumber, offset += increment)
                                        {
                                            ...
                                            case AD_LONG_BLOCK_FORMAT_ADDRESS_DESCRIPTOR:
                                                ptrDefects->block[elementNumber].longBlockAddress =
                                                    M_BytesTo8ByteValue(defectData[offset + 0], defectData[offset + 1],
                                                                        defectData[offset + 2], defectData[offset + 3],
                                                                        defectData[offset + 4], defectData[offset + 5],
                                                                        defectData[offset + 6], defectData[offset + 7]);
                                                break;
                                            case AD_EXTENDED_BYTES_FROM_INDEX_FORMAT_ADDRESS_DESCRIPTOR:
                                                ptrDefects->bfi[elementNumber].cylinderNumber =
                                                    M_BytesTo4ByteValue(0, defectData[offset + 0],
                                                                        defectData[offset + 1], defectData[offset + 2]);
                                                ptrDefects->bfi[elementNumber].headNumber = defectData[offset + 3];
                                                ptrDefects->bfi[elementNumber].multiAddressDescriptorStart =
                                                    defectData[offset + 4] & BIT7;
                                                ptrDefects->bfi[elementNumber].bytesFromIndex = M_BytesTo4ByteValue(
            ...
            {
                // defect list length is zero, so we don't have anything else to do but
                // allocate the struct and populate the data, then return it
...
void print_SCSI_Defect_List(ptrSCSIDefectList defects)
{
    if (defects)
    {
        ...
        {
            printf("\tNOTE: At this time, reported defects are for the entire "
                   "device, not a single logical unit\n");
        }
        ...
        case AD_EXTENDED_PHYSICAL_SECTOR_FORMAT_ADDRESS_DESCRIPTOR:
            ...
            if (defects->numberOfElements > 0)
            {
                ...
                for (uint64_t iter = 0; iter < defects->numberOfElements; ++iter)
                {
                    ...
                    {
                        // multi was on, but now off...this is the last descriptor for the
                        // same error
                        multi          = '+';
                        switchMultiOff = true;
                    }
                    ...
                    else
                    {
                        printf("%c %8" PRIu32 "  %3" PRIu8 "  %10" PRIu32 " \n", multi,
                               defects->physical[iter].cylinderNumber, defects->physical[iter].headNumber,
        ...
        case AD_EXTENDED_BYTES_FROM_INDEX_FORMAT_ADDRESS_DESCRIPTOR:
            ...
            if (defects->numberOfElements > 0)
            {
                ...
                for (uint64_t iter = 0; iter < defects->numberOfElements; ++iter)
                {
                    ...
                    {
                        printf("%c %8" PRIu32 "  %3" PRIu8 "  %10" PRIu32 " \n", multi,
                               defects->bfi[iter].cylinderNumber, defects->bfi[iter].headNumber,
                                           ...
                                           void*         updateData)
{
    ...
    for (iterator = 0; iterator < numberOfRandomLBAs; ++iterator)
    {
        ...
        // call the function to create an uncorrectable with the range set to 1 so
        // we only corrupt 1 physical block at a time randomly
        if (flaggedErrors)
        {
                                    ...
                                    M_ATTR_UNUSED void*         updateData)
{
    ...
    {
        // changing the increment amount to 1 because the ATA read/write long
        // commands can only do a single LBA at a time.
        increment = 1;
    }
    ...
    for (iterator = startingLBA; iterator < (startingLBA + range); iterator += increment)
    {
        ...
        {
            ret = corrupt_LBA_Read_Write_Long(device, iterator, UINT16_MAX); // saying to corrupt all the data bytes
                                                                             // to make sure we do get an error.
        }
        else
        {
            ret = NOT_SUPPORTED;
        ...
        {
            size_t dataBufSize =
                C_CAST(size_t, device->drive_info.deviceBlockSize) * C_CAST(size_t, logicalPerPhysicalSectors);
            uint8_t* dataBuf =
                C_CAST(uint8_t*, safe_calloc_aligned(dataBufSize, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!dataBuf)
            {
                return MEMORY_FAILURE;
            }
            // don't check return status since we expect this to fail after creating
            // the error
            if (device->deviceVerbosity > VERBOSITY_QUIET)
            {
            ...
            // scsi_Read_16(device, 0, false, false, false, iterator, 0,
            // logicalPerPhysicalSectors, dataBuf);
                                  ...
                                  M_ATTR_UNUSED void*         updateData)
{
    ...
    if (is_Write_Flagged_Uncorrectable_Supported(device))
    {
        ...
        // device->drive_info.deviceBlockSize; This function will only flag
        // individual logical sectors since flagging works differently than pseudo
        // uncorrectables, which we write the full sector with since a psuedo
...
bool is_Read_Long_Write_Long_Supported(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        /*a value of zero may be valid on really old drives which otherwise accept
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // Trying to use report supported operation codes/inquiry cmdDT first on the
        // read long command. Using read long since it was removed in latest specs,
        // so if it is not supported, then we know write long won't work unless
        // using the write uncorrectable bit.
        DECLARE_ZERO_INIT_ARRAY(uint8_t, commandSupportInformation,
                                30); // should be more than big enough
        ...
        else if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC &&
                 device->drive_info.scsiVersion < SCSI_VERSION_SPC_3)
        {
            dataLength += 6;
        }
        if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3 &&
            !device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations &&
            SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE, operationCode, 0,
                                                             dataLength, commandSupportInformation))
        {
            reportSuccess = true;
            ...
            default:
                break;
                 ...
                 device->drive_info.scsiVersion < SCSI_VERSION_SPC_3 &&
                 SUCCESS == scsi_Inquiry(device, commandSupportInformation, dataLength, operationCode, false, true))
        {
            reportSuccess = true;
            switch (commandSupportInformation[1] & 0x07)
            {
        ...
        {
            // try issuing a read long command with no data transfer and see if it's
...
eReturnValues corrupt_LBA_Read_Write_Long(tDevice* device, uint64_t corruptLBA, uint16_t numberOfBytesToCorrupt)
{
    ...
    {
        // since this device has multiple logical blocks per physical block, we also
        // need to adjust the LBA to be at the start of the physical block do this
        // by dividing by the number of logical sectors per physical sector. This
        // integer division will get us aligned
        uint64_t tempLBA = corruptLBA / logicalPerPhysicalBlocks;
        tempLBA *= logicalPerPhysicalBlocks;
        // do we need to adjust for alignment? We'll add it in later if I ever get a
        // drive that has an alignment other than 0 - TJE
        corruptLBA = tempLBA;
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
            ...
            device->drive_info.IdentifyData.ata.Word206 & BIT1)
        {
            ...
            uint8_t* data =
                C_CAST(uint8_t*, safe_calloc_aligned(dataSize, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!data)
            {
                return MEMORY_FAILURE;
            }
            ...
            if (ret == SUCCESS)
            {
                ...
                for (uint32_t iter = 0; iter < numberOfBytesToCorrupt && iter < device->drive_info.deviceBlockSize - 1;
                     ++iter)
                {
                ...
                {
                    // The drive responded through SAT enough to tell us exactly how many
                    // blocks are expected...so we can set the data transfer length as is
                    // expected...since this wasn't clear on non 512B logical sector
                    // drives.
            ...
            safe_free_aligned(&data);
                 ...
                 corruptLBA < MAX_28_BIT_LBA) /*a value of zero may be valid on really old
                                                 drives which otherwise accept this
                                                 command, but this should be ok for now*/
        {
            bool setFeaturesToChangeECCBytes = false;
            if (device->drive_info.IdentifyData.ata.Word022 != 4)
            {
                // need to issue a set features command to specify the number of ECC
                // bytes before doing a read or write long (according to old Seagate ATA
                // reference manual from the web)
            ...
            // This drive supports the legacy 28bit read/write long commands from
            // ATA... These commands are really old and transfer weird byte based
            // values. While these transfer lengths shouldbe supported by SAT, there
            // are some SATLs that won't handle this odd case. It may or may not go
            // through...-TJE
            if (device->drive_info.ata_Options.chsModeOnly)
            {
                ...
                if (SUCCESS == convert_LBA_To_CHS(device, C_CAST(uint32_t, corruptLBA), &cylinder, &head, &sector))
                {
                    ...
                    if (ret == SUCCESS)
                    {
                        ...
                        {
                            data[iter] = M_2sCOMPLEMENT(data[iter]); // C_CAST(uint8_t,
                                                                     // random_Range_64(0, UINT8_MAX));
                        }
                        ret = ata_Legacy_Write_Long_CHS(device, true, cylinder, head, sector, data, dataSize);
                    }
                }
                else // Couldn't convert or the LBA is greater than the current CHS
                     // mode
                {
                    ret = NOT_SUPPORTED;
                }
            ...
            {
                // reverting back to drive defaults again so that we don't mess anyone
                // else up.
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        uint16_t dataLength =
            C_CAST(uint16_t,
                   device->drive_info.deviceBlockSize * logicalPerPhysicalBlocks); // start with this size for now...
        uint8_t* dataBuffer =
            C_CAST(uint8_t*, safe_calloc_aligned(dataLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (device->drive_info.deviceMaxLba > UINT32_MAX)
        {
        ...
        // ret should not be success and we should have an illegal length indicator
        // set so we can reallocate and read the ecc bytes
        memset(&senseFields, 0, sizeof(senseDataFields));
        ...
        if (senseFields.illegalLengthIndication &&
            senseFields.valid) // spec says these bit should both be zero since we didn't
                               // do this request with enough bytes to read the ECC bytes
        {
            if (senseFields.fixedFormat)
            {
                dataLength += C_CAST(uint16_t,
                                     M_2sCOMPLEMENT(senseFields.fixedInformation)); // length different is a twos
                                                                                    // compliment value since we
                                                                                    // requested less than is available.
            }
            else
            {
                dataLength += C_CAST(uint16_t,
                                     M_2sCOMPLEMENT(senseFields.descriptorInformation)); // length different is a
                                                                                         // twos compliment value
                                                                                         // since we requested less
                                                                                         // than is available.
            }
            uint8_t* temp =
                C_CAST(uint8_t*, safe_realloc_aligned(dataBuffer, 0, dataLength, device->os_info.minimumAlignment));
            if (temp)
            {
                dataBuffer = temp;
                ...
                else
                {
                    // seed_64(C_CAST(uint64_t, time(M_NULLPTR)));
                    // modify the user data to cause a uncorrectable error
                    for (uint32_t iter = 0; iter < numberOfBytesToCorrupt &&
                                            iter < (device->drive_info.deviceBlockSize * logicalPerPhysicalBlocks - 1);
                         ++iter)
                    {
                        // Originally using random values, but it was recommended to do 2's
                        // compliment of the original data instead.
                        dataBuffer[iter] = M_2sCOMPLEMENT(dataBuffer[iter]); // C_CAST(uint8_t, random_Range_64(0,
                                                                             // UINT8_MAX));
                    }
...
eReturnValues get_LBAs_From_SCSI_Pending_List(tDevice* device, ptrPendingDefect defectList, uint32_t* numberOfDefects)
{
    ...
    *numberOfDefects                   = 0; // set to zero since it will be incremented as we read in the bad LBAs
    ...
    if (SUCCESS == get_Pending_List_Count(device, &totalPendingReported)) // This is useful so we know whether
                                                                          // or not to bother reading the log
    {
        validPendingReportedCount = true;
        ret                       = SUCCESS; // change this to SUCCESS since we know we will get a valid
                                             // count & list before we read it
    }
    if (totalPendingReported > 0 && validPendingReportedCount)
    {
        ...
        if (pendingLogSize > 0)
        {
            ...
            {
                // First, validate that we got the right SCSI log page...I've seen some
                // USB devices ignore the subpage code and return the wrong data. - TJE
                    ...
                    pendingDefectsLog[1] == 0x01)
                {
                    ...
                    if (pageLength > 4)
                    {
                        uint32_t pendingDefectCount = 1; // will be set in loop shortly...but use this for now to
                                                         // enter the loop
                        uint8_t  parameterLength = 0;
                        uint32_t offset          = LOG_PAGE_HEADER_LENGTH; // setting to this so we start with the
                                                                           // first parameter we are given...which
                                                                           // should be zero
                        for (uint32_t defectCounter = 0;
                             ...
                             offset += (parameterLength + 4))
                        {
                            ...
                            parameterLength = pendingDefectsLog[offset + 3]; // does not include 4 byte
                                                                             // header. The increment in the
...
eReturnValues get_LBAs_From_ATA_Pending_List(tDevice* device, ptrPendingDefect defectList, uint32_t* numberOfDefects)
{
    ...
    *numberOfDefects                   = 0; // set to zero since it will be incremented as we read in the bad LBAs
    ...
    if ((totalPendingReported > 0 && validPendingReportedCount) ||
        !validPendingReportedCount) // if we got a valid count and a number
                                    // greater than zero, we read the list. If we
                                    // didn't get a valid count, then we still
                                    // read the list...this may be an optimization
                                    // in some cases
    {
        // Check if the ACS pending log is supported and use that INSTEAD of the
        // Seagate log...always use std spec when we can
        uint32_t pendingLogSize = 0;
        ...
        if (pendingLogSize > 0)
        {
                                       ...
                                       pendingList, pendingLogSize, M_NULLPTR, 0, 0))
            {
                ...
                for (uint32_t descriptorIter = 0, offset = 16;
                     descriptorIter < numberOfDescriptors && offset < pendingLogSize;
                     ++descriptorIter, offset += 16, ++(*numberOfDefects))
                {
    ...
    if (SUCCESS == get_SCSI_Log_Size(device, LP_BACKGROUND_SCAN_RESULTS, 0, &backgroundScanResultsLength))
    {
        if (backgroundScanResultsLength > 0)
        {
            ...
            uint8_t* backgroundScanResults =
                C_CAST(uint8_t*, safe_calloc_aligned(backgroundScanResultsLength, sizeof(uint8_t),
                                                     device->os_info.minimumAlignment));
                                                     ...
                                                     uint32_t*        numberOfDefects)
{
    ...
    *numberOfDefects = 0;
    ptrBackgroundResults bmsResults =
        C_CAST(ptrBackgroundResults, safe_malloc(sizeof(backgroundResults) * MAX_BACKGROUND_SCAN_RESULTS));
    if (!bmsResults)
    {
        return MEMORY_FAILURE;
    }
...
eReturnValues get_LBAs_From_DST_Log(tDevice* device, ptrPendingDefect defectList, uint32_t* numberOfDefects)
{
    ...
    *numberOfDefects = 0;
    ...
    if (ret == SUCCESS)
    {
        // Got the DST log entries, so we need to find error LBAs from read element
        // failures to add that to the list
