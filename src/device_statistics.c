...
// \brief This file defines the functions related to getting/displaying device
// statistics
...
// this is ued to determine which device statistic is being talked about by the
// DSN log on ata
/*
eReturnValues
map_Page_And_Offset_To_Device_Statistic_And_Set_Threshold_Data(tDevice *device,
ptrDeviceStatistics deviceStats)
{
    ...
    if (SUCCESS == get_ATA_Log_Size(device, ATA_LOG_DEVICE_STATISTICS, &deviceStatsSize, true, true))
    {
                                        ...
                                        true, false))
        {
            uint8_t* devStatsNotificationsLog =
                C_CAST(uint8_t*, safe_calloc_aligned(deviceStatsNotificationsSize, sizeof(uint8_t),
            ...
            if (SUCCESS == get_ATA_Log(device, ATA_LOG_DEVICE_STATISTICS_NOTIFICATION, M_NULLPTR, M_NULLPTR, true,
                                       ...
                                       0, 0))
            {
                // Start at page 1 since we want all the details, not just the summary
                // from page 0 increment by 2 qwords and go through each statistic and
                // it's condition individually
                for (uint32_t offset = LEGACY_DRIVE_SEC_SIZE; offset < deviceStatsNotificationsSize; offset += 16)
                {
                    ...
                    uint8_t statisticByteOffset = M_Byte0(statisticLocation); // first byte of the statistic offset
                                                                              // (view ACS3/4 spec for this)
                    ...
                    //   010b = triggers on the device statistic value equal to the
                    //   threshold value 011b = triggers on the device statistic value
                    //   less than the threshold value 100b = triggers on the device
                    //   statistic value greater than the threshold value
                    ...
                    case ATA_DEVICE_STATS_LOG_GEN_ERR:
                        ...
                        case 8: // number of reported uncorrectable errors
                            ...
                            break;
                        case 16: // number of resets between command acceptance and command
                                 // completion
                                   ...
                                   deviceStatsLog, deviceStatsSize, M_NULLPTR, 0, 0))
        {
            ...
            for (uint8_t pageIter = 0; pageIter < deviceStatsLog[ATA_DEV_STATS_SUP_PG_LIST_LEN_OFFSET]; ++pageIter)
            {
                ...
                // TODO: Find a better way to change this code, but for now, on big
                // endian systems, we need to byte swap all qwords of the buffer to make
                // the code below work properly
...
static eReturnValues get_SCSI_DeviceStatistics(tDevice* device, ptrDeviceStatistics deviceStats)
{
    ...
    // read list of supported logs, the with that list we'll populate the
    // statistics data
    ...
    // this is really a work-around for USB drives since some DO support pages,
    // but the don't actually list them (same as the VPD pages above). Most USB
    // drives don't work though - TJE
    if (dummyUpLogPages)
    {
        ...
        // descriptors (2 bytes per page for pages + subpage format)) if you add a
        // new page here, make the page length above bigger
        supportedLogPages[4] = LP_SUPPORTED_LOG_PAGES; // just to be
                                                       // correct/accurate
        ...
        supportedLogPages[25] = 0x01; // subpage (page number is same as
                                      // temperature)
        supportedLogPages[26] = LP_ENVIRONMENTAL_LIMITS;
        supportedLogPages[27] = 0x02; // subpage (page number is same as
                                      // temperature)
        ...
        supportedLogPages[45] = 0x20; // subpage (page number is same as general
                                      // statistics and performance)
    }
    uint16_t logPageIter = LOG_PAGE_HEADER_LENGTH; // log page descriptors start on offset 4 and are
                                                   // 2 bytes long each
         ...
         logPageIter += incrementAmount)
    {
        ...
        {
            subpageCode = supportedLogPages[logPageIter + 1]; // inner switch
                                                              // statement
        }
        switch (pageCode)
        {
        case LP_WRITE_ERROR_COUNTERS:
            if (subpageCode == 0)
            {
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                    ...
                    // loop through the data and gather the data from each parameter we
                    // care about getting.
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        case 0: // Errors corrected without substantial delay
                            ...
                            case 8: // quad word
                                ...
                                break;
                            default: // don't bother trying to read the data since it's in a
                                     // more complicated format to read than we care to handle
            ...
            case 1: // environmental reporting
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        switch (parameterCode)
                        {
                        case 0: // temperature report. (note: parameters 0000-00FF are for
                                // each temperature location reported...we are only going to
                                // care about the first one right now...)-TJE
                            if (tempLogBuf[iter + 2] & BIT4)
                            {
                            ...
                            break;
                        case 0x100: // humidity report. (note: parameters 0100-01FF are for
                                    // each humidity location reported...we are only going
                                    // to care about the first one right now...)-TJE
                            if (tempLogBuf[iter + 2] & BIT4)
                            {
            ...
            case 2: // environmental limits
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        switch (parameterCode)
                        {
                        case 0: // temperature limits. (note: parameters 0000-00FF are for
                                // each temperature location reported...we are only going to
                                // care about the first one right now...)-TJE
                            if (tempLogBuf[iter + 2] & BIT4)
                            {
                            ...
                            ++deviceStats->sasStatistics.statisticsPopulated;
                            break;
                        case 0x100: // humidity limits. (note: parameters 0100-01FF are for
                                    // each humidity location reported...we are only going
                                    // to care about the first one right now...)-TJE
                            //
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        case 1: // date of manufacture
                            // set into the buffer as week-year and parse it out that way
                            // later.
            ...
            case 0: // background scan results
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        switch (parameterCode)
                        {
                        case 0: // POM, # scans performed, bg scan progress, # bgms
                                // performed
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        case 1: // general access statistics and performance
                            ...
                            ++deviceStats->sasStatistics.statisticsPopulated;
                            // weighted number of read command processing plus write command
                            // processing
                            deviceStats->sasStatistics.weightedReadCommandProcessingPlusWriteCommandProcessing
                                .isSupported = true;
                            deviceStats->sasStatistics.weightedReadCommandProcessingPlusWriteCommandProcessing
                             ...
                             iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                        {
                            ...
                            case 1: // general access statistics and performance
                                ...
                                if (tempLogBuf[iter + 2] & BIT4)
                                {
                                    ...
                                    scsi_Threshold_Comparison(
                                        &deviceStats->sasStatistics.weightedNumberOfReadCommandsPlusWriteCommands);
                                    // weighted number of read command processing plus write
                                    // command processing
                                    deviceStats->sasStatistics.weightedReadCommandProcessingPlusWriteCommandProcessing
                                        .isThresholdValid = true;
                                    deviceStats->sasStatistics.weightedReadCommandProcessingPlusWriteCommandProcessing
        ...
        case LP_ZONED_DEVICE_STATISTICS: // subpage 1
            ...
            case 0x01: // ZBD statistics
            {
                                                  ...
                                                  tempLogBuf, LEGACY_DRIVE_SEC_SIZE))
                {
                         ...
                         iter += (C_CAST(uint16_t, parameterLength + UINT16_C(4))))
                    {
                        ...
                        case 10: // write rule violations
                            ...
                            ++deviceStats->sasStatistics.statisticsPopulated;
                            break;
                        case 11: // Maximum implicitly open sequential or before required
                                 // zones
                ...
                // Thresholds are not defined/obsolete so no need to read them or
        ...
        case LP_PROTOCOL_SPECIFIC_PORT:
            ...
            case 0:
                ...
                //       I am not aware of other transports implementing this page at
                //       this time - TJE
                // This page is read in a 64k size to make sure we get as much as
                // possible in a single command.
                {
                    uint16_t protocolSpecificDataLength = UINT16_MAX;
                    uint8_t* protSpData =
                        C_CAST(uint8_t*, safe_calloc_aligned(protocolSpecificDataLength, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (protSpData)
                    {
                        ...
                        {
                            // mimimum page length for a SAS drive assuming only 1 port and 1
                            // phy is 64B. Each additional port adds a minimum of another 60
                            // bytes
                            uint32_t pageLength =
                                M_BytesTo2ByteValue(protSpData[2], protSpData[3]) + LOG_PAGE_HEADER_LENGTH;
                            uint16_t parameterLength = 4;
                            uint16_t portCounter     = 0;
                            for (uint32_t offset = 4; offset < pageLength && portCounter < SAS_STATISTICS_MAX_PORTS &&
                                 ...
                                 offset += parameterLength + 4, ++portCounter)
                            {
                                ...
                                parameterLength = protSpData[offset + 3]; // 4 bytes for the length of the
                                                                          // header for the parameter code
                                if (parameterLength > 0)
                                {
                                    ...
                                    if (protocolIdentifier == SCSI_PROTOCOL_ID_SAS)
                                    {
                                        ...
                                        for (uint8_t phyIter = 0; phyIter < numberOfPhys && phyOffset < pageLength &&
                                                                  phyCounter < SAS_STATISTICS_MAX_PHYS;
                                             ++phyIter, phyOffset += phyDescriptorLength + 4, ++phyCounter)
                                        {
                                            // now at the actual phy data, so we can read what we want
                                            // to report
                                            ...
                                            if (phyDescriptorLength > 0)
                                            {
                                                ...
                                                ++deviceStats->sasStatistics.sasProtStats
                                                      .sasStatsPerPort[deviceStats->sasStatistics.sasProtStats
                                                                           .portCount]
                                                      .phyCount;
                                                // Phy event descriptors? Not sure this is needed right
                                                // now -TJE
                                                //       Events would be yet another loop depending on
                                ...
                                {
                                    // parameters without a length mean move on to the next one
                                    // since no additional data was provided.
                                    continue;
                                }
                            }
                        }
    ...
    {
        // This loop is so that we can retry with different formats if it does not
        // work the first time - TJE Attempt LBA mode, then attempt pchs, then call
        // it quits if neither works. If the drive has a large LBA (>32b max) then
        // use extended formats, otherwise use short formats NOTE: SBC2 and later
    ...
    // most likely the primary list in block format won't work, but trying it
    // anyways as a first step - TJE
    ...
    while (!gotPrimaryDefectCount)
    {
        ...
        if (defectRet != SUCCESS && !gotPrimaryDefectCount)
        {
            ...
            {
                // special case to restart the loop again with long address types in
                // case short are not supported, but it isn't a high capacity devices
                defectFormat = AD_LONG_BLOCK_FORMAT_ADDRESS_DESCRIPTOR;
            }
            else
            {
...
static void print_SCSI_Time_Interval_Statistic(statistic theStatistic, const char* statisticName)
{
    if (theStatistic.isSupported)
    {
        ...
        if (theStatistic.isValueValid)
        {
            ...
            // now byteswap the double words to get the correct endianness (for LSB
            // machines)
...
// This is a different function to be more specific to SAS environmental
// limits/reporting pages
...
static eReturnValues print_ATA_DeviceStatistics(tDevice* device, ptrDeviceStatistics deviceStats)
{
    ...
    if (deviceStats->sataStatistics.vendorSpecificStatisticsSupported)
    {
        ...
        for (uint8_t vendorSpecificIter = 0, statisticsFound = 0;
             vendorSpecificIter < 64 && statisticsFound < deviceStats->sataStatistics.vendorSpecificStatisticsPopulated;
             ++vendorSpecificIter)
        {
...
static eReturnValues print_SCSI_DeviceStatistics(M_ATTR_UNUSED tDevice* device, ptrDeviceStatistics deviceStats)
{
    ...
    printf("\t! = monitored condition met (Requires Threshold to be set and "
           "comparison enabled)\n");
    printf("\t- = supports notification (requires log page thresholds to be "
           "supported)\n");
    printf(" %-60s %-16s %-16s\n", "Statistic Name:", "Threshold:", "Value:");
    ...
    if (deviceStats->sasStatistics.protocolSpecificStatisticsSupported)
    {
        if (deviceStats->sasStatistics.protocolStatisticsType == STAT_PROT_SAS)
        {
            ...
            // So this needs to loop and output which port ID and phy ID each
            // statistic is for
...
eReturnValues print_DeviceStatistics(tDevice* device, ptrDeviceStatistics deviceStats)
{
    ...
    // as I write this I'm going to try and keep ATA and SCSI having the same
    // printout format, but that may need to change...-TJE
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
