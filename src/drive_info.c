...
// This is an internal structure that's purpose is to report capabilities out of
// the subfunctions processing identify data or other data so that additional
// commands to run for device discovery can be performed as needed based on
// these capabilities.
typedef struct _idDataCapabilitiesForDriveInfo
{
    eSeagateFamily seagateFamily;
    bool           supportsIDDataLog; // used to help figure out what needs to be parsed or
                                      // not out of std identify data, not already available
                                      // in the log
    struct
    {
        bool copyOfIdentify;
    ...
    bool processedStdIDData; // set when the function that reviews the standard ID
                             // data (ECh) has already been called.
                                                      ...
                                                      uint32_t                          dataLength)
{
    ...
    // start by assuming 512B per sector. This is updated later if the drive
    // supports a different setting.
    ...
    // check if the really OLD Mb/s bits are set...if they are, set the speed
    // based off of them This will be changed later if other words are set.-TJE
    if (is_ATA_Identify_Word_Valid(wordPtr[0]) && M_GETBITRANGE(wordPtr[0], 10, 8) > 0)
    {
        ...
        if (wordPtr[0] & BIT9)
        {
            driveInfo->interfaceSpeedInfo.ancientHistorySpeed.dataTransferGt5MbSLte10MbS =
                true; // 7.5Mb/s - 0.9375MB/s - used 7.5Mb/s as the middle of these
                      // values
        }
        if (wordPtr[0] & BIT8)
        {
        ...
        is_ATA_Identify_Word_Valid(wordPtr[6]))
    {
        ...
        // According to ATA, word 53, bit 0 set to 1 means the words 54,-58 are
        // valid. if set to zero they MAY be valid....so just check validity on
        // everything
    }

    // buffer type is in word 20. According to very old product manuals, if this
    // is set to 3, then read-look ahead is supported.
    if (is_ATA_Identify_Word_Valid(wordPtr[20]))
    {
        if (wordPtr[20] == 0x0003)
        {
            driveInfo->readLookAheadSupported = true;
            // NOTE: It is not possible to determine whether this is currently enabled
            // or not.
        }
    }
    // cache size (legacy method - from ATA 1) Word 21
    // note: Changed from multiplying by logical sector size to 512 as that is
    // what ATA says this is increments of.
    ...
    {
        // NOTE: ATA 1 lists this as can or cannot perform doubleword I/O. This is
        // listed as vendor unique as well.
        //       This is reserved in ATA-2 until it gets used later. This is
        //       PROBABLY safe to use without additional version checks. Most likely
        //       this was only used by one vendor
    ...
    bool dmaSupported     = false; // to be used later when determining transfer
                                   // speeds
    ...
    if (is_ATA_Identify_Word_Valid(wordPtr[51]))
    {
        ...
        {
            memset(&driveInfo->interfaceSpeedInfo, 0,
    ...
    // If SN is invalid, or all ASCII zeroes, then this is likely an ESDI drive
    // managed by an ATA compatible controller.
    //       In this case, we may not want to change the speed to say "PIO-0"
    //       since it has a different transfer rate.
    ...
    else if (dmaSupported)
    {
        ...
        if (driveInfo->interfaceSpeedInfo.parallelSpeed.maxSpeed < 2.1)
        {
            ...
            // this will be changed later for any drives supporting the other
            // MWDMA/UDMA fields
            driveInfo->interfaceSpeedInfo.parallelSpeed.maxSpeed         = 2.1;
            driveInfo->interfaceSpeedInfo.parallelSpeed.maxModeNameValid = true;
            snprintf(driveInfo->interfaceSpeedInfo.parallelSpeed.maxModeName, PARALLEL_INTERFACE_MODE_NAME_MAX_LENGTH,
    ...
    {
        lbaModeSupported = true; // workaround for some devices that may not have
                                 // set the earlier LBA mode bit
        driveInfo->maxLBA = M_WordsTo4ByteValue(wordPtr[60], wordPtr[61]);
    }

    // interface speed: NOTE: for old drives, word 51 indicates highest supported
    // PIO mode 0-2 supported
    //                        word 52 indicates highest supported single word DMA
    //                        mode 0, 1, 2 supported
    //                  See ATA-2
    if (is_ATA_Identify_Word_Valid(wordPtr[62]))
    {
        // SWDMA (obsolete since MW is so much faster...) (word 52 also holds the
        // max supported value, but is also long obsolete...it can be checked if
        // word 62 is not supported)
    ...
    // get which specifications are supported and the number of them added to the
    // list (ATA Spec listed in word 80)
    ...
    else
    {
        ...
        // NOTE: May be able to check other fields to determine if a later standard
        // is supported or not, but this is a fair assumption-TJE
        add_Specification_To_Supported_List(driveInfo->specificationsSupported,
    ...
    // Some words mirror other bits and other hold support while another holds
    // enabled. Some of these are also paired between supported and enabled. The
    // following code assumes these pairs to parse this data as best it can
    // without making things too complicated-TJE
    ...
    if ((word84Valid && wordPtr[84] & BIT8) || (word87Valid && wordPtr[87] & BIT8))
    {
        ...
        memcpy(&driveInfo->worldWideName, &wordPtr[108],
    ...
    // NOTE: In ATA8-APT there is table36 which goes over how to determine the
    // cabling type.
    //       This table is mean to help a host know which speed to use at most
    //       based on results from both drive 0 and drive 1. In here we do not
    //       have this information, but we will report what this individual device
    //       is reporting. Annex A gives additional detail of a non-standard way
    //       to determine it, but it is not recommended.
    // TLDR: The drive may not detect this properly in certain cases...see
    // ATA8-APT for details
    ...
    {
        // bits 13:0 are valid for alignment. bit 15 will be 0 and bit 14 will be 1.
        // remove bit 14 with an xor
    ...
    {
        // NV Cache Size logical blocks - needs testing against different drives to
    ...
    // Special case for SSD detection. One of these SSDs didn't set the media_type
    // to SSD but it is an SSD. So this match will catch it when this happens. It
    // should be uncommon to find though -TJE
    ...
    // adjust as reported value is one larger than last accessible LBA on the
    // drive-TJE
...
// This function expects a complete ID data log, so the supported pages followed
// by all supported pages that were read.
                                                         ...
                                                         uint32_t                          dataLength)
{
    ...
    if (pageNumber == C_CAST(uint8_t, ATA_ID_DATA_LOG_SUPPORTED_PAGES) && revision >= ATA_ID_DATA_VERSION_1)
    {
        ...
        for (uint16_t iter = ATA_ID_DATA_SUP_PG_LIST_OFFSET;
             iter < C_CAST(uint16_t, listLen + ATA_ID_DATA_SUP_PG_LIST_OFFSET) &&
             iter < UINT16_C(ATA_LOG_PAGE_LEN_BYTES);
             ++iter)
        {
            ...
            (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
                ...
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
                ...
                // Utilization (IDK if we need anything from this log for this
                // information)
        ...
        if (ataCapabilities->supportedIDDataPages.strings && (offset +
        ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        idDataLog[offset + 0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT &&
        M_Byte2(qword0) == ATA_ID_DATA_LOG_ATA_STRINGS && M_Word0(qword0) >=
        ATA_ID_DATA_VERSION_1)
            {
            }
        }
        offset = ATA_LOG_PAGE_LEN_BYTES * ATA_ID_DATA_LOG_SECURITY;
        if (ataCapabilities->supportedIDDataPages.security && (offset +
        ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        idDataLog[offset + 0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT &&
        M_Byte2(qword0) == ATA_ID_DATA_LOG_SECURITY && M_Word0(qword0) >=
        ATA_ID_DATA_VERSION_1)
            {
            }
        }
        offset = ATA_LOG_PAGE_LEN_BYTES * ATA_ID_DATA_LOG_PARALLEL_ATA;
        if (ataCapabilities->supportedIDDataPages.parallelATA && (offset +
        ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        idDataLog[offset + 0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT &&
        M_Byte2(qword0) == ATA_ID_DATA_LOG_PARALLEL_ATA && M_Word0(qword0) >=
        ATA_ID_DATA_VERSION_1)
            {
            }
        }
        offset = ATA_LOG_PAGE_LEN_BYTES * ATA_ID_DATA_LOG_SERIAL_ATA;
        if (ataCapabilities->supportedIDDataPages.serialATA && (offset +
        ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
            uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        idDataLog[offset + 0]); if (qword0 & ATA_ID_DATA_QWORD_VALID_BIT &&
        M_Byte2(qword0) == ATA_ID_DATA_LOG_SERIAL_ATA && M_Word0(qword0) >=
        ATA_ID_DATA_VERSION_1)
            {
            }
        }*/
        ...
        if (ataCapabilities->supportedIDDataPages.zac2 && (offset + ATA_LOG_PAGE_LEN_BYTES) <= dataLength)
        {
                ...
                M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
            {
                ...
                if (versionQWord & ATA_ID_DATA_QWORD_VALID_BIT)
                {
                    ...
                    default:
                        // add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                        // &driveInfo->numberOfSpecificationsSupported, "Unknown ZAC Minor
                        // version: %04" PRIX16, M_Word0(versionQWord));
                        break;
                    }
                }
        ...
        if (ataCapabilities->supportedIDDataPages.copyOfIdentify && !ataCapabilities->processedStdIDData)
        {
            ...
            // std identify data was not already processed, so process it here for
            // anything not covered by the above code.
                                                                   ...
                                                                   uint32_t                          dataLength)
{
    ...
    if (pageNumber == C_CAST(uint8_t, ATA_DEVICE_STATS_LOG_LIST) && revision >= ATA_DEV_STATS_VERSION_1)
    {
        ...
        // if (freeFallStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <=
        // dataLength)
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        //     idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        //     idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) ==
        //     ATA_DEVICE_STATS_LOG_FREE_FALL && M_Word0(qword0) >=
        ...
        // if (rotatingMediaStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <=
        // dataLength)
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        //     idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        //     idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) ==
        //     ATA_DEVICE_STATS_LOG_ROTATING_MEDIA && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        // offset = ATA_LOG_PAGE_LEN_BYTES * ATA_DEVICE_STATS_LOG_GEN_ERR;
        // if (generalErrorStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <=
        // dataLength)
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        //     idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        //     idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) ==
        //     ATA_DEVICE_STATS_LOG_GEN_ERR && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        // if (transportStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <=
        // dataLength)
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        //     idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        //     idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) ==
        //     ATA_DEVICE_STATS_LOG_TRANSPORT && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
        ...
        // if (zonedDeviceStatistics && (offset + ATA_LOG_PAGE_LEN_BYTES) <=
        // dataLength)
        //{
        //     uint64_t qword0 = M_BytesTo8ByteValue(idDataLog[offset + 7],
        //     idDataLog[offset + 6], idDataLog[offset + 5], idDataLog[offset + 4],
        //     idDataLog[offset + 3], idDataLog[offset + 2], idDataLog[offset + 1],
        //     idDataLog[offset + 0]); if (M_Byte2(qword0) ==
        //     ATA_DEVICE_STATS_LOG_ZONED_DEVICE && M_Word0(qword0) >=
        //     ATA_DEV_STATS_VERSION_1)//validating we got the right page
        //     {
        //     }
        // }
                                                        ...
                                                        uint32_t                          dataLength)
{
    ...
    if (smartData && dataLength >= LEGACY_DRIVE_SEC_SIZE)
    {
        ...
        // read temperature (194), poh (9) for all, then read 241, 242, and 231 for
        // Seagate only
        ...
        for (smartIter = 2; smartIter < 362; smartIter += 12)
        {
            ...
            case 9: // POH (This attribute seems shared between vendors)
            {
                ...
                uint32_t powerOnMinutes =
                    ...
                    60;
                powerOnMinutes += (millisecondsSinceIncrement / 60000); // convert the milliseconds to minutes, then add
            ...
            case 194:
                ...
                /* TODO: This can be improved with better filters/interpretations
                 * defined per vendor to read this attribute. */
                if (ataCapabilities->seagateFamily != MAXTOR)
                {
                    if (ataCapabilities->seagateFamily == SEAGATE_VENDOR_K)
                    {
                        if (!driveInfo->temperatureData.highestValid &&
                            currentAttribute.worstEver != ATA_SMART_ATTRIBUTE_WORST_COMMON_START) // Filter out 253 as
                                                                                                  // that is an
                                                                                                  // unreasonable
                                                                                                  // measurement, and
                                                                                                  // more likely
            ...
            case 231: // SSD Endurance
                ...
                {
                    // SCSI was implemented first, and it returns a value where 0 means
                    // 100% spares left, ATA is the opposite, so we need to subtract our
                    // number from 100 On SATA drives below, we had firmware reporting in
                    // the range of 0-255 instead of 0-100. Lets check for them and
                    // normalize the value so it can be reported Note that this is only
                    // for some firmwares...we should figure out a better way to do this
                        ...
                        strcmp(driveInfo->firmwareRevision, "0004") == 0)
                    {
                        driveInfo->percentEnduranceUsed =
                            100 - ((C_CAST(uint32_t, currentAttribute.nominal) * 100) / 255);
                    }
                    else
                    {
            ...
            case 233: // Lifetime Write to Flash (SSD)
                    ...
                    ataCapabilities->seagateFamily == SEAGATE_VENDOR_F)
                {
                    ...
                    // convert this to match what we're doing below since this is likely
                    // also in GiB written (BUT IDK BECAUSE IT ISN'T IN THE SMART SPEC!)
                                                                  ...
                                                                  uint32_t              dataLength)
{
    ...
    if (device && info && securityProtocolList && dataLength > 8)
    {
        ...
        uint32_t bufIter = 8;
        // Check endianness. ATA is SUPPOSED to report little endian and SCSI/NVMe
        // report big endian, but that doesn't seem like it's always followed.
        //       So to check this, check the bytes after the length are all zero for
        //       either interpretation. One will be right, usually the shorter of
        //       the lengths. This is necessary for any security protocol 0
        //       (information) buffers as I've seen all kinds of weird combinations
        //       - TJE
        // For now, just take the shorter value of the two lengths given....seems
        // true for everything I've encountered so far.
        ...
        for (; (bufIter - 8) < length && (bufIter - 8) < dataLength; bufIter++)
        {
            ...
            case SECURITY_PROTOCOL_TCG_6:
                // TODO: Level 0 discovery to look up which TCG feature(s) are being
                // implemented.
...
eReturnValues get_ATA_Drive_Information(tDevice* device, ptrDriveInformationSAS_SATA driveInfo)
{
    ...
    uint8_t* logBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(logBufferSize, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!logBuffer)
    {
    ...
    if (ataCap.gplSupported &&
        SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_DIRECTORY, 0, logBuffer, LEGACY_DRIVE_SEC_SIZE, 0))
    {
        gotLogDirectory = true;
    }
    else if (ataCap.smartErrorLoggingSupported &&
             SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_DIRECTORY, logBuffer, LEGACY_DRIVE_SEC_SIZE))
    {
        gotLogDirectory = true;
    }
    ...
    if (gotLogDirectory || ataCap.smartErrorLoggingSupported)
    {
        ...
        uint32_t devStatsSize = 0, idDataLogSize = 0, hybridInfoSize = 0, smartSelfTest = 0, extSelfTest = 0,
                 hostlogging = 0, sctStatus = 0, concurrentRangesSize = 0, farmLogSize = 0;
        if (gotLogDirectory)
        {
            devStatsSize = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_DEVICE_STATISTICS * 2) + 1],
                                               logBuffer[(ATA_LOG_DEVICE_STATISTICS * 2)]) *
                           LEGACY_DRIVE_SEC_SIZE;
            idDataLogSize = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2) + 1],
                                                logBuffer[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2)]) *
                            LEGACY_DRIVE_SEC_SIZE;
            hybridInfoSize = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_HYBRID_INFORMATION * 2) + 1],
                                                 logBuffer[(ATA_LOG_HYBRID_INFORMATION * 2)]) *
                             LEGACY_DRIVE_SEC_SIZE;
            smartSelfTest = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_SMART_SELF_TEST_LOG * 2) + 1],
                                                logBuffer[(ATA_LOG_SMART_SELF_TEST_LOG * 2)]) *
                            LEGACY_DRIVE_SEC_SIZE;
            extSelfTest = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG * 2) + 1],
                                              logBuffer[(ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG * 2)]) *
                          LEGACY_DRIVE_SEC_SIZE;
            sctStatus = M_BytesTo2ByteValue(logBuffer[(ATA_SCT_COMMAND_STATUS * 2) + 1],
                                            logBuffer[(ATA_SCT_COMMAND_STATUS * 2)]) *
                        LEGACY_DRIVE_SEC_SIZE;
            hostlogging = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_HOST_SPECIFIC_80H * 2) + 1],
                                              logBuffer[(ATA_LOG_HOST_SPECIFIC_80H * 2)]) *
                          LEGACY_DRIVE_SEC_SIZE;
            concurrentRangesSize = M_BytesTo2ByteValue(logBuffer[(ATA_LOG_CONCURRENT_POSITIONING_RANGES * 2) + 1],
                                                       logBuffer[(ATA_LOG_CONCURRENT_POSITIONING_RANGES * 2)]) *
                                   LEGACY_DRIVE_SEC_SIZE;
            farmLogSize = M_BytesTo2ByteValue(logBuffer[(SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS * 2) + 1],
                                              logBuffer[(SEAGATE_ATA_LOG_FIELD_ACCESSIBLE_RELIABILITY_METRICS * 2)]) *
                          LEGACY_DRIVE_SEC_SIZE;
        }
        else
        {
            // This is a case for old drives. They will only support single sector
            // logs when this is set like this
            if (is_Self_Test_Supported(device))
            {
                smartSelfTest = ATA_LOG_PAGE_LEN_BYTES;
            }
            // The only other logs to look at in here are SMART error log (summary and
            // comprehensive) and selective self test
            //  as these are the only other single sector logs that will show up on
            //  old drives like these
        }
        ...
        {
            uint8_t* idDataLog =
                C_CAST(uint8_t*, safe_calloc_aligned(idDataLogSize, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (idDataLog)
            {
        ...
        {
            uint8_t* devStats =
                C_CAST(uint8_t*, safe_calloc_aligned(devStatsSize, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (devStats)
            {
        ...
        if (ataCap.sctSupported && sctStatus > 0) // GPL or SMART
        {
                                       ...
                                       logBuffer, ATA_LOG_PAGE_LEN_BYTES, M_NULLPTR, 0, 0))
            {
                ...
                if (sctFormatVersion > 1) // cannot find spec for revision 1 of this log, but we'll keep
                                          // this safe until I find it with this check
                {
        ...
        if (ataCap.gplSupported && concurrentRangesSize)
        {
            ...
            // NOTE: Only reading first 512B since this has the counter we need. Max
            // log size is 1024 in ACS5 - TJE
        ...
        {
            uint8_t* farmData =
                C_CAST(uint8_t*, safe_calloc_aligned(16384, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (farmData)
            {
                // read "page 0" or first 4KB for header/top level information and
                // verify this is in fact the FARM log
                                                         ...
                                                         0, farmData, 16384, 0))
                {
                        ...
                        (farmSignature & UINT64_C(0x00FFFFFFFFFFFF)) == SEAGATE_FARM_LOG_SIGNATURE)
                    {
                        ...
                        // Now read the next page to get the DOM (and possibly other useful
                        // data)
                        if (SUCCESS ==
                                                      ...
                                                      (UINT16_C(16384) / LEGACY_DRIVE_SEC_SIZE), farmData, 16384, 0))
                        {
                                ...
                                (farmPage1ID & UINT64_C(0x00FFFFFFFFFFFF)) == 1)
                            {
                                    ...
                                    dateOfManufactureQWord & BIT62) // supported and valid info
                                {
                                    ...
                                    {
                                        // year is 2 digits, but this log was not in existance until
                                        // after 2018 or so
    ...
    {
        // TCG - SED drive (need to test a trusted command to see if it is being
        // blocked or not)
        ...
        else
        {
            ...
            uint8_t* protocolList = C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
                                           ...
                                           uint32_t                    dataLength)
{
    ...
    if (driveInfo && scsiInfo && inquiryData && dataLength >= INQ_RETURN_DATA_LENGTH_SCSI2)
    {
        ...
        case 0:
            /*add_Specification_To_Supported_List(driveInfo->specificationsSupported,
            &driveInfo->numberOfSpecificationsSupported, "Does not conform to a SCSI
            standard");

            break;*/
            // Note: Old SCSI/SPC standards may report multiple specifications
            // supported in this byte Codes 08-0Ch, 40-44h, 48-4Ch, & 88h-8Ch are
            // skipped in there. These were valious ways to report ISO/ECMA/ANSI
            // standard conformance seperately for the same SCSI-x specification. New
            // standards (SPC6 or whatever is next) may use these values and they
            // should be used.
        case 0x81:
        case 0x01:
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
        ...
        if (responseFormat == 1)
        {
            // response format of 1 means there is compliance with the Common Command
            // Set specification, which is partial SCSI2 support.
            add_Specification_To_Supported_List(driveInfo->specificationsSupported,
                                       ...
                                       ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    {
        // VPD pages (read list of supported pages...if we don't get anything back,
        // we'll dummy up a list of things we are interested in trying to
        // read...this is to work around crappy USB bridges
        uint8_t* tempBuf = C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE * 2, sizeof(uint8_t),
                                                                device->os_info.minimumAlignment));
        if (!tempBuf)
        {
            return MEMORY_FAILURE;
        }
        // some devices (external) don't support VPD pages or may have issue when
        // trying to read them, so check if this hack is set before attempting to
        // read them
        if ((!device->drive_info.passThroughHacks.scsiHacks.noVPDPages ||
             device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable) &&
            (scsiInfo->version >= 2 ||
             device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable)) // VPD pages indroduced in SCSI 2...also a
                                                                             // USB hack
        {
            ...
            {
                // for whatever reason, this device didn't return support for the list
                // of supported pages, so set a flag telling us to dummy up a list so
                // that we can still attempt to issue commands to pages we do need to
            ...
            if (!dummyUpVPDSupport)
            {
                ...
                {
                    // this case means that the command was successful, but we got nothing
                    // but zeros....which happens on some craptastic USB bridges
                    dummyUpVPDSupport = true;
                }
            }
            if (dummyUpVPDSupport)
            {
                uint16_t offset = 4;
                // in here we will set up a fake supported VPD pages buffer so that we
                // try to read the unit serial number page, the SAT page, and device
                // identification page
                ...
                if (device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable)
                {
                    // This is a hack for devices that will only support this page and
                    // MAYBE the device identification VPD page. Not adding the device ID
                    // page here because it almost always contains only a name string
                    // and/or a T10 string which isn't useful for the information being
                    // gathered in here today.
                    tempBuf[offset] = UNIT_SERIAL_NUMBER; // SCSI2
                    ++offset;
                }
                else
                {
                    // now each byte will reference a supported VPD page we want to dummy
                    // up. These should be in ascending order
                    ...
                    {
                        tempBuf[offset] = ATA_INFORMATION; // SAT. Going to leave this in here no matter
                                                           // what other version info is available since
                                                           // SATLs needing this dummy data may support
                                                           // this regardless of other version info
                        ++offset;
                    }
                    ...
                    // Add more pages to the dummy information as we need to. This may be
                    // useful to do in the future in case a device decides not to support
                    // a MANDATORY page or another page we care about
                }
                // set page length (n-3)
            ...
            // now loop through and read pages as we need to, only reading the pages
            // that we care about
            uint16_t vpdIter = 0;
            for (vpdIter = 0;
                 ...
                 vpdIter++)
            {
                ...
                case UNIT_SERIAL_NUMBER:
                {
                    ...
                    uint8_t* unitSerialNumber =
                        C_CAST(uint8_t*, safe_calloc_aligned(unitSerialNumberPageLength, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!unitSerialNumber)
                    {
                                                ...
                                                supportedVPDPages[vpdIter], true, false))
                    {
                        ...
                        {
                            if (strncmp(driveInfo->vendorID, "SEAGATE",
                                        safe_strlen("SEAGATE")) == 0 &&
                                serialNumberLength == 0x14) // Check SEAGATE Vendor ID And check that the length
                                                            // matches the SCSI commands reference manual
                            {
                                // get the SN and PCBA SN separetly. This is unique to Seagate
                                // drives at this time.
                            ...
                            else
                            {
                                ...
                                // For Seagate and LaCie USB drives, need to remove leading or
                                // trailing zeroes.
                ...
                case DEVICE_IDENTIFICATION:
                {
                    uint8_t* deviceIdentification =
                        C_CAST(uint8_t*, safe_calloc_aligned(INQ_RETURN_DATA_LENGTH, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!deviceIdentification)
                    {
                        perror("Error allocating memory to read device identification VPD "
                               "page");
                        continue;
                    }
                                                ...
                                                DEVICE_IDENTIFICATION, true, false))
                    {
                        ...
                        if (devIDPageLen + 4 > INQ_RETURN_DATA_LENGTH)
                        {
                            ...
                            uint8_t* temp =
                                C_CAST(uint8_t*, safe_reallocf_aligned(C_CAST(void**, &deviceIdentification), 0,
                                                                       C_CAST(size_t, devIDPageLen) + 4,
                                                                       device->os_info.minimumAlignment));
                            if (!temp)
                            {
                                perror("Error trying to realloc for larget device "
                                       "identification VPD page data!\n");
                                return 101;
                            }
                        ...
                        // Below we loop through to the designator descriptors to find the
                        // WWN, and on SAS set the active port. we get the active phy from
                        // the low byte of the WWN when we find the association field set to
                        // 01b
                             ...
                             deviceIdentificationIter += designatorLength)
                        {
                            ...
                            case 0: // associated with the addressed logical unit
                                ...
                                {
                                    driveInfo->worldWideNameSupported = true;
                                    // all NAA values other than 6 currently are 8 bytes long, so
                                    // get all 8 of those bytes in.
                                ...
                                break;
                            case 1: // associated with the target port that received the
                                    // command
                                if (is_Seagate_Family(device))
                                {
                                    if (protocolIdentifier == 0x06 &&
                                        designatorType == 0x03) // SAS->only place that getting a port number
                                                                // makes sense right now since we aren't
                                                                // gathering port speed for other interfaces
                                                                // since it isn't reported.
                                    {
                                        // we know we have found the right designator, so read the
                                        // WWN, and check the lowest nibble for the port number
                                        accotiatedWWN =
                                            M_BytesTo8ByteValue(deviceIdentification[deviceIdentificationIter + 4],
                                                                deviceIdentification[deviceIdentificationIter + 5],
                                                                deviceIdentification[deviceIdentificationIter + 6],
                                ...
                                break;
                            case 2: // associated with SCSI target device that contains the
                ...
                case EXTENDED_INQUIRY_DATA:
                {
                    uint8_t* extendedInquiryData =
                        C_CAST(uint8_t*, safe_calloc_aligned(VPD_EXTENDED_INQUIRY_LEN, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                            ...
                            // read supported lengths and protection types VPD page
                            {
                                ...
                                if (supportedBlockSizesAndProtectionTypes)
                                {
                                                                ...
                                                                false))
                                    {
                                        ...
                                        uint8_t* temp = C_CAST(
                                            uint8_t*, safe_reallocf_aligned(
                                                          C_CAST(void**, &supportedBlockSizesAndProtectionTypes), 0,
                                                                    ...
                                                                    false))
                                        {
                                                 ...
                                                 offset += UINT16_C(8))
                                            {
                                                ...
                                                {
                                                    // all protection types supported so we can leave the
                                                    // loop
                                                    break;
                                                }
                                            }
                ...
                {
                    uint8_t* blockDeviceCharacteristics =
                        C_CAST(uint8_t*, safe_calloc_aligned(VPD_BLOCK_DEVICE_CHARACTERISTICS_LEN, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!blockDeviceCharacteristics)
                    {
                        perror("Error allocating memory to read block device "
                               "characteistics VPD page");
                        continue;
                    }
                ...
                case POWER_CONDITION:
                    // reading this information has been moved to the mode pages below. -
                    // TJE add_Feature_To_Supported_List(driveInfo->featuresSupported,
                ...
                {
                    uint8_t* logicalBlockProvisioning =
                        C_CAST(uint8_t*, safe_calloc_aligned(VPD_LOGICAL_BLOCK_PROVISIONING_LEN, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!logicalBlockProvisioning)
                    {
                        perror("Error allocating memory to read logical block provisioning "
                               "VPD page");
                        continue;
                    }
                ...
                case ATA_INFORMATION:
                {
                    uint8_t* ataInformation =
                        C_CAST(uint8_t*, safe_calloc_aligned(VPD_ATA_INFORMATION_LEN, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!ataInformation)
                    {
                ...
                {
                    uint32_t concurrentRangesLength = (15 * 32) + 64; // max of 15 ranges at 32 bytes each, plus 64
                                                                      // bytes that show ahead as a "header"
                    uint8_t* concurrentRangesData =
                        C_CAST(uint8_t*, safe_calloc_aligned(concurrentRangesLength, sizeof(uint8_t),
                                                             device->os_info.minimumAlignment));
                    if (!concurrentRangesData)
                    {
                        perror("Error allocating memory to read concurrent positioning "
                               "ranges VPD page");
                        continue;
                    }
                                                ...
                                                CONCURRENT_POSITIONING_RANGES, true, false))
                    {
                        ...
                        driveInfo->concurrentPositioningRanges =
                            C_CAST(uint8_t, (M_BytesTo2ByteValue(concurrentRangesData[2],
                                                                 concurrentRangesData[3]) -
                                             60) /
                                                32); //-60 since page length doesn't include first
                                                     // 4 bytes and descriptors start at offset 64.
                                                     // Each descriptor is 32B long
                    }
                ...
                {
                    uint8_t* zbdCharacteristics =
                        C_CAST(uint8_t*, safe_calloc_aligned(VPD_ZONED_BLOCK_DEVICE_CHARACTERISTICS_LEN,
                                                             sizeof(uint8_t), device->os_info.minimumAlignment));
                    if (!zbdCharacteristics)
                    {
                        perror("Error allocating memory to read zoned block device "
        ...
        {
            // SCSI(1)/SASI/CCS don't have VPD pages. Try getting the SN from here
    ...
    if (device && driveInfo && scsiInfo)
    {
            ...
            !device->drive_info.passThroughHacks.scsiHacks.noLogPages) // SCSI2 introduced log pages
        {
            ...
            if (scsiLogBuf)
            {
                ...
                {
                    // either device doesn't support logs, or it just doesn't support
                    // subpages, so let's try reading the list of supported pages (no
                    // subpages) before saying we need to dummy up the list
                ...
                {
                    // device doesn't support subpages, so read the list of pages without
                    // subpages before continuing.
                    if (SUCCESS != scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, LP_SUPPORTED_LOG_PAGES, 0,
                                                      0, scsiLogBuf, 512))
                    {
                        dummyUpLogPages = true;
                ...
                {
                    // trying to read the list of supported pages can trigger this to show
                    // up due to invalid operation code when this happens, just return to
                ...
                // this is really a work-around for USB drives since some DO support
                // pages, but the don't actually list them (same as the VPD pages
                // above). Most USB drives don't work though - TJE
                if (dummyUpLogPages)
                {
                    uint16_t offset    = 4;
                    ...
                    if (subpagesSupported)
                    {
                        ...
                        offset += increment;
                        scsiLogBuf[offset] = LP_SUPPORTED_LOG_PAGES_AND_SUBPAGES; // just to be
                                                                                  // correct/accurate
                    ...
                    // if SBC3! (sequential access device page, so also check peripheral
                    // device type)
                    if (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                        scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
                    {
                        if (scsiInfo->version >= 6) // SBC3 is to be used in conjunction with SPC4. We may need
                                                    // to drop this one level later, but this should be ok
                        {
                    ...
                    if (scsiInfo->version >= 4) // SPC2
                    {
                        scsiLogBuf[offset] = LP_START_STOP_CYCLE_COUNTER; // just to be correct, we're not
                                                                          // reading this today
                        offset += increment;
                    }
                    ...
                    if (scsiInfo->version >= 6)
                    {
                        scsiLogBuf[offset] = LP_GENERAL_STATISTICS_AND_PERFORMANCE; // not likely available
                                                                                    // on USB
                        offset += increment;
                    }
                    ...
                    if (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                        scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
                    {
                        if (scsiInfo->version >= 5) // SBC2 is to be used in conjunction with SPC3. We may need
                                                    // to drop this one level later, but this should be ok
                        {
                ...
                uint16_t logPageIter = LOG_PAGE_HEADER_LENGTH; // log page descriptors start on offset 4
                                                               // and are 2 bytes long each
                     ...
                     logPageIter += incrementAmount)
                {
                    ...
                    case LP_WRITE_ERROR_COUNTERS:
                        if (subpageCode == 0)
                        {
                            ...
                            {
                                // check the length before we start trying to read the number of
                                // bytes in.
                                if (M_BytesTo2ByteValue(writeErrorData[4], writeErrorData[5]) == 0x0005)
                                {
                                    ...
                                    case 8: // quad word
                                        ...
                                        break;
                                    default: // don't bother trying to read the data since it's in
                                             // a more complicated format to read than we care to
                                             // handle in this code right now
                                        break;
                                    }
                    ...
                    case LP_READ_ERROR_COUNTERS:
                        if (subpageCode == 0)
                        {
                                                              ...
                                                              subpageCode, 0x0005, readErrorData, 16))
                            {
                                ...
                                {
                                    // check the length before we start trying to read the number
                                    // of bytes in.
                    ...
                    case LP_TEMPERATURE: // also environmental reporting
                        ...
                        case 1: // environmental reporting
                        {
                                                              ...
                                                              subpageCode, 0, environmentReporting, 16))
                            {
                                ...
                                driveInfo->temperatureData.highestTemperature =
                                    C_CAST(int8_t, environmentReporting[10]);
                    ...
                    case LP_UTILIZATION: // also start-stop cycle counter
                        ...
                        case 0x00: // start-stop cycle count
                        {
                                                              ...
                                                              subpageCode, 0x0001, startStopCounterLog, 14))
                            {
                                ...
                                {
                                    // check the parameter code in case a device/translator with
                                    // this page is responding but did not give this parameter
                                    // code
                    ...
                    case LP_SELF_TEST_RESULTS:
                        if (subpageCode == 0)
                        {
                            uint8_t* selfTestResults =
                                C_CAST(uint8_t*, safe_calloc_aligned(LP_SELF_TEST_RESULTS_LEN, sizeof(uint8_t),
                                                                     device->os_info.minimumAlignment));
                            if (!selfTestResults)
                            {
                                break;
                    ...
                    case LP_INFORMATION_EXCEPTIONS:
                        if (subpageCode == 0)
                        {
                                                              ...
                                                              subpageCode, 0, informationExceptions, 11))
                            {
                                ...
                                else // we have a trip condition...don't care what the
                                     // specific trip is though
                                {
                                    driveInfo->smartStatus = 1;
                                }
                                if (!driveInfo->temperatureData.temperatureDataValid && informationExceptions[10] > 0)
                                {
                                    // temperature log page was not read, neither was
                                    // environmental reporting, but we do have a current
                                    // temperature reading here, so use it
                                    driveInfo->temperatureData.temperatureDataValid = true;
                    ...
                    case SEAGATE_LP_FARM:
                        ...
                        {
                            // Currently only reading first parameter to check if this is the
                            // farm log.
                                                              ...
                                                              subpageCode, 0, farmData, 76))
                            {
                                ...
                                // NOTE: If for any reason DOM was not already read from
                                // standard page, can read it here too
                            }
                            safe_free_aligned(&farmData);
                        }
                        break;
                    case 0x3C: // Vendor specific page. we're checking this page on
                               // Seagate drives for an enhanced usage indicator on SSDs
                               // (PPM value)
        ...
        if (!smartStatusRead)
        {
            // we didn't read the informational exceptions log page, so we need to set
            // this to SMART status unknown
            driveInfo->smartStatus = 2;
        }
    }
    return ret;
                                                 ...
                                                 ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        ...
        // read capacity data - try read capacity 10 first, then do a read
        // capacity 16. This is to work around some USB bridges passing the command
        ...
        case PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE:
            if (SUCCESS == scsi_Read_Capacity_10(device, readCapBuf, READ_CAPACITY_10_LEN))
            {
                ...
                if (scsiInfo->version > 3) // SPC2 and higher can reference SBC2 and
                                           // higher which introduced read capacity 16
                {
                    // try a read capacity 16 anyways and see if the data from that was
                    // valid or not since that will give us a physical sector size whereas
                    // readcap10 data will not
                    uint8_t* temp = C_CAST(uint8_t*, safe_realloc_aligned(readCapBuf, READ_CAPACITY_10_LEN,
                    ...
                    if (SUCCESS == scsi_Read_Capacity_16(device, readCapBuf, READ_CAPACITY_16_LEN))
                    {
                        ...
                        copy_Read_Capacity_Info(&logicalBlockSize, &physicalBlockSize, &maxLBA, &sectorAlignment,
                                                readCapBuf, true);
                        // some USB drives will return success and no data, so check if this
                        // local var is 0 or not...if not, we can use this data
                        if (maxLBA != 0)
                        {
                                        ...
                                        ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        if (!device->drive_info.passThroughHacks.scsiHacks.noModePages && (scsiInfo->version >= 2 || scsiInfo->ccs))
        {
            ...
            uint16_t offset        = 0;
            // create a list of mode pages (and any subpages) we care about reading
            // and go through that list reading each one
            DECLARE_ZERO_INIT_ARRAY(uint8_t, listOfModePagesAndSubpages,
                                    512); // allow 10 entries in the list...update the
                                          // loop condition below if this is adjusted
            // format for page list is first byte = page, 2nd byte = subpage, then
            // increment and look at the next page
                ...
                device->drive_info.interface_type != MMC_INTERFACE && device->drive_info.interface_type != SD_INTERFACE)
            {
                if (driveInfo->rotationRate == 0 &&
                    (scsiInfo->peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                     ...
                     scsiInfo->peripheralDeviceType == PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE))
                {
                    listOfModePagesAndSubpages[offset] = MP_RIGID_DISK_GEOMETRY; // To get medium rotation rate if we
                                                                                 // didn't already get it. This page is
                                                                                 // long obsolete
                    offset += 2;
                }
            }
                ...
                scsiInfo->version >= 5) // SPC3 added subpage codes
            {
                    ...
                    device->drive_info.interface_type != SD_INTERFACE)
                {
                    ...
                    listOfModePagesAndSubpages[offset] = MP_CONTROL; // IO Advice Hints (can we read this page or not
                                                                     // basically)
                ...
                scsiInfo->version >= 5) // SPC3 added subpage codes
            {
                ...
                listOfModePagesAndSubpages[offset + 1] = 0xF1; // reading this for the ATA APM settings (check if
                                                               // supported really)
                offset += 2;
            }
            if (scsiInfo->version >= 3) // Added in SPC
            {
                listOfModePagesAndSubpages[offset] = MP_INFORMATION_EXCEPTIONS_CONTROL; // SMART/informational
                                                                                        // exceptions & MRIE value.
                                                                                        // Dexcept? Warnings?
                listOfModePagesAndSubpages[offset + 1] = 0;
                offset += 2;
            }
            ...
            uint16_t modeIter           = 0;
            uint8_t  protocolIdentifier = 0;
            for (uint16_t pageCounter = 0; modeIter < offset && pageCounter < numberOfPages &&
                 ...
                 modeIter += 2, ++pageCounter)
            {
                ...
                case MP_READ_WRITE_ERROR_RECOVERY:
                    ...
                    {
                    case 0:
                        // check if AWRE and ARRE are supported or can be changed before
                        // checking if they are enabled or not.
                        {
                            ...
                            DECLARE_ZERO_INIT_ARRAY(uint8_t, readWriteErrorRecovery,
                                                    12 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include
                                                                                            // header length in
                                                                                            // this
                            bool     pageRead     = false;
                            ...
                            if (defaultsRead)
                            {
                                ...
                                if (readWriteErrorRecovery[headerLength + 2] & BIT7)
                                {
                                    ...
                                    else
                                    {
                                        awreStringLength = 30;
                                        char* temp =
                                            C_CAST(char*, safe_realloc(awreString, awreStringLength * sizeof(char)));
                                        if (temp)
                                        {
                                            awreString = temp;
                                ...
                                if (readWriteErrorRecovery[headerLength + 2] & BIT6)
                                {
                                    ...
                                    else
                                    {
                                        arreStringLength = 30;
                                        char* temp =
                                            C_CAST(char*, safe_realloc(arreString, arreStringLength * sizeof(char)));
                                        if (temp)
                                        {
                                            arreString = temp;
                ...
                case MP_CACHING:
                    ...
                    case 0:
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, cachingPage,
                                                20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include
                                                                                        // header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = 0;
                        ...
                        {
                            // NV_DIS
                            driveInfo->nvCacheEnabled = !M_ToBool(cachingPage[headerLength + 13] &
                                                                  BIT0); // bit being set means disabled the cache,
                                                                         // being set to 0 means cache is enabled.
                            ...
                            {
                                // we didn't get is supported from above, so check the changable
                                // page
                                memset(cachingPage, 0, 20 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH);
                                pageRead = false; // reset to false before reading the changable
                                                  // values page
                                ...
                                if (pageRead)
                                {
                                    // changable dictates if a bit can be changed or not. So
                                    // unlike above where it is indicating state, this indicates
                                    // if it is supported or can be changed at all from the
                                    // current state
                ...
                case MP_CONTROL:
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, controlPage,
                                                MP_CONTROL_LEN +
                                                    SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header
                                                                                       // length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = 0;
                        ...
                        if (pageRead)
                        {
                            ...
                            {
                                // check length since the page needs to be long enough for this
                                // data. Earlier specs this page was shorter
                                if (controlPage[headerLength + 1] == 0x0A)
                                {
                                    if (driveInfo->longDSTTimeMinutes ==
                                        0) // checking for zero since we may have already gotten
                                           // this from the Extended Inquiry VPD page
                                    {
                                        driveInfo->longDSTTimeMinutes =
                    ...
                    {
                        // check if DLC is supported or can be changed before checking if
                        // they are enabled or not.
                        char*    dlcString       = M_NULLPTR;
                        uint32_t dlcStringLength = 0;
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, controlExtensionPage,
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, ioAdviceHints,
                                                1040 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header
                                                                                          // length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = 0;
                        ...
                        if (pageRead)
                        {
                            // check if any of the Hints valid bits are set so we know it is
                            // enabled. TODO: add checking for the cache enabled bit?
                            bool valid = false;
                    ...
                    case 0xF1: // PATA control
                               // if we can read this page, then the device supports PATA
                               // Control
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, pataControl,
                                                8 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header
                                                                                       // length in this
                        // bool pageRead = false,
                        bool sixByte = false;
                        // uint16_t headerLength = 0;
                                                          ...
                                                          8 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH, M_NULLPTR, &sixByte))
                        {
                            ...
                            // {
                            //     headerLength = MODE_PARAMETER_HEADER_10_LEN;
                            //     blockDescLen = M_BytesTo2ByteValue(pataControl[6],
                            //     pataControl[7]); if (pataControl[3] & BIT7)
                            //     {
                            //         driveInfo->isWriteProtected = true;
                            //     }
                            // }
                ...
                case MP_PROTOCOL_SPECIFIC_PORT:
                    switch (subPageCode)
                    {
                    case 0x00: // Protocol specific port (Use this to get whether SAS or
                               // FC or SCSI, etc)
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, protocolSpecificPort,
                                                LEGACY_DRIVE_SEC_SIZE +
                                                    SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include header
                                                                                       // length in this
                        bool     pageRead     = false;
                ...
                case MP_POWER_CONDTION:
                    ...
                    case 0x00: // EPC
                    {
                        ...
                        // read the default values to check if it's supported...then try the
                        // current page...
                        bool defaultsRead = false;
                        bool sixByte      = false;
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, powerConditions,
                                                40 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include
                                                                                        // header length in this
                        bool     pageRead     = false;
                        uint16_t headerLength = 0;
                        if (SUCCESS == get_SCSI_Mode_Page(device, MPC_DEFAULT_VALUES, pageCode, subPageCode, M_NULLPTR,
                        ...
                        if (defaultsRead)
                        {
                            if (powerConditions[1 + headerLength] > 0x0A)
                            {
                                ...
                                else
                                {
                                    epcFeatureStringLength = 4;
                                    char* temp             = C_CAST(
                                        char*, safe_realloc(epcFeatureString, epcFeatureStringLength * sizeof(char)));
                                    if (temp)
                                    {
                    ...
                    {
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, ataPowerConditions,
                                                16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include
                                                                                        // header length in this
                        bool     pageRead     = false;
                        bool     sixByte      = false;
                        uint16_t headerLength = 0;
                ...
                case MP_INFORMATION_EXCEPTIONS_CONTROL:
                    ...
                    case 0x01: // Background control
                    {
                        ...
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, backgroundControl,
                                                16 + SCSI_MODE_PAGE_MIN_HEADER_LENGTH); // need to include
                                                                                        // header length in this
                        bool     pageRead     = false;
                        bool     defaultsRead = false;
                        ...
                        if (defaultsRead)
                        {
                            ...
                            if (backgroundControl[headerLength + 5] & BIT0)
                            {
                                ...
                                else
                                {
                                    bmsPSStringLength = 50;
                                    char* temp =
                                        C_CAST(char*, safe_realloc(bmsPSString, bmsPSStringLength * sizeof(char)));
                                    if (temp)
                                    {
                                        bmsPSString = temp;
                                              ...
                                              ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        // skip diag pages on USB/IEEE1394 as it is extremly unlikely these requests
        // will be handled properly and unlikely that any standard diag pages will
        // be supported.-TJE
        if (device->drive_info.interface_type != USB_INTERFACE &&
            device->drive_info.interface_type != IEEE_1394_INTERFACE &&
            device->drive_info.interface_type != MMC_INTERFACE && device->drive_info.interface_type != SD_INTERFACE)
        {
            // Read supported Diagnostic parameters and check for rebuild assist.
            // (need SCSI2 and higher since before that, this is all vendor unique)
            ...
            if (supportedDiagnostics)
            {
                ...
                {
                    // try this request first. If it fails, then most likely we need to
                    // use the SCSI 2 method below instead
                ...
                if (!gotDiagData)
                {
                    // old backwards compatible way to request the diag data is to send a
                    // send diagnostic with all zeroes to request the supported pages in
                ...
                if (gotDiagData)
                {
                    // confirm the page code in case old devices did not respond correctly
                    // due to not supporting the page format bit. confirm the list of
                    // supported pages by checking the page code and byte 1 are both set
                    // to zero
                    if (supportedDiagnostics[0] == DIAG_PAGE_SUPPORTED_PAGES && supportedDiagnostics[1] == 0)
                    {
                             ...
                             iter < C_CAST(uint32_t, pageLength + UINT16_C(4)) && iter < supportedDiagsLength; ++iter)
                        {
                            ...
                            {
                                // Add more diagnostic pages in here if we want to check them
                                                   ...
                                                   ptrSCSIIdentifyInfo         scsiInfo)
{
    ...
    if (device && driveInfo && scsiInfo)
    {
        if (!device->drive_info.passThroughHacks.scsiHacks
                 .noReportSupportedOperations) // mostly for USB devices to prevent
                                               // sending commands that don't
                                               // usually work in the first place.
        {
            uint8_t* supportedCommands =
                C_CAST(uint8_t*, safe_calloc_aligned(36, sizeof(uint8_t), device->os_info.minimumAlignment));
            // allocating 36 bytes for 4 byte header + max length of 32B for the CDB
            if (supportedCommands)
            {
                // Most SAT devices won't report all at once, so try asking for
                // individual commands that are supported one at a time instead of
                // asking for everything all at once. Format unit
                bool formatSupported     = false;
                bool fastFormatSupported = false;
                if (scsiInfo->version >= 5 &&
                    SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE,
                                                                     SCSI_FORMAT_UNIT_CMD, 0, 10, supportedCommands))
                {
                    ...
                    case 5: // supported in vendor specific mannor in same format as case
                            // 3
                        formatSupported = true;
                        // now check for fast format support
                        if (!(supportedCommands[7] == 0xFF &&
                              supportedCommands[8] == 0xFF)) // if both these bytes are FFh, then the drive
                                                             // conforms to SCSI2 where this was the
                                                             // "interleave" field
                        {
                ...
                else if (scsiInfo->version >= 3 && scsiInfo->version < 5 &&
                         SUCCESS == scsi_Inquiry(device, supportedCommands, 12, SCSI_FORMAT_UNIT_CMD, false, true))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                             device, false, REPORT_OPERATION_CODE,
                                                             SCSI_FORMAT_WITH_PRESET_CMD, 0, 14, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                memset(supportedCommands, 0, 36);
                // Sanitize (need to check each service action to make sure at least one
                // is supported.
                bool sanitizeSupported = false;
                if (scsiInfo->version >= 5 &&
                    SUCCESS == scsi_Report_Supported_Operation_Codes(
                                   device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, SANITIZE_CMD,
                                   SCSI_SANITIZE_OVERWRITE, 14, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                else if (scsiInfo->version >= 5 &&
                         SUCCESS == scsi_Report_Supported_Operation_Codes(
                                        device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, SANITIZE_CMD,
                                        SCSI_SANITIZE_BLOCK_ERASE, 14, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                    ...
                    default:
                        break;
                         ...
                         SUCCESS == scsi_Report_Supported_Operation_Codes(
                                        device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, SANITIZE_CMD,
                                        SCSI_SANITIZE_CRYPTOGRAPHIC_ERASE, 14, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                    ...
                    default:
                        break;
                         ...
                         SUCCESS == scsi_Report_Supported_Operation_Codes(
                                        device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, SANITIZE_CMD,
                                        SCSI_SANITIZE_EXIT_FAILURE_MODE, 14, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                             device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             0x9E, 0x17, 20, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                             device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             0x9E, 0x18, 20, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                             device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             0x9E, 0x19, 20, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 &&
                    scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE &&
                    SUCCESS == scsi_Report_Supported_Operation_Codes(device, false,
                                                                     REPORT_OPERATION_CODE_AND_SERVICE_ACTION, 0x9E,
                                                                     0x1A, 20, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                    ...
                    scsiInfo->peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
                {
                    if (scsiInfo->version >= 5 &&
                        SUCCESS == scsi_Report_Supported_Operation_Codes(device, false,
                                                                         REPORT_OPERATION_CODE_AND_SERVICE_ACTION, 0x95,
                                                                         0x07, 20, supportedCommands))
                    {
                        switch (supportedCommands[1] & 0x07)
                        {
                        case 0: // not available right now...so not supported
                        case 1: // not supported
                            break;
                        case 3: // supported according to spec
                        case 5: // supported in vendor specific manor in same format as case
                                // 3
                            add_Feature_To_Supported_List(driveInfo->featuresSupported,
                                                          &driveInfo->numberOfFeaturesSupported, "Zone Domains");
                            break;
                ...
                {
                    // Check security protocol in case the earlier attempt did not work to
                    // detect when the OS/driver/HBA are blocking these commands
                    if (scsiInfo->version >= 6 &&
                        SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE, 0xA2, 0,
                                                                         16, supportedCommands))
                    {
                        switch (supportedCommands[1] & 0x07)
                        {
                        case 0: // not available right now...so not supported
                ...
                // check write buffer (firmware download) call info firmware download.h
                ...
                // change the device type to scsi before we enter here! Doing this so
                // that --satinfo is correct!
                ...
                if (scsiInfo->version >= 5 &&
                    SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE,
                                                                     ATA_PASS_THROUGH_12, 0, 16, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                    ...
                    case 5: // supported in vendor specific mannor in same format as case
                            // 3
                        // TODO: make sure this isn't the "blank" command being supported by
                        // a MMC device.
                        add_Feature_To_Supported_List(driveInfo->featuresSupported,
                ...
                if (scsiInfo->version >= 5 &&
                    SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE,
                                                                     ATA_PASS_THROUGH_16, 0, 20, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
                ...
                if (scsiInfo->version >= 5 && SUCCESS == scsi_Report_Supported_Operation_Codes(
                                                             device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             0x7F, 0x1FF0, 36, supportedCommands))
                {
                    switch (supportedCommands[1] & 0x07)
                    {
                    case 0: // not available right now...so not supported
...
eReturnValues get_SCSI_Drive_Information(tDevice* device, ptrDriveInformationSAS_SATA driveInfo)
{
    ...
    uint8_t* inquiryData =
        C_CAST(uint8_t*, safe_calloc_aligned(255, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (inquiryData)
    {
    ...
    // TODO: add checking peripheral device type as well to make sure it's only
    ...
    {
        // send report luns to see how many luns are attached. This SHOULD be the
        // way to detect multi-actuator drives for now. This could change in the
        // future.
        // TODO: Find a better way to remove the large check above which may not
        // work out well in some cases, but should reduce false detection on USB
    ...
    if (scsiInfo.version == 6 &&
        (device->drive_info.passThroughHacks.scsiHacks.securityProtocolSupported ||
         SUCCESS == scsi_SecurityProtocol_In(device, SECURITY_PROTOCOL_INFORMATION, 0, false, 0,
                                             M_NULLPTR))) // security protocol commands introduced in
                                                          // SPC4. TODO: may need to drop to SPC3 for
                                                          // some devices. Need to investigate
    {
        // Check for TCG support - try sending a security protocol in command to get
        // the list of security protocols (check for security protocol EFh? We can
        // do that for ATA Security information)
                ...
                scsi_SecurityProtocol_In(device, SECURITY_PROTOCOL_INFORMATION, 0, false, 512, securityProtocols))
            {
                                                                            ...
                                                                            securityProtocols, 512))
                {
                    ...
                    if (driveInfo->securityInfo.ataDeviceServer)
                    {
                        ...
                        memcpy(&driveInfo->ataSecurityInformation, &driveInfo->securityInfo.ataSecurityInfo,
                               sizeof(ataSecurityStatus));
                    }
                }
            }
    ...
    {
        // these old standards didn't report it, but we can reasonably guess the
        // speed
...
// currently using the bitfields in here, other commands are sometimes run to
// read additional information may need to reorganize more in the future to
// eliminate needing to pass in tDevice -TJE
                                                       ...
                                                       uint32_t                identifyDataLength)
{
    ...
    {
        // Supports security send/receive. Check for TCG and other security
        // protocols
        ...
        {
            // NOTE: For whatever reason the security commands did not complete
            // despite the drive supporting them to read the list of supported
            // protocols set the "blocked commaands" flag
            //  This is not currently enabled as it has not been observed in any
            //  system yet like it has for ATA and SCSI
            // driveInfo->trustedCommandsBeingBlocked = true;
        }
    }
...
// This is for use with ATA or SCSI drives where we only want to show the
// applicable information for each drive type. NOT RECOMMENDED ON EXTERNAL
// USB/IEEE1394 PRODUCTS!
...
void print_NVMe_Device_Information(ptrDriveInformationNVMe driveInfo)
{
    ...
    // Putting SMART & DST data here so that it isn't confused with the namespace
    // data below - TJE
    if (driveInfo->smartData.valid)
    {
        ...
        if (driveInfo->controllerData.longDSTTimeMinutes > 0)
        {
            ...
            minutes = 0;
            seconds = 0;
            convert_Seconds_To_Displayable_Time(driveInfo->controllerData.longDSTTimeMinutes * 60, &years, &days,
...
#    define MINUTES_IN_1_YEAR 525600.0
        ...
        double calculatedUsage =
            C_CAST(double, totalTerabytesRead + totalTerabytesWritten) *
            C_CAST(double, MINUTES_IN_1_YEAR / C_CAST(double, driveInfo->smartData.powerOnHoursD) * 60.0);
        printf("%0.02f\n", calculatedUsage);
        // Total Bytes Read
    ...
    case ENCRYPTION_SELF_ENCRYPTING:
        ...
        {
            printf("\t\tWARNING: OS is blocking TCG commands over passthrough.
        Please enable it before running any TCG commands\n");
        }*/
        break;
...
#    define MINUTES_IN_1_YEAR 525600.0
    ...
    if (driveInfo->trustedCommandsBeingBlocked)
    {
        printf("\t\tWARNING: OS/driver/HBA is blocking TCG commands over "
    ...
    {
        if (driveInfo->lowCurrentSpinupViaSCT) // to handle differences in reporting
                                               // between 2.5" products and others
        {
    ...
    {
        printf("Full"); // changed to "Full" from "Immediate" since this makes more
                        // sense...-TJE
    ...
    {
        printf("\t\tNone reported or an error occurred while trying to "
               "determine\n\t\tthe features.\n");
    }
    // Adapter information
    ...
    if (driveInfo->adapterInformation.specifierIDValid) // IEEE1394 only, so it will only print when we get
                                                        // this set to true for now - TJE
    {
        printf("\t\tSpecifier ID: ");
    ...
    {
        printf("This device has multiple actuators. Some commands/features may "
               "affect more than one actuator.\n");
    }
    return;
}

// This exists so we can print out SCSI reported and ATA reported information
// for comparison purposes. (SAT test/check) NOT FOR USE WITH A SAS DRIVE
...
// This function ONLY exists because we need to show a mix of SCSI and ATA
// information on USB.
                                         ...
                                         ptrDriveInformationSAS_SATA ataDriveInfo)
{
    ...
    {
        // take data from each of the inputs, and plug it into a new one, then call
        // the standard print function
        memcpy(externalDriveInfo, ataDriveInfo, sizeof(driveInformationSAS_SATA));
        // we have a copy of the ata info, now just change the stuff we want to show
        // from scsi info
        ...
        memcpy(externalDriveInfo->firmwareRevision, scsiDriveInfo->firmwareRevision,
        ...
        // But this is being done for some newer products that are...strange...so we
        // need to do some additional checks to figure out whether the SCSI drive
        // info is telling us something that is not reported in the ATA drive info.
        ...
        {
            memcpy(&externalDriveInfo->specificationsSupported[extSpecNumber],
                                              ...
                                              ptrDriveInformationNVMe     nvmeDriveInfo)
{
    ...
    // After that take the POH, temperature, DST information, workload, and
    // combine the features. Also add the NVMe spec version to the output as well.
    if (externalDriveInfo && scsiDriveInfo && nvmeDriveInfo)
    {
        // take data from each of the inputs, and plug it into a new one, then call
        // the standard print function
        memcpy(externalDriveInfo, scsiDriveInfo, sizeof(driveInformationSAS_SATA));
        // Keep the SCSI information, minus a few things to copy from NVMe if the
        // NVMe info needed was read properly
        if (nvmeDriveInfo->smartData.valid)
        {
            ...
            externalDriveInfo->totalBytesRead =
                C_CAST(uint64_t,
                       nvmeDriveInfo->smartData.dataUnitsReadD * 512 *
            ...
            externalDriveInfo->totalBytesWritten =
                C_CAST(uint64_t,
                       nvmeDriveInfo->smartData.dataUnitsWrittenD * 512 *
                           1000); // this is a count of 512B units, so converting to bytes
        ...
        {
            memcpy(&externalDriveInfo->featuresSupported[extFeatNumber],
            ...
            if (strcmp(nvmeDriveInfo->controllerData.controllerFeaturesSupported[nvmeFeatNumber], "Firmware Update") ==
                0)
            {
                // this is not the best way to handle this, but will keep capabilities
                // listed more consistent with NVMe
        ...
        if (nvmeDriveInfo->namespaceData.valid)
        {
            ...
            {
                memcpy(&externalDriveInfo->featuresSupported[extFeatNumber],
                       &nvmeDriveInfo->namespaceData.namespaceFeaturesSupported[nvmeFeatNumber], MAX_FEATURE_LENGTH);
                ++(externalDriveInfo->numberOfFeaturesSupported);
            }
        }
    }
...
eReturnValues print_Drive_Information(tDevice* device, bool showChildInformation)
{
    ...
    // Always allocate scsiDrive info since it will always be available no matter
    // the drive type we are talking to!
    ...
    if (ret == SUCCESS && (ataDriveInfo || scsiDriveInfo || usbDriveInfo || nvmeDriveInfo))
    {
        ...
        {
            // ONLY call the external function when we are able to get some
            // passthrough information back as well
            if ((device->drive_info.interface_type == USB_INTERFACE ||
                 device->drive_info.interface_type == IEEE_1394_INTERFACE) &&
