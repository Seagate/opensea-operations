...
eReturnValues ata_Get_Native_Max_LBA(tDevice* device, uint64_t* nativeMaxLBA)
{
    ...
    if ((is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
         device->drive_info.IdentifyData.ata.Word086 & BIT15) /*validate words 119,120 are valid first, then validate
                                                                 that word*/
        && (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) &&
            device->drive_info.IdentifyData.ata.Word119 & BIT8)) // accessible max address feature set
    {
        ret = ata_Get_Native_Max_Address_Ext(device, nativeMaxLBA);
    }
    else if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word082) &&
             device->drive_info.IdentifyData.ata.Word082 & BIT10) // HPA feature set
    {
        ret = ata_Read_Native_Max_Address(device, nativeMaxLBA,
                                          device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported);
    }
    else // no other feature sets to check. DCO is handled in a different file
         // since it affects more than just maxLBA
    {
...
eReturnValues get_Native_Max_LBA(tDevice* device, uint64_t* nativeMaxLBA)
{
    ...
    *nativeMaxLBA     = UINT64_MAX; // this is invalid, but useful for scsi since
                                    // reseting to native max means using this value
                                    // (see the reset code for scsi_Set_Max_LBA)
...
eReturnValues scsi_Set_Max_LBA_2(tDevice* device, uint64_t newMaxLBA, bool reset, bool changeId)
{
    ...
    // always do a mode sense command to get back a block descriptor. Always
    // request default values because if we are doing a reset, we just send it
    // right back, otherwise we will overwrite the returned data ourselves
    if (SUCCESS == scsi_Mode_Sense_10(device, 0, 0x18, 0, false, true, MPC_DEFAULT_VALUES, scsiDataBuffer))
    {
        newMaxLBA += 1; // Need to add 1 for SCSI so that this will match the -i
                        // report. If this is not done, then  we end up with 1 less
...
eReturnValues ata_Set_Max_LBA_2(tDevice* device, uint64_t newMaxLBA, bool reset, bool changeId)
{
    ...
    // first do an identify to figure out which method we can use to set the
    // maxLBA (legacy, or new Max addressable address feature set)
    uint64_t nativeMaxLBA = 0;
    // always get the native max first (even if that's only a restriction of the
    // HPA feature set)
    if (SUCCESS == (ret = get_Native_Max_LBA(device, &nativeMaxLBA)))
    {
        ...
        if ((reset && (newMaxLBA - 1) == device->drive_info.deviceMaxLba) ||
            (newMaxLBA == device->drive_info.deviceMaxLba)) // The -1 is due to how maxlba is read
                                                            // and saved in the tDevice struct.
        {
            // already at maxLBA. Do not make a change.
            // Both HPA and AMAC will require a power cycle between calls to setting
            // the maxLBA, so no need to use that up if this has already been set.
            ret = SUCCESS;
        }
        else
        {
            if ((is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
                 device->drive_info.IdentifyData.ata.Word086 & BIT15) && /*validate 119 and 120 will be valid first*/
                (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) &&
                     ...
                     device->drive_info.IdentifyData.ata.Word082 & BIT10) // HPA feature set
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            ret = ata_Set_Max_Address(device, C_CAST(uint32_t, newMaxLBA),
                                                      true); // this is a non-volitile command
                        }
                        else
                        {
                            ret = NOT_SUPPORTED;
...
eReturnValues set_Max_LBA_2(tDevice* device, uint64_t newMaxLBA, bool reset, bool changeId)
{
    ...
    else
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            printf("Setting the max LBA is not supported on this device type at this "
                   "time\n");
        }
        ret = NOT_SUPPORTED;
    }
...
// This function is specifically named since it has a main purpose: restoring
// max LBA to erase a drive as much as possible or to allow validation of an
// erase as much as possible. Because of this, it handles all the ATA checks to
// make sure all features are restored or a proper error code for frozen or
// access denied is returned (HPA/AMAC/DCO and HPA security are all handled)
eReturnValues restore_Max_LBA_For_Erase(tDevice* device)
{
    ...
    {
        // before calling the reset, check if HPA security might be active. This
        // could block this from working
        ...
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word083) &&
            device->drive_info.IdentifyData.ata.Word083 & BIT8)
        {
            // HPA security is supported...check if enabled to be able to return
            // proper error code if cannot do restoration
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
                device->drive_info.IdentifyData.ata.Word086 & BIT8)
            {
        ...
        // Before attempting any restore commands, get the current MaxLBA, HPA/AMAC
        // native MaxLBA, and the DCO identify MaxLBA. Compare them to see which, if
        // any, needs to be restored. This will reduce command aborts and errors in
        // this process.

        eReturnValues readnativemaxret = ata_Get_Native_Max_LBA(device, &hpaamacMax);

        // First compare read native max to current max
        if (readnativemaxret == SUCCESS &&
            ((hpaamacMax - UINT64_C(1)) > currentMaxLBA)) // minus 1 is due to how tDevice saves maxLBA as a SCSI
                                                          // value
        {
            ret = ata_Set_Max_LBA(device, 0, true);
            if (ret != SUCCESS && hpaSecurityEnabled)
            {
                // most likely command aborted because this feature is active and has
                // locked the HPA from being changed/restored
                return DEVICE_ACCESS_DENIED;
            }
            else if (ret != SUCCESS)
            {
                // HPA and AMAC require a power cycle between each change to the max
                // LBA. this could also cover when these features are frozen.
            ...
            if (ret == SUCCESS && is_DCO_Supported(device, M_NULLPTR))
            {
                ...
                if (SUCCESS == dcoret)
                {
                    if ((dcoIDData.maxLBA - UINT64_C(1)) > currentMaxLBA ||
                        ((dcoIDData.maxLBA - UINT64_C(1)) > hpaamacMax))
                    {
                        ret = dco_Restore(device);
                        if (ret != SUCCESS)
                        {
                            // DCO restore is needed, but we cannot do it until the drive has
                            // been power cycled
...
static uint64_t get_ATA_MaxLBA(tDevice* device)
{
    ...
    if (SUCCESS == ata_Identify(device, C_CAST(uint8_t*, &device->drive_info.IdentifyData.ata.Word000), 512))
    {
        ...
        {
            // acs4 - word 69 bit3 means extended number of user addressable sectors
            // word supported (words 230 - 233) (Use this to get the max LBA since
            // words 100 - 103 may only contain a value of FFFF_FFFF)
            if ((is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word053) &&
                ...
                && (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word069) &&
                    device->drive_info.IdentifyData.ata.Word069 & BIT3))
            {
                maxLBA =
        ...
        // adjust to make it report more like SCSI since that is how all this
        // library works
        if (maxLBA > 0)
        {
            maxLBA -= 1;
...
static uint64_t get_SCSI_MaxLBA(tDevice* device)
{
    ...
    uint8_t* readCapBuf =
        C_CAST(uint8_t*, safe_calloc_aligned(READ_CAPACITY_10_LEN, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!readCapBuf)
    {
        return maxLBA;
    }
    if (SUCCESS == scsi_Read_Capacity_10(device, readCapBuf, READ_CAPACITY_10_LEN))
    {
        ...
        if (device->drive_info.scsiVersion > 3) // SPC2 and higher can reference SBC2 and higher which introduced
                                                // read capacity 16
        {
            // try a read capacity 16 anyways and see if the data from that was valid
            // or not since that will give us a physical sector size whereas readcap10
            // data will not
            uint8_t* temp =
                C_CAST(uint8_t*, safe_realloc_aligned(readCapBuf, READ_CAPACITY_10_LEN, READ_CAPACITY_16_LEN,
    ...
    else
    {
        ...
        uint8_t* temp = C_CAST(uint8_t*, safe_realloc_aligned(readCapBuf, READ_CAPACITY_10_LEN, READ_CAPACITY_16_LEN,
                                                              device->os_info.minimumAlignment));
        if (temp == M_NULLPTR)
        {
            safe_free_aligned(&readCapBuf);
...
bool is_Max_LBA_In_Sync_With_Adapter_Or_Driver(tDevice* device, bool issueReset)
{
    ...
    else if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        // opensea-transport has a software based translator present so all requests
        // in here will be handled by it if there is not a driver or hardware
        // translator We need to first try reading VPD page 89h. This is the SAT
        // page and it requires pulling fresh identify data....it MIGHT update the
        // controller, but it might not.
        ...
        {
            // Note that the identify data in this page can be modified by the SATL in
            // some versions of SAT, so do not trust it.
        }
        if (issueReset)
        {
            // NOTE: This is an IOCTL implemented by the low-level system files.
            //       There is no guarantee this is implemented or works at all. For
            //       example, in Windows the IOCTLs for this are obsolete and do not
            //       work on any modern hardware I can find. While testing a Broadcom
            //       controller on Linux, this does seem to fix the synchronization
            //       issue with the SG reset IOCTL. This may not work on a different
            //       controller and there is no guarantee the driver implements this
            //       IOCTL
            if (SUCCESS == os_Device_Reset(device))
            {
                delay_Seconds(1);
                // issue test unit ready after this to clear out a sense code for the
                // device having received a bus reset
                scsi_Test_Unit_Ready(device, M_NULLPTR);
            }
        }
        ataMaxLBA  = get_ATA_MaxLBA(device);
        scsiMaxLBA = get_SCSI_MaxLBA(device);
        // before comparing, know that ATA and SCSI report max LBA slightly
        // differently. It's an off by 1 scenario. In SCSI, max LBA is reported as
        // last readable LBA of that value read lba <=maxLBA In ATA, maxLBA is a
        // count of readable LBAs...so                 read lba < maxLBA The
        // functions used to get max lba are already taking this difference into
        // account The other special case to have in mind is USB. USB drives may
        // report a lower value as well. USB drives may also do reverse 4k emulation
        // which will also change how this is reported. So USB has yet another
        // special case to handle when this happens.
        if (is_Sector_Size_Emulation_Active(device))
        {
            // likely only USB drives from the Windows XP era
            // in this case take the ata max LBA and divide it by the scsi sector size
            // before comparing.
            ...
            {
                // possibly USB being off by one since it can use maxLBA to save info
                // for the adapter. Considering this in sync because of this special
                // case
                inSync = true;
            }
        }
    ...
    {
        // NOTE: There are not the same style of commands on nvme to change capacity
        // as there is with SAS and SATA
        //       In NVMe there is the namespace management feature, but that is far
        //       more complicated than this command. For now just returning true for
        //       NVMe since the only real encapsulated NVMe drives will be on USB
        //       and it is extremely unlikely any namespace management commands will
        //       work over USB
        inSync = true;
    }
    return inSync;
...
bool is_Change_Identify_String_Supported(tDevice* device)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
                                                 ...
                                                 LEGACY_DRIVE_SEC_SIZE, 0))
        {
                ...
                M_Word0(qword0) >= 0x0001)
            {
                ...
                return (supportedCapabilitiesQWord & BIT63 &&
                        supportedCapabilitiesQWord & BIT58); // check bit63 since it should always be 1, then bit
                                                             // 58 for Capacity/Model Number Mapping
            }
        }
    }
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS == get_SCSI_VPD_Page_Size(device, CAPACITY_PRODUCT_IDENTIFICATION_MAPPING, &capIDVPDSizeBytes) &&
            capIDVPDSizeBytes > 0)
        {
...
ptrcapacityModelNumberMapping get_Capacity_Model_Number_Mapping(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (SUCCESS ==
                get_ATA_Log_Size(device, ATA_LOG_CAPACITY_MODELNUMBER_MAPPING, &capMNLogSizeBytes, true, false) &&
            capMNLogSizeBytes > 0)
        {
            ...
            if (SUCCESS == get_ATA_Log(device, ATA_LOG_CAPACITY_MODELNUMBER_MAPPING, M_NULLPTR, M_NULLPTR, true, false,
                                       true, capMNMappingLog, capMNLogSizeBytes, M_NULLPTR, 0, 0))
            {
                ...
                if (capModelMapping != M_NULLPTR)
                {
                         ...
                         offset += 48, ++descriptorCounter)
                    {
                        ...
                        memcpy(capModelMapping->descriptor[descriptorCounter].modelNumber, &capMNMappingLog[offset + 8],
                               mnLimit);
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
            ...
            capIDVPDSizeBytes > 0)
        {
            ...
            if (SUCCESS == get_SCSI_VPD(device, CAPACITY_PRODUCT_IDENTIFICATION_MAPPING, M_NULLPTR, M_NULLPTR, true,
                                        capProdIDMappingVPD, capIDVPDSizeBytes, M_NULLPTR))
            {
                // calculate number of descriptors based on page length
                uint32_t numberOfDescriptors = M_BytesTo2ByteValue(capProdIDMappingVPD[2],
                                                                   capProdIDMappingVPD[3]) /
                                               48; // Each descriptor is 48B long
                uint32_t capProdIDMappingSz =
                    C_CAST(uint32_t, (sizeof(capacityModelNumberMapping) - sizeof(capacityModelDescriptor)) +
                                         (sizeof(capacityModelDescriptor) * numberOfDescriptors));
                capModelMapping =
                    C_CAST(ptrcapacityModelNumberMapping, safe_calloc(capProdIDMappingSz, sizeof(uint8_t)));
                if (capModelMapping != M_NULLPTR)
                {
                    capModelMapping->numberOfDescriptors = numberOfDescriptors;
                    // loop through descriptors
                    for (uint32_t offset = 4, descriptorCounter = 0;
                         ...
                         offset += 48, ++descriptorCounter)
                    {
                        ...
                        capModelMapping->descriptor[descriptorCounter].capacityMaxAddress -=
                            1; // Need to -1 for SCSI so that this will match the -i report.
                               // If this is not done, then  we end up with 1 less than the
                               // value provided.
                        uint16_t mnLimit = M_Min(MODEL_NUM_LEN, 16);
                        memset(capModelMapping->descriptor[descriptorCounter].modelNumber, 0, mnLimit + 1);
                        memcpy(capModelMapping->descriptor[descriptorCounter].modelNumber,
...
void print_Capacity_Model_Number_Mapping(ptrcapacityModelNumberMapping capModelMapping)
{
    if (capModelMapping)
    {
        ...
        for (uint32_t descriptorCounter = 0; descriptorCounter < capModelMapping->numberOfDescriptors;
             descriptorCounter++)
        {
            printf("%20" PRIu64 " %s\n", capModelMapping->descriptor[descriptorCounter].capacityMaxAddress,
