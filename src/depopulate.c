...
//
// \file depopulate.h
// \brief This file defines the functions for depopulating physical/storage
// elements on a drive (Remanufacture)
...
bool is_Depopulation_Feature_Supported(tDevice* device, uint64_t* depopulationTime)
{
                                                 ...
                                                 LEGACY_DRIVE_SEC_SIZE, 0))
        {
                                                                 ...
                                                                 // for bit63 to be 1 and page number to be 3
            {
                ...
                if (depopulationTime)
                {
                    ...
                    {
                        *depopulationTime = UINT64_MAX; // so we can set the timeout on the command or say
                                                        // "time not reported"
                    }
                }
            }
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             0x9E, 0x17, 20, reportOpCodes))
        {
...
{
    // NOTE: Seagate legacy method uses head numbers starting at zero, but STD
    // spec starts at 1. Add 1 to anything from Seagate legacy method
    eReturnValues ret = NOT_SUPPORTED;
    if (!elementList)
    {
        return BAD_PARAMETER;
    }
    // This should be a number of 512B blocks based on how many physical elements
    // are supported by the drive. NOTE: If this ever starts requesting a LOT of
    // data, then this may need to be broken into multiple commands. - TJE
                                       ...
                                       uint64_t           depopulateTime)
{
    ...
    printf("\nElement #\tType\tHealth\tStatus\t\tAssociated MaxLBA\tRebuild "
           "Allowed\n");
    printf("---------------------------------------------------------------------"
           "-------------\n");
...
#define PHYSICAL_ELEMENT_REBUILD_ALLOWED_STRING_MAX_LENGTH 4
    ...
    printf("\nNOTE: At least one element must be able to be rebuilt to "
...
// NOTE: This may NOT give percentage. This will happen on ATA drives, but you
// can check that it is still running or not. - TJE On ATA drives, if in
// progress, the progress variable will get set to 255 since it is not possible
...
eReturnValues get_Depopulate_Progress(tDevice* device, eDepopStatus* depopStatus, double* progress)
{
    ...
    *depopStatus = DEPOP_NOT_IN_PROGRESS;
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        {
            // Send the get physical element status command and check if any say
            // depopulation/repopulation in progress or had an error. read physical
            // element status to see if any of the specified element number matches
            // any that were found
            ...
            {
                ptrPhysicalElement elementList =
                    C_CAST(ptrPhysicalElement, safe_malloc(numberOfDescriptors * sizeof(physicalElement)));
                if (elementList)
                {
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (SUCCESS == scsi_Request_Sense_Cmd(device, true, senseData, SPC3_SENSE_LEN))
        {
            ...
            // now that we've read all the fields, check for known sense data and fill
            // in progress if any.
        ...
        {
            // If this failed, there is likely a bigger problem! But we can try
            // getting physical element status
            ret = FAILURE;
        }
    }
    return ret;
...
eReturnValues show_Depop_Repop_Progress(tDevice* device)
{
    ...
    if (SUCCESS == get_Depopulate_Progress(device, &depopStatus, &progress))
    {
        ...
        case DEPOP_MICROCODE_NEEDS_ACTIVATION:
            printf("Depopulation/repopulation requires microcode activation before "
                   "it can be run.\n");
            break;
        default:
            printf("Unknown depopulation/repopulation status. The feature may not be "
                   "supported, or is not running.\n");
            break;
        }
    }
    else
    {
        ret = FAILURE;
        printf("A failure was encountered when checking for progress on "
                                                  ...
                                                  bool     pollForProgress)
{
    ...
    if (is_Depopulation_Feature_Supported(device, &depopTime))
    {
        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
        {
            ...
            {
                printf("Starting depopulation. Approximate time until completion is "
                       "not available.\n");
            }
            else
            {
        ...
        if (ret != SUCCESS)
        {
            ...
            {
                // On SAS, we'll have sense data, on ATA we can attempt to request
                // sense, but some systems/controllers do this for us and make this
                // impossible to retrieve...so we need to work around this
                uint8_t senseKey = 0;
                uint8_t asc      = 0;
                uint8_t ascq     = 0;
                uint8_t fru      = 0;
                ...
                get_Sense_Key_ASC_ASCQ_FRU(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, &senseKey, &asc,
                                           &ascq, &fru);
                // if this matches known cases, we're good to go...otherwise if ATA try
                // requesting sense data ext command.
                if (senseKey == SENSE_KEY_NOT_READY && asc == 0x04 && ascq == 0x1E) // microcode activation required
                {
                    if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                    {
                        printf("Depopulation cannot be started. Microcode must be "
                               "activated first.\n");
                    }
                    ret = FAILURE;
                }
            ...
            else if (device->drive_info.drive_type == ATA_DRIVE)
            {
                ...
                if (workaroundIncompleteSense)
                {
                    ...
                    // This means that something about the command was not liked...first
                    // check if microcode needs activation
            ...
            if (determineInvalidElementOrMaxLBA)
            {
                ...
                ret                 = FAILURE;
                // read physical element status to see if any of the specified element
                // number matches any that were found
                uint32_t numberOfDescriptors = 0;
                ...
                if (numberOfDescriptors > 0)
                {
                    ...
                    if (elementList)
                    {
                        ...
                        if (SUCCESS == get_Physical_Element_Descriptors(device, numberOfDescriptors, elementList))
                        {
                            // loop through and check associatedCapacity and
                            // elementIdentifiers
                            ...
                            for (uint32_t elementID = 0; elementID < numberOfDescriptors; ++elementID)
                            {
                                if (elementList[elementID].elementIdentifier == elementDescriptorID)
                                {
                                    ...
                                    {
                                        // tried requesting a new capacity greater than what the
                                        // device can support...so this will trigger an error
                                        invalidMaxLBA = true;
                                    }
                                }
                            }
                ...
                if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                {
                    ...
                    {
                        printf("Depopulation failed with invalid field. Invalid element "
                               "specified, or invalid new max LBA or some other error\n");
                    }
                }
            }
            else
            {
                if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                {
                    printf("An unknown error was encountered when attempting to "
                           "depopulate elements.\n");
                }
                ret = FAILURE;
            }
        ...
        else
        {
            if (pollForProgress)
            {
                ...
                // SCSI can report progress percentage in sense data. ATA does not do
                // this. Furthermore, request sense data from ATA may or may not work or
                ...
                do
                {
                    ...
                    if (depopStatus == DEPOP_IN_PROGRESS)
                    {
                        if (progress > 100.0)
                        {
                            if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                            {
                                printf("\rDepopulation is progress, but progress indication is "
                                       "not available.");
                            }
                        }
                        else
...
bool is_Depopulate_And_Modify_Zones_Supported(tDevice* device, uint64_t* depopulationTime)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
                                                     ...
                                                     LEGACY_DRIVE_SEC_SIZE, 0))
            {
                ...
                if (supportedCapabilitiesQWord0 & BIT63 &&
                    M_GETBITRANGE(supportedCapabilitiesQWord0, 23, 16) ==
                        ATA_ID_DATA_LOG_SUPPORTED_CAPABILITIES) // make sure required
                                                                // bits/fields are
                                                                // there...checking for
                                                                // bit63 to be 1 and
                                                                // page number to be 3
                                                 ...
                                                 LEGACY_DRIVE_SEC_SIZE, 0))
        {
            ...
            if (supportedCapabilitiesQWord0 & BIT63 &&
                M_GETBITRANGE(supportedCapabilitiesQWord0, 23, 16) ==
                    ATA_ID_DATA_LOG_ZONED_DEVICE_INFORMATION) // make sure required
                                                              // bits/fields are
                                                              // there...checking for
                                                              // bit63 to be 1 and
                                                              // page number to be 9
...
eReturnValues perform_Repopulate_Physical_Element(tDevice* device, bool pollForProgress)
{
    ...
    if (is_Repopulate_Feature_Supported(device, &depopTime))
    {
        if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
        {
            if (depopTime == UINT64_MAX || depopTime == 0)
            {
                printf("Starting repopulation. Approximate time until completion is "
                       "not available.\n");
            }
            else
            {
        ...
        if (ret != SUCCESS)
        {
            if (device->drive_info.drive_type == SCSI_DRIVE)
            {
                ...
                if (senseKey == SENSE_KEY_NOT_READY && asc == 0x04 && ascq == 0x1E) // microcode activation required
                {
                    if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                    {
                        printf("Repopulation cannot be started. Microcode must be "
                               "activated first.\n");
                    }
                    ret = FAILURE;
                }
                else if (senseKey == SENSE_KEY_ILLEGAL_REQUEST && asc == 0x2C &&
                         ascq == 0x00) // command sequence error means all depopulated
                                       // elements do not allow restoration (at least
                                       // one needs to support this)
                {
                    ret = FAILURE;
                }
            }
            else if (device->drive_info.drive_type == ATA_DRIVE)
            {
                ...
                if (workaroundIncompleteSense)
                {
                    ...
                    if (!reasonFound)
                    {
                        // read physical element status to see if any of the specified
                        // element number matches any that were found
                        ...
                        if (numberOfDescriptors > 0)
                        {
                            ...
                            if (elementList)
                            {
                                ...
                                {
                                    // figure out if any depopulated elements support being
                                    // repopulated
                        ...
                        if (currentlyDepopulatedElements > 0)
                        {
                            ...
                            else
                            {
                                if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                                {
                                    printf("Repopulation of elements is not supported as "
                                           "currently depopulated elements do not\n");
                ...
                do
                {
                    ...
                    if (depopStatus == DEPOP_REPOP_IN_PROGRESS)
                    {
                        if (progress > 100.0)
                        {
                            if (device->deviceVerbosity >= VERBOSITY_DEFAULT)
                            {
                                printf("\rRepopulation is progress, but progress indication is "
                                       "not available.");
                            }
                        }
                        else
