...
//                     The intention of the file is to be generic & not OS
//                     specific
...
eReturnValues get_Ready_LED_State(tDevice* device, bool* readyLEDOnOff)
{
    ...
    {
        uint8_t* modeSense =
            C_CAST(uint8_t*, safe_calloc_aligned(24, sizeof(uint8_t), device->os_info.minimumAlignment));
        if (!modeSense)
        {
    ...
    else // ata cannot control ready LED since it is managed by the host, not
         // the drive (drive just reads a signal to change operation as per ATA
         // spec). Not sure if other device types support this change or not at
         // this time.
    {
        ret = NOT_SUPPORTED;
    }
...
eReturnValues change_Ready_LED(tDevice* device, bool readyLEDDefault, bool readyLEDOnOff)
{
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        if (readyLEDDefault)
        {
            // we need to read the default AND current page this way we only touch 1
            // bit on the page
...
eReturnValues scsi_Set_NV_DIS(tDevice* device, bool nv_disEnableDisable)
{
    ...
    uint8_t* cachingModePage = C_CAST(uint8_t*, safe_calloc_aligned(MP_CACHING_LEN + MODE_PARAMETER_HEADER_10_LEN,
    ...
    if (ret == SUCCESS)
    {
        ...
        cachingModePage[6] = 0;
        cachingModePage[7] = 0;
        // now go change the bit to what we need it to, then send a mode select
        // command
...
eReturnValues nvme_Set_Write_Cache(tDevice* device, bool writeCacheEnableDisable)
{
    ...
    if (device->drive_info.IdentifyData.nvme.ctrl.vwc & BIT0) // This bit must be set to 1 to control whether write
                                                              // caching is enabled or disabled.
    {
        ...
        // featuresOptions.sv //TODO: take extra "volatile" parameter? The drive may
        // or may not support saving this accross power cycles (see spec). - TJE
...
// NOTE: this uses the RCD bit. Old drives do not support this bit. Checking the
// changable values to detect support before trying to change it.
bool scsi_Is_Read_Look_Ahead_Supported(tDevice* device)
{
    ...
    // check default to see if it is enabled and just cannot be disabled
    // (unlikely)
...
bool is_NV_Cache_Enabled(tDevice* device)
{
    if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        return !scsi_is_NV_DIS_Bit_Set(device); // since this function returns when the bit is set to 1
                                                // (meaning cache disabled), then we need to flip that bit for
                                                // this use.
    }
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE && !device->drive_info.passThroughHacks.scsiHacks.noVPDPages)
    {
                                    ...
                                    BLOCK_DEVICE_CHARACTERISTICS, true, false))
        {
                ...
                device->drive_info.currentProtectionType > 0)
            {
                ...
                // So we need to check if the device supports logical block provisioning
                // management. If it does, we are done, but otherwise we need to set a
                // flag for may require an overwrite. Devices that support logical block
                // provisioning will not require an overwrite because they automatically
                // unmap at the end of crypto or block erase which resets the PI bytes
                // and does not cause a read conflict. -TJE NOTE: It is possible for a
                // vendor unique behavior on other devices to allow reading after these,
                // but we have no way of detecting that -TJE In SBC, a device supporting
                // this shall support the logical block provisioning VPD page...so just
                // try requesting that first.
                ...
                {
                    // check if lbpu, lbpws, or lbpws10 are set since this can indicate
                    // support for provisioning. If none are set, provisioning is not
                    // supported.
                ...
                if (needPIWriteAfterErase)
                {
                    ...
                    {
                        // only change when this is set to some other value because that can
                        // help to set this only when crypto is supported
                        writeReq->cryptoErase = WAEREQ_PI_FORMATTED_MAY_REQUIRE_OVERWRITE;
                    }
                    if (writeReq->blockErase != WAEREQ_NOT_SPECIFIED)
                    {
                        // only change when this is set to some other value because that can
                        // help to set this only when block is supported
                                          ...
                                          uint32_t*   overwriteEraseTimeEstimateMinutes)
{
    ...
    if (sanitizeInfo.crypto)
    {
        ...
        if (writeAfterEraseRequirements.cryptoErase >= WAEREQ_MEDIUM_ERROR_OTHER_ASC)
        {
            ...
            {
                snprintf(sanitizeWarning, MAX_ERASE_WARNING_LENGTH,
                         "PI formatting may require write after crypto erase.");
            }
            else
            {
                snprintf(sanitizeWarning, MAX_ERASE_WARNING_LENGTH,
                         "Cannot be stopped, even with a power cycle. Write after "
                         "crypto erase required.");
            }
        }
        else
        {
    ...
    // format on SAS SSD will take only a couple seconds since it basically does a
    // unmap operation
    if (isFormatUnitSupported && device->drive_info.drive_type == SCSI_DRIVE && !formatUnitAdded && is_SSD(device))
    {
        ...
        currentErase->sanitizationLevel = ERASE_SANITIZATION_CLEAR; // While an SSD may do a block erase, there is
                                                                    // no guarantee it is the same as
                                                                    // Sanitize.-TJE
        ++currentErase;
    ...
    {
        // next up for NVMe is to list the format with user and crypto erase
        // support.
        if (nvmeFormatInfo.cryptographicEraseSupported)
        {
            ...
            // NOTE: We can create a list of known devices capable of meeting purge
            // and a list of those know to meet clear to set this more clearly
            ++currentErase;
        }
        // if NOT an NVM HDD, user erase should be next since it will most likely be
        // as fast as a sanitize block erase
        if (is_SSD(device))
        {
            ...
            currentErase->eraseWeight       = 1;
            currentErase->warningValid      = false;
            currentErase->sanitizationLevel = ERASE_SANITIZATION_CLEAR; // only crypto erase is a possible purge in
                                                                        // IEEE2883-2022
            // NOTE: We can create a list of known devices capable of meeting purge
            // and a list of those know to meet clear to set this more clearly
            ++currentErase;
    ...
    // trim/unmap/deallocate are not allowed since they are "hints" that those
    // LBAs are not needed rather than guaranteed erasure of those blocks

    // This weight value is reserved for TCG revert (this is placed in another
    // library)

    bool enhancedEraseAddedToList = false;
    // maybe enhanced ata security erase (check time...if it's set to 2 minutes,
    // then that is the lowest possible time and likely a TCG drive doing a crypto
    // erase)
    if (ataSecurityInfo.enhancedEraseSupported && ataSecurityInfo.enhancedSecurityEraseUnitTimeMinutes == 2)
    {
        ...
        {
            snprintf(currentErase->eraseWarning, MAX_ERASE_WARNING_LENGTH,
                     "Use the password set in the BIOS/UEFI or disable it from "
                     "BIOS/UEFI.");
        }
        else
        {
    ...
    // this weight value is reserved for TCG revertSP (this is placed in another
    // library)
    ...
    // format unit (I put this above write same since on SAS, we cannot get
    // progress indication from write same)
    if (isFormatUnitSupported && device->drive_info.drive_type == SCSI_DRIVE && !formatUnitAdded)
    {
        ...
        currentErase->sanitizationLevel = ERASE_SANITIZATION_CLEAR; // If security initialize is supported in
                                                                    // format it can be a purge. Seagate drives do
                                                                    // not support this bit, other vendors might.
        ++currentErase;
        formatUnitAdded = true;
    }
    ...
    if (device->drive_info.drive_type == NVME_DRIVE && nvmeFormatInfo.formatCommandSupported && !nvmFormatAdded)
    {
        currentErase->eraseIdentifier = ERASE_NVM_FORMAT_USER_SECURE_ERASE;
        snprintf(currentErase->eraseName, MAX_ERASE_NAME_LENGTH, "NVM Format: User Data Erase");
        currentErase->eraseWeight = 8; // assuming that this will do a full drive
                                       // overwrite format which will be slow
        // NOTE: If crypto is supported, a request for user secure erase may run a
        // crypto erase, but no way to know for sure-TJE
        currentErase->warningValid      = false;
        currentErase->sanitizationLevel = ERASE_SANITIZATION_CLEAR;
        // Note: We can create a list of known devices capable of meeting purge and
        // a list of those know to meet clear to set this more clearly
        ++currentErase;
        nvmFormatAdded = true;
    }
    ...
    if (ataSecurityInfo.securitySupported)
    {
        ...
        // if enhanced erase has not been added, but is supported, add it here
        // (since it's longer than short)
        if (!enhancedEraseAddedToList && ataSecurityInfo.enhancedEraseSupported &&
            ataSecurityInfo.enhancedSecurityEraseUnitTimeMinutes >= ataSecurityInfo.securityEraseUnitTimeMinutes)
        {
            enhancedEraseAddedToList = true;
        ...
        // Using normal for the estimate since it will always be an overwrite of the
        // disk (no crypto) and because we cannot access reassigned sectors during a
        // host overwrite, so this is the closest we'll get.-TJE
    ...
    if (overwriteEraseTimeEstimateMinutes) // make sure the incoming value is zero
                                           // in case time was set by something
                                           // above here (like ata security erase)
    {
        uint8_t hours   = 0;
        uint8_t minutes = 0;
        // let's set a time estimate!
        // base it off of the long DST time...as that is probably the closest match
        // we'll get since that does access every LBA
        ...
        *overwriteEraseTimeEstimateMinutes = M_Max(*overwriteEraseTimeEstimateMinutes, longDSTTimeMinutes);
        ...
        {
            // This drive doesn't support anything that gives us time estimates, so
            // let's make a guess.
            // TODO: Make this guess better by reading the drive capabilities and
            // interface speed to determine a more accurate estimate.
                                   ...
                                   uint32_t*         overwriteEraseTimeEstimateMinutes)
{
    ...
    printf("\tRecommendation - Restore the MaxLBA of the device prior to any "
           "erase in\n");
    printf("\t                 order to allow the drive to erase all user "
           "addressable\n");
    printf("\t                 sectors. For ATA devices this means restoring \n");
    printf("\t                 HPA + DCO / AMAC to restore the maxLBA.\n");
    printf("\t                 Restoring the MaxLBA also allows full "
           "verification of\n");
    printf("\t                 all user addressable space on the device without "
           "a\n");
    ...
    printf("\tClear, Possible Purge - Cryptographic erase is a purge if the "
           "vendor\n");
    printf("\t        implementation meets the requirements in IEEE 2883-2022.\n");
    printf("\tPurge - Logical techniques that target user data, "
           "overprovisioning,\n");
    printf("\t        unused space, and bad blocks rendering data recovery "
           "infeasible\n");
...
#define ERASE_SANITIZATION_CAPABILITIES_STR_LEN (24)
    ...
    if (overwriteEraseTimeEstimateMinutes)
    {
        ...
        // Example output:
        // The minimum time to overwrite erase this drive is approximately x days y
        // hours z minutes. The actual time may take longer. Cryptographic erase
        // completes in seconds. Trim/Unmap & blockerase should also complete in
        // under a minute
...
eReturnValues set_Sense_Data_Format(tDevice* device, bool defaultSetting, bool descriptorFormat, bool saveParameters)
{
    ...
    // Should there be an interface check here? We should allow this anywhere
    // since SAT might be being used and would also be affected by this bit
    if (defaultSetting)
    {
        ...
        {
            // figure out what D_Sense is set to, then change it in the current
            // settings
...
eReturnValues get_Current_Free_Fall_Control_Sensitivity(tDevice* device, uint16_t* sensitivity)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
            device->drive_info.IdentifyData.ata.Word086 & BIT15)
        {
            if (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) &&
                device->drive_info.IdentifyData.ata.Word119 & BIT5) // supported
            {
                ret          = SUCCESS;
                *sensitivity = UINT16_MAX; // this can be used to filter out invalid value, a.k.a.
                                           // feature is not enabled, but is supported.
                if (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word120) &&
                    device->drive_info.IdentifyData.ata.Word120 & BIT5) // enabled
                {
...
void show_Test_Unit_Ready_Status(tDevice* device)
{
    ...
    else
    {
        ...
        printf("NOT READY\n");
        device->deviceVerbosity = VERBOSITY_COMMAND_NAMES; // the function below will print out a sense
                                                           // data translation, but only it we are at this
                                                           // verbosity or higher which is why it's set
                                                           // before this call.
...
eReturnValues enable_Disable_AAM_Feature(tDevice* device, bool enable)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
            ...
            device->drive_info.IdentifyData.ata.Word083 & BIT9)
        {
            if (enable)
            {
                // set value to the vendor recommended value reported in identify data
                // when requesting an enable operation
            ...
            else
            {
                ...
                if (ret != SUCCESS)
                {
                    // the disable AAM feature is not available on all devices according
                    // to ATA spec.
                    ret = NOT_SUPPORTED;
                }
            }
...
// AAM Levels:
//  0 - vendor specific
//  1-7Fh = These are labelled as "Retired" in every spec I can find, so no idea
//  what these even mean. - TJE 80h = minimum acoustic emanation 81h - FDh =
//  intermediate acoustic management levels FEh = maximum performance.
...
eReturnValues get_AAM_Level(tDevice* device, uint8_t* aamLevel)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word083) &&
            device->drive_info.IdentifyData.ata.Word083 & BIT9) // word 86 says "enabled". We may or may not want to
                                                                // check for that.
        {
...
eReturnValues scsi_Update_Mode_Page(tDevice* device, uint8_t modePage, uint8_t subpage, eSCSI_MP_UPDATE_MODE updateMode)
{
    ...
    if (modePage == MP_RETURN_ALL_PAGES ||
        subpage == MP_SP_ALL_SUBPAGES) // if asking for all mode pages, all mode pages
                                       // and subpages, or all subpages of a specific
                                       // page, we need to handle it in here.
    {
        // if resetting all pages, check if the RTD bit is supported to simplify the
        // process...-TJE
        ...
        {
            // requesting to reset all mode pages. Send the mode select command with
            // the RTD bit set.
        ...
        else
        {
            if (SUCCESS == get_SCSI_Mode_Page_Size(device, MPC_CURRENT_VALUES, modePage, subpage, &modePageLength))
            {
                ...
                {
                    // now we need to loop through each page, and send it to the drive as
                    // a new mode select command.
                    uint32_t offset                = 0;
                    ...
                    for (; offset < modePageLength; offset += currentPageLength, ++counter)
                    {
                        ...
                        if (modeData[offset] & BIT6)
                        {
                            ...
                            currentPageLength = M_BytesTo2ByteValue(modeData[offset + 2],
                                                                    modeData[offset + 3]) +
                                                4; // add 4 bytes for page code, subpage code,
                                                   // & page length bytes
                        }
                        else
                        {
                        ...
                        currentPageToSet = C_CAST(uint8_t*, safe_calloc_aligned(currentPageToSetLength, sizeof(uint8_t),
                        ...
                        else
                        {
                            ...
                            memcpy(currentPageToSet, &modeData[0],
                                   MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength);
                            // now zero out the reserved bytes for the mode select command
                            currentPageToSet[0] = 0; // mode data length is reserved for mode select commands
                        ...
                        // now we need to copy the default data over now, then send it to
                        // the drive.
    ...
    else
    {
        ...
        if (SUCCESS == get_SCSI_Mode_Page_Size(device, MPC_CURRENT_VALUES, modePage, subpage, &modePageLength))
        {
                                              ...
                                              modePageLength, M_NULLPTR, &used6ByteCmd))
            {
                ...
                bool savable    = modeData[offset + 0] & BIT7;  // use this to save pages. This bit says whether
                                                                // the page/settings can be saved or not.
                if (used6ByteCmd)
                {
...
// NOTE: This rely's on NOT having the mode page header in the passed in buffer,
// just the raw mode page itself!
eReturnValues scsi_Set_Mode_Page(tDevice* device, uint8_t* modePageData, uint16_t modeDataLength, bool saveChanges)
{
    ...
    // even though we have the data we want to send, we must ALWAYS request the
    // page first, then modify the data and send it back.
    if (SUCCESS == get_SCSI_Mode_Page_Size(device, MPC_CURRENT_VALUES, modePage, subpage, &modePageLength))
    {
        uint8_t* modeData =
                                          ...
                                          modeData, modePageLength, M_NULLPTR, &used6ByteCmd))
        {
            ...
            if (used6ByteCmd)
            {
                ...
                offset                = MODE_PARAMETER_HEADER_6_LEN + blockDescriptorLength;
                // now zero out the reserved bytes for the mode select command
                modeData[0] = 0; // mode data length is reserved for mode select
                                 // commands
                                 // leave medium type alone
                                 // leave device specific parameter alone???
                                 // leave block descriptor length alone in case we got some.
            ...
            // now send the mode select command
            bool pageFormat = modePage == 0 ? false : true; // set to false when reading vendor unique page zero
            // bool savable = modeData[offset + 0] & BIT7;// use this to save pages.
            // This bit says whether the page/settings can be saved or not.
            if (used6ByteCmd)
            {
...
//       Reviewing all the various SCSI standards for different device types
//       would be necessary in order to make this 100% correct and complete. We
//       are currently focussed on block and zoned block devices, however some
...
{
    scsiDeviceType = M_GETBITRANGE(scsiDeviceType, 4,
                                   0); // strip off the qualifier if it was passed
    switch (modePage)
    {
    ...
    case 0x0D:
        switch (subpage)
        {
        case 0x00: // Power Condition (direct accessblock device) or CD Device
                   // Parameters
            switch (scsiDeviceType)
            {
            case PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE:
    ...
    case 0x1C:
        switch (subpage)
        {
        case 0x00: // Varies on name depending on device type. All related to
                   // failure reporting though!!!
            switch (scsiDeviceType)
            {
            case PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE:
            case PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE:
...
// this should only have the mode data. NO block descriptors or mode page header
// (4 or 8 bytes before the mode page starts)
                            ...
                            bool                 outputWithPrintDataBuffer)
{
    if (modeData && modeDataLen > UINT32_C(2))
    {
        ...
        int equalsLengthToPrint = C_CAST(int, (M_Min(C_CAST(uint32_t, pageLength), modeDataLen) * UINT32_C(3)) -
                                                  UINT32_C(1)); // printf for variable width fields requires an int, so
                                                                // this shuold always calculate to a non-negative value,
                                                                // so it should be safe to case to int...plus it should
                                                                // be small...mode pages are not that long that they
                                                                // come close to signed 32bit max values.-TJE
        // print the header
        ...
        // before going further, check if we have a page name to lookup and printout
        // to adjust the size for
...
// shows a single mode page for the selected control(current, saved, changable,
// default)
                                       ...
                                       // page, we need to handle it in here.
    {
        if (SUCCESS == get_SCSI_Mode_Page_Size(device, mpc, modePage, subpage, &modePageLength))
        {
            ...
            if (SUCCESS == get_SCSI_Mode_Page(device, mpc, modePage, subpage, M_NULLPTR, M_NULLPTR, true, modeData,
                                              modePageLength, M_NULLPTR, &used6ByteCmd))
            {
                // Loop through each page returned in the buffer and print it to the
                // screen
                uint32_t offset                = 0;
                uint16_t blockDescriptorLength = 0;
                if (!used6ByteCmd)
...
// should we return an error when asking for all mode pages since that output
// will otherwise be really messy???
    ...
    if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3)
    {
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, 1, LOG_SELECT_CMD, 0, 14, supportData))
        {
            ...
            case 0x03: // supports in conformance with a scsi standard
                // check the CDB usage data to see if we can set a page code or subpage
                // code
                if (cdbSize > 0)
                {
...
// doing this in SCSI way for now...should handle nvme separately at some point
// since a namespace is similar to a lun
...
eMLU get_MLU_Value_For_SCSI_Operation(tDevice* device, uint8_t operationCode, uint16_t serviceAction)
{
    ...
    if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, reportingOptions, operationCode, serviceAction,
                                                         4, reportOp))
    {
...
bool scsi_Mode_Pages_Shared_By_Multiple_Logical_Units(tDevice* device, uint8_t modePage, uint8_t subPage)
{
    ...
    uint8_t* vpdModePagePolicy =
        C_CAST(uint8_t*, safe_calloc_aligned(modePagePolicyLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        ...
        if (SUCCESS == scsi_Inquiry(device, vpdModePagePolicy, modePagePolicyLength, MODE_PAGE_POLICY, true, false))
        {
            ...
            if (vpdModePagePolicy)
            {
                    ...
                    scsi_Inquiry(device, vpdModePagePolicy, modePagePolicyLength, MODE_PAGE_POLICY, true, false))
                {
                    ...
                    for (uint32_t vpdMPOffset = 4; vpdMPOffset < UINT16_MAX && vpdMPOffset < modePagePolicyLength;
                         vpdMPOffset += 4)
                    {
                        ...
                        {
                            // This is the "report all mode pages", no subpages to indicate
                            // that the mlus applies to all mode pages on the device.
                            mlus = M_ToBool(vpdModePagePolicy[vpdMPOffset + 2] & BIT7);
                            break;
                        }
                        ...
                        {
                            // This is the "report all mode pages and subpages", to indicate
                            // that the mlus applies to all mode pages and subpages on the
                            // device.
                            mlus = M_ToBool(vpdModePagePolicy[vpdMPOffset + 2] & BIT7);
                            break;
                        }
...
eReturnValues get_Concurrent_Positioning_Ranges(tDevice* device, ptrConcurrentRanges ranges)
{
    ...
    if (ranges && ranges->size >= sizeof(concurrentRangesV1) && ranges->version >= CONCURRENT_RANGES_VERSION_V1)
    {
        if (device->drive_info.drive_type == ATA_DRIVE)
        {
            uint32_t concurrentLogSizeBytes = 0; // NOTE: spec currently says this is at most 1024 bytes, but may be
                                                 // as low as 512
                ...
                concurrentLogSizeBytes > 0)
            {
                uint8_t* concurrentRangeLog =
                    C_CAST(uint8_t*, safe_calloc_aligned(concurrentLogSizeBytes, sizeof(uint8_t),
                                                         device->os_info.minimumAlignment));
                                           ...
                                           false, true, concurrentRangeLog, concurrentLogSizeBytes, M_NULLPTR, 0, 0))
                {
                    ...
                    for (uint32_t offset = 64, rangeCounter = 0;
                         offset < concurrentLogSizeBytes && rangeCounter < ranges->numberOfRanges && rangeCounter < 15;
                         offset += 32, ++rangeCounter)
                    {
        ...
        else if (device->drive_info.drive_type == SCSI_DRIVE)
        {
                ...
                concurrentLogSizeBytes > 0)
            {
                                            ...
                                            concurrentRangeVPD, concurrentLogSizeBytes, M_NULLPTR))
                {
                    ...
                    ranges->numberOfRanges = C_CAST(uint8_t, (M_BytesTo2ByteValue(concurrentRangeVPD[2],
                                                                                  concurrentRangeVPD[3]) -
                                                              60) /
                                                                 32); //-60 since page length doesn't include
                                                                      // first 4 bytes and descriptors start at
                                                                      // offset 64. Each descriptor is 32B long
...
void print_Concurrent_Positioning_Ranges(ptrConcurrentRanges ranges)
{
    ...
    {
        printf("ERROR: Incompatible concurrent ranges data structure. Cannot print "
               "the data.\n");
    }
}
...
eReturnValues get_Write_Read_Verify_Info(tDevice* device, ptrWRVInfo info)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        {
            if (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) &&
                device->drive_info.IdentifyData.ata.Word119 & BIT1) // supported
            {
                info->supported = true;
                if (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word120) &&
                    device->drive_info.IdentifyData.ata.Word120 & BIT1)
                {
                    info->enabled = true;
                }
                ...
                // filling in remaining data because it is possible some drives will
                // report valid data for these other modes even if not enabled.
                if (info->currentWRVMode == ATA_WRV_MODE_USER &&
                    (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word211) &&
                     is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word210)))
                {
                    info->wrv3sectorCount = M_WordsTo4ByteValue(device->drive_info.IdentifyData.ata.Word211,
                                                                device->drive_info.IdentifyData.ata.Word210);
                }
                if (info->currentWRVMode == ATA_WRV_MODE_VENDOR &&
                    (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word213) &&
                     is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word212)))
                {
                    info->wrv2sectorCount = M_WordsTo4ByteValue(device->drive_info.IdentifyData.ata.Word213,
                ...
                if (info->enabled)
                {
                    ...
                    case ATA_WRV_MODE_USER:
                        ...
                        break;
                    default: // handle any case not currently defined in the
                             // specifications.
...
eReturnValues set_Write_Read_Verify(tDevice* device, bool all, bool vendorSpecific, uint32_t wrvSectorCount)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        else if (wrvSectorCount == 65536) // Detecting this very specific number to make this mode
                                          // since it is in the spec and this is probably what would
                                          // be wanted in this case.
        {
        ...
        else
        {
            ...
            // Before doing the devision, round up to nearest. If this would overflow,
            ...
            else
            {
                ...
                // If someone selected a value not evenly divisible by 1024, they likely
                // want at LEAST that many sectors being verified rather than not
                // enough, so rounding up here makes the most sense -TJE
