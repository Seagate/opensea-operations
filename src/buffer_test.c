...
//
// \file buffer_test.c
// \brief This file defines the function calls for performing buffer/cabling
// tests
...
static bool are_Buffer_Commands_Available(tDevice* device)
{
    ...
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        // Only asking about read buffer command, since write buffer will likely be
        // implemented for at least FWDL, so if this is supported, the equivalent
        // write buffer command should also be supported
        ...
        if (!device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations &&
            SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             READ_BUFFER_CMD, 0x02, 14,
                                                             supportedCommandData)) // trying w/ service action (newer
                                                                                    // SPC spec allows this)
        {
            driveReportedSupport = true;
        }
        else if (!device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations &&
                 SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE, READ_BUFFER_CMD,
                                                                  0, 14,
                                                                  supportedCommandData)) // older SPC3
        {
            // NOTE: we are just going to assume that if this gives us success, that
            // this mode is supported, even though we don't know for sure...it SHOULD
            // be supported if we got that this command is supported, which is good
            // enough
            driveReportedSupport = true;
        }
        else if (SUCCESS == scsi_Inquiry(device, supportedCommandData, 16, READ_BUFFER_CMD, false,
                                         true)) // really old SPC spec used inquiry to
        ...
        if (driveReportedSupport)
        {
            ...
            case 3: // supported in accordance with SCSI specification
                supported = true;
                break;
            default: // not supported, unknown support, or vendor unique/non-standard
                     // support
                supported = false;
                break;
            }
        ...
        {
            // this means the command to ask about support didn't work, so we're just
            // going to try asking the size of the buffer and if that works, it is
            // supported
...
static eReturnValues get_Buffer_Size(tDevice* device, uint32_t* bufferSize, uint8_t* offsetBoundary)
{
    ...
    *bufferSize = LEGACY_DRIVE_SEC_SIZE; // default to this size. Change this only
                                         // if the drive reports a different size
    *offsetBoundary = 0x09;              // default to this size. Change this only if the drive
                                         // reports a different size
...
static eReturnValues send_Read_Buffer_Command(tDevice* device, uint8_t* ptrData, uint32_t dataSize)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        // return ata_Read_Buffer(device, ptrData,
        // device->drive_info.ata_Options.readBufferDMASupported); Switching to this
        // new function since it will automatically try DMA mode if supported by the
        // drive. If the controller or driver don't like issuing DMA mode, this will
...
static eReturnValues send_Write_Buffer_Command(tDevice* device, uint8_t* ptrData, uint32_t dataSize)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        // return ata_Write_Buffer(device, ptrData,
        // device->drive_info.ata_Options.writeBufferDMASupported); Switching to
        // this new function since it will automatically try DMA mode if supported
        // by the drive. If the controller or driver don't like issuing DMA mode,
        // this will detect it and retry the command with PIO mode.
    ...
    if (checkSenseData)
    {
        if (senseKey == SENSE_KEY_ABORTED_COMMAND) // INFORMATION UNIT iuCRC ERROR DETECTED
        {
            ...
            case 0x47:
                ...
                case 0x03: // INFORMATION UNIT iuCRC ERROR DETECTED - SAT will translate
                           // a CRC error into this! Definitely need this one. Less sure
                           // about the others...-TJE
...
// Function for simple byte pattern tests. take counter for number of times to
// try it?
                                      ...
                                      ptrPatternTestResults testResults)
{
    ...
    if (patternBuffer && returnBuffer)
    {
        ...
        {
            bool breakFromLoop = false; // this will be set to true when we need to
                                        // exit the loop for one reason or another
            ...
            default:
                ...
                continue; // continue loop since this will miscompare no matter what on
...
{
    uint8_t* patternBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(deviceBufferSize, sizeof(uint8_t),
                                             device->os_info.minimumAlignment)); // only send this to the drive
    uint8_t* returnBuffer =
        C_CAST(uint8_t*, safe_malloc_aligned(deviceBufferSize,
                                             device->os_info.minimumAlignment)); // only receive this from the drive
    if (patternBuffer && returnBuffer)
    {
        for (uint32_t bitNumber = 0, byteNumber = 0; byteNumber < deviceBufferSize; ++bitNumber)
        {
            ...
            if (bitNumber > 7)
            {
                // this means we've shifted the bit through each bit of this byte, so
                // offset to the next byte and start again
                ++byteNumber;
                bitNumber = 0;
...
static void perform_Random_Pattern_Test(tDevice* device, uint32_t deviceBufferSize, ptrPatternTestResults testResults)
{
    ...
    if (patternBuffer && returnBuffer)
    {
        ...
        {
            bool breakFromLoop = false;
            fill_Random_Pattern_In_Buffer(patternBuffer,
                                          deviceBufferSize); // set a new random pattern each time
            eReturnValues wbResult = send_Write_Buffer_Command(device, patternBuffer, deviceBufferSize);
            ++(testResults->totalCommandsSent);
            switch (wbResult)
            {
...
//                          R_ERR = mulltiple possible causes from bad
//                          connection to bad cable. Recommend redoing the
//                          connection or replacing cable.
// SATA Device statistics: CRC = definitely bad
//                         ASR events = bad cable as well. (asynchronous signal
//                         recovery)
// SAS SPL error counters: Invalid Dword = definitely bad
//                         Running disparity or loss of sync = mulltiple
//                         possible causes from bad connection to bad cable.
//                         Recommend redoing the connection or replacing cable.
...
eReturnValues perform_Cable_Test(tDevice* device, ptrCableTestResults testResults)
{
    ...
    {
        uint8_t offsetPO2 = 0; // This shouldn't actually be needed...but I have it
                               // here in case I do
        ...
        if (SUCCESS == get_Buffer_Size(device, &bufferSize, &offsetPO2) && bufferSize > 0)
        {
            ...
            // drive supports the read/write buffer commands we need and we know what
            // size the buffer is we can test with. now we need to begin testing.
            ...
            {
                perform_Byte_Pattern_Test(device, UINT32_C(0x00000000), bufferSize,
                                          &testResults->zerosTest[count]); // arbitrary number 10 was chosen
                                                                           // since it sounded good for number
                                                                           // of times to try this pattern
            }
            for (uint8_t count = 0; count < ALL_F_TEST_COUNT; ++count)
            {
                perform_Byte_Pattern_Test(device, UINT32_C(0xFFFFFFFF), bufferSize,
                                          &testResults->fTest[count]); // arbitrary number 10 was chosen since
                                                                       // it sounded good for number of times
                                                                       // to try this pattern
            }
            for (uint8_t count = 0; count < ALL_5_TEST_COUNT; ++count)
            {
                perform_Byte_Pattern_Test(device, UINT32_C(0x55555555), bufferSize,
                                          &testResults->fivesTest[count]); // arbitrary number 10 was chosen
                                                                           // since it sounded good for number
                                                                           // of times to try this pattern
            }
            for (uint8_t count = 0; count < ALL_A_TEST_COUNT; ++count)
            {
                perform_Byte_Pattern_Test(device, UINT32_C(0xAAAAAAAA), bufferSize,
                                          &testResults->aTest[count]); // arbitrary number 10 was chosen since
                                                                       // it sounded good for number of times
                                                                       // to try this pattern
            }
            for (uint8_t count = 0; count < ZERO_F_5_A_TEST_COUNT; ++count)
            {
                perform_Byte_Pattern_Test(device, UINT32_C(0x00FF55AA), bufferSize,
                                          &testResults->zeroF5ATest[count]); // arbitrary number 10 was chosen since
                                                                             // it sounded good for number of times
                                                                             // to try this pattern
            }
            // now walking 1's
            for (uint8_t count = 0; count < WALKING_1_TEST_COUNT; ++count)
            {
                perform_Walking_Test(device, false, bufferSize,
                                     &testResults->walking1sTest[count]); // arbitraty number 5 was chose
                                                                          // since it sounded good for
                                                                          // trying this test
            }
            // walking 0's
            for (uint8_t count = 0; count < WALKING_0_TEST_COUNT; ++count)
            {
                perform_Walking_Test(device, true, bufferSize,
                                     &testResults->walking0sTest[count]); // arbitraty number 5 was chose
                                                                          // since it sounded good for
                                                                          // trying this test
            }
            ...
            {
                perform_Random_Pattern_Test(device, bufferSize,
                                            &testResults->randomTest[count]); // arbitrary number 10 was chosen since
                                                                              // it sounded good for number of times to
                                                                              // try this pattern
            }
