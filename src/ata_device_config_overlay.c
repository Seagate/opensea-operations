...
// Important notes:
// If HPA is set (max < native max) commands to restore or set will be aborted.
// if (device->drive_info.IdentifyData.ata.Word082 & BIT10) //HPA feature set
// DMA mode commands for identify and set only possible if supported (check
// identify bit) If frozen, identify, identify dma, resotre, set will all be
// aborted. If id bit shows supported, but the dco ident fails, consider the
// drive in a frozen state -TJE
...
eReturnValues dco_Freeze_Lock(tDevice* device)
{
    ...
    if (is_DCO_Supported(device, M_NULLPTR))
    {
        ...
        if (ret == ABORTED)
        {
            ret = FROZEN; // device is already in a frozen state, so return this
                          // instead of aborted.
        }
    }
    return ret;
...
eReturnValues dco_Identify(tDevice* device, ptrDcoData data)
{
    ...
    if (is_DCO_Supported(device, &dcoDMASupport))
    {
        if (data)
        {
            ...
            if (ret == ABORTED)
            {
                // if the command aborted, then device is in the frozen state so return
                // this instead.-TJE
                ret = FROZEN;
            }
            else
            {
                ...
                // data->features3 = M_BytesTo2ByteValue(dcoIdentData[45],
                // dcoIdentData[44]); words 23-207 reserved words 208-254 reserved
            }
        }
        else
        {
...
eReturnValues dco_Set(tDevice* device, ptrDcoData data)
{
    ...
    if (is_DCO_Supported(device, &dcoDMASupport))
    {
        if (data)
        {
            ...
            {
                // go through the user-provided details and make changes to the
                // requested fields mwdma bits
                if (!data->mwdma.mwdma2)
                {
