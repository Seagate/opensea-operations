                         ...
                         bool        automaticReadReallocationEnabled)
{
        ...
        (device->drive_info.drive_type == ATA_DRIVE || device->drive_info.drive_type == ATAPI_DRIVE))
    {
        if (device->drive_info.interface_type != IDE_INTERFACE)
        {
            ...
            temp               = C_CAST(uint8_t*, safe_realloc_aligned(dataBuf, 0, dataSize * sizeof(uint8_t),
    ...
    else
    {
        ...
        {
            // write reallocation is not allowed, but try to write it anyways...it may
            // reduce a reallocation. If a write succeeds, we're done and don't need
            // to reallocate. If a write fails, then send the reassign blocks command
            // since the sector needs reassignment
            ret = write_LBA(device, LBA->errorAddress, false, dataBuf, dataSize);
            if (ret == SUCCESS)
            {
                ret = flush_Cache(device);
            ...
            if (ret != SUCCESS &&
                device->drive_info.drive_type != NVME_DRIVE) // make sure the write and verify did actually work!
                                                             // NOTE: NVMe does not have a reassign command or
                                                             // translation for it in translation spec
            {
                // need to use the reallocate command (SCSI...ATA interfaces should
                // attempt translating it through SAT)
                bool    longLBA   = false;
                ...
                // create the list of LBAs. 1 for 1 logical per physical, 8 for 8
                // logical per physical
                ...
                do
                {
                    // always using short list since we are doing single reallocations at
                    // a time...not using enough data to need a long list.
                    ...
                    if (senseFields.validStructure)
                    {
                            ...
                            senseFields.scsiStatusCodes.senseKey != SENSE_KEY_HARDWARE_ERROR)
                        {
                            ...
                            // if we have a valid LBA, then we need to remove all LBAs prior
                            // to that one and reissue the command.
                        ...
                        if (updateList)
                        {
                            ...
                            // Check both the LBAs we saved above since it's not clear if both
                            // conditions can happen at the same time or not. Most likely only
                            ...
                            if (commandSpecificLba != UINT64_MAX)
                            {
                                ...
                                // update list length
                                reassignListLength = offset - 4; // minus 4 to get just length of list minus the
                                                                 // parameter header
                                if (longLBA)
                                {
                                ...
                                reassignListLength += 4; // add the 4 back in now before we come
                                                         // back around and reissue the command
                            }
    ...
    {
        // We are going to call this function recursively to try it again forcing
        // ATA passthrough
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
...
void print_LBA_Error_List(ptrErrorLBA const LBAs, uint16_t numberOfErrors)
{
    ...
    printf("                            Bad LBAs                            \n");
    printf("Defect Number          Defect LBA                            Repair "
           "Status\n");
    ...
    {
        printf("\nNOTE: Some LBAs could not be repaired because access to them was "
               "denied.\n");
        printf("This may happen when a secondary drive with a file system "
               "installed on\n");
        printf("it is recognized by the current host OS, but the current host "
               "doesn't have\n");
...
void sort_Error_LBA_List(ptrErrorLBA LBAList, uint32_t* numberOfLBAsInTheList)
{
    ...
    if (*numberOfLBAsInTheList > 1)
    {
        ...
        // Remove duplicates and update the number of items in the list (local var
        // only). This should be easy since we've already sorted the list
