...
//
// \file sanitize.c
// \brief This file defines the functions for sanitize operations on SCSI and
// ATA drives
                                               ...
                                               eSanitizeStatus* sanitizeStatus)
{
    ...
    if (result == SUCCESS)
    {
        *percentComplete =
        ...
        // NOTE: If more status bits are added to the spec, then we will need to add
        // support for detecting them here.
    ...
    else
    {
        ...
        // first check that the abort bit was set because if that isn't there, then
        // we won't be able to identify a reason due to a SATL issue
                                                ...
                                                eSanitizeStatus* sanitizeStatus)
{
    ...
    uint8_t       senseKey = 0;
    uint8_t       fru      = 0;
    eReturnValues result   = scsi_Request_Sense_Cmd(device, false, req_sense_buf,
                                                    SPC3_SENSE_LEN); // get fixed format sense data to make this easier to
                                                                     // parse the progress from.
    ...
    *percentComplete = 0;
    if (result == SUCCESS || result == IN_PROGRESS)
    {
        ...
        {
            *sanitizeStatus  = SANITIZE_STATUS_IN_PROGRESS;
            *percentComplete = M_BytesTo2ByteValue(req_sense_buf[16],
                                                   req_sense_buf[17]); // sense key specific information
        }
    }
    else
    {
...
eReturnValues show_Sanitize_Progress(tDevice* device)
{
    ...
    {
        printf("\tSanitize command is not currently in progress. It is either "
    ...
    {
        printf("\tSanitize is frozen on this device. It must be power cycled to "
               "clear the freeze lock.\n");
    }
    else if (sanitizeInProgress == SANITIZE_STATUS_UNSUPPORTED_FEATURE)
    {
        printf("\tThe last sanitize command specified an unsupported sanitize "
               "mode.\n");
    }
    ...
    {
        // ATA- Completed with physical sectors that are available to be allocated
        // for user data that were not successfully sanitized
        printf("\tSanitize command failed: completed with physical sectors that are "
               "available for user data and were not successfully sanitized!\n");
    }
    else
    {
...
eReturnValues get_ATA_Sanitize_Device_Features(tDevice* device, sanitizeFeaturesSupported* sanitizeOptions)
{
    ...
    if (ret == SUCCESS)
    {
            ...
            device->drive_info.IdentifyData.ata.Word059 & ATA_IDENTIFY_SANITIZE_SUPPORTED)
        {
            ...
            // ata identify device data log also contains "Definitive ending pattern
            // supported" bit on capabilities page ata identify device data log sets
            // copies of word 59 on security page. another bit describes commands
            // allowed matching ACS-2 or not. Modern drives should set this to one to
...
eReturnValues get_SCSI_Sanitize_Supported_Features(tDevice* device, sanitizeFeaturesSupported* sanitizeOpts)
{
    ...
    if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3) // check for this version of SPC first since the
                                                              // report supported Operation codes and Sanitize
                                                              // command should only be on drives with this version
                                                              // or highter.
    {
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             SANITIZE_CMD, SCSI_SANITIZE_OVERWRITE, 14,
                                                             supportedCommands))
        {
            ret = SUCCESS;
            ...
            case 3:                                           // supported according to spec
                sanitizeOpts->definitiveEndingPattern = true; // only setting this in this case since the "vendor
                                                              // specific mannor" may be that definitive ending pattern
                                                              // is not possible.-TJE
                M_FALLTHROUGH;
            case 5: // supported in vendor specific mannor in same format as case 3
                ...
                {
                    // Assuming that only a compliant translator will support this, so
                    // screening for ATA and NVMe which are limited to 16 passes
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             SANITIZE_CMD, SCSI_SANITIZE_BLOCK_ERASE, 14,
                                                             supportedCommands))
        {
            ret = SUCCESS;
            switch (supportedCommands[1] & 0x07)
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             SANITIZE_CMD, SCSI_SANITIZE_CRYPTOGRAPHIC_ERASE, 14,
                                                             supportedCommands))
        {
            ret = SUCCESS;
            switch (supportedCommands[1] & 0x07)
        ...
        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION,
                                                             SANITIZE_CMD, SCSI_SANITIZE_EXIT_FAILURE_MODE, 14,
                                                             supportedCommands))
        {
            ret = SUCCESS;
            switch (supportedCommands[1] & 0x07)
...
eReturnValues get_NVMe_Sanitize_Supported_Features(tDevice* device, sanitizeFeaturesSupported* sanitizeOpts)
{
    ...
    if (device->drive_info.IdentifyData.nvme.ctrl.sanicap > 0)
    {
        ...
        {
            // get the sanitize config feature status to know which mode it is
            // operating in.
...
static eReturnValues sanitize_Poll_For_Progress(tDevice* device, uint32_t delayTime)
{
    ...
    while (sanitizeInProgress == SANITIZE_STATUS_IN_PROGRESS)
    {
        ...
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            ...
            {
                if (sanitizeInProgress != SANITIZE_STATUS_IN_PROGRESS &&
                    percentComplete < 100) // if we get to the end, percent complete may not say 100%,
                                           // so we need this condition to correct it
                {
...
// These must be preserved to offer backwards compatibility if this changes in
// the future -TJE
...
typedef struct _sanitizeOperationOptions_V1
{
    ...
    struct
    {
        bool invertPatternBetweenPasses; // SATA note: Some drives may or may not
                                         // set a definitive ending pattern upon
                                         // completion. By default, this function
                                         // will set the definitive ending pattern
                                         // bit whenever possible-TJE
        uint8_t numberOfPasses;          // 0 = BAD_PARAMETER, 1 = 1, 2 = 2, etc. NVMe and
                                         // SATA max at 16. SCSI maxes at 32
        uint32_t pattern;
...
eReturnValues run_Sanitize_Operation2(tDevice* device, sanitizeOperationOptions sanitizeOptions)
{
    ...
    {
        // NOTE: If sanitizeoptions changes version, need to adjust code to offer
        // backwards compatibility when possible.-TJE
        //       If new options are added, set to backwards compatible defaults.
        uint32_t        delayTime           = 1;
        double          percentComplete     = 0.0;
        eSanitizeStatus sanitizeInProgress  = 0;
        bool            sendExitFailureMode = false;
        // first check if a sanitize test is in progress (and that the drive isn't
        // frozen or in a failure state)
        ret = get_Sanitize_Progress(device, &percentComplete, &sanitizeInProgress);
        ...
        if (sendExitFailureMode)
        {
            ...
            if (ret != SUCCESS)
            {
                ...
                if (sanitizeInProgress == SANITIZE_STATUS_FAILED ||
                    sanitizeInProgress == SANITIZE_STATUS_FAILED_PHYSICAL_SECTORS_REMAIN)
                {
                    // need to run the sanitize command in restricted exit mode, so you
                    // can only get it to run by reissuing the command in restricted mode
                    // again.
                    return DEVICE_ACCESS_DENIED; // use this to inform the user that you
                                                 // cannot use AUSE and need to retry
                                                 // sanitize without that field.
                }
            }
        }
        ...
        case OVERWRITE_ERASE:
            ...
            if (sanitizeOptions.overwriteOptions.numberOfPasses >= 16) // 16 is max number of passes for NVMe/SATA
            {
                ...
                else
                {
                    ...
                    // In SCSI, a number of passes == 0 is reserved, so do not use
                    // this-TJE
                }
            }
