...
//
// \file host_erase.c
// \brief This file defines the function for performing a host based erase
// functions (host issues a series of write commands)
                          ...
                          bool     hideLBACounter)
{
    ...
    if (eraseRangeStart == 0)
    {
        ...
        {
            // At least in WIndows, you MIGHT get a permissions issue trying to write
            // LBA 0 and maxlba. So if this erase is erasing the whole drive, do this
            // first to make sure we use a low-level IOCTL to do this. Once that
            // completes it should be possible to erase the drive without an error.
            if (SUCCESS == os_Erase_Boot_Sectors(device))
            {
                flush_Cache(device);                 // in case the OS call didn't flush the writes,
                                                     // make sure we flush them
                os_Update_File_System_Cache(device); // tell the OS to update what it knows about the disk (that
                                                     // the partition table is gone now basically)
            }
        }
    }
    if (eraseRangeStart != alignedLBA)
    {
        ...
        // read the LBA, modify ONLY the data the user wants to erase, then write it
        // to the drive.
        if (SUCCESS == read_LBA(device, alignedLBA, false, writeBuffer, dataLength))
        {
            ...
            if (alignedLBA == 0)
            {
                // update the filesystem cache after writing the boot partition sectors
                // so that no other LBA writes have permission errors - TJE
    ...
    if (ret == SUCCESS)
    {
        for (iter = eraseRangeStart; iter < eraseRangeEnd; iter += sectors)
        {
            if (iter + sectors > eraseRangeEnd)
            {
                ...
                else // we aren't going to the end of the drive and may need to read
                     // the nearby data to keep anything the user didn't want to
                     // overwrite
                {
                         ...
                         bool     hideLBACounter)
{
    ...
    time_t        startTime   = 0;
    // first figure out how many writes we'll need to issue, then allocate the
    // memory we need
    uint32_t sectors    = get_Sector_Count_For_Read_Write(device);
    uint64_t iter       = 0;
    uint32_t dataLength = sectors * device->drive_info.deviceBlockSize;
         ...
         iter += sectors, time(&currentTime))
    {
        ...
        if (iter == 0)
        {
            // update the filesystem cache after writing the boot partition sectors so
            // that no other LBA writes have permission errors - TJE
            os_Update_File_System_Cache(device);
        }
        if (iter + sectors >= device->drive_info.deviceMaxLba)
        {
...
eReturnValues erase_Boot_Sectors(tDevice* device)
{
    ...
    // Try this first. Currently in Windows this is needed for some devices as raw
    // writes return permision denied
    ret = os_Erase_Boot_Sectors(device);
    if (ret == SUCCESS)
    {
        os_Update_File_System_Cache(device);
    }
    // even if the OS erase boot sectors succeeds, issue the following writes
    // anyways to make sure it really did erase everything.
    if (VERBOSITY_QUIET < device->deviceVerbosity)
    {
        printf("\rWriting LBA: %-40" PRIu64 "", iter);
        fflush(stdout);
    }
    // NOTE: this currently only issues 2 writes counting on the sector count
    // being either enough for 32KiB or 64KiB. This is far from perfect, but will
    // work for now.-TJE write sector zero
    ret = write_LBA(device, iter, false, writeBuffer, dataLength);
    if (ret == SUCCESS)
    {
