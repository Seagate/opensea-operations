...
// There is no specific way to enable or disable this on SCSI, so this simulates
// the bahaviour according to what we see with ATA
static eReturnValues scsi_Enable_Disable_EPC_Feature(tDevice* device, eEPCFeatureSet lba_field)
{
    ...
    {
        // read the default mode page and write it back to the drive to set the
        // default values from the drive...should be the same as enabling
        ret = scsi_Set_Device_Power_Mode(device, true, true, PWR_CND_ALL, 0, false);
    }
    else if (lba_field == DISABLE_EPC)
    {
        // read the current settings, disable any timers that are enabled and set
        // the timer values to zero...should be the same as disabling.
...
eReturnValues print_Current_Power_Mode(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        if (SUCCESS == ata_Check_Power_Mode(device, &powerMode))
        {
            ...
            case 0x00:
                ...
                {
                    printf("PM2: Standby state and device is in the Standby_z power "
                           "condition\n");
                }
                break;
            case 0x01: // should only happen when EPC is enabled according to the
                       // spec...so not checking EPC feature
                printf("PM2: Standby state and the device is in the Standby_y power "
                       "condition\n");
                break;
            case 0x40: // NV cache
                printf("PM0: Active state. NV Cache power is enabled and spindle is "
                       "spun/spinning down\n");
                break;
            case 0x41: // NV cache
                printf("PM0: Active state. NV Cache power is enabled and spindle is "
            ...
            case 0x80:
                ...
                break;
            case 0x81: // should only happen when EPC is enabled according to the
                       // spec...so not checking EPC feature
                printf("PM1: Idle state and the device is in the Idle_a power "
                       "condition\n");
                break;
            case 0x82: // should only happen when EPC is enabled according to the
                       // spec...so not checking EPC feature
                printf("PM1: Idle state and the device is in the Idle_b power "
                       "condition\n");
                break;
            case 0x83: // should only happen when EPC is enabled according to the
                       // spec...so not checking EPC feature
                printf("PM1: Idle state and the device is in the Idle_c power "
                       "condition\n");
                break;
            case 0xFF:
    ...
    {
        /*
        NOTE: Removed the code which was checking to see if the power mode is
        supported mainly because it was changing the power state of the drive. -MA
        */
        uint8_t* senseData =
        ...
        {
            bool issuetur = false;
            // requested fixed format sensedata, so parse it. If we don't find what we
            // are looking for post the "unable to retrieve current power mode"
            // message
...
eReturnValues transition_Power_State(tDevice* device, ePowerConditionID newState)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        case PWR_CND_IDLE_C:
            ...
            break;
        case PWR_CND_ACTIVE:
            // No such thing in ATA. Attempt by sending read-verify to a few sectors
            // on the disk randomly (Early SAT translation recommended this behavior)
            ...
            // TODO: zoned devices will require a different method. See SAT4 or later
            // for details. SAT-6 says: EPC: set features for all supported power
            // conditions: ID set to FFh, enable set to zero, save set to zero to
            // disable all current timers
            //      Then issue an idle immediate. If no errors: In active condition,
            //      else return an error.
            // non-EPC: send idle with feature 0, count 0, lba 0. If no error: In
            // active state.
            ret = SUCCESS;
            break;
        ...
        default:
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
                printf("Power State Transition is not supported on this device type at "
                       "this time\n");
            }
            ret = NOT_SUPPORTED;
            break;
    ...
    else // if (device->drive_info.drive_type == SCSI_DRIVE) /*removed the if
         // SCSI here to handle NVMe or other translations*/
    {
        switch (newState)
        {
        case PWR_CND_ACTIVE:
            if (device->drive_info.scsiVersion > SCSI_VERSION_SCSI2) // checking for support after SCSI2. This isn't
                                                                     // perfect, but should be ok for now.
            {
                ret = scsi_Start_Stop_Unit(device, false, 0, PC_ACTIVE, false, false, false);
            }
            else
            {
                // before you could specify a power condition, you used the "Start" bit
                // as a way to move from standby to active
...
eReturnValues get_NVMe_Power_States(tDevice* device, ptrNVMeSupportedPowerStates nvmps)
{
    ...
    if (device && device->drive_info.drive_type == NVME_DRIVE && nvmps)
    {
        ...
        for (uint16_t powerIter = 0; powerIter < driveMaxPowerStates && powerIter < MAXIMUM_NVME_POWER_STATES;
             ++powerIter)
        {
            ...
            // r/w throughput and latency are all 5bit fields, so stripping off the
            // top 3 bits when assigning in case future revisions make changes.
            ...
            if (device->drive_info.IdentifyData.nvme.ctrl.psd[powerIter].idlePower > 0)
            {
                ...
                {
                    // cannot handle these cases, so disable reporting idle power since we
                    // cannot report it correctly
        ...
        // finish by reading which is the current power state that the device is
        // operating in
...
// This was basically copy-pasted from common_public.c in opensea-transport.
// Need to make this a function in opensea-common instead
#define NVM_POWER_ENT_EX_TIME_MAX_STR_LEN 10
static const char* convert_NVM_Latency_To_HR_Time_Str(uint64_t timeInNanoSeconds,
                                                      char     timeStr[NVM_POWER_ENT_EX_TIME_MAX_STR_LEN])
{
...
#define NVM_LAT_UNIT_STR_LEN 3
    ...
    switch (unitCounter)
    {
    case 6: // we shouldn't get to a days value, but room for future large drives
            // I guess...-TJE
    ...
    case 0:
        ...
        break;
    default: // couldn't get a good conversion or something weird happened so show
             // original nanoseconds.
        snprintf(units, NVM_LAT_UNIT_STR_LEN, "ns");
...
void print_NVM_Power_States(ptrNVMeSupportedPowerStates nvmps)
{
    if (nvmps)
    {
        ...
        printf("\t\tRead/Write throughput and latency values are scaled from 0 - "
               "100%%.\n");
        printf("\t100%% = max performance, 0%% = minimum relative performance.\n");
        // flags | # | max power | idle power | active power | latencies and
        // throughputs (can be N/A when not reported)
        printf("\n   #  Max Power: Idle Power: Active Power: RRT: RRL: RWT: RWL: "
               "Entry Time: Exit Time:\n");
        printf("-------------------------------------------------------------------"
               "------------------\n");
        // all values should be in watts. 1.00 watts, 0.25 watts, etc
        // should relative values be scaled to percentages? 100% = max performance.
        // This may be easier for some to understand than a random number
                                            ...
                                            ptrPowerConditionSettings powerConditionSettings)
{
    ...
    if (powerConditionSettings->powerConditionValid)
    {
        ...
        {
            // this command is restoring the power conditions from the drive's default
            // settings (bit6) and saving them upon completion (bit4)...the other
            // option is to return to the saved settings, but we aren't going to
        ...
        else // we aren't restoring settings, so we need to set things up to save
             // settings
        {
            uint8_t  lbalo  = 0;
            uint8_t  lbaMid = 0;
            ...
            if (powerConditionSettings->timerValid)
            {
                ...
                {
                    // need to convert to a number of minutes to send to the drive
                    // instead!
            ...
            else // they didn't enter a timer value so this command will do the
                 // EXACT same thing...just decided to use a different feature to
                 // EPC
            {
...
eReturnValues ata_Set_Device_Power_Mode(tDevice*          device,
                                        ...
                                        bool              powerModeTimerValid)
{
    ...
    uint8_t* ataDataBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (!ataDataBuffer)
    {
        perror("calloc failure!\n");
        return MEMORY_FAILURE;
...
{
    // If restore all to defaults, then read the default mode page, then write it
    // back. Write all applicable changes back to the drive, checking each
    ...
    if (restoreAllToDefaults)
    {
        ...
        {
            ret = scsi_Mode_Select_10(device, 0, true, true, true, M_NULLPTR,
                                      0); // RTD bit is set and supported by the drive which will reset the
                                          // page to defaults for us without a data transfer or multiple
                                          // commands.
        }
        else
        {
            ...
            powerConditionsPageLength =
                MODE_PARAMETER_HEADER_10_LEN + MP_POWER_CONDITION_LEN; //*should* be maximum size we need assuming no
                                                                       // block descriptor
            powerConditionsPage = C_CAST(uint8_t*, safe_calloc_aligned(powerConditionsPageLength, sizeof(uint8_t),
                                                     ...
                                                     false, MPC_DEFAULT_VALUES, powerConditionsPage)))
            {
                ...
                ret = scsi_Mode_Select_10(device, powerConditionsPageLength, true, true, false, powerConditionsPage,
                                          powerConditionsPageLength);
            }
    ...
    else
    {
        ...
        // Check if anything in the incoming list is requesting default values so we
        // can allocate and read the defaults for those conditions before sending to
        // the drive.
        ...
        {
            // Read the default mode page, then save whichever things need defaults
            ...
            if (SUCCESS == (ret = scsi_Mode_Sense_10(device, MP_POWER_CONDTION, powerConditionsPageLength, 0, true,
                                                     false, MPC_DEFAULT_VALUES, powerConditionsPage)))
            {
                // uint16_t modeDataLength = M_BytesTo2ByteValue(powerConditionsPage[0],
                // powerConditionsPage[1]);
                ...
                if (powerConditions->idle_a.powerConditionValid && powerConditions->idle_a.restoreToDefault)
                {
                    ...
                    powerConditions->idle_a.restoreToDefault = false; // turn this off now that we have the other
                                                                      // settings stored.
                }
                if (powerConditions->standby_z.powerConditionValid && powerConditions->standby_z.restoreToDefault)
                {
                    ...
                    powerConditions->standby_z.restoreToDefault = false; // turn this off now that we have the other
                                                                         // settings stored.
                }
                    ...
                    0x0A) // newer EPC drives support these, but older devices do not.
                {
                    if (powerConditions->idle_b.powerConditionValid && powerConditions->idle_b.restoreToDefault)
                    {
                        ...
                        powerConditions->idle_b.restoreToDefault = false; // turn this off now that we have the other
                                                                          // settings stored.
                    }
                    if (powerConditions->idle_c.powerConditionValid && powerConditions->idle_c.restoreToDefault)
                    {
                        ...
                        powerConditions->idle_c.restoreToDefault = false; // turn this off now that we have the other
                                                                          // settings stored.
                    }
                    if (powerConditions->standby_y.powerConditionValid && powerConditions->standby_y.restoreToDefault)
                    {
                        ...
                        powerConditions->standby_y.restoreToDefault = false; // turn this off now that we have the other
                                                                             // settings stored.
                    }
                    // CCF fields
        ...
        // Now, read the current settings mode page, make any necessary changes,
        // then send it to the drive and we're done.
        powerConditionsPageLength =
            MODE_PARAMETER_HEADER_10_LEN + MP_POWER_CONDITION_LEN; //*should* be maximum size we need assuming no
                                                                   // block descriptor
        powerConditionsPage = C_CAST(uint8_t*, safe_calloc_aligned(powerConditionsPageLength, sizeof(uint8_t),
...
// The first failure that happens will cause the function to fail and not
// proceed to set any other timer values.
        ...
        device->drive_info.IdentifyData.ata.Word086 & BIT15) // words 119, 120 valid
    {
            ...
            device->drive_info.IdentifyData.ata.Word119 & BIT7)
        {
            ...
            else
            {
                if (powerConditions)
                {
                    // go through each and every power condition and for each valid one,
                    // pass it to the ATA function. If unsuccessful, return immediately
                    // This does it in the same top-down order as the SCSI mode page to
...
// enableDisable = true means enable, false means disable
eReturnValues scsi_Set_Device_Power_Mode(tDevice*          device,
                                         bool              restoreDefaults,
                                         bool              enableDisable,
                                         ePowerConditionID powerCondition,
                                         uint32_t          powerModeTimer,
...
{
    eReturnValues ret = NOT_SUPPORTED;
    // first we need to check that VPD page 8Ah (power condition) exists...and we
    // can possibly use that information to return "not supported, etc"
    uint8_t* powerConditionVPD =
        C_CAST(uint8_t*, safe_calloc_aligned(VPD_POWER_CONDITION_LEN, sizeof(uint8_t),
                                             device->os_info.minimumAlignment)); // size of 18 is defined in
                                                                                 // SPC4 for this VPD page
    if (!powerConditionVPD)
    {
        perror("calloc failure!");
        return MEMORY_FAILURE;
    }
    if (SUCCESS == scsi_Inquiry(device, powerConditionVPD, VPD_POWER_CONDITION_LEN, POWER_CONDITION, true,
                                false)) // Not technically necessary, but will keep this only
                                        // functional on EPC drives.
    {
        if (powerConditionVPD[1] != POWER_CONDITION) // make sure we got the correct page...if we did, we
                                                     // will proceed to do what we can with the other passed
                                                     // in options
        {
            if (VERBOSITY_QUIET < device->deviceVerbosity)
            {
    ...
    // Calling new scsi_Set_Power_Conditions() function since it's setup to handle
    // EPC and legacy drives. Just need to setup structures in here then call that
    // function properly.
    if (restoreDefaults)
    {
    ...
    else
    {
        ...
        powerConditionTimers powerConditions;
        memset(&powerConditions, 0, sizeof(powerConditionTimers));
        // All checks for NOT_SUPPORTED are based off of VPD page support bits.
        // Current assumption is if that timer is supported by the device, then so
        // is changing that timer or changing it from enabled to disabled. It would
        // probably be a good idea to also check the changable mode page, but that
        // is not done right now - TJE
        switch (powerCondition)
        {
        case PWR_CND_IDLE_A:
...
// enableDisable = true means enable, false means disable
eReturnValues set_Device_Power_Mode(tDevice*          device,
                                    bool              restoreDefaults,
                                    bool              enableDisable,
                                    ePowerConditionID powerCondition,
                                    uint32_t          powerModeTimer,
                                    bool              powerModeTimerValid)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ret = ata_Set_Device_Power_Mode(device, restoreDefaults, enableDisable, powerCondition, powerModeTimer,
                                        powerModeTimerValid);
    }
    else if (device->drive_info.drive_type == SCSI_DRIVE)
    {
        ret = scsi_Set_Device_Power_Mode(device, restoreDefaults, enableDisable, powerCondition, powerModeTimer,
                                         powerModeTimerValid);
    }
    else
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            printf("Power choice configuration not supported on this device type at "
                   "this time.\n");
        }
        ret = NOT_SUPPORTED;
    }
...
eReturnValues get_Power_State(tDevice* device, uint32_t* powerState, eFeatureModeSelect selectValue)
{
    ...
    else
    {
        if (VERBOSITY_QUIET < device->deviceVerbosity)
        {
            printf("Get Power State is currently not supported on this device type "
                   "at this time.\n");
        }
        ret = NOT_SUPPORTED;
    }
    ...
    if (device->drive_info.drive_type == SCSI_DRIVE) // this is only available on SCSI drives.
    {
        ...
        if (SUCCESS == get_SCSI_VPD_Page_Size(device, POWER_CONSUMPTION, &powerConsumptionLength))
        {
                ...
                scsi_Inquiry(device, powerConsumptionPage, powerConsumptionLength, POWER_CONSUMPTION, true, false))
            {
                ...
                uint32_t pcIter                    = 4;
                uint32_t counter                   = 0;
                // ctc changed the "<" conditions to "<=" so all the identifiers get
                // parsed (was an "off-by-1" problem")
                for (; pcIter <= powerConsumptionLength &&
                       pcIter <= C_CAST(uint32_t, identifiers->numberOfPCIdentifiers * 4);
                     pcIter += 4, counter++)
                {
        ...
        {
            uint8_t* pcModePage =
                C_CAST(uint8_t*, safe_calloc_aligned(MODE_PARAMETER_HEADER_10_LEN + 16, sizeof(uint8_t),
                                                     device->os_info.minimumAlignment));
            if (!pcModePage)
            {
                return MEMORY_FAILURE;
            ...
            if (SUCCESS == scsi_Mode_Sense_10(device, MP_POWER_CONSUMPTION, MODE_PARAMETER_HEADER_10_LEN + 16, 0x01,
                                              true, false, MPC_CURRENT_VALUES, pcModePage))
            {
                ret = SUCCESS;
                ...
                if (identifiers->activeLevel == 0)
                {
                    ...
                    // ctc 10 lines of code after the comments are necessary because the
                    // pcIdentifier and the identfiers->identifiers[] are NOT necessarily
                    // in order ctc need to step through the indentifiers to find the
                    // correct one
...
#define POWER_CONSUMPTION_UNIT_BUFFER_LENGTH 25
                ...
                // convert this to a smaller value that can be reprsented with minimal
                // floating point (13500mw->13.5w)
            ...
            // show a list of the values supported (in watts). If the value is less
            // than 1 watt, exclude it
            ...
            for (; pcIter < identifiers->numberOfPCIdentifiers; pcIter++)
            {
                ...
                printf(" %g |", watts); // use %g to use shortest possible notation for
                                        // the output. This keeps 12w 13.5w without
                                        // extra zeros all over the place
            }
            ...
            printf(" default ]\n"); // always allow default so that we can restore
                                    ...
                                    bool           resetToDefault)
{
    ...
    {
        uint8_t* pcModePage = C_CAST(uint8_t*, safe_calloc_aligned(16 + MODE_PARAMETER_HEADER_10_LEN, sizeof(uint8_t),
                                                                   device->os_info.minimumAlignment));
                                          ...
                                          false, mpControl, pcModePage))
        {
            if (!resetToDefault)
            {
                ...
                case PC_ACTIVE_LEVEL_LOWEST:
                    // set the active level to what was requested (power consumption
                    // identifier is ignored here)
                    pcModePage[MODE_PARAMETER_HEADER_10_LEN + 6] &= 0xFC; // clear lower 2 bits to 0
...
eReturnValues map_Watt_Value_To_Power_Consumption_Identifier(tDevice* device, double watts, uint8_t* pcIdentifier)
{
    ...
    *pcIdentifier = 0xFF; // invalid
    ...
    /*/
    //This is a dummied up test to make sure this code REALLY REALLY works by
    putting these in a random order (since order is not specified in the SPC
    specification) ret = SUCCESS; identifiers.currentIdentifier = 0;
    ...
    if (ret == SUCCESS)
    {
        ...
        // ctc had to change variable initialization of iter1 and inter2 to match
        ...
        ret = NOT_SUPPORTED;
        // ctc changed to nested for loops here... not sure it's needed, but it's
        // clearer
        //         for (; iter1 < identifiers.numberOfPCIdentifiers /* && iter2 >=
        //         0*/; iter1++, iter2--)
        for (; iter1 < identifiers.numberOfPCIdentifiers; iter1++)
        {
            // ctc needed to reset iter2=0 to go through the for loop the next
            // times... not sure why the code doesn't follow convention ctc and use
            // for(initializer, condition, increment), but whatever.  Nonstandard and
            // goofy coding sytle, I guess
            iter2 = 0;
        ...
        if (!exactMatchFound)
        {
            // now compare the best results between the two iterators to see which is
            // closer to the best match, or is the best match need to check which one
            // is closer and select it
...
eReturnValues enable_Disable_APM_Feature(tDevice* device, bool enable)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
            ...
            device->drive_info.IdentifyData.ata.Word083 & BIT3)
        {
            if (enable)
            {
                // subcommand 05..set value to 0x7F when requesting an enable operation
                // so that it's a good mix of performance and power savings.
            ...
            else
            {
                ...
                if (ret != SUCCESS)
                {
                    // the disable APM feature is not available on all devices according
                    // to ATA spec.
                    ret = NOT_SUPPORTED;
                }
            }
...
static eReturnValues ata_Get_EPC_Settings(tDevice* device, ptrEpcSettings epcSettings)
{
    ...
    // get_ATA_Log_Size(device, ATA_LOG_POWER_CONDITIONS, &epcLogSize, true,
    // false) //uncomment this line to ask the drive for the EPC log size rather
    // than use the hard coded value above.
...
static eReturnValues scsi_Get_EPC_Settings(tDevice* device, ptrEpcSettings epcSettings)
{
    ...
    {
        // NOTE: Recovery times are in milliseconds, not 100 milliseconds like other
        // timers, so need to convert to 100 millisecond units for now-TJE idle a
    ...
    // now time to read the mode pages for the other information (start with
    // current, then saved, then default)
         ...
         ++modePageControl)
    {
        ...
        if (gotData)
        {
            ...
            case MPC_CURRENT_VALUES:
                ...
                // special case. If reading the current values, check the page for the
                // PS bit (parameters savable) to note this for all power conditions.
                ...
                *idleAenabledBit = true;
                if (powerConditionVPDsupported == false)
                {
                    if (modePageControl == MPC_CHANGABLE_VALUES || modePageControl == MPC_CURRENT_VALUES)
                    {
                        ...
                        // SCSI has supported this mode page for a while, so if it's the
                        // "changeable" page set "supported" bits for different power
                        // conditions.
                        epcSettings->idle_a.powerConditionSupported = true;
                    }
                }
            ...
            *standbyZtimerSetting = M_BytesTo4ByteValue(epcModePage[headerLength + 8], epcModePage[headerLength + 9],
            ...
            if (epcModePage[headerLength + 1] > 0x0A) // before EPC this page was shorter, so do not try to access the
                                                      // rest of it as the data is invalid
            {
                // idle b
...
// NOTE: This is intended for legacy drives that don't support extra timers for
// EPC. An EPC drive will still process this command though!!! - TJE More Notes:
// This function is similar to the EPC function, BUT this will not check for the
// VPD page as it doesn't exist on old drives.
...
{
    // Check the changable page for support of idle and standby timers before
    // beginning???
...
// using 100 millisecond increments since that is what SCSI uses and the
// methodology in here will match SAT spec. This seemed simpler - TJE
static eReturnValues ata_Set_Standby_Timer(tDevice* device, uint32_t hundredMillisecondIncrements)
{
    ...
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word049) &&
        device->drive_info.IdentifyData.ata.Word049 & BIT13) // this is the only bit across all ATA standards that will
                                                             // most likely work. Prior to ATA3, there was no other
                                                             // support bit for the power management feature set.
    {
        ...
        // Check the current power mode. If the drive is in standby already, use
        // standby, otherwise use idle to set the timer. NOTE: Previously only the
        // standby command was used like in SAT-5 for non-EPC. Changed to checking
        ...
        {
            // not in standby mode, so use idle. NOTE: This does not take into account
            // EPC, but this is meant for legacy drives anyways, so this does not need
            // to be complicated. This may affect standby_y, but users should be using
            // EPC instead if they want better, more granular timers anyways. -TJE
...
eReturnValues set_Standby_Timer(tDevice* device, uint32_t hundredMillisecondIncrements, bool restoreToDefault)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        if (restoreToDefault)
        {
            // Cannot restore ATA to default values. This is done with a power cycle
            // or some SATL that has a capability to remember the original timer (in
            // which case, the scsi function should be used instead)
            return NOT_SUPPORTED;
        }
...
eReturnValues sata_Set_Device_Automatic_Partioan_To_Slumber_Transtisions(tDevice* device, bool enable)
{
    ...
    if ((device->drive_info.drive_type == ATA_DRIVE || device->drive_info.drive_type == ATAPI_DRIVE) && is_SATA(device))
    {
        ...
        eReturnValues getDIPM     = sata_Get_Device_Initiated_Interface_Power_State_Transitions(
            device, M_NULLPTR, &dipmEnabled); // DIPM must be ENABLED before we
...
eReturnValues transition_To_Active(tDevice* device)
{
    ...
    {
        // no ATA command to do this, so we need to issue something to perform a
        // medium access.
    ...
    else // treat as SCSI
    {
        ...
        {
            // before you could specify a power condition, you used the "Start" bit as
            // a way to move from standby to active
            ret = scsi_Start_Stop_Unit(device, false, 0, PC_START_VALID, false, false, true);
        }
    }
    return ret;
...
eReturnValues transition_To_Idle(tDevice* device, bool unload)
{
        ...
        if (unload)
        {
            if ((is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word084) &&
                ...
                (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word087) &&
                 device->drive_info.IdentifyData.ata.Word087 & BIT13))
            {
                // send the command since it supports the unload feature...otherwise we
                // return NOT_SUPPORTED
                ret = ata_Idle_Immediate(device, true);
            }
        }
    ...
    else
    {
                                                                 ...
                                                                 // perfect, but should be ok for now.
        {
            if (unload)
            {
                // unload can happen if power condition modifier set to 1. Needs
                // SBC3/SPC3.
                if (device->drive_info.scsiVersion > SCSI_VERSION_SPC_2)
                {
                    ret = scsi_Start_Stop_Unit(device, false, 1, PC_IDLE, false, false, false);
                                       ...
                                       uint8_t  phyNumber)
{
    ...
    uint16_t enhPhyControlLength =
        MODE_PARAMETER_HEADER_10_LEN + 8 + 40; // first 8 bytes are a "header" followed by 20 bytes per phy and
                                               // setting this for 2 phys since that is most common right now. -TJE
    uint8_t* enhSasPhyControl =
        C_CAST(uint8_t*, safe_calloc_aligned(enhPhyControlLength * sizeof(uint8_t), sizeof(uint8_t),
                                             device->os_info.minimumAlignment));
    if (!enhSasPhyControl)
    {
        return MEMORY_FAILURE;
    }
    // read first 4 bytes to get total mode page length, then re-read the part
    // with all the data
    ...
    if (gotFullPageLength)
    {
        if (alreadyHaveAllData ||
            SUCCESS == scsi_Mode_Sense_10(device, MP_PROTOCOL_SPECIFIC_PORT, enhPhyControlLength, 0x03, true, false,
                                          MPC_CURRENT_VALUES, enhSasPhyControl))
        {
            // make sure we got the header as we expect it, then validate we got all
            // the data we needed. uint16_t modeDataLength =
            // M_BytesTo2ByteValue(enhSasPhyControl[0], enhSasPhyControl[1]);
                ...
                (enhSasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & BIT6) > 0)
            {
                    ...
                    6) // make sure it's the SAS protocol page
                {
                         ...
                         ++phyIter, phyDescriptorOffset += descriptorLength)
                    {
                        ...
                        // check if the caller requested changing all phys or a specific phy
                        // and only modify it's descriptor if either of those are true.
                    ...
                    // we've finished making our changes to the mode page, so it's time to
                    // write it back!
...
eReturnValues get_SAS_Enhanced_Phy_Control_Number_Of_Phys(tDevice* device, uint8_t* phyCount)
{
    ...
    uint8_t* enhSasPhyControl =
        C_CAST(uint8_t*, safe_calloc_aligned((MODE_PARAMETER_HEADER_10_LEN + enhPhyControlLength) * sizeof(uint8_t),
    ...
    if (SUCCESS == (ret = scsi_Mode_Sense_10(device, MP_PROTOCOL_SPECIFIC_PORT,
                                             ...
                                             MPC_CURRENT_VALUES, enhSasPhyControl)))
    {
        // uint16_t modeDataLength = M_BytesTo2ByteValue(enhSasPhyControl[0],
        // enhSasPhyControl[1]);
        uint16_t blockDescriptorLength = M_BytesTo2ByteValue(enhSasPhyControl[6], enhSasPhyControl[7]);
        // validate we got the right page
        if ((enhSasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & 0x3F) == 0x19 &&
                                          ...
                                          MPC_CURRENT_VALUES, enhSasPhyControl))
        {
                ...
                (enhSasPhyControl[MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength + 0] & BIT6) > 0)
            {
                    ...
                    6) // make sure it's the SAS protocol page
                {
                    ...
                    for (uint16_t phyIter = 0; phyIter < C_CAST(uint16_t, numberOfPhys) &&
...
eReturnValues puis_Spinup(tDevice* device)
{
    ...
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        ...
        {
            // this command is not required to spinup the drive. Any media access will
            // spin it up.
        }
    }
    return ret;
