...
// \brief This file defines the function for performing a firmware download to a
// drive
...
// In order to be able to validate the data on any CPU, we don't want to
// hardcode any lengths in case things get packed or aligned differently. So
// define each struct version here internally so we can do sizeof(v1), etc to
// check it. Each time the structure changes, add a new one that we can use to
// verify size, version, length, etc
...
typedef struct _firmwareUpdateDataV1
{
    ...
    eDownloadMode dlMode;          // how to do the download. Full, Segmented, Deferred,
                                   // etc
    uint16_t segmentSize;          // size of segments to use when doing segmented. If 0,
                                   // will use 64.
    uint8_t* firmwareFileMem;      // pointer to the firmware file read into memory to
                                   // send to the drive.
    uint32_t firmwareMemoryLength; // length of the memory the firmware file was read
                                   // into. This should be a multiple of 512B sizes...
    uint64_t avgSegmentDlTime;     // stores the average segment time for the download
    uint64_t activateFWTime;       // stores the amount of time it took to issue the last
                                   // segment and activate the new code (on segmented). On
                                   // deferred this is only the time to activate.
    union
    {
        uint8_t firmwareSlot; // NVMe
        uint8_t bufferID;     // SCSI
    };
    bool existingFirmwareImage;      // set to true means you are activiting an
                                     // existing firmware image in the specified slot.
                                     // - NVMe only
    bool ignoreStatusOfFinalSegment; // This is a legacy compatibility option. Some
                                     // old drives do not return status on the last
                                     // segment, but the download is successful and
                                     // this ignores the failing status from the OS
                                     // and reports SUCCESS when set to true.
...
typedef struct _firmwareUpdateDataV2
{
    ...
    uint8_t forceCommitAction;    // NVMe only. forceCommitActionValid must be true
                                  // to use this.
...
static eReturnValues check_For_Power_Cycle_Required(eReturnValues ret, tDevice* device)
{
    ...
    // Check if the device needs a power cycle to complete the update...This has
    // been necessary in Windows with the Intel NVMe driver in some cases
    if (ret == SUCCESS && device->drive_info.drive_type == NVME_DRIVE)
    {
        // we do not already have a "Power cycle is requied" return code, so need to
        // check the firmware log for the NVMe device to see if the "next active"
        // slot value is non-zero. If it is non-zero then the low-level driver did
        // not issue the necessary reset for activation. read the firmware log for
        // more information
        ...
        if (SUCCESS == nvme_Get_Log_Page(device, &firmwareLogOpts))
        {
            ...
            {
                // if the driver is saying this command passed, but the data is empty,
                // it is a driver bug but this is a driver with a known need for the
                // power cycle to happen. For some reason, this driver will do the
                // update, but attempting to read this log gives back zeros until you
                // close all software (closing all handles is not enough) and return
                // empty data for this log.
            ...
            else
            {
                ...
                // this is a workaround where this log is reported empty, which is NOT
                // correct, but seems to match when the power cycle is actually needed
                // to complete activation -TJE This is most likely an Intel Driver
                // caching problem from what I can tell when debugging this issue.
                if (is_Empty(firmwareLog, 512))
                {
                    ret = POWER_CYCLE_REQUIRED;
                }
                // set the firmware revision in each slot
                // for (uint32_t slotIter = 0, offset = 8; slotIter <=
                // M_GETBITRANGE(device->drive_info.IdentifyData.nvme.ctrl.frmw, 3, 1)
                // && slotIter <= 7 /*max of 7 slots in spec and structure*/ && offset <
                // 512; ++slotIter, offset += 8)
                //{
...
eReturnValues firmware_Download(tDevice* device, firmwareUpdateData* options)
{
    ...
    if (options && options->version >= FIRMWARE_UPDATE_DATA_VERSION_V1 && options->size >= sizeof(firmwareUpdateDataV1))
    {
        ...
        if (options->version < FIRMWARE_UPDATE_DATA_VERSION_V3)
        {
            ...
            // While v3 should be backwards compatible, do a translation here before
            // entering the remaining code
            ...
            // visual studio complains about this NOT being here and GCC does the
            // opposite...so only add this case for visual studio.
        ...
        if (options->dlMode == FWDL_UPDATE_MODE_AUTOMATIC)
        {
            ...
            // special case. Some SAS controllers (PMC 8070) filter the SCSI write
            // buffer command mode and will not allow deferred download So change this
            // to segmented for this case.
        ...
        // send test unit ready to determine if spinup command is required. If it
        // is, send the SCSI start-stop unit command to do the spinup.
        scsiStatus turStatus;
        memset(&turStatus, 0, sizeof(scsiStatus));
        scsi_Test_Unit_Ready(device,
                             &turStatus); // Note: Not checking for success because
                                          // we want to evaluate the received sense
                                          // data ourselves in this case-TJE
        if (turStatus.senseKey == SENSE_KEY_NOT_READY)
        {
            ...
            if (turStatus.asc == 0x04 && turStatus.ascq == 0x02)
            {
                ...
                // We should only hit this on anything scsi encapsulated either by a
                // SATL or RAID controller or a native SCSI drive that needs a spin up
                // before receiving any other commands.
                scsi_Start_Stop_Unit(device, false, 0, 0, false, false, true);
            }
        }

        // Adding a flush cache here because it is occasionally needed on some
        // devices. It is just a precaution to avoid potential issues with firmware
        // not flushing when it activates new code.
        flush_Cache(device);
        ...
        if (options->dlMode == FWDL_UPDATE_MODE_ACTIVATE)
        {
            ...
            {
                // cannot activate slot 0 for an existing image. Value should be between
                // 1 and 7 Activating with slot 0 is only allowed for letting the
                // controller choose an image for replacing after sending it to the
                // drive. Not applicable for switching slots
            ...
            {
                // This means that we encountered a driver that is not allowing us to
                // issue the Win10 API Firmware activate call for some unknown reason.
                // This doesn't happen with Microsoft's AHCI driver though...
                // Instead, we should disable the use of the API and retry with
                // passthrough to perform the activation. This is not preferred at all.
                // We want to use the Win10 API whenever possible so the system is ready
                // for the changes to the bus and drive information so that it is less
                // likely to BSOD like we used to see in older versions of Windows.
        ...
        else
        {
            ...
            case FWDL_UPDATE_MODE_DEFERRED:
                ...
                if (device->drive_info.passThroughHacks.scsiHacks.writeBufferNoDeferredDownload)
                {
                    if (device->deviceVerbosity > VERBOSITY_QUIET)
                    {
                        printf("\nWARNING: This controller is known to filter the SCSI "
                               "write-buffer command and block deferred download.\n");
                        printf("         If the firmware update fails, try using segmented "
                               "download instead.\n\n");
                    }
                }
                break;
            ...
            {
                // If segment size is not specified, set to compatible defaults for now.
                // This is more complicated on Windows due to old workarounds - TJE
...
#    if WINVER >= SEA_WIN32_WINNT_WIN10
                    if (device->os_info.fwdlIOsupport.fwdlIOSupported)
                    {
                        ...
                        // this driver supports the FWDL ioctl, so it likely does not have a
                        // problem with multi-sector transfers - TJE
                    }
                    else
                    {
                        // changing the transfer size to single blocks as a workaround for
                        // old drivers. This is more generic than I would like, but do not
                        // currently have a better solution for this old issue. This issue
                        // goes back to Windows XP ATA passthrough days and only single
                        // sector transfers work properly on these old, strange drivers.
                        // Ideally this check is more enhanced for specific drivers that are
                        // known to have this issue, but there is not currently enough
                        // information to setup this more complicated check. -TJE
...
#    endif // winver >= win10
                ...
                {
                    // not ATA passthrough, so do not worry about working around strange
                    // driver issues
                    options->segmentSize = 64;
                }
#else
                // Not Windows, so no strange driver workarounds necessary at this time
                // - TJE
                options->segmentSize = 64;
#endif
            }
...
#    if WINVER >= SEA_WIN32_WINNT_WIN10
            ...
            if (device->drive_info.drive_type != NVME_DRIVE && deviceSupportsWinAPI && downloadMode == DL_FW_DEFERRED)
            {
                ...
                {
                    // turn off use of the windows API since this file cannot meet the
                    // alignment requirements for the whole download
                    device->os_info.fwdlIOsupport.fwdlIOSupported = false;
                }
                // check if transfer size requirements will be met
                else if (downloadSize > device->os_info.fwdlIOsupport.maxXferSize ||
                         downloadRemainder > device->os_info.fwdlIOsupport.maxXferSize)
                {
                    // transfer size for download or remainder are greater than the API
                    // can handle, so just turn it off... (Unlikely to happen)
                    device->os_info.fwdlIOsupport.fwdlIOSupported = false;
                }
            }
#    endif // WINVER >= WIN10
            ...
            while (currentDownloadBlock < downloadBlocks)
            {
                ...
                ret = firmware_Download_Command(device, downloadMode, downloadOffset, downloadSize,
                ...
                if (ret != SUCCESS)
                {
                        ...
                        downloadOffset == 0 && device->drive_info.drive_type != NVME_DRIVE)
                    {
                        ...
                        // This retry is only when not forcing a specific mode and when the
                        // very first segment fails. 2 more conditions for this.
                        //  1. For SCSI/SAS, need to look for invalid field in CDB. An
                        //  invalid firmware returns invalid field in parameter list, so if
                        //  you get invalid field in parameter, do NOT retry.
                        //  2. For ATA/SATA, need to look for a command abort in the rtfrs.
                        //  There is no other way for SATA to specify things, so it is a
                        //  less robust check
                            ...
                            device->drive_info.lastCommandRTFRs.error & ATA_ERROR_BIT_ABORT)
                        {
                            ...
                            if (device->deviceVerbosity > VERBOSITY_QUIET)
                            {
                                printf("\nAutomatic deferred download failed. Either the drive "
                                       "does not support this mode\n");
                                printf("or this is an invalid firmware image for this device.\n");
                                printf("Retrying the download with segmented download mode to "
                                       "verify.\n");
                                fflush(stdout);
                            }
                            continue;
                        ...
                        else if (device->drive_info.drive_type == SCSI_DRIVE)
                        {
                            ...
                            if (senseKey == SENSE_KEY_ILLEGAL_REQUEST && asc == 0x24 && ascq == 0x00)
                            {
                                ...
                                if (device->deviceVerbosity > VERBOSITY_QUIET)
                                {
                                    printf("\nAutomatic deferred download failed. Either the "
                                           "drive does not support this mode\n");
                                    printf("or this is an invalid firmware image for this "
                                           "device.\n");
                                    printf("Retrying the download with segmented download mode "
                                           "to verify.\n");
                                    fflush(stdout);
                                }
                                continue;
                            ...
                            {
                                // this is not a case to issue a retry as it looks like the
                                // drive rejected the command for another reason
                                break;
                            }
                        }
                        else
                        {
                            // this is not a case to issue a retry as it looks like the drive
                            // rejected the command for another reason
                            break;
                        }
                    }
                    else
                    {
                        // error occurred for a specific mode rather than automatic mode for
                        // deferred download, so exit and do not continue trying the
                        // download.
                        break;
                    }
                }
            ...
            {
                if (downloadMode == DL_FW_SEGMENTED && downloadRemainder == 0 &&
                    (currentDownloadBlock + 1) == downloadBlocks)
                {
                    // this means that we had an error on the last sector, which is a
                    // drive bug in old products. Check that we don't have RTFRs from the
                    // last command and that the sense data does not say "unaligned write
                    // command" We may need to expand this check if we encounter this
                    // problem in other OS's or on other kinds of controllers (currently
                    // this is from a motherboard)
            ...
            if (downloadRemainder > 0 && ret == SUCCESS)
            {
                if (device->drive_info.drive_type == ATA_DRIVE)
                {
                    // round the remainder to the nearest sector since ATA talks in terms
                    // of sectors
...
#if defined(_WIN32) && defined(WINVER)
#    if WINVER >= SEA_WIN32_WINNT_WIN10
                // If we are here, then this is windows and the Windows 10 API may be
                // being used below. Because of this, there are additional allignment
                // requirements for the segments that we must meet. So now we are going
                // to check if this meets those requirements...if not, we need to
                // allocate a different buffer that meets the requirements, copy the
                // data to it, then send the command. - TJE
                if (device->drive_info.drive_type != NVME_DRIVE && device->os_info.fwdlIOsupport.fwdlIOSupported &&
                    downloadMode == DL_FW_DEFERRED) // checking to see if Windows says
                                                    // the FWDL API is supported
                {
                    if (downloadRemainder < device->os_info.fwdlIOsupport.maxXferSize &&
                        (downloadRemainder % device->os_info.fwdlIOsupport.payloadAlignment == 0))
                    {
                        // we're fine, just issue the command
                        ret = firmware_Download_Command(device, downloadMode, downloadOffset, downloadRemainder,
                                                        &options->firmwareFileMem[downloadOffset], options->bufferID,
                                                        false, false, true, 60, nvmeForceCA, nvmeForceCommitAction,
                                                        nvmeforceDisableReset);
                    }
                    else if (!(downloadRemainder < device->os_info.fwdlIOsupport.maxXferSize))
                    {
                        // This shouldn't happen. We should catch that the individual chunks
                        // or remainder are too big and shouldn't fall into here...
                        ret = OS_PASSTHROUGH_FAILURE;
                    }
                    else if (downloadRemainder % device->os_info.fwdlIOsupport.payloadAlignment)
                    {
                        // This SHOULDN'T happen. We should catch that this won't work
                        // earlier when we check if the file meets alignment requirements or
                        // not.
                        ret = OS_PASSTHROUGH_FAILURE;
                    }
                }
                else // not supported, so nothing else needs to be done other than
                     // issue the command
                {
                    ret = firmware_Download_Command(device, downloadMode, downloadOffset, downloadRemainder,
                                                    &options->firmwareFileMem[downloadOffset], options->bufferID, false,
                                                    false, true, 60, nvmeForceCA, nvmeForceCommitAction,
                                                    nvmeforceDisableReset);
                }
#    else
...
#    endif
                    ...
                    ret != SUCCESS)
                {
                    ...
                    {
                        // this means that we had an error on the last sector, which is a
                        // drive bug in old products. Check that we don't have RTFRs from
                        // the last command and that the sense data does not say "unaligned
                        // write command" We may need to expand this check if we encounter
                        // this problem in other OS's or on other kinds of controllers
                        // (currently this is from a motherboard)
                        if (device->drive_info.drive_type == ATA_DRIVE &&
            ...
            {
                delay_Milliseconds(100); // This is here because there seems to be a need for a delay.
                                         // If activating too quickly after the Firmware is downloaded,
                                         // it seems to fail. This works - TJE
                // send an activate command (not an existing slot, this is a new image
                // activation)
                ret = firmware_Download_Command(device, DL_FW_ACTIVATE, 0, 0, options->firmwareFileMem,
...
typedef struct _supportedDLModesV1
{
    ...
    bool     downloadMicrocodeSupported; // should always be true unless it's a super
                                         // old drive that doesn't support a download
                                         // command
    bool fullBuffer;
    bool segmented;
    ...
    bool deferredPowerCycleActivationSupported;         // SATA will always set this to
                                                        // true!
    bool     deferredHardResetActivationSupported;      // SAS only
    bool     deferredVendorSpecificActivationSupported; // SAS only
    uint32_t minSegmentSize;                            // in 512B blocks...May not be accurate for SAS Value
                                                        // of 0 means there is no minumum
    uint32_t maxSegmentSize;                            // in 512B blocks...May not be accurate for SAS.
                                                        // Value of all F's means there is no maximum
    uint16_t recommendedSegmentSize;                    // in 512B blocks...check SAS
    uint8_t  driveOffsetBoundary;                       // this is 2^<value>
    uint32_t driveOffsetBoundaryInBytes;                // This is the bytes value from the PO2
                                                        // calculation in the comment above.
    eDownloadMode           recommendedDownloadMode;
    SCSIMicrocodeActivation codeActivation;               // SAS Only
    eMLU                    multipleLogicalUnitsAffected; // This will only be set for multi-lun
                                                          // devices. NVMe will set this since
                                                          // firmware affects all namespaces on the
                                                          // controller
    firmwareSlotInfo firmwareSlotInfo;                    // Basically NVMe only at this point since
                                                          // such a concept doesn't exist for ATA or
                                                          // SCSI at this time - TJE
...
eReturnValues get_Supported_FWDL_Modes(tDevice* device, ptrSupportedDLModes supportedModes)
{
        ...
        supportedModes->size >= sizeof(supportedDLModesV1))
    {
        ...
        case ATA_DRIVE:
        {
            ...
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word234))
            {
                supportedModes->minSegmentSize =
                    M_BytesTo2ByteValue(M_Byte1(device->drive_info.IdentifyData.ata.Word234),
                                        M_Byte0(device->drive_info.IdentifyData.ata.Word234));
            }
            else
            {
            ...
            if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word235))
            {
                supportedModes->maxSegmentSize =
                    M_BytesTo2ByteValue(M_Byte1(device->drive_info.IdentifyData.ata.Word235),
                                        M_Byte0(device->drive_info.IdentifyData.ata.Word235));
            }
            else
            {
            ...
            // now try reading the supportd capabilities page of the identify device
            // data log for the remaining info (deferred download)
                                                     ...
                                                     ATA_LOG_PAGE_LEN_BYTES, 0))
            {
                ...
                uint64_t dlMicrocodeBits =
                    M_BytesTo8ByteValue(supportedCapabilities[23], supportedCapabilities[22], supportedCapabilities[21],
                                        supportedCapabilities[20], supportedCapabilities[19], supportedCapabilities[18],
                                        supportedCapabilities[17], supportedCapabilities[16]);
                ...
                if (dlMicrocodeBits & ATA_ID_DATA_QWORD_VALID_BIT)
                {
                    ...
                    supportedModes->maxSegmentSize =
                        M_BytesTo2ByteValue(M_Byte3(dlMicrocodeBits), M_Byte2(dlMicrocodeBits));
                    if (supportedModes->downloadMicrocodeSupported &&
                        !(supportedModes->maxSegmentSize > 0 && supportedModes->maxSegmentSize < 0xFFFF))
                    {
                        supportedModes->maxSegmentSize = UINT32_MAX;
                    }
                    supportedModes->minSegmentSize =
                        M_BytesTo2ByteValue(M_Byte1(dlMicrocodeBits), M_Byte0(dlMicrocodeBits));
                    if (supportedModes->downloadMicrocodeSupported &&
                        !(supportedModes->minSegmentSize > 0 && supportedModes->minSegmentSize < 0xFFFF))
                    {
                        supportedModes->minSegmentSize = 0;
                    }
                }
        ...
        case NVME_DRIVE:
            if (device->drive_info.IdentifyData.nvme.ctrl.oacs & BIT2)
            {
                supportedModes->downloadMicrocodeSupported   = true;
                supportedModes->deferred                     = true;
                supportedModes->multipleLogicalUnitsAffected = MLU_AFFECTS_ALL_LU; // Firmware affects all namespaces
                ...
                else
                {
                    ...
                    if (device->os_info.fwdlIOsupport.fwdlIOSupported)
                    {
                        // If we got here in Windows, then we need to make sure we follow
                        // any default rules MS sets for using their API
                ...
                if (device->os_info.fwdlIOsupport.fwdlIOSupported)
                {
                    // If Windows, then we need to make sure we don't break their
                    // rules.-TJE
                ...
                if (SUCCESS == nvme_Get_Log_Page(device, &firmwareLogOpts))
                {
                    ...
                    // set the firmware revision in each slot
                    for (uint32_t slotIter = 0, offset = 8;
                         slotIter <= supportedModes->firmwareSlotInfo.numberOfSlots &&
                         slotIter <= 7 /*max of 7 slots in spec and structure*/ && offset < 512;
                         ++slotIter, offset += 8)
                    {
                        memcpy(supportedModes->firmwareSlotInfo.slotRevisionInfo[slotIter].revision,
            ...
            /*
            //NOTE: This is the code that was previously for when running in SCSI
            translation, but showed up as an NVMe drive. It probably wasn't used and
            falling into the next case will be ok. Keeping it as a comment for now -
            TJE
            //running in SCSI translation mode, so only set full & deferred download
            modes supportedModes->downloadMicrocodeSupported = true;
            supportedModes->fullBuffer = true;
            supportedModes->deferred = true;
            supportedModes->minSegmentSize = 0;
            supportedModes->maxSegmentSize = UINT32_MAX;
            //need to set the offset requirement...for now I'm setting the minimum the
            NVMe spec says can be reported...should be OK...-TJE
            supportedModes->driveOffsetBoundaryInBytes = 4096;//4Kb is the minimum
            specified in the NVMe specification that the drive may conform to..this
            should be good enough for the translation.
            supportedModes->driveOffsetBoundary = 12;//power of 2
            */
            break;
        case SCSI_DRIVE:
        {
            // before trying all the code below, look at the extended inquiry data
            // page so see if the download modes are supported or not.
            uint8_t* extendedInq = C_CAST(uint8_t*, safe_calloc_aligned(VPD_EXTENDED_INQUIRY_LEN, sizeof(uint8_t),
            ...
            if (extendedInq)
            {
                    ...
                    scsi_Inquiry(device, extendedInq, VPD_EXTENDED_INQUIRY_LEN, EXTENDED_INQUIRY_DATA, true, false))
                {
                    ...
                    {
                        supportedModes->downloadMicrocodeSupported = true;
                        // bit 7 = dm_md_4 - temporary...not saving at this time since it's
                        // rarely used. bit 6 = dm_md_5
                        supportedModes->fullBuffer = M_ToBool(extendedInq[19] & BIT6);
                        // bit 5 = dm_md_6 - segmented temporary...not saving at this time
                        // since it's rarely used bit 4 = dm_md_7
                        ...
                        // bit 1 = dm_md_f - activate deferred code (part of mode e. If mode
                        // e is supported, so should f - TJE
                    }
                }
                safe_Free_aligned(C_CAST(void**, &extendedInq)); // PRH valgrind check
            }

            // PMC 8070 fails this command for some unknown reason even if a drive
            // supports it, so skip these requests when this hack is set.-TJE
            if (!device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations)
            {
                uint8_t* writeBufferSupportData =
                    C_CAST(uint8_t*, safe_calloc_aligned(14, sizeof(uint8_t), device->os_info.minimumAlignment));
                // first try asking for supported operation code for Full Buffer
                // download
                if (SUCCESS == scsi_Report_Supported_Operation_Codes(
                                   device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, WRITE_BUFFER_CMD,
                                   SCSI_WB_DL_MICROCODE_SAVE_ACTIVATE, 14, writeBufferSupportData))
                {
                    ...
                    case 3: // supported according to spec
                        supportedModes->downloadMicrocodeSupported = true;
                        supportedModes->fullBuffer                 = true;
                        supportedModes->multipleLogicalUnitsAffected =
                            C_CAST(eMLU, M_GETBITRANGE(writeBufferSupportData[1], 6, 5));
                        break;
                    default:
                        break;
                    }
                    // if this worked, then we know we can ask about other supported
                    // operation codes.
                    if (SUCCESS == scsi_Report_Supported_Operation_Codes(
                                       device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, WRITE_BUFFER_CMD,
                                       SCSI_WB_DL_MICROCODE_OFFSETS_SAVE_ACTIVATE, 14, writeBufferSupportData))
                    {
                        switch (writeBufferSupportData[1] & 0x07)
                        {
                        case 3: // supported according to spec
                        {
                            ...
                            uint32_t length = M_BytesTo4ByteValue(
                                0, writeBufferSupportData[10], writeBufferSupportData[11], writeBufferSupportData[12]);
                    ...
                    if (SUCCESS == scsi_Report_Supported_Operation_Codes(
                                       device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, WRITE_BUFFER_CMD,
                                       SCSI_WB_DL_MICROCODE_OFFSETS_SAVE_DEFER, 14, writeBufferSupportData))
                    {
                        switch (writeBufferSupportData[1] & 0x07)
                        {
                        case 3: // supported according to spec
                    ...
                    if (SUCCESS == scsi_Report_Supported_Operation_Codes(
                                       device, false, REPORT_OPERATION_CODE_AND_SERVICE_ACTION, WRITE_BUFFER_CMD,
                                       SCSI_WB_DL_MICROCODE_OFFSETS_SAVE_SELECT_ACTIVATE_DEFER, 14,
                                       writeBufferSupportData))
                    {
                        switch (writeBufferSupportData[1] & 0x07)
                        {
                ...
                else
                {
                    ...
                    {
                        // report supported operation codes isn't supported, so try legacy
                        // inquiry cmdDT method
                        uint8_t* temp =
                            C_CAST(uint8_t*, safe_reallocf_aligned(C_CAST(void**, &writeBufferSupportData), 14, 16,
                                                                   device->os_info.minimumAlignment));
                        if (!temp)
                        {
                        ...
                        // if that still doesn't work, we can try the obsolete method using
                        // the inquiry command
                        if (SUCCESS == scsi_Inquiry(device, writeBufferSupportData, 16, WRITE_BUFFER_CMD, false, true))
                        {
                            ...
                            case 3: // supported according to spec
                            {
                                // try to look at the mode bit field and determine which modes
                                // are supported...
                                uint8_t mode =
                                    writeBufferSupportData[7] & 0x1F; // byte 1 of the write buffer cdb itself
                                if ((mode & 0x07) == 0x07)
                                {
                                ...
                                else if (mode & BIT2) // we'll just say full only...no really
                                                      // good way to do this honestly
                                {
                                    supportedModes->downloadMicrocodeSupported = true;
                                    supportedModes->fullBuffer                 = true;
                                }
                                uint32_t length =
                                    M_BytesTo4ByteValue(0, writeBufferSupportData[12], writeBufferSupportData[13],
                                                        writeBufferSupportData[14]);
                                if (length == UINT32_C(0xFFFFFF) || length == 0)
                                {
                                    supportedModes->maxSegmentSize = UINT32_MAX;
                                    supportedModes->minSegmentSize = 0;
                        ...
                        else
                        {
                            ...
                            if (strncmp(device->drive_info.T10_vendor_ident, "NVMe", 4) == 0)
                            {
                                ...
                                supportedModes->downloadMicrocodeSupported = true;
                                supportedModes->fullBuffer                 = true;
                                supportedModes->scsiInfoPossiblyIncomplete =
                                    false; // setting this since we know it's NVMe, so we're
                                           // pretty sure that this is what we'll have from SCSI
                                           // translation
                                supportedModes->recommendedSegmentSize       = 64;
                                supportedModes->maxSegmentSize               = UINT32_MAX;
                                supportedModes->minSegmentSize               = 0;
                            ...
                            {
                                supportedModes->downloadMicrocodeSupported =
                                    true; // set this to on so we return good status...
                                supportedModes->scsiInfoPossiblyIncomplete = true;
                                // Setting supported stuff below even though we don't know for
                                // sure...should be safe enough.
                                supportedModes->fullBuffer             = true;
                                supportedModes->segmented              = true;
                                supportedModes->recommendedSegmentSize = 64;
                                supportedModes->maxSegmentSize         = UINT32_MAX;
                    ...
                    else
                    {
                                                                             ...
                                                                             writeBufferSupportData))
                        {
                            ...
                            if (writeBufferCmdSupported)
                            {
                                ...
                                // are supported...
                                uint8_t mode =
                                    writeBufferSupportData[5] & 0x1F; // byte 1 of the write buffer cdb itself
                                if ((mode & 0x07) == 0x07)
                                {
                                    // full and segmented supported
                                    supportedModes->downloadMicrocodeSupported = true;
                                ...
                                uint32_t length =
                                    M_BytesTo4ByteValue(0, writeBufferSupportData[10], writeBufferSupportData[11],
                                                        writeBufferSupportData[12]);
                                if (length == UINT32_C(0xFFFFFF) || length == 0)
                                {
                                    supportedModes->maxSegmentSize = UINT32_MAX;
                                    supportedModes->minSegmentSize = 0;
                        ...
                        // else try requesting all supported OPs and parse that
                        // information??? It could be report all is supported, but other
                        // modes are not
                        else
                        {
                            ...
                            uint8_t* reportAllOPs =
                                C_CAST(uint8_t*, safe_calloc_aligned(reportAllOPsLength, sizeof(uint8_t),
                                                                     device->os_info.minimumAlignment));
                            if (reportAllOPs)
                            {
                                                                                     ...
                                                                                     reportAllOPsLength, reportAllOPs))
                                {
                                    ...
                                    reportAllOPsLength = M_BytesTo4ByteValue(reportAllOPs[0], reportAllOPs[1],
                                                                             reportAllOPs[2], reportAllOPs[3]) +
                                                         4;
                                    ...
                                    if (reportAllOPs)
                                    {
                                        if (SUCCESS == scsi_Report_Supported_Operation_Codes(device, false, REPORT_ALL,
                                                                                             0, 0, reportAllOPsLength,
                                                                                             reportAllOPs))
                                        {
                                            // loop through the data and check for the commands and
                                            // service actions we are interested in.
                                            uint32_t supportedCmdsIter   = 4;
                                            uint16_t cmdDescriptorLength = 8;
                                            uint32_t supportedCmdsLength =
                                                M_BytesTo4ByteValue(reportAllOPs[0], reportAllOPs[1], reportAllOPs[2],
                                                                    reportAllOPs[3]) +
                                                4;
                                            for (; supportedCmdsIter < supportedCmdsLength;
                                                 supportedCmdsIter += cmdDescriptorLength)
                                            {
                                                uint8_t  operationCode = reportAllOPs[supportedCmdsIter];
                                                uint16_t serviceAction =
                                                    M_BytesTo2ByteValue(reportAllOPs[supportedCmdsIter + 2],
                                                                        reportAllOPs[supportedCmdsIter + 3]);
                                                bool serviceActionValid =
                                                    M_ToBool(reportAllOPs[supportedCmdsIter + 5] & BIT0);
                                                eMLU mlu = C_CAST(
                                                    eMLU, M_GETBITRANGE(reportAllOPs[supportedCmdsIter + 5], 5, 4));
                                                cmdDescriptorLength =
                                                    (reportAllOPs[supportedCmdsIter + 5] & BIT1) ? 20 : 8;
                                                ...
                                                case WRITE_BUFFER_CMD:
                                                    if (serviceActionValid)
                                                    {
                                                        ...
                                                        case SCSI_WB_DL_MICROCODE_OFFSETS_SAVE_ACTIVATE:
                                                            supportedModes->downloadMicrocodeSupported = true;
                                                            supportedModes->segmented                  = true;
                                                            // In this format, we cannot determine minimum or
                                                            // maximum transfer sizes. so set to max
                                                        ...
                                                        case SCSI_WB_DL_MICROCODE_OFFSETS_SAVE_DEFER:
                                                            ...
                                                            supportedModes->multipleLogicalUnitsAffected = mlu;
                                                            break;
                                                        case SCSI_WB_ACTIVATE_DEFERRED_MICROCODE: // not
                                                                                                  // currently
                                                                                                  // handled
                                                                                                  // since
                                                                                                  // it is
                                                    ...
                                                    else
                                                    {
                                                        supportedModes->downloadMicrocodeSupported = true;
                                                        supportedModes->scsiInfoPossiblyIncomplete = true;
                                                        // setting segmented and full buffer download modes
                                                        // in here because they SHOULD work on the products
                                                        // we care about supporting.
                                                        supportedModes->fullBuffer = true;
                                                        if (device->drive_info.scsiVersion >
                                                            2) // SPC added segmented. Earlier products only
                                                               // supported full buffer
                                                        {
                                                            supportedModes->segmented = true;
                                                        }
                                                    }
            ...
            {
                // assume 512B boundaries unless vendor ID is NVMe, in which case assume
                // 4k
                if (strncmp(device->drive_info.T10_vendor_ident, "NVMe", 4) == 0)
                {
            ...
            // The code below is Seagate specific...should this be in Seagate
            // Operations? - TJE
            ...
            if ((family == SEAGATE || family == SEAGATE_VENDOR_A) && supportedModes->scsiInfoPossiblyIncomplete)
            {
                ...
                if (c3VPD)
                {
                    // If the drive is a Seagate SCSI drive, then try reading the C3 mode
                    // page which is Seagate specific for the supported features
                    if (SUCCESS == scsi_Inquiry(device, c3VPD, 255, 0xC3, true, false))
                    {
                        supportedModes->downloadMicrocodeSupported = true;
                        supportedModes->scsiInfoPossiblyIncomplete = false; // turning this off because if we read this
                                                                            // page we SHOULD know it's capabilities
                        supportedModes->fullBuffer = true;
                        // byte 63  bit7 = QNR
                        if (c3VPD[63] & BIT6)
                        {
                        ...
                        // DO NOT turn the flag to false. It should already be false. If it
                        // was set to true, then the drive has already reported it supports
                        // this mode some other way.
                        if (c3VPD[82] & BIT6)
                        {
                            supportedModes->deferred = true;
                        }
        ...
        if (supportedModes->downloadMicrocodeSupported)
        {
            ...
            else
            {
                ...
                if (!supportedModes->deferred ||
                    device->drive_info.passThroughHacks.scsiHacks.writeBufferNoDeferredDownload)
                {
                    // even older ATA drives have no choice, so set these modes when
                    // needed
        ...
        supportedModes->size >= sizeof(supportedDLModesV1))
    {
        ...
        if (supportedModes->downloadMicrocodeSupported)
        {
            ...
            if (supportedModes->deferred)
            {
                ...
                {
                    printf("\tDeferred (Requires power cycle or activation command to "
                           "activate)\n");
                }
            }
            ...
            case SCSI_MICROCODE_ACTIVATE_BEFORE_COMMAND_COMPLETION:
                printf("Microcode Activation: Activated before completion of final "
                       "command in write buffer sequence\n");
                break;
            case SCSI_MICROCODE_ACTIVATE_AFTER_EVENT:
                printf("Microcode Activation: Activated after vendor specific event, "
            ...
            case MLU_NOT_REPORTED:
                printf("Not reported. Device may not support multiple logical
            units.\n"); break; case MLU_AFFECTS_ONLY_THIS_UNIT: printf("FW Updates
            affect only this unit.\n"); break; case MLU_AFFECTS_MULTIPLE_LU:
            ...
            if (supportedModes->firmwareSlotInfo.firmwareSlotInfoValid)
            {
                ...
                {
                    // slot number, read only?, active slot?, next active slot?, firmware
                    // revision in that slot
