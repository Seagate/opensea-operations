...
extern "C"
{
    ...
    //
    //! \brief   Description:  Function to send Get Error Information Log Page NVMe
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    //!   \param[in] numOfErrToPrint = set to 0 to print all, otherwise set a
    //!   reasonable value (e.g. 32)
    //!                                [NVMe Identify data shows how many entries
    //!                                are present]
    //!
    //  Exit:
    ...
    //
    //! \brief   Description:  Function to print Firmware Slots Log Page NVMe
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //
    //! \brief   Description:  Function to print Commands Supported and Effects Log
    ...
    //
    //! \brief   Description:  Function to print Device Self-test Log Page NVMe
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    //!   \param[in] selectType eNvmeFeaturesSelectValue, i.e. current, default,
    //!   saved etc. \param[in] listOnlySupportedFeatures = !!NOT USED!! list only
    //!   supported features.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //  nvme_Get_Log_Size
    //
    //! \brief   Description:  Function to get the size for GetLog Page command by a
    //! utility.
    //!                        NOTE: Some variable length logs will not return a
    //!                        size at this time. Vendor unique logs are not
    //!                        supported i nthis function
    //
    //  Entry:
    //!   \param[in] device = pointer to the device structure. This is needed in
    //!   order to calculate some log sizes that are not fixed \param[in] logPageId
    //!   = Log Page Identifier. \param[out] logSize = size of the Log to return
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
