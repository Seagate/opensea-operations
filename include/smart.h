...
// SMART attributes are NOT standardized. Use these definitions with caution as
// they may have different meanings between vendors and firmwares. - TJE
...
#define ATTRB_NUM_CRC_ERROR      (199)
    ...
    typedef struct _smartTripInfo
    {
        ...
        char reasonString[UINT8_MAX]; // This is a string with a translatable reason
                                      // for the trip. Only valid when the above bool
                                      // is set. The fields below will also give the
                                      // reason if you would like to use those instead.
        uint8_t reasonStringLength;   // length of the string. If it is zero, then it
                                      // was not used.
        ...
        union
        {
            ...
            struct
            {
                uint8_t attributeNumber; // NOTE: This may not be available since the
                                         // threshold sector has been obsolete for a long
                                         // time. If this is zero, it is an invalid
                                         // attribute number
    ...
    //  run_SMART_Check()
    //
    //! \brief   Description:  Function to run a SMART check on and ATA or SCSI
    //! device
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] tripInfo = OPTIONAL pointer to a struct to get why a drive has
    //!   been tripped (if available).
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = SMART tripped, IN_PROGRESS = warning
    //!   condition detected, all others - unknown status or error occured
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  prints the SFF-8055 SMART trip approved warning
    //! message. Output modified to handle modern SSDs if passed in
    //
    //  Entry:
    //!   \param[in] ssd = true means output showing the SSD is tripping SMART,
    //!   otherwise output indicating it is an HDD tripping SMART
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //  ata_SMART_Check()
    //
    //! \brief   Description:  Function to perform an ATA SMART check on a device
    //! (sends SMART return status and checks that the rtfrs come back as expected)
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] tripInfo = OPTIONAL pointer to a struct to get why a drive has
    ...
    //!           COMMAND_FAILURE = unknown error/smart not enabled undefined
    //!           status, UNKNOWN - didn't get back rtfrs, so unable to verify SMART
    //!           status
    //
    //-----------------------------------------------------------------------------
    ...
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = SMART tripped, IN_PROGRESS = warning
    //!   condition detected, COMMAND_FAILURE = unknown error/smart not enabled
    //!   undefined status, UNKNOWN - didn't get back rtfrs, so unable to verify
    //!   SMART status
    //
    //-----------------------------------------------------------------------------
    ...
    //!
    //  Exit:
    //!   \return true = enabled, false = not enabled (may not be supported or just
    //!   not enabled)
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Set_Feature_Control(tDevice *device, eSCTFeature sctFeature, bool
    //  enableDisable, bool defaultValue, bool isVolatile, uint16_t
    //  hdaTemperatureIntervalOrState)
    //
    //! \brief   Description:  set a SCT feature to a specific value using SCT
    ...
    //!   \param isVolatile - true = volatile change (cleared on reset/power cycle),
    //!   false = non-volatile change \param hdaTemperatureIntervalOrState - used
    //!   for the HDA Temperature interval feature code only to change the state or
    //!   frequency of the logging.
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = sct not supported or feature not supported
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Get_Feature_Control(tDevice *device, eSCTFeature sctFeature, bool
    //  enableDisable, bool defaultValue, bool isVolatile, uint16_t
    //  hdaTemperatureIntervalOrState)
    //
    //! \brief   Description:  get a SCT feature's information using SCT (SMART
    //! command transport)
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //!   \param enableDisable - true = enable, false = disable
    //!   \param defaultValue - restore to drive's default value
    //!   \param hdaTemperatureIntervalOrState - used for the HDA Temperature
    //!   interval feature code only to change the state or frequency of the
    //!   logging. \param featureOptionFlags - option flags specific to the feature
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = sct not supported or feature not supported
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Set_Command_Timer(tDevice *device, eSCTErrorRecoveryCommand ercCommand,
    ...
    //!   \param timerValueMilliseconds - how long to set the timer to in
    //!   milliseconds \param isVolatile - true = volatile change (cleared on
    //!   reset/power cycle), false = non-volatile change
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = sct not supported or feature not supported
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Get_Command_Timer(tDevice *device, eSCTErrorRecoveryCommand ercCommand,
    //  uint32_t timerValueMilliseconds)
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Restore_Command_Timer(tDevice *device, eSCTErrorRecoveryCommand
    //  ercCommand)
    //
    //! \brief   Description:  Restore the SCT Error recovery command timeout value
    //! to default
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //-----------------------------------------------------------------------------
    //
    //  sct_Get_Min_Recovery_Time_Limit(tDevice *device, uint32_t
    //  *minRcvTimeLmtMilliseconds)
    //
    //! \brief   Description:  Get the Minimum supported value for SCT Error
    //! recovery command timeout
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param minRcvTimeLmtMilliseconds - Minimum supported value for the
    //!   RECOVERY TIME LIMIT field in milliseconds
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = sct not supported or feature not supported
    //
    ...
    //
    //! \brief   Description:  Enable or disable the SMART Attribute Autosave
    //! feature on a device
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //
    //! \brief   Description:  Enable or disable the SMART Auto Offline feature on a
    //! device
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //
    //! \brief   Description:  Get SMART information from an ATA device (excludes
    //! vendor unique data and attributes)
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //
    //! \brief   Description:  Print SMART information from an ATA device (excludes
    //! vendor unique data and attributes)
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    typedef struct _informationalExceptionsControl
    {
        ...
        bool sixByteCommandUsed;         // don't change this. for use when calling a set
                                         // function after a get function.
        bool ps;                         // don't change this. for use when calling a set function after a get
                                         // function.
        uint8_t deviceSpecificParameter; // only stored here in case we are performing a
                                         // mode select command after reading the data.
        ...
        bool logerr;   // log error. 1 = will log the error to informational exceptions
                       // log, 0 means it may or maynot.
        uint8_t  mrie;
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_SCSI_Informational_Exceptions_Info(tDevice *device, eScsiModePageControl
    //  mpc, ptrInformationalExceptionsControl controlData,
    ...
    //  ptrInformationalExceptionsLog logData)
    //
    //! \brief   Description:  Set SCSI Informational Exceptions information
    //! (SMART). This should be called AFTER the
    //! get_SCSI_Informational_Exceptions_Info function since a mode sense is
    //! required before a mode select...
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param save - Set to true to make this change save across a power cycle.
    //!   False to make this change without saving it. \param controlData - pointer
    //!   to structure to save mode page data to
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = feature not supported on this device
    //
    ...
    //
    //! \brief   Description:  Set SCSI Informational Exceptions MRIE (Method of
    //! reporting informational exceptions) (SMART Check) (Changes when the
    //! condition is reported and the sense code used)
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param mrieMode - A value of 0 - 6. 0 = off, 6 = on request. See spec for
    //!   mode details or use enum from scsi_helper.h \param driveDefault - restore
    //!   to the drive's default value
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = feature not supported on this device
    //
...
#define SMART_ERROR_STATE_MASK                                                                                         \
    0x0F                           // highnibble is vendor unique. use this to look at the low nibble and
                                   // match a state to the enum below
    typedef enum _eSMARTErrorState // Low nibble only!!! high nibble is vendor
                                   // unique!
    {
    ...
    {
        uint8_t transportSpecific; // when command was initiated. If FFh, then this is
                                   // a hardware reset
        uint8_t  feature;
        uint8_t  count;
        uint8_t  lbaLow;
    ...
    {
        uint8_t version;
        uint8_t numberOfCommands; // number of commands logged before the error.
                                  // Between 0 and 5
        bool extDataStructures;   // when true, the data structures are for ext commands
                                  // (ext comprehensive vs comprehensive/summary logs)
        union
        {
...
#define SMART_SUMMARY_ERRORS_MAX UINT8_C(5) // defined as this in ATA spec
    typedef struct _summarySMARTErrorLog
    {
        ...
        SMARTErrorDataStructure smartError[SMART_SUMMARY_ERRORS_MAX]; // sorted in order from most recent
                                                                      // to oldest
...
#define SMART_COMPREHENSIVE_ERRORS_MAX                                                                                 \
    UINT8_C(25) // 255 is the maximum allowed by the spec. We are doing less than
                // this since we don't have products supporting more than this.
#define SMART_EXT_COMPREHENSIVE_ERRORS_MAX                                                                             \
    UINT8_C(100) // 65532 is the maximum allowed by the spec. We are doing less
                 // than this since we don't have products supporting more than
                 // this. Other vendors might though...
    ...
    typedef struct _comprehensiveSMARTErrorLog
    {
        ...
        union
        {
            SMARTErrorDataStructure smartError[SMART_COMPREHENSIVE_ERRORS_MAX];        // sorted in order from most
                                                                                       // recent to oldest
            SMARTErrorDataStructure extSmartError[SMART_EXT_COMPREHENSIVE_ERRORS_MAX]; // sorted in order
                                                                                       // from most recent
                                                                                       // to oldest
        };
        uint16_t deviceErrorCount;
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_ATA_Summary_SMART_Error_Log(tDevice * device, ptrSummarySMARTErrorLog
    //  smartErrorLog)
    //
    //! \brief   Description:  Get the ATA Summary SMART Error Log (will be ordered
    //! from most recent to oldest according to ATA spec) (only holds 28bit commands
    //! accurately)
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param smartErrorLog - pointer to the summary SMART error log structure to
    //!   fill in
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = feature not supported on this device
    ...
    //-----------------------------------------------------------------------------
    //
    //  print_ATA_Summary_SMART_Error_Log(ptrSummarySMARTErrorLog errorLogData, bool
    //  genericOutput)
    //
    //! \brief   Description:  Print the ATA Summary SMART Error Log (will be
    //! ordered from most recent to oldest according to ATA spec) (only prints 28bit
    //! commands accurately due to 48bit truncation)
    //
    //  Entry:
    //!   \param errorLogData - pointer to the summary SMART error log structure to
    //!   print out \param genericOutput - true = generic output showing registers
    //!   in hex. false = detailed output that is translated from the reported
    //!   regiters according to ATA spec
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //  ptrComprehensiveSMARTErrorLog smartErrorLog, bool forceSMARTLog)
    //
    //! \brief   Description:  Get the ATA (ext) Comprehensive SMART Error Log (will
    //! be ordered from most recent to oldest according to ATA spec). Automatically
    //! pulls Ext log when GPL is supported (48bit drive) for most accurate
    //! information
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param smartErrorLog - pointer to the comprehensive SMART error log
    //!   structure to fill in \param forceSMARTLog - set this to true to force a
    //!   48bit drive with GPL to read the SMART log instead. NOTE: not recommended
    //!   as the SMART log can only hold 28bit commands. 48Bit may show up, but will
    //!   be truncated to fit.
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = failed to change the feature,
    //!   NOT_SUPPORTED = feature not supported on this device
    ...
    //-----------------------------------------------------------------------------
    //
    //  print_ATA_Comprehensive_SMART_Error_Log(ptrComprehensiveSMARTErrorLog
    //  errorLogData, bool genericOutput)
    //
    //! \brief   Description:  Print the ATA (ext) comprehensive SMART Error Log
    //! (will be ordered from most recent to oldest according to ATA spec)
    //
    //  Entry:
    //!   \param errorLogData - pointer to the summary SMART error log structure to
    //!   print out \param genericOutput - true = generic output showing registers
