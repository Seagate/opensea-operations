...
#include "operations_Common.h"
    ...
    // There are a few other types out there that are not currently supported in
    // here:
    ...
    typedef enum _ePartTableType
    {
        ...
        PARTITION_TABLE_MRB, // master boot record...note there are subtypes...should
                             // EBR be here too?
    ...
    typedef enum _eMBRType
    {
        ...
        MBR_TYPE_UEFI,                 // 1 record most likely, but follows modern/classic other than
                                       // zeroes in bootstrap code area
        MBR_TYPE_AAP,                  // advanced active partitions. AAP is always at 5 if available.
                                       // Check partition type for this offset
        MBR_TYPE_NEWLDR,               // 4 records + newldr and aap. newldr always at 4, aap at 5
                                       // if available (check partition type)
    ...
    {
        uint8_t status; // bit7 = active. 01h-7Fh are invalid. 0x00 = inactive. AAP
                        // and newldr uses some of this differently! If one of these
                        // is detected, change how to part this struct
...
#define MBR_SIGNATURE_LO (0x55)
    ...
    {
        eMBRType mbrType;
        uint8_t  numberOfPartitions; // set to how many of the following partitions
                                     // contain something filled in.
    ...
    // Please read through the lists on both of these websites. This list may be
    // incomplete and be aware that some identifiers are reused between OSs
    // https://en.m.wikipedia.org/wiki/Partition_type
    // https://www.win.tue.nl/~aeb/partitions/partition_types.html
    // some of these that share codes may be able to be figured out with other
    // available partitions or if LBAs are provided for access -TJE Reading the
    // partition format structures may also be able to further clarrify what it is.
    // Ex: & can be NTFS, HPFS, exFAT, so checking this could help
    typedef enum _eMBRPartitionType
    {
        ...
        MBR_PART_TYPE_CONFIG_OR_DIAG_PARTITION               = 0x12, // multiple vendors seems to have used this for a
                                                                     // config/recovery/diag partition of some kind
        ...
        MBR_PART_TYPE_FAT_16_VOLUME_SET         = 0x86, // legacy fault tolerant fat16. NT4 or earlier will add 80h to
                                                        // partitions part of fault tolerant set
    ...
    // NOTE: Block 0 is a device descriptor map. actual APM may begin at sector 1 or
    // even later
...
#define APM_PROCESSOR_TYPE_LEN (16)
    ...
    {
        uint32_t firstPartitionSectorNumber; // more useful for debugging this code
                                             // than anything else.-TJE Was this found
                                             // on sector 0, 1, 2, etc...
...
#define GPT_MIN_PARTITIONS                                                                                             \
    (128) // This is the minimum number that is required per UEFI spec for space
          // in the array. There can be more than this.
...
#define GPT_PARTITION_NAME_LENGTH_BYTES (72)
    ...
    // NOTE: This struct will be byte swapped as needed to host endianness so it can
    // be easier to print/compare/etc
    ...
    {
        gptPartitionTypeName partitionTypeGUID; // NOTE: This will use an internal lookup when
                                                // populating to set name and enum type. Be aware not
                                                // all types are known, so it may be set to unknown!
        gptGUID  uniquePartitionGUID;
        uint64_t startingLBA;
        uint64_t endingLBA;      // inclusive
        uint64_t attributeFlags; // some flags depend on partition type! bits48-63 are
                                 // type specific
        uint16_t partitionName[GPT_PARTITION_NAME_LENGTH_BYTES / 2]; // NOTE: This is described as a null-terminated
                                                                     // string. Unclear if ascii or utf-16, but assuming
                                                                     // utf-16 for now-TJE
    } gptPartitionEntry;
    ...
    typedef struct _gptData
    {
        ...
        uint32_t revision;
        bool     crc32HeaderValid; // if this is false, then something is wrong and the
                                   // data may be invalid
        uint64_t currentLBA;       // This will be "1" for primary copy. If this was read
                                   // from the backup, this will be maxlba (or close to it)
        uint64_t backupLBA;
        uint64_t firstUsableLBA;
        uint64_t lastUsableLBA;
        gptGUID  diskGUID;
        uint32_t numberOfPartitionEntries; // reported in GPT header. may be greater
                                           // than number read depending on how many
                                           // empty entries are in the list
        bool crc32PartitionEntriesValid;
        bool validBackupGPT;             // gpt was able to read from last LBA. If reading from
                                         // the backup, this bool means the primary copy...which
                                         // will likely be false since the primary was not the
                                         // data source
        uint32_t partitionDataAvailable; // number of partitions that were successfully
                                         // read into the following partition entires
        gptPartitionEntry partition[1];  // NOTE: This must be allocated based on how many partitions
                                         // are actually available! ex: malloc(sizeof(gptData) +
    ...
    // Ideas when reading this info. Note whether the partitions are aligned per the
    // drive's requirements (physical sector size for SAS/SATA, nvme alignment???)
