...
#include "operations_Common.h"
    ...
    //
    typedef enum ePartTableTypeEnum
    {
    ...
    typedef enum eMBRTypeEnum
    {
        ...
        MBR_TYPE_AAP,     // advanced active partitions. AAP is always at 5 if available. Check partition type for this
                          // offset
    ...
    typedef struct s_mbrCHSAddress
    {
        uint8_t head;
    ...
    typedef struct s_mbrPartitionEntry
    {
        uint8_t status; // bit7 = active. 01h-7Fh are invalid. 0x00 = inactive. AAP and newldr uses some of this
                        // differently! If one of these is detected, change how to part this struct
...
#define MBR_SIGNATURE_LO (0x55)
    ...
    // identifiers are reused between OSs https://en.m.wikipedia.org/wiki/Partition_type
    // https://www.win.tue.nl/~aeb/partitions/partition_types.html
    // some of these that share codes may be able to be figured out with other available partitions or if LBAs are
    // provided for access -TJE Reading the partition format structures may also be able to further clarrify what it is.
    // Ex: & can be NTFS, HPFS, exFAT, so checking this could help
    typedef enum eMBRPartitionTypeEnum
    {
        ...
        MBR_PART_TYPE_CONFIG_OR_DIAG_PARTITION =
            0x12, // multiple vendors seems to have used this for a config/recovery/diag partition of some kind
        ...
        MBR_PART_TYPE_FAT_16_VOLUME_SET =
            0x86, // legacy fault tolerant fat16. NT4 or earlier will add 80h to partitions part of fault tolerant set
...
#define APM_PROCESSOR_TYPE_LEN (16)
    ...
    typedef struct s_apmPartitionEntry
    {
    ...
    typedef struct s_apmData
    {
        uint32_t firstPartitionSectorNumber; // more useful for debugging this code than anything else.-TJE Was this
                                             // found on sector 0, 1, 2, etc...
...
#define GPT_MIN_PARTITIONS                                                                                             \
    (128) // This is the minimum number that is required per UEFI spec for space in the array. There can be more than
          // this.

#define GPT_PARTITION_NAME_LENGTH_BYTES (72)

    typedef enum eGPTPartitionTypeEnum
    {
    ...
    {
        gptPartitionTypeName
            partitionTypeGUID; // NOTE: This will use an internal lookup when populating to set name and enum type. Be
                               // aware not all types are known, so it may be set to unknown!
        ...
        uint16_t partitionName[GPT_PARTITION_NAME_LENGTH_BYTES /
                               2]; // NOTE: This is described as a null-terminated string. Unclear if ascii or utf-16,
                                   // but assuming utf-16 for now-TJE
    } gptPartitionEntry;

    typedef struct s_gptData
    {
        bool     mbrValid;
        mbrData  protectiveMBR;
        uint32_t revision;
        bool     crc32HeaderValid; // if this is false, then something is wrong and the data may be invalid
        uint64_t currentLBA; // This will be "1" for primary copy. If this was read from the backup, this will be maxlba
                             // (or close to it)
        uint64_t backupLBA;
        uint64_t firstUsableLBA;
        uint64_t lastUsableLBA;
        gptGUID  diskGUID;
        uint32_t numberOfPartitionEntries; // reported in GPT header. may be greater than number read depending on how
                                           // many empty entries are in the list
        bool crc32PartitionEntriesValid;
        bool validBackupGPT; // gpt was able to read from last LBA. If reading from the backup, this bool means the
    ...
    // sector size for SAS/SATA, nvme alignment???)

    typedef struct s_partitionInfo
    {
