...
#define VENDOR_SPECIFIC_PHY_EVENT_ID_CHECK BIT15
    ...
    typedef struct _phyEventCounter
    {
        ...
        uint16_t rawID;           // the raw ID value including vendor unique bit and lenth
                                  // bits.
        uint64_t counterMaxValue; // use this to check if a counter has hit the
                                  // maximum value it can hold or not.
        uint64_t counterValue;
    } phyEventCounter;

// This should be enought to read standard and vendor counters, but can be
// adjusted if needed.
#define MAX_PHY_EVENT_COUNTERS UINT8_C(32)

    typedef struct _sataPhyEventCounters
    {
        bool valid;                       // must be true for any other data to have meaning in this
                                          // structure.
        bool validChecksumReceived;       // if this is false, then the following data may
                                          // be corrupt, but some of it may be valid.
