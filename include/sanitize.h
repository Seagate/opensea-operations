...
extern "C"
{
    ...
    typedef enum _noDeallocateResponseMode
    {
        ...
        NO_DEALLOC_RESPONSE_WARNING, // a warning is generated and sanitize commands
                                     // are still processed when no deallocate is set
                                     // in the command
        NO_DEALLOC_RESPONSE_ERROR    // a error is generated and santize commands are
                                     // aborted when no deallocate is set in the command
    ...
    typedef struct _sanitizeFeaturesSupported
    {
        ...
        bool definitiveEndingPattern;               // SAS & NVMe set this to true. SATA this comes
                                                    // from identify device data log
        eWriteAfterEraseReq writeAfterCryptoErase;  // SAS only
        eWriteAfterEraseReq writeAfterBlockErase;   // SAS only
        uint8_t             maximumOverwritePasses; // based on ATA/NVMe/SCSI standards. Not
                                                    // reported by the drive.-TJE
    ...
    //!   \param[out] sanitizeOptions - pointer to the sanitizeFeaturesSupported
    //!   structure to populate some information
    //!
    //  Exit:
    ...
    //
    //! \brief   Description:  Function to get the SANITIZE Device Features from an
    //! ATA drive
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] sanitizeOptions = pointer to struct that will have which
    //!   operations are supported set
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to find out which of the sanitize feature
    //! options are supported, if any
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    ...
    typedef enum _eSanitizeStatus
    {
        ...
        SANITIZE_STATUS_NOT_IN_PROGRESS, // SCSI/SAS - May be the same thing as
                                         // success, which is why they share the same
                                         // value.
        SANITIZE_STATUS_IN_PROGRESS,
        SANITIZE_STATUS_NEVER_SANITIZED,                // Only useful on a fresh drive that's never
                                                        // been sanitized. Some amount of support to
                                                        // detect this on ATA is also present.
        SANITIZE_STATUS_FAILED,                         // generic failure
        SANITIZE_STATUS_FAILED_PHYSICAL_SECTORS_REMAIN, // ATA - Completed with
                                                        // physical sectors that are
                                                        // available to be allocated
                                                        // for user data that were not
                                                        // successfully sanitized
        SANITIZE_STATUS_UNSUPPORTED_FEATURE,            // ATA - the specified sanitize value in
                                                        // the feature register is not supported
    ...
    //
    //! \brief   Description:  Function to get the progress of an active Sanitize
    //! test
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] percentComplete = pointer to a double type that will hold the
    //!   test progress \param[out] sanitizeStatus = pointer to a enum that holds
    //!   the current sanitize status
    //!
    //  Exit:
    ...
    //
    //! \brief   Description:  This function calls get_Sanitize_Progress and prints
    //! out the progress based on the result
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!
    ...
    //
    //! \brief   Description: (Obsolete) This function will start, and optionally
    //! poll for progress for the duration of a sanitize operation
    //
    //  Entry:
    //!   \param[in] device = pointer to device structure containing open device
    //!   handle \param[in] sanitizeOperation = enum type specifying which sanitize
    //!   operation to perform \param[in] pollForProgress = set to true to poll for
    //!   progress until complete. This will output percent complete to stdout. For
    //!   GUI's, it's recommended that this is set to false and progress is polled
    //!   elsewhere. \param[in] pattern = pointer to a data pattern to use in an
    //!   overwrite. NOTE This param is only used for Sanitize overwrite. It will be
    //!   ignored otherwise. \param[in] patternLength = length of the pattern
    //!   pointed to by pattern argument. Must be at least 4 for ATA (first 4 bytes
    //!   are used). Pattern cannot be larger than 1 logical sector for SCSI
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = fail
    //
...
#define SANITIZE_OPERATION_OPTIONS_VERSION (1)
    typedef struct _sanitizeOperationOptions
    {
        ...
        struct
        {
            ...
            union
            {
                // These bits mean the same thing today. ZBC and ZAC use ZNR, NVMe Zoned
                // Namespaces uses the no-deallocate for the same purpose-TJE
                bool zoneNoReset;  // zoned devices only - SATA and SAS
                bool noDeallocate; // NVMe only today. May not be supported by a
                                   // controller.
            };
            uint8_t reserved[6];
