...
extern "C"
{
    ...
    typedef enum eEncryptionSupportEnum
    {
        ENCRYPTION_NONE,
    ...
    typedef struct s_firmwareDownloadSupport
    {
        bool downloadSupported; // also known as immediate
        bool segmentedSupported;
        bool deferredSupported;
        bool dmaModeSupported;
        bool seagateDeferredPowerCycleRequired; // When this is set, a segmented download is treated as a deferred
                                                // download, requiring a power cycle, in the Seagate drive's firmware.
    } firmwareDownloadSupport;

    typedef struct s_humidityInformation // all values are relative humidity percentages from 0% to 100%
    {
    ...
    typedef struct s_temperatureInformation // all values in degrees celsius
    {
    ...
    typedef struct s_lastDSTInformation
    {
...
#define MAX_PORTS                                                                                                      \
    UINT8_C(2) // Change this number if more ports are added to SAS drives that we want to retrieve the port speed from.
               // For now, 2 is enough - TJE

    typedef enum eInterfaceSpeedTypeEnum
    {
    ...
    typedef struct s_ifSerialSpeed
    {
        uint8_t numberOfPorts;    // SATA will always be set to 1
        uint8_t activePortNumber; // this will be set to the port number we are currently talking over. This is
                                  // determined by parsing the accosiation field of a device identification designator.
                                  // (SPC spec). Note: This is SCSI/SAS only
        uint8_t portSpeedsMax[MAX_PORTS]; // 0 = not reported, 1 = gen 1 (1.5Gb/s), 2 = gen 2 (3.0Gb/s), 3 = gen 3
                                          // (6.0Gb/s), 4 = gen 4 (12.0Gb/s)
        uint8_t portSpeedsNegotiated[MAX_PORTS]; // 0 = not reported, 1 = gen 1 (1.5Gb/s), 2 = gen 2 (3.0Gb/s), 3 = gen
                                                 // 3 (6.0Gb/s), 4 = gen 4 (12.0Gb/s)
    } ifSerialSpeed;

    typedef enum eCablingInfoTypeEnum
    {
        CABLING_INFO_NONE,
        CABLING_INFO_ATA,
    } eCablingInfoType;

    typedef struct s_ataCablingInfo
    {
...
#define PARALLEL_INTERFACE_MODE_NAME_MAX_LENGTH 20
    ...
    typedef struct s_ifParallelSpeed
    {
    ...
    typedef struct s_interfaceSpeed
    {
...
#define CRYPTO_MODULE_MODULE_NAME_LENGTH      256
    typedef struct s_securityCompliance
    {
        bool valid;
    ...
    typedef struct s_securityProtocolInfo
    {
        ...
        ataSecurityStatus ataSecurityInfo; // to report out info about ATA security for any interface supporting the
                                           // security protocol.
...
#define MAX_SPEC_LENGTH                                                                                                \
    ...
    typedef struct s_driveInformationSAS_SATA
    {
        ...
        char                   vendorID[T10_VENDOR_ID_LEN +
                      1]; // This is the T10 vendor ID. ATA will be set to "ATA", NVMe will be set to "NVMe"
        ...
        bool powerOnMinutesValid; // check this to make sure the time is valid. It is possible this is zero on a brand
                                  // new out of the box product
        uint64_t powerOnMinutes;
        uint64_t maxLBA;
        uint64_t
            nativeMaxLBA; // ATA Only since SCSI doesn't have a way to get the native max without changing the drive
                          // (not ok to do for this function) if set to 0, or UINT64_MAX, then the value is invalid
        ...
        bool trustedCommandsBeingBlocked; // Linux blocks ATA trusted send/receive commands by default. So this bool is
                                          // a going to be true on most linux systems that haven't had the kernel boot
                                          // parameter to allow them set. All other systems will likely see this allowed
        uint64_t cacheSize;               // Bytes
        uint64_t hybridNANDSize;          // Bytes
        double   percentEnduranceUsed; // This is a double so that some drives can report a more finite percentage used
                                       // with a decimal point.
        uint64_t totalLBAsRead;    // LBA count, will need to be multiplied by the logical Sector size in order to know
                                   // number of bytes
        uint64_t totalLBAsWritten; // LBA count, will need to be multiplied by the logical Sector size in order to know
                                   // number of bytes
        ...
        char featuresSupported[MAX_FEATURES][MAX_FEATURE_LENGTH]; // max of 50 different features, 50 characters allowed
        ...
        uint8_t smartStatus; // 0 = good, 1 = bad, 2 = unknown (unknown will happen on many USB drives, everything else
                             // should work)
        uint8_t zonedDevice; // set to 0 for non-zoned devices (SMR). If non-zero, then this matches the latest ATA/SCSI
                             // specs for zoned devices
        ...
        uint64_t longDSTTimeMinutes; // This is the drive's reported Long DST time (if supported). This can be used as
                                     // an approximate time to read the whole drive on HDD. Not sure this is reliable on
                                     // SSD since the access isn't limited in the same way a HDD is.
        bool        isWriteProtected;   // Not available on SATA!
        adapterInfo adapterInformation; // Not populated on all OSs. This is only obtainable from low-level OS calls and
                                        // copied from the tDevice structure.
        uint8_t lunCount; // for help detecting multi-actuator drives. Will be zero if not checked for, and an invalid
                          // value. Will be 1 or more when checked. Each lun represents and actuator today. - TJE
        uint8_t concurrentPositioningRanges; // For multi-actuator drives with multiple actuators per LUN, this will be
                                             // set to a value greater than zero, if zero then this is not supported.
    ...
    typedef struct s_driveInformationNVMe
    {
        ...
        struct
        {
            ...
            char    controllerFeaturesSupported[MAX_FEATURES]
                                            [MAX_FEATURE_LENGTH]; // max of 50 different features, 50 characters allowed
                                                                  // for each feature name
        ...
        {
            bool    valid;
            uint8_t smartStatus; // 0 = good, 1 = bad, 2 = unknown (unknown will happen on many USB drives, everything
                                 // else should work) (this is to be similar to ATA and SCSI-TJE
        ...
        struct
        {
            ...
            uint64_t namespaceCapacity;    // maximum number of logical blocks to be allocated in the namespace at any
                                           // point in time
    ...
    typedef enum eDriveInfoTypeEnum
    {
        DRIVE_INFO_SAS_SATA = 0,
        DRIVE_INFO_NVME
    } eDriveInfoType;

    typedef struct s_driveInformation
    {
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_SCSI_Drive_Information(ptrDriveInformation externalDriveInfo, ptrDriveInformation scsiDriveInfo,
    //  ptrDriveInformation ataDriveInfo)
    //
    //! \brief   Description:  This function takes ATA drive information and SCSI drive information and combines it into
    //! the externalDriveInfo struct for what we want to show a user about an external drive
    //
    //  Entry:
    ...
    //! for SAS/SATA drive information type
    //
    //  Entry:
    ...
    //
    //! \brief   Description:  This function is generic and prints out the data in the driveInfo structure to the
    //! screen.
    //
    //  Entry:
    ...
    //
    //! \brief   Description:  This function will print out both SCSI reported and ATA reported information. This should
    //! only be done for SAT interfaces.
    //
    //  Entry:
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param showChildInformation = set to true to show information about both the scsi reported info (what the OS
    //!   sees) and the ata reported info (what the bridge reads and interprets to show the OS)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = one of the operations being called inside of this function failed.
    //
