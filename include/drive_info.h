...
extern "C"
{
    ...
    typedef struct _firmwareDownloadSupport
    {
        ...
        bool seagateDeferredPowerCycleRequired; // When this is set, a segmented
                                                // download is treated as a deferred
                                                // download, requiring a power cycle,
                                                // in the Seagate drive's firmware.
    } firmwareDownloadSupport;

    typedef struct _humidityInformation // all values are relative humidity
    ...
    {
        bool informationValid;   // If this is not true, then the device doesn't support
                                 // DST
        uint8_t  testNumber;     // short/long/background/foreground test number
        uint64_t errorLBA;       // error lba if any (if no error, this is set to UINT64_MAX
                                 // (all F's)
...
#define MAX_PORTS                                                                                                      \
    UINT8_C(2) // Change this number if more ports are added to SAS drives that we
               // want to retrieve the port speed from. For now, 2 is enough - TJE
    ...
    typedef struct _ifSerialSpeed
    {
        ...
        uint8_t activePortNumber;                // this will be set to the port number we are currently
                                                 // talking over. This is determined by parsing the
                                                 // accosiation field of a device identification
                                                 // designator. (SPC spec). Note: This is SCSI/SAS only
        uint8_t portSpeedsMax[MAX_PORTS];        // 0 = not reported, 1 = gen 1 (1.5Gb/s), 2
                                                 // = gen 2 (3.0Gb/s), 3 = gen 3 (6.0Gb/s), 4
                                                 // = gen 4 (12.0Gb/s)
        uint8_t portSpeedsNegotiated[MAX_PORTS]; // 0 = not reported, 1 = gen 1 (1.5Gb/s),
                                                 // 2 = gen 2 (3.0Gb/s), 3 = gen 3
                                                 // (6.0Gb/s), 4 = gen 4 (12.0Gb/s)
    } ifSerialSpeed;
    ...
    typedef struct _ataCablingInfo
    {
        ...
        bool    device1;
        uint8_t deviceNumberDetermined; // 0 = reserved, 1 = jumper, 2 = cable select,
                                        // 3 = other unknown method
    } ataCablingInfo;
...
#define PARALLEL_INTERFACE_MODE_NAME_MAX_LENGTH 20
    ...
    {
        bool negotiatedValid;   // may be false if old parallel interface that doesn't
                                // report this...
        double negotiatedSpeed; // MB/s
        double maxSpeed;        // MB/s
        bool   negModeNameValid;
        char   negModeName[PARALLEL_INTERFACE_MODE_NAME_MAX_LENGTH]; // Hold something
                                                                     // like UDMA6, or
                                                                     // FAST320, etc
        bool maxModeNameValid;
        char maxModeName[PARALLEL_INTERFACE_MODE_NAME_MAX_LENGTH]; // Hold something
                                                                   // like UDMA6, or
                                                                   // FAST320, etc
    ...
    // This struct is only for ATA drives...more specifically legacy drives. Can be
    // used if any ATA drive that populates these fields.
    typedef struct _legacyCHSInfo
    {
        ...
        bool     currentInfoconfigurationValid;         // Must be true for the following parts of
                                                        // this structure
...
#define CRYPTO_MODULE_MODULE_NAME_LENGTH      256
    ...
    typedef struct _securityProtocolInfo
    {
        ...
        ataSecurityStatus ataSecurityInfo;        // to report out info about ATA security for any
                                                  // interface supporting the security protocol.
        securityCompliance cryptoModuleCompliance[MAX_COMPLIANCE_DESCRIPTORS]; // setting to 2 for now...not sure there
                                                                               // are more than 1 reported currently.
...
#define MAX_FEATURE_LENGTH                                                                                             \
    UINT8_C(50) // maximum number of characters to allow for use when storing
                // feature names.
...
#define MAX_SPEC_LENGTH UINT8_C(40)
    ...
    typedef struct _driveInformationSAS_SATA
    {
        ...
        char vendorID[T10_VENDOR_ID_LEN + 1];        // This is the T10 vendor ID. ATA will be set to "ATA", NVMe
                                                     // will be set to "NVMe"
        ...
        bool     worldWideNameSupported; // set to true when worldWideName contains valid
                                         // data
        ...
        bool                   powerOnMinutesValid; // check this to make sure the time is valid. It is
                                                    // possible this is zero on a brand new out of the
                                                    // box product
        uint64_t powerOnMinutes;
        uint64_t maxLBA;
        uint64_t nativeMaxLBA; // ATA Only since SCSI doesn't have a way to get the
                               // native max without changing the drive (not ok to do
                               // for this function) if set to 0, or UINT64_MAX, then
        ...
        uint16_t      sectorAlignment;           // first logical sector offset within the first
                                                 // physical sector
        uint16_t rotationRate;                   // Value matches the spec. 0 = not reported. 1 = SSD,
                                                 // everything else is an RPM
        uint8_t formFactor;                      // matches SBC and ACS specs
        uint8_t numberOfSpecificationsSupported; // number of specifications added to
                                                 // the list in the next field
        char               specificationsSupported[MAX_SPECS][MAX_SPEC_LENGTH];
        eEncryptionSupport encryptionSupport;
        bool               trustedCommandsBeingBlocked; // Linux blocks ATA trusted send/receive
                                                        // commands by default. So this bool is a
                                                        // going to be true on most linux systems
                                                        // that haven't had the kernel boot
                                                        // parameter to allow them set. All other
                                                        // systems will likely see this allowed
        uint64_t cacheSize;                             // Bytes
        uint64_t hybridNANDSize;                        // Bytes
        double   percentEnduranceUsed;                  // This is a double so that some drives can report a
                                                        // more finite percentage used with a decimal point.
        uint64_t totalLBAsRead;                         // LBA count, will need to be multiplied by the logical
                                                        // Sector size in order to know number of bytes
        uint64_t totalLBAsWritten;                      // LBA count, will need to be multiplied by the logical
                                                        // Sector size in order to know number of bytes
        uint64_t totalWritesToFlash;                    // SSD Only (SATA only). This is used for
                                                        // calculating write amplification
        uint64_t totalBytesRead;
        uint64_t totalBytesWritten;
        double   deviceReportedUtilizationRate; // ACS4 or SBC4 required for this to be
                                                // valid
        ...
        char           featuresSupported[MAX_FEATURES][MAX_FEATURE_LENGTH]; // max of 50 different features,
                                                                            // 50 characters allowed for each
                                                                            // feature name
        ...
        uint8_t                 smartStatus;      // 0 = good, 1 = bad, 2 = unknown (unknown will happen on
                                                  // many USB drives, everything else should work)
        uint8_t zonedDevice;                      // set to 0 for non-zoned devices (SMR). If non-zero, then
                                                  // this matches the latest ATA/SCSI specs for zoned devices
        lastDSTInformation dstInfo;
        bool               lowCurrentSpinupValid; // will be set to true for ATA, set to false for
                                                  // SAS
        bool lowCurrentSpinupViaSCT;
        int  lowCurrentSpinupEnabled;        // only valid when lowCurrentSpinupValid is set
                                             // to true
        uint64_t longDSTTimeMinutes;         // This is the drive's reported Long DST time (if
                                             // supported). This can be used as an approximate
                                             // time to read the whole drive on HDD. Not sure
                                             // this is reliable on SSD since the access isn't
                                             // limited in the same way a HDD is.
        bool        isWriteProtected;        // Not available on SATA!
        adapterInfo adapterInformation;      // Not populated on all OSs. This is only
                                             // obtainable from low-level OS calls and
                                             // copied from the tDevice structure.
        uint8_t lunCount;                    // for help detecting multi-actuator drives. Will be zero if not
                                             // checked for, and an invalid value. Will be 1 or more when
                                             // checked. Each lun represents and actuator today. - TJE
        uint8_t concurrentPositioningRanges; // For multi-actuator drives with
                                             // multiple actuators per LUN, this will
                                             // be set to a value greater than zero,
                                             // if zero then this is not supported.
    ...
    typedef struct _driveInformationNVMe
    {
        ...
        struct
        {
            ...
            uint8_t  hostIdentifier[16];  // get features (when supported by drive...and
                                          // if host has set this)
            ...
            eEncryptionSupport encryptionSupport;
            uint8_t            reserved; // previously part of numberOfControllerFeatures as a
                                         // word, but changed to a byte
            uint8_t numberOfControllerFeatures;
            char    controllerFeaturesSupported[MAX_FEATURES]
                                            [MAX_FEATURE_LENGTH]; // max of 50 different features, 50 characters
                                                                  // allowed for each feature name
        ...
        {
            bool    valid;
            uint8_t smartStatus;       // 0 = good, 1 = bad, 2 = unknown (unknown will happen
                                       // on many USB drives, everything else should work)
                                       // (this is to be similar to ATA and SCSI-TJE
        ...
        struct
        {
            ...
            uint64_t namespaceCapacity;    // maximum number of logical blocks to be allocated
                                           // in the namespace at any point in time
            uint64_t namespaceUtilization; // current number of logical blocks allocated
                                           // in the namespace
            ...
            // Namespace features will include protection information types, and
            // security protocols supported
            uint8_t reserved; // previously part of numberOfControllerFeatures as a
                              // word, but changed to a byte
            uint8_t numberOfNamespaceFeatures;
            char    namespaceFeaturesSupported[MAX_FEATURES]
                                           [MAX_FEATURE_LENGTH]; // max of 50 different features, 50 characters
                                                                 // allowed for each feature name
        } namespaceData;
        securityProtocolInfo securityInfo; // TCG, IEEE1667, etc
    ...
    //
    //! \brief   Description:  This function fills in all the driveInformation into
    //! a driveInformation structure
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param driveInfo = pointer to the struct to fill in with ATA drive
    //!   information.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = one of the operations being called
    //!   inside of this function failed.
    //
    //-----------------------------------------------------------------------------
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param driveInfo = pointer to the struct to fill in with SCSI drive
    //!   information.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = one of the operations being called
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_SCSI_Drive_Information(ptrDriveInformation externalDriveInfo,
    //  ptrDriveInformation scsiDriveInfo, ptrDriveInformation ataDriveInfo)
    //
    //! \brief   Description:  This function takes ATA drive information and SCSI
    //! drive information and combines it into the externalDriveInfo struct for what
    //! we want to show a user about an external drive
    //
    //  Entry:
    //!   \param externalDriveInfo = pointer to the struct to fill in with SCSI and
    //!   ATA drive information for displaying \param scsiDriveInfo = pointer to the
    //!   struct to filled in with SCSI drive information \param ataDriveInfo =
    //!   pointer to the struct to filled in with SCSI drive information
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  This function is generic and prints out the data in
    //! the driveInfo structure to the screen for SAS/SATA drive information type
    //
    //  Entry:
    //!   \param driveInfo = pointer to the driveInformationSAS_Sata struct with
    ...
    //
    //! \brief   Description:  This function is generic and prints out the data in
    //! the driveInfo structure to the screen.
    //
    //  Entry:
    //!   \param driveInfo = pointer to the driveInformation struct with information
    ...
    //-----------------------------------------------------------------------------
    //
    //  print_Parent_And_Child_Information(ptrDriveInformation scsiDriveInfo,
    //  ptrDriveInformation ataDriveInfo)
    //
    //! \brief   Description:  This function will print out both SCSI reported and
    //! ATA reported information. This should only be done for SAT interfaces.
    //
    //  Entry:
    //!   \param scsiDriveInfo = pointer to the driveInformation struct with SCSI
    //!   information to print to the screen \param ataDriveInfo = pointer to the
    //!   driveInformation struct with ATA information to print to the screen
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  This function prints out identifying information
    //! about a device.
    //!                        The model number, serial number, current temperature,
    //!                        power on hours, link rate, and maxLBA will be printed
    //!                        to the screen among other things
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param showChildInformation = set to true to show information about both
    //!   the scsi reported info (what the OS sees) and the ata reported info (what
    //!   the bridge reads and interprets to show the OS)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, FAILURE = one of the operations being called
    //!   inside of this function failed.
    ...
    //
    //! \brief   Description:  Function to determine SCSI interface speeds (called
    //! by print drive info)
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] scsiport0negSpeed = pointer to string for port0 negotiated
    //!   speed value \param[in] scsiport1negSpeed = pointer to string for port1
    //!   negotiated speed value \param[in] scsiport0maxSpeed = pointer to string
    //!   for port0 max speed value \param[in] scsiport1maxSpeed = pointer to string
    //!   for port1 max speed value
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
