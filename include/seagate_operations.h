...
extern "C"
{
    ...
    //
    //! \brief   Description:  This issues a Seagate Specific SCT command to change
    //! the SATA PHY speed. Only available on Seagate HDD's
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param speedGen - Which SATA generation speed to set. 1 = 1.5Gb/s, 2
    //!   = 3.0Gb/s, 3 = 6.0Gb/s. All other inputs return BAD_PARAMETER
    //!
    //  Exit:
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Set_Phy_Speed(tDevice *device, uint8_t phySpeedGen, bool allPhys,
    //  uint8_t phyNumber)
    //
    //! \brief   Description:  This issues a mode sense and mode select to the SAS
    //! phy page to change the programmed maximum link rate of 1 or all phys.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param phySpeedGen - Which SAS generation speed to set. 1 = 1.5Gb/s, 2
    //!   = 3.0Gb/s, 3 = 6.0Gb/s, 4 = 12.0Gb/s, 5 = 22.5Gb/s All other inputs return
    //!   BAD_PARAMETER \param allPhys - on SAS, set this to true to set all Phys to
    //!   the same speed. \param phyNumber - if allPhys is false, this is used to
    //!   specify which phy to change speed on.
    //!
    //  Exit:
    //!   \return SUCCESS = successfully set Phy Speed, !SUCCESS = check return code
    //
...
#define SET_PHY_SPEED_SATA_MAX_GENERATION                                                                              \
    3 // SATA only has 3 generations, so it's a lower number than the overall
      // limit above which covers SAS as well.
    //-----------------------------------------------------------------------------
    //
    //  set_phy_speed()
    //
    //! \brief   Description:  This is the friendly call to use that does all the
    //! input checking for you before calling the proper functions to set Phy Speed
    //! for SATA or SAS.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param phySpeedGen - Which generation speed to set. 1 = 1.5Gb/s, 2
    //!   = 3.0Gb/s, 3 = 6.0Gb/s, 4 = 12.0Gb/s (SAS), 5 = 22.5Gb/s (SAS). All other
    //!   inputs return BAD_PARAMETER \param allPhys - on SAS, set this to true to
    //!   set all Phys to the same speed. This is ignored on SATA \param
    //!   phyIdentifier - if allPhys is false, this is used to specify which phy to
    //!   change speed on. This is ignored on SATA
    //!
    //  Exit:
    //!   \return SUCCESS = successfully set Phy Speed, !SUCCESS = check return code
    //
    ...
    //
    //! \brief   Description:  This function checks if the SCT command for low
    //! current spinup is supported or not.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!
    ...
    //
    //! \brief   Description:  This function will check if low current spin up is
    //! enabled on Seagate ATA drives. Not all drives support this feature.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param sctCommandSupported - set to true means the SCT command is
    //!   supported and to be used to determine the enabled value, otherwise false
    //!   means check an identify bit on 2.5" products. Should be set to the return
    //!   value of is_SCT_Low_Current_Spinup_Supported(device)
    //!
    //  Exit:
    //!   \return sctCommandSupported = true: 0 - invalid state or could not be
    //!   detected due to SAT translation failure, 1 = low, 2 = default, 3 =
    //!   ultralow
    //!                               = false: 0 - not enabled or not supported. 1 =
    //!                               low. The set features method does not have the
    //!                               same granularity as the SCT command.
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  seagate_SCT_Low_Current_Spinup(tDevice *device, eSeagateLCSpinLevel
    //  spinupLevel)
    //
    //! \brief   Description:  This function will send the SCT command to set the
    //! state of the low-current spinup feature on a Seagate drive that supports
    //! this SCT command. NOTE: Not all Seagate products support this command.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    ...
    //!
    //  Exit:
    //!   \return SUCCESS = successfully enabled low current spin up, NOT_SUPPORTED
    //!   = not Seagate or drive doesn't support this feature.
    //
    //-----------------------------------------------------------------------------
    ...
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param useSCTCommand - set to true to use the SCT command to set spinup
    //!   current level. Should be set to true when
    //!   is_SCT_Low_Current_Spinup_Supported() == true \param state - state to set
    //!   low current spinup to. Tt should be set to one of eSeagateLCSpinLevel
    //!   regardless of the value of useSCTCommand. This will properly translate the
    //!   SCT value to a value for the Set Features command version as necessary.
    //!
    //  Exit:
    //!   \return SUCCESS = successfully enabled low current spin up, NOT_SUPPORTED
    //!   = not Seagate or drive doesn't support this feature.
    ...
    //
    //! \brief   Description:  This function will send the command to set the SSC
    //! (Spread Spectrum Clocking) state of a Seagate SATA drive. A power cycle is
    //! required to make changes take affect
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param mode - set to enum value saying whether to enable, disable, or set
    //!   to defaults
    //!
    //  Exit:
    //!   \return SUCCESS = successfully set SSC state, FAILURE = failed to set SSC
    //!   state, NOT_SUPPORTED = not Seagate or drive doesn't support this feature.
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  This function will send the command to get the SSC
    //! (Spread Spectrum Clocking) state of a Seagate SATA drive.
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!   \param mode - pointer to enum variable that will hold the state upon
    //!   successful completion
    //!
    //  Exit:
    //!   \return SUCCESS = successfully got SSC state, FAILURE = failed to get SSC
    //!   state, NOT_SUPPORTED = not Seagate or drive doesn't support this feature.
    //
    //-----------------------------------------------------------------------------
    ...
    {
        bool valid; // this must be set to true for the remaining fileds to have
                    // meaning.
    ...
    OPENSEA_OPERATIONS_API eReturnValues
    seagate_Get_Power_Balance(tDevice* device,
                              bool*    supported,
                              bool*    enabled); // SATA only. SAS should use the set power consumption
                                              // options in power_control.h
    ...
    OPENSEA_OPERATIONS_API eReturnValues
    seagate_Set_Power_Balance(tDevice*          device,
                              ePowerBalanceMode powerMode); // SATA only. SAS should use the set power
                                                            // consumption options in power_control.h
    ...
    //  get_IDD_Support()
    //
    //! \brief   Description:  Gets which IDD features/operations are supported by
    //! the device
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] iddSupport = pointer to a iddSupportedFeatures structure that
    //!   will hold which features are supported
    //!
    //  Exit:
    //!   \return SUCCESS on successful completion, FAILURE = fail, NOT_SUPPORTED =
    //!   IDD not supported
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  Gets an approximate time for how long a specific IDD
    //! operation may take
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] iddTest = enum value describing the IDD test to get the time
    //!   for \param[in] timeInSeconds = pointer to a uint64_t that will hold the
    //!   amount of time in seconds that IDD is estimated to take
    //!
    //  Exit:
    //!   \return SUCCESS on successful completion, FAILURE = fail, NOT_SUPPORTED =
    //!   IDD not supported
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] IDDtest = enum value describing the IDD test to run (CAPTIVE
    //!   not supported right now) \param[in] pollForProgress = 0 = don't poll, just
    //!   start the test. 1 = poll for progress and display the progress on the
    //!   screen. \param[in] captive = set to true to force running the test in
    //!   captive mode. Long test only!
    //!
    //  Exit:
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] status = returns a status value for an ongoing IDD operation.
    //!   This is similar to a DST status code value.
    //!
    //  Exit:
    //!   \return SUCCESS on successful completion, FAILURE = fail
    //
    ...
    //
    //! \brief   Description:  Checks if the Seagate power telemetry feature is
    //! supported or not
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    ...
    // NOTE: While these 2 structures are common, keep them in this layer since they
    // are meant to be read into for an operation. Putting these in
    // opensea-transport may confuse users into thinking a memcpy can be done to use
    // them, but these are modified to make things easier for operations
    ...
    // NOTE: This structure should not be used for memcpy or pointing over a data
    // buffer!
    //       This will be filled in by parsing the return data from the drive to
    //       make it easily usable in a utility.
    typedef struct _seagatePwrTelemetry
    {
        ...
        uint64_t driveTimeStampForHostRequestedMeasurement; // in microseconds //should
                                                            // this be a double???
        uint64_t driveTimeStampWhenTheLogWasRetrieved;      // in microseconds //should
                                                            // this be a double???
    ...
    //
    //! \brief   Description:  Gets the power telemetry data into a structure that
    //! can be used to display the data
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] pwrTelData = must be allocated before calling. Will be filled
    //!   with power telemetry data upon success
    //!
    //  Exit:
    //!   \return SUCCESS on successful completion, FAILURE = fail
    //
    ...
    //
    //  Entry:
    //!   \param[in] pwrTelData = pointer to power telemetry data already retrieved
    //!   from the drive.
    //!
    //  Exit:
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  request_Power_Measurement(tDevice *device, uint16_t timeMeasurementSeconds,
    ...
    //-----------------------------------------------------------------------------
    //
    //  pull_Power_Telemetry_Log(tDevice *device, const char * const filePath,
    ...
    //!   transferSizeBytes = OPTIONAL. If set to zero, this is ignored. Should be
    //!   rounded to 512B for ATA devices
    //!
    //  Exit:
    //!   \return VOID
    //
...
//-----------------------------------------------------------------------------
//
//  translate_IDD_Status_To_String(uint8_t status, char *translatedString, bool
//  justRanDST)
//
//! \brief   Description: Translates the status code from IDD to a string. NOTE:
//! This is basically the same as the DST translation. This may be able to be
//! improved in the future.
//
//  Entry:
//!   \param[in] status = status code from last run IDD (from log or elsewhere)
//!   \param[out] translatedString = pointer to memory to set the information
//!   string about IDD that needs to be printed \param[in] justRanDST = set to
//!   true if IDD just finished running. Set to false if not known
//!
//  Exit:
//!   \return VOID
//
...
#define MAX_IDD_STATUS_STRING_LENGTH 160
    ...
    //
    //! \brief   Description:  This function checks if the Seagate SATA quick format
    ...
    //
    //! \brief   Description:  This function issues the Seagate SATA quick format
    //! command. This is a captive operation, so you must wait for it to complete,
    //! no matter how long it takes, but should be a couple minutes at most
    //
    //  Entry:
    //!   \param device - pointer to the device structure.
    //!
    //  Exit:
    //!   \return SUCCESS on successful completion, FAILURE = fail, NOT_SUPPORTED
    //!   not supported on this device. This only happens on non-sata right now. -
    //!   TJE
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  Function to send Get Extended SMART Information Log
    //! Page NVMe command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    //!   \param[in] nsid = Namespace ID for the namespace of 0xFFFFFFFF for entire
    //!   controller. \param[out] pData = Data buffer (suppose to be 512 bytes)
...
#define SERVO_PRODUCT_FAMILY_LEN       4
    ...
    // This is defined in the Seagate SCSI commands reference manual available on
    // the web
