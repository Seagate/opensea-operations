...
extern "C"
{
    ...
    // This is written towards the SATA device statistics log, however all SAS
    // statistics read should be able to be put into here without too much issue.
    typedef struct _statistic
    {
        ...
        uint64_t statisticValue; // may need additional parsing depending on the
                                 // statistic this represents
    ...
    {
        uint16_t statisticsPopulated; // just a count of how many were populated...not
                                      // any specific order
        // general statistics
    ...
    typedef enum _eProtocolSpecificStatisticsType
    {
        ...
        STAT_PROT_SAS,  // SAS protocol specific port page info. Up to 2 ports and 2
                        // phys?
                        // Other protocol specific pages. I did not see one for SPI, SSA, SRP, Fibre
                        // Channel, UAS, or SOP. If these other protocols add data to output, we can
                        // add it here. - TJE
    } eProtocolSpecificStatisticsType;

// setting maximum number of ports to 2 for now. This could change in the
// future, but is not super likely - TJE
#define SAS_STATISTICS_MAX_PORTS 2
// setting maximum phys to 2. Current drives have 1 phy per port, so this is
// more than necessary - TJE
#define SAS_STATISTICS_MAX_PHYS 2
    ...
    typedef struct _sasDeviceStatitics
    {
        ...
        // Read Reverse Error Counters - These might be for tape drives, not
        // HDDs...may remove this-TJE
        ...
        // TODO: How do we want to handle multiple port SAS? Currently limiting this
        // output to 2 ports since that is the most supported today-TJE
        union
        {
            sasProtocolStatistics sasProtStats;
            // Other data structures for other protocols that implement the protocol
            // specific port log page
        };
        ...
        //      Informational exceptions??? Not sure how we should track this data yet
        //      - TJE
        //
    } sasDeviceStatitics;
