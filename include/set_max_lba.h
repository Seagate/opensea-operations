...
extern "C"
{
    ...
    //
    //! \brief   Gets the native maxLBA from an ATA device with the HPA or AMA
    //! feature set
    //
    //  Entry:
    //!   \param[in]  device file descriptor
    //!   \param[out]  nativeMaxLBA the maxLBA of the device. If neither HPA or AMA
    //!   are supported, then this will be set to UINT64_MAX
    //!
    //  Exit:
    ...
    //  Entry:
    //!   \param[in]  device file descriptor
    //!   \param[out]  nativeMaxLBA the maxLBA of the device. If unable to retrieve
    //!   or not supported, UINT64_MAX is returned
    //!
    //  Exit:
    //!   \return SUCCESS = good, !SUCCESS something went wrong see error codes
    //
    ...
    //!   \param[in]  reset if set to 1 (or higher), this will reset to the native
    //!   max, otherwise it will use the newMaxLBA param to set the maxLBA
    //!   \param[in]  changeId if set to 1 (or higher), this will change model
    //!   number if available in AMAC feature set
    //!
    //  Exit:
    //!   \return SUCCESS = good, !SUCCESS something went wrong see error codes
    //
    ...
    //
    //! \brief   Sets the maxLBA of the selected device using HPA or AMA feature
    //! sets
    //
    //  Entry:
    //!   \param[in]  device file descriptor
    ...
    //
    //! \brief   Sets the maxLBA of the selected device. This will work with new and
    //! old methods.
    ...
    //
    //! \brief   This function is specifically named since it has a main purpose:
    //! restoring max LBA to erase a drive as much as possible
    //!          or to allow validation of an erase as much as possible.
    //!          Because of this, it handles all the ATA checks to make sure all
    //!          features are restored or a proper error code for frozen or access
    //!          denied is returned (HPA/AMAC/DCO and HPA security are all handled)
    //
    //  Entry:
    //!   \param[in]  device file descriptor
    //!
    //  Exit:
    //!   \return SUCCESS = good, DEVICE_ACCESS_DENIED means HPA security is active
    //!   and blocked the restoration of the maxLBA
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   This function checks if the adapter and device are reporting the
    //! same maxLBA to the host.
    //!          SATA drives behind USB adapters or SAS HBAs work through a SATL
    //!          (translator) that may not be in sync after changing the maxLBA.
    //!          This checks if these things are in sync or not and reports the
    //!          status. If not in sync, a powercycle or reboot is necessary for the
    //!          adapter to update what it knows about the attached device.
    //
    //  Entry:
    //!   \param[in]  device = file descriptor
    //!   \param[in]  issueReset = attempt to issue a low-level reset to synchronize
    //!   scsi and ata reported info. This reset will not work in all scenarios. if
    //!   set to false, this will be called recursively to try the reset for you as
    //!   needed.
    //!
    //  Exit:
    //!   \return true = in sync, false = out of sync. Recommend rebooting/power
    //!   cycling to resync the adapters knowledge of the device.
    //
    //-----------------------------------------------------------------------------
    ...
    typedef struct _capacityModelNumberMapping
    {
        ...
        capacityModelDescriptor descriptor[1]; // NOTE: This must be allocated based on how many
                                               // descriptors are actually available! ex:
                                               // malloc(sizeof(capacityModelNumberMapping) +
                                               // (get_capacityModelDescriptor_Count() *
    ...
    //!
    //  Exit:
    //!   \return SUCCESS = pointer to the struct to fill in with Capacity/Product
    //!   Mapping, FAILURE = M_NULLPTR.
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  delete_Capacity_Model_Number_Mapping(ptrcapacityModelNumberMapping
    //  capModelMapping)
    //
    //! \brief   Description:  This function free allocated Capacity/Product Mapping
    //
    //  Entry:
    //!   \param[in] capModelMapping = pointer to the struct to fill in with
    //!   Capacity/Product Mapping
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
