...
extern "C"
{
    ...
    //
    //! \brief   Checks the current power mode of the device and prints it to the
    //! screen. (SATA will always work. SAS only works if the drive has transitioned
    //! to another state)
    //
    //  Entry:
    ...
    //! This can only do EPC settings, and single power states at a time.
    //!          This function has been replaced by a set_EPC_Power_Conditions below
    //!          which can handle changing multiple power states at a time.
    //
    //  Entry:
    //!   \param device - file descriptor
    //!   \param restoreDefaults - set to true to restore the drives default
    //!   settings. All other options will be ignored \param enableDisable - set to
    //!   true for enabling a powerCondition and false to disable one \param
    //!   powerCondition - set to the power condition you wish to have changed
    //!   \param powerModeTimer - the timer value to set. If this is zero, a timer
    //!   value will not be set and the power mode will only be enabled. This option
    //!   is ignored when enableDisable is false or when restoreDefaults is true
    //!   \param powerModeTimerValid - when set to true, this means the incoming
...
#define MAXIMUM_NVME_POWER_STATES                                                                                      \
    UINT16_C(32) // the npss can technically report up to 256 since it is a 8bit
                 // field, but spec only defines 32 (0 to 31)

    // note: this does not exactly match the NVMe identify data. It takes that and
    // converts it to this "friendly" structure that is a little easier to parse
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_Power_State(tDevice *device, uint32_t * powerState, eFeatureModeSelect
    //  selectValue )
    //
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_OPERATIONS_API eReturnValues get_Power_State(tDevice*           device,
                                                         uint32_t*          powerState,
    ...
    typedef struct _powerConsumptionIdentifiers
    {
        ...
        powerConsumptionIdentifier identifiers[0xFF]; // Maximum number of power consumption
                                                      // identifiers...probably won't get this many, but
                                                      // might as well make this possible to do.
        bool    currentIdentifierValid;
        uint8_t currentIdentifier;
        bool    activeLevelChangable; // this may be changable or not depending on what
                                      // the drive reports
        uint8_t activeLevel;          // From power consumption mode page. This is a
                                      // high/medium/low value. Only use this if non-zero
    } powerConsumptionIdentifiers, *ptrPowerConsumptionIdentifiers;

    //-----------------------------------------------------------------------------
    //
    //  get_Power_Consumption_Identifiers(tDevice *device,
    ...
    //  Entry:
    //!   \param [in] device - file descriptor
    //!   \param [out] identifiers - pointer to a struct that will be filled in with
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_OPERATIONS_API eReturnValues get_Power_Consumption_Identifiers(tDevice*                       device,
                                                                           ptrPowerConsumptionIdentifiers identifiers);

    //-----------------------------------------------------------------------------
    //
    //  print_Power_Consumption_Identifiers(ptrPowerConsumptionIdentifiers
    //  identifiers)
    //
    //! \brief  Print out all the power consumption identifiers in the struct (the
    //! number that is supported anyways).
    //
    //  Entry:
    //!   \param [in] identifiers - pointer to a struct containing the power
    //!   consumption identifier information
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  set_Power_Consumption(tDevice *device, ePCActiveLevel activeLevelField,
    //  uint8_t powerConsumptionIdentifier, bool resetToDefault)
    //
    //! \brief  Set the power consumption rate for a SCSI drive. (ATA not supported
    //! right now, may never be depending on future ata spec implementation)
    //
    //  Entry:
    //!   \param [in] device - file descriptor
    //!   \param [in] activeLevelField - set to an enum value matching SCP spec.
    //!   When set to PC_ACTIVE_LEVEL_IDENTIFIER, the powerConsumptionIdentifier
    //!   value will be used. \param [in] powerConsumptionIdentifier - only valid
    //!   when activeLevelField is set to PC_ACTIVE_LEVEL_IDENTIFIER. This value
    //!   must match one the device supports. \param [in] resetToDefault - when set
    //!   to true, all other inputs are ignored. The default mode is restored by
    //!   reading the default settings and setting the current settings to the
    //!   defaults.
    //!
    //  Exit:
    //!   \return SUCCESS = good, !SUCCESS something went wrong see error codes
    //
    //-----------------------------------------------------------------------------
    OPENSEA_OPERATIONS_API eReturnValues set_Power_Consumption(tDevice*       device,
    ...
    //-----------------------------------------------------------------------------
    //
    //  map_Watt_Value_To_Power_Consumption_Identifier(tDevice *device, double
    //  watts, uint8_t *powerConsumptionIdentifier)
    //
    //! \brief  Maps a value in Watts to a power condition identifier supported by
    //! the drive. Will return NOT_SUPPORTED if no suitable match can be found
    //
    //  Entry:
    //!   \param [in] device - file descriptor
    //!   \param [in] watts - value in watts to search for. Will be rounded down
    //!   when searching for a match. \param [out] powerConsumptionIdentifier - will
    //!   be set to the value of the power consumption identifier that most closely
    //!   matches the incoming watt value.
    //!
    //  Exit:
    //!   \return SUCCESS = good, !SUCCESS something went wrong see error codes
    //
    ...
    typedef struct _powerConditionInfo // written according to ATA spec
                                       // fields...will try to populate as much as
    ...
    OPENSEA_OPERATIONS_API eReturnValues sata_Get_Device_Initiated_Interface_Power_State_Transitions(tDevice* device,
                                                                                                     bool*    supported,
                                                                                                     bool*    enabled);

    OPENSEA_OPERATIONS_API eReturnValues sata_Set_Device_Initiated_Interface_Power_State_Transitions(tDevice* device,
                                                                                                     bool     enable);

    OPENSEA_OPERATIONS_API eReturnValues sata_Get_Device_Automatic_Partioan_To_Slumber_Transtisions(tDevice* device,
                                                                                                    bool*    supported,
                                                                                                    bool*    enabled);

    OPENSEA_OPERATIONS_API eReturnValues sata_Set_Device_Automatic_Partioan_To_Slumber_Transtisions(tDevice* device,
                                                                                                    bool     enable);

    // Following functions allow power mode transitions on Non-EPC drives (pre SBC3
    // for SCSI) These will work with EPC, but may cause changes to the current
    // timer values. See SCSI/ATA specs for details on interactions with these and
    // EPC timers
    ...
    OPENSEA_OPERATIONS_API eReturnValues transition_To_Idle(tDevice* device,
                                                            bool     unload); // unload feature must be supported

    // NOTE: Do not call this unless you know what you are doing. This requires a
    // reset to wake up from, which may not be callable from an application.
    OPENSEA_OPERATIONS_API eReturnValues transition_To_Sleep(tDevice* device);

    // Be careful changing partial and slumber settings. Not every controller will
    // support it properly!
    OPENSEA_OPERATIONS_API eReturnValues scsi_Set_Partial_Slumber(tDevice* device,
                                                                  bool     enablePartial,
                                                                  bool     enableSlumber,
                                                                  bool     partialValid,
                                                                  bool     slumberValid,
    ...
    OPENSEA_OPERATIONS_API eReturnValues get_SAS_Enhanced_Phy_Control_Number_Of_Phys(tDevice* device,
                                                                                     uint8_t* phyCount);
    ...
    // If doing all phys, use get_SAS_Enhanced_Phy_Control_Number_Of_Phys first to
    ...
    {
        bool powerConditionValid; // specifies whether to look at anything in this
                                  // structure or not.
        bool restoreToDefault;    // If this is set, none of the other values in this
                                  // matter
        bool enableValid;         // holds if enable bool below should be referenced at all or
                                  // not.
    ...
    typedef struct _powerConditionTimers
    {
        ...
        // All fields below here only apply to EPC. Everything above is supported by
        // legacy devices that support the power conditions mode page
        powerConditionSettings idle_b;
        powerConditionSettings idle_c;
        powerConditionSettings standby_y;
        // Fields below are only for SAS/SCSI devices. Attempting to change these for
        // other devices will be ignored. PM_BG_Preference and CCF fields are handled
        // below. EPC only
    ...
    // When ATA drive, the restoreToDefaults is not allowed. Also, translation of
    // timer value is done according to SAT spec
    ...
    //  Entry:
    //!   \param device - file descriptor
    //!   \param enable - set to true to enable the PUIS feature. Set to False to
    //!   disable the PUIS feature.
    //!
    //  Exit:
    ...
    typedef struct _puisInfo
    {
        bool puisSupported;
        bool puisEnabled;
        bool spinupCommandRequired; // requires the spinup command to be issued to
                                    // spin up. If false, then a media access will
                                    // spinup as needed.
