...
extern "C"
{
    ...
    // The enum below can be used to specify which type of reservation is used.
    // NOTE: These are not defined to values for NVMe or SCSI since they are
    // different so that they can be translated as necessary for the device type.

    // Scopes defined for clarity in reporting, but not supported for actual use
    // since scopes other than logical unit are obsolete.
...
#define PERSISTENT_RESERVATION_CAPABILITIES_VERSION 1
    ...
    typedef enum _eAllowedCommandDetail
    {
        ...
        RES_CMD_UNKNOWN = 0xFF, // for forward compatibility with values that are currently reserved
    ...
    OPENSEA_OPERATIONS_API void show_Persistent_Reservations_Capabilities(
...
#define RESERVATION_DATA_VERSION 1
    ...
    typedef struct _reservationsData
    {
        ...
        reservationInfo reservation[1]; // variable length depending on how it was allocated.
...
#define FULL_RESERVATION_INFO_VERSION 1
    ...
    typedef struct _fullReservationInfo
    {
        ...
        uint32_t               generation;
        uint16_t               numberOfKeys;
        fullReservationKeyInfo reservationKey[1]; // Variable size depending on how many will be reported
