...
extern "C"
{
    ...
    M_PARAM_RO(1) OPENSEA_OPERATIONS_API bool is_Persistent_Reservations_Supported(const tDevice* device);
...
#define PERSISTENT_RESERVATION_CAPABILITIES_VERSION 1
    ...
    get_Persistent_Reservations_Capabilities(const tDevice*                       device,
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Registration_Key_Count(const tDevice* device, uint16_t* keyCount);
...
#define REGISTRATION_KEY_DATA_VERSION 1
    ...
    M_PARAM_RO(1)
    M_PARAM_RW(3)
    OPENSEA_OPERATIONS_API eReturnValues get_Registration_Keys(const tDevice*          device,
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Reservation_Count(const tDevice* device, uint16_t* reservationKeyCount);
...
#define RESERVATION_DATA_VERSION 1
    ...
    M_PARAM_RO(1)
    M_PARAM_RW(3)
    OPENSEA_OPERATIONS_API eReturnValues get_Reservations(const tDevice*      device,
    ...
    M_PARAM_RO(1)
    M_PARAM_WO(2)
    OPENSEA_OPERATIONS_API eReturnValues get_Full_Status_Key_Count(const tDevice* device, uint16_t* keyCount);
...
#define FULL_RESERVATION_INFO_VERSION 1
    ...
    M_PARAM_RO(1)
    M_PARAM_RW(3)
    OPENSEA_OPERATIONS_API eReturnValues get_Full_Status(const tDevice*         device,
                                                         uint16_t               numberOfKeys,
    ...
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues register_Key(const tDevice* device,
    ...
    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues unregister_Key(const tDevice* device, uint64_t currentRegistrationKey);

    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues acquire_Reservation(const tDevice*   device,
                                                             uint64_t         key,
                                                             eReservationType resType);

    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues release_Reservation(const tDevice*   device,
                                                             uint64_t         key,
                                                             eReservationType resType);

    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1) OPENSEA_OPERATIONS_API eReturnValues clear_Reservations(const tDevice* device, uint64_t key);

    M_NONNULL_PARAM_LIST(1)
    M_PARAM_RO(1)
    OPENSEA_OPERATIONS_API eReturnValues
    preempt_Reservation(const tDevice* device, uint64_t key, uint64_t preemptKey, bool abort, eReservationType resType);
