...
extern "C"
{
    ...
    //  run_DST()
    //
    //! \brief   Description:  Function to send a ATA Spec DST or SCSI spec DST to a
    //! device and poll it for updates. Recommended for utility usage since this
    //! will also poll or wait
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] DSTType = see enum above
    //!   \param[in] pollForProgress = 0 = don't poll, just start the test. 1 = poll
    //!   for progress and display the progress on the screen. \param[in]
    //!   captiveForeground = when set to true, the self test is run in
    //!   captive/foreground mode. This is only for ATA or SCSI. When set, this will
    //!   wait for the entire test to complete before returning. This is ignored on
    //!   NVMe \param[in] ignoreMaxTime = when this is set to true, the timeout for
    //!   the maximum time to wait for DST before aborting it will be ignored and
    //!   will wait indefinitely to complete the DST. This is useful if a system is
    //!   having high disc usage and DST is unable to progress
    //!
    //  Exit:
    ...
    //-----------------------------------------------------------------------------
    //
    //  send_DST(tDevice *device, eDSTType DSTType, bool captiveForeground, uint32_t
    //  commandTimeout)
    //
    //! \brief   Description:  Function to send a ATA Spec DST or SCSI spec DST to a
    //! device
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] percentComplete = pointer to a uint32 that will hold the DST
    //!   %complete \param[out] status = pointer to a value to hold the current DST
    //!   status
    //!
    //  Exit:
    ...
    //  print_DST_Progress()
    //
    //! \brief   Description:  Function to get and print the progress of an active
    //! DST test
    //
    //  Entry:
    //!   \param[in] device = file descriptor
...
#define MAX_DST_STATUS_STRING_LENGTH 160
    ...
    //
    //! \brief   Get the long DST timeout in hours and minutes. This function fills
    //! in the data referenced by the passed in pointers so it's up to the called to
    //! use the data as they wish
    //
    //  Entry:
    ...
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[out] percentComplete = pointer to the variable that will be filled
    //!   in with the percent Complete \param[out] status = pointer to the variable
    //!   that will be filled in with the DST status/result
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to get the current DST progress and
    //! Status/result
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[out] percentComplete - pointer to a variable to hold the percentage
    //!   completed \param[out] status - pointer to a variable to hold the status
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to get the error LBA from the attached
    //! device from the DST log. Will auto detect ATA vs SCSI
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[out] lba - pointer to a uint64_t that will hold the LBA of the last
    //!   error
    //!
    //  Exit:
    //!   \return true = valid LBA returned, false = invalid LBA. Could not read
    //!   log, or status indicates failure other than read failure
    //
    //-----------------------------------------------------------------------------
    ...
    {
        ptrErrorLBA ptrToErrorList; // pointer to the list so there is no need to copy
                                    // memory all over the place
        uint64_t* errorIndex;       // pointer to the current index value so that DST and
                                    // clean can update this and give it back to the caller
    ...
    //  run_DST_And_Clean()
    //
    //! \brief   Description:  This function performs a DST and clean. It starts
    //! DST, gets the error, fixes it, then repeats until all errors are fixed or
    //! the error limit is reached.
    //!                        The error limit to this function should NOT be
    //!                        adjusted for number of logical sectors per physical
    //!                        sector like in the generic test functions.
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[in] errorLimit - value representing number of errors to fix. This
    //!   must be 1 or higher. \param[in] updateFunction - \param[in] updateData -
    //!   \param[in] externalErrorList - optional. Only use if you intend to do
    //!   other things before or after DST & Clean. With this parameter, the ending
    //!   result error list will not print. \param[in] repaired - flag for Tattoo
    //!   log for when the drive has been repaired.
    //!
    //  Exit:
    //!   \return SUCCESS = completed DST and clean successfully, !SUCCESS = error
    //!   limit reached, or unrepairable DST condition
    //
    //-----------------------------------------------------------------------------
    ...
    typedef struct _dstDescriptor
    {
        ...
        uint8_t selfTestRun; //"Content of LBA 0:7" in ATA spec, or from self test
                             // status in NVMe
        ...
        }; // union isn't necessary but might help make someone's life easier if they
           // understand this data one way or another
        union
        {
        ...
        union
        {
            // sense info below is translated according to SAT for ATA. SCSI will set
            // this right from the log
            struct
            {
                uint8_t senseKey;
...
// max of 21 allowed by ATA spec for SMART log. 19/page for GPL, but I've never
// seen more than 21 entries supported...-TJE Max of 20 in NVMe specification
#define MAX_DST_ENTRIES 21
    ...
    {
        uint8_t numberOfEntries; // count of how many valid entries are placed into
                                 // this struct
        dstLogType logType;      // can be used by things printing this data to parse
                                 // things correctly
