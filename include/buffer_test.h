...
#define RANDOM_TEST_COUNT     5
    ...
    typedef struct _patternTestResults
    {
        ...
        uint32_t totalCommandTimeouts;   // how many take longer than the timeout set
                                         // for the drive
        uint32_t totalCommandCRCErrors;  // how many commands return a CRC error
        uint32_t totalBufferComparisons; // how many write-read buffer, then compare
                                         // the two have been done in the test;
    ...
    //
    //! \brief   Description: Perform a cable/buffer test using read/write buffer
    //! commands to check for mismatches and other bus errors
    //
    //  Entry:
    ...
    //
    //! \brief   Description: Print the cable/buffer test results after running a
    //! test.
    //
    //  Entry:
    //!   \param[in] testResults = structure that holds results from a cable/buffer
    //!   test
    //!
    //  Exit:
    //
